...
sns_rc sns_oem1_inst_init(sns_sensor_instance *this, sns_sensor_state const *state)
{
  ...
  sns_oem1_inst_state *inst_state = (sns_oem1_inst_state *)this->state->state;
  sns_oem1_sensor_state *sensor_state = (sns_oem1_sensor_state *)state->state;
  ...
  stream_service->api->create_sensor_instance_stream(stream_service, this, inst_state->accel_suid,
                                       ...
                                       sns_request const *client_request)
{
...
#endif
  {
    ...
    sns_std_sensor_config *client_req = (sns_std_sensor_config *)(client_request->request);
    sns_memscpy(&state->client_config, sizeof(state->client_config), client_request->request,
    ...
    encoded_len = pb_encode_request(buffer, sizeof(buffer), &accel_config,
                                    sns_std_sensor_config_fields, NULL);
    if(0 < encoded_len && NULL != state->accel_stream)
    {
      sns_request request = (sns_request){.message_id = SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_CONFIG,
    ...
    SNS_INST_PRINTF(LOW, this,
                    "Processed oem1 config request: enabled accel with"
                    "sample rate %d, result: %u",
                    state->config.sample_rate, rc);
#else
    ...
    sns_memscpy(&resampler_config.sensor_uid, sizeof(resampler_config.sensor_uid),
                &state->accel_suid, sizeof(state->accel_suid));
    ...
    stream_service->api->create_sensor_instance_stream(stream_service, this, state->resampler_suid,
    ...
    if(NULL != state->resampler_stream)
    {
      ...
      if(0 < encoded_len)
      {
        sns_request request = (sns_request){.message_id = SNS_RESAMPLER_MSGID_SNS_RESAMPLER_CONFIG,
                                            .request_len = encoded_len,
                                            .request = buffer};
        rc = state->resampler_stream->api->send_request(state->resampler_stream, &request);
        SNS_INST_PRINTF(MED, this, "oem1 sent accel request to resampler at rate %d",
