/**============================================================================
  ...
  @brief The oem1 virtual Sensor Instance implementation
...
/* Used only for demo/ debugging purposes for island print functionality */
static void sns_oem1_island_print(sns_sensor_instance *const this)
{
  sns_oem1_inst_state *state = (sns_oem1_inst_state *)this->state->state;
  sns_time ts = sns_get_system_time();
  state->island_print_count++;
  SNS_INST_UPRINTF(MED, this, "OEM1-%u, timestamp-0x%x%08x", state->island_print_count,
...
static sns_rc sns_oem1_process_resampler_event(sns_sensor_instance *const this)
{
  ...
  while(NULL != resampler_event_in)
  {
    if(SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_EVENT == resampler_event_in->message_id)
    {
      ...
      uint8_t arr_index = 0;
      pb_float_arr_arg arg = {.arr = data, .arr_len = ARR_SIZE(data), .arr_index = &arr_index};

      pb_istream_t stream = pb_istream_from_buffer((pb_byte_t *)resampler_event_in->event,
                                                   resampler_event_in->event_len);
      sns_std_sensor_event resampler_data = sns_std_sensor_event_init_default;

      resampler_data.data =
          (struct pb_callback_s){.funcs.decode = &pb_decode_float_arr_cb, .arg = &arg};

      if(!pb_decode_noinit(&stream, sns_std_sensor_event_fields, &resampler_data))
      {
      ...
      else
      {
        if(SNS_STD_SENSOR_SAMPLE_STATUS_UNRELIABLE == resampler_data.status)
        {
          SNS_INST_PRINTF(LOW, this,
                          " OEM1 received unreliable accel data"
                          " from resampler ignoring..");
          resampler_event_in =
              state->resampler_stream->api->get_next_input(state->resampler_stream);
          continue;
        }
        ...
        SNS_INST_PRINTF(LOW, this, "resampler accel data: x %d/1000, y %d/1000, z %d/1000",
        ...
        SNS_INST_PRINTF(LOW, this, "OEM1 output:  x %d/1000, y %d/1000, z %d/1000",
                        (int)(1000 * oem1_payload[0]), (int)(1000 * oem1_payload[1]),
                        (int)(1000 * oem1_payload[2]));

#ifdef OEM1_SUPPORT_EVENT_TYPE
        if(state->inst_config.current_state != state->inst_config.previous_state)
        {
#endif
          SNS_INST_PRINTF(MED, this, "OEM1 last %d current %d", state->inst_config.current_state,
                          state->inst_config.previous_state);
          rc = pb_send_sensor_stream_event(this, NULL, resampler_event_in->timestamp,
                                           SNS_OEM1_MSGID_SNS_OEM1_DATA,
                                           SNS_STD_SENSOR_SAMPLE_STATUS_ACCURACY_HIGH, oem1_payload,
                                           OEM1_NUM_ELEMS, state->config.encoded_data_event_len);
          if(SNS_RC_SUCCESS != rc)
          {
            ...
            resampler_event_in->message_id)
    {
      SNS_INST_PRINTF(LOW, this,
                      "Received SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_PHYSICAL_CONFIG_EVENT");
    }
    // check next sample
 ...
 */
static sns_rc sns_oem1_inst_notify_event(sns_sensor_instance *const this)
{
  sns_rc rc = SNS_RC_SUCCESS;
  ...
  while(NULL != accel_event_in)
  {
    if(SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_EVENT == accel_event_in->message_id)
    {
      ...
      pb_float_arr_arg arg = {
          .arr = (float *)temp, .arr_len = OEM1_NUM_ELEMS, .arr_index = &arr_index};

      stream_event.data =
          (struct pb_callback_s){.funcs.decode = &pb_decode_float_arr_cb, .arg = &arg};

      if(!pb_decode_noinit(&stream, sns_std_sensor_event_fields, &stream_event))
      {
      ...
      else
      {
        ...
        SNS_INST_PRINTF(LOW, this, "accel data: x %d/1000, y %d/1000, z %d/1000",
        ...
        {
#endif
          rc = pb_send_sensor_stream_event(
              this, NULL, accel_event_in->timestamp, SNS_OEM1_MSGID_SNS_OEM1_DATA,
              SNS_STD_SENSOR_SAMPLE_STATUS_ACCURACY_HIGH, accel_payload, OEM1_NUM_ELEMS,
...
const sns_sensor_instance_api sns_oem1_sensor_instance_api = {
