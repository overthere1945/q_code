/**============================================================================
  ...
  @brief The oem1 virtual Sensor implementation
...
extern const sns_sensor_uid oem1_suid;
...
#ifdef OEM1_SUPPORT_REGISTRY
static sns_rc sns_oem1_sensor_send_error(sns_sensor *const this, sns_std_error error)
{
      ...
      instance = this->cb->get_sensor_instance(this, false))
  {
    ...
    if(!pb_send_event(instance, sns_std_error_event_fields, &error_event, sns_get_system_time(),
                      SNS_STD_MSGID_SNS_STD_ERROR_EVENT, this->sensor_api->get_sensor_uid(this)))
    {
...
static void sns_oem1_registry_req(sns_sensor *const this)
{
  sns_oem1_sensor_state *state = (sns_oem1_sensor_state *)this->state->state;
  ...
  pb_buffer_arg data = (pb_buffer_arg){.buf = "sns_oem1", .buf_len = (strlen("sns_oem1") + 1)};
  ...
  encoded_len = pb_encode_request(buffer, sizeof(buffer), &sns_oem1_registry,
...
static bool sns_oem1_parse_registry(pb_istream_t *stream, const pb_field_t *field, void **arg)
{
  UNUSED_VAR(field);
...
static sns_rc handle_oem1_registry_event(sns_sensor *const this)
{
  ...
  for(event = state->registry_stream->api->peek_input(state->registry_stream); NULL != event;
      event = (NULL == state->registry_stream)
                  ? NULL
...
{
  sns_oem1_inst_state const *inst_state = (sns_oem1_inst_state *)instance->state->state;
                                                 ...
                                                 sns_request const *new_req, bool remove)
{
  ...
  if((NULL != curr_inst) && (NULL != new_req) && (NULL != curr_req) &&
  ...
  else if(remove)
  {
    if(NULL != curr_inst)
    {
      curr_inst->cb->remove_client_request(curr_inst, curr_req);
          ...
          ((new_req->message_id == SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_CONFIG) ||
           (new_req->message_id == SNS_STD_SENSOR_MSGID_SNS_STD_ON_CHANGE_CONFIG)))
  {
    ...
    pb_simple_cb_arg arg = {.decoded_struct = &config, .fields = sns_std_sensor_config_fields};
    request.payload = (struct pb_callback_s){.funcs.decode = &pb_decode_simple_cb, .arg = &arg};
    ...
    if(pb_decode_noinit(&stream, sns_std_request_fields, &request))
    {
      ...
      match_inst = sns_sensor_util_find_instance_match(this, &decoded_req, &find_instance_match);
    }
    else
    {
    ...
    if(status == true)
    {
      if(NULL == curr_inst)
      {
        ...
        if(NULL == match_inst)
        {
          rv_inst = this->cb->create_instance(this, (sizeof(sns_oem1_inst_state)));
        }
        else
        {
          rv_inst = match_inst;
...
static sns_sensor_uid const *sns_oem1_get_sensor_uid(sns_sensor const *this)
{
  UNUSED_VAR(this);
  return &oem1_suid;
}
...
const sns_sensor_api sns_oem1_api = {
