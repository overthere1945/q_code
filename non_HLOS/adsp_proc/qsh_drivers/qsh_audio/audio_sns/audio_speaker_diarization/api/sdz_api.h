...
{
    NON_BUFFERED = 0,                     /* Send SDZ engine output immediately. Each buffer shall have one payload
                                             corresponding to a partial/complete segment of speech.
                                             Typical usage : live display of output. */

    BUFFERED,                             /* Buffer up SDZ engine output based on configured buffer size from client.
...
struct param_id_sdz_output_config_t
{
    ...
    /**< @h2xmle_description {Configuration to specify transcription output mode depending on usecase requirement.
                              NON_BUFFERED - Output of each SDZ algo process is immediately sent to client.
                              BUFFERED     - Output of each SDZ algo process is buffered and sent to client intermittently.}
   ...
   uint32_t num_bufs;
    /**< @h2xmle_description {Configuration to specify number of output buffers, 0 for NON_BUFFERED output mode.}
         @h2xmle_default     {0}
         @h2xmle_range       {0.. 16} */
}
