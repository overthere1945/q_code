...
#define CSR_HCI_RESET_TRANSPORT_REQ                             ((CsrHciPrim) (0x000C + CSR_HCI_PRIM_DOWNSTREAM_LOWEST))
#define CSR_PERI_REGISTER_EVENT_HANDLER_REQ                     ((CsrHciPrim) (0x000D + CSR_HCI_PRIM_DOWNSTREAM_LOWEST))
#define CSR_PERI_COMMAND_REQ                                    ((CsrHciPrim) (0x000E + CSR_HCI_PRIM_DOWNSTREAM_LOWEST))

#define CSR_HCI_PRIM_DOWNSTREAM_HIGHEST                                       (0x000E + CSR_HCI_PRIM_DOWNSTREAM_LOWEST)
...
#define CSR_HCI_RESET_TRANSPORT_CFM                             ((CsrHciPrim) (0x000B + CSR_HCI_PRIM_UPSTREAM_LOWEST))
#define CSR_PERI_REGISTER_EVENT_HANDLER_CFM                     ((CsrHciPrim) (0x000C + CSR_HCI_PRIM_UPSTREAM_LOWEST))
#define CSR_PERI_EVENT_IND                                      ((CsrHciPrim) (0x000D + CSR_HCI_PRIM_UPSTREAM_LOWEST))

#define CSR_HCI_PRIM_UPSTREAM_HIGHEST                                         (0x000D + CSR_HCI_PRIM_UPSTREAM_LOWEST)
...
{
    CsrHciPrim  type;
    CsrSchedQid queueId;                 /* queueId to which incoming PERI events should be sent */
} CsrPeriRegisterEventHandlerReq;

typedef struct
{
    CsrHciPrim type;
} CsrPeriRegisterEventHandlerCfm;
...
typedef struct
{
    ...
    CsrMblk   *data;                /* Pointer to MBLK data. */
    CsrUint8   event;               /* Event code. 0x00 and 0xFF corresponds to vendor specific events.
                                       Rest of values implies use of HCI events for vendor specific purposes.
                                       For example QCA chips use HCI command complete events as response for vendor specific commands. */
...
typedef struct
{
    ...
    CsrSchedQid queueId;
    CsrUint8 baudId;
} CsrHciResetTransportReq;

typedef struct
{
    CsrHciPrim type;
    CsrBool    restarted;
} CsrHciResetTransportCfm;
