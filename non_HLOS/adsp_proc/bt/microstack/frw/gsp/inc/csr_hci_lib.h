...
#define CsrPeriRegisterEventHandlerReqSend(_queueId){ \
        CsrPeriRegisterEventHandlerReq *__msg; \
        __msg = CsrPeriRegisterEventHandlerReq_struct(_queueId); \
        CsrMsgTransport(CSR_HCI_IFACEQUEUE, CSR_HCI_PRIM, __msg); \
}
...
#define CsrPeriCommandReqSend(_payloadLength, _payload){ \
        CsrPeriCommandReq *__msg; \
        __msg = CsrPeriCommandReq_struct(_payloadLength, _payload); \
        CsrMsgTransport(CSR_HCI_IFACEQUEUE, CSR_HCI_PRIM, __msg); \
}
 ...
 *
 * Description
 *      Restarts the transport with new baudrate, if any.
 *
 * PARAMETERS
 *      qid         :  Task ID
 * -------------------------------------------------------------------- */
CsrHciResetTransportReq *CsrHciResetTransportReq_struct(CsrSchedQid queueId,
                                                        CsrUint8 baudId);
#define CsrHciResetTransportReqSend(_qid, _baudId)                           \
    do                                                                       \
    {                                                                        \
        CsrHciResetTransportReq *__msg;                                      \
        __msg = CsrHciResetTransportReq_struct(_qid, _baudId);               \
        CsrMsgTransport(CSR_HCI_IFACEQUEUE, CSR_HCI_PRIM, __msg);            \
    } while (0)
