...
/* Log Text Handle */
CSR_LOG_TEXT_HANDLE_DEFINE(LHci);

#ifdef CSR_BLUECORE_ONOFF
/* - 2 below because we don't process CSR_HCI_INCOMING_DATA_REQ & CSR_HCI_RESET_TRANSPORT_REQ */
static const CsrHciEventType csrHciMsgHandlers[CSR_HCI_PRIM_DOWNSTREAM_COUNT - 2] =
{
...
static const CsrHciEventType csrHciMsgHandlers[][CSR_HCI_PRIM_DOWNSTREAM_COUNT] =
{
    { /* Jump tabel to handle downstream CSR_HCI messages send from the upper layers                                  */
        ...
        CsrHciUnregisterVendorSpecificEventHandlerReqHandler, /* CSR_HCI_UNREGISTER_VENDOR_SPECIFIC_EVENT_HANDLER_REQ */
        /*  For QCA chip, a HCI_RESET command should be fired 
           after downloading NVM file. */
        CsrHciCommandReqHandler,                              /* CSR_HCI_COMMAND_REQ                                  */
#else
        ...
        CsrHciVendorSpecificCommandReqHandler,                /* CSR_HCI_VENDOR_SPECIFIC_COMMAND_REQ                  */
        NULL,                                                 /* CSR_HCI_INCOMING_DATA_REQ                            */
        CsrHciResetTransportReqHandler,                       /* CSR_HCI_RESET_TRANSPORT_REQ                          */
        CsrPeriRegisterEventHandlerReqHandler,                /* CSR_PERI_REGISTER_EVENT_HANDLER_REQ                  */
        CsrPeriCommandReqHandler,                             /* CSR_PERI_COMMAND_REQ                                 */
    },
    { /* Jump tabel to handle downstream CSR_HCI messages send from the upper layers                                  */
        CsrHciRegisterEventHandlerReqHandler,                 /* CSR_HCI_REGISTER_EVENT_HANDLER_REQ                   */
        CsrHciRegisterAclHandlerReqHandler,                   /* CSR_HCI_REGISTER_ACL_HANDLER_REQ                     */
        ...
        CsrHciUnregisterScoHandlerReqHandler,                 /* CSR_HCI_UNREGISTER_SCO_HANDLER_REQ                   */
#else
        NULL,                                                 /* CSR_HCI_REGISTER_SCO_HANDLER_REQ                     */
        NULL,                                                 /* CSR_HCI_UNREGISTER_SCO_HANDLER_REQ                   */
#endif
        CsrHciRegisterVendorSpecificEventHandlerReqHandler,   /* CSR_HCI_REGISTER_VENDOR_SPECIFIC_EVENT_HANDLER_REQ   */
        CsrHciUnregisterVendorSpecificEventHandlerReqHandler, /* CSR_HCI_UNREGISTER_VENDOR_SPECIFIC_EVENT_HANDLER_REQ */
        CsrHciCommandReqHandler,                              /* CSR_HCI_COMMAND_REQ                                  */
...
void CsrHciInit(void **gash)
{
    ...
    inst->periEventHandler = CSR_SCHED_QID_INVALID;
    ...
    CSR_LOG_TEXT_REGISTER(&LHci, "HCI", 0, NULL);
}
    ...
    switch (event)
    {
        case CSR_TM_BLUECORE_PRIM:
        {
            ...
            if (*primType == CSR_TM_BLUECORE_ACTIVATE_TRANSPORT_CFM)
            {
                ...
                CSR_LOG_TEXT_CONDITIONAL_ERROR(prim->result != CSR_RESULT_SUCCESS, (LHci, 0, "TM BC Activate Failed %d", prim->result));
                CSR_UNUSED(prim);

#ifdef CSR_TARGET_PRODUCT_WEARABLE
                CsrTmBluecoreRegisterHciEventHandlerReqSend(CSR_HCI_IFACEQUEUE, inst->hciEventHandler);
        ...
        {
#ifndef CSR_BLUECORE_ONOFF
            if (inst->recvMsgP && inst->state <= CSR_HCI_STATE_DEACTIVATING)
#endif
            {
                CsrHciPrim *primType = (CsrHciPrim *) inst->recvMsgP;
#ifdef CSR_BLUECORE_ONOFF
                if ((*primType < CSR_HCI_PRIM_DOWNSTREAM_COUNT - 2) && (csrHciMsgHandlers[*primType] != NULL))
                {
                    csrHciMsgHandlers[*primType](inst);
                }
#else
                if ((*primType < CSR_HCI_PRIM_DOWNSTREAM_COUNT) && 
                        (inst->state < CSR_HCI_STATE_DEACTIVATING) && 
                            (csrHciMsgHandlers[inst->state][*primType] != NULL))
                {
