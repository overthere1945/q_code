...
static CsrUint32 resetBaudrate;
...
#define HCI_NOP_PAYLOAD_LEN           6
#define HCI_COMMAND_STATUS_EV_CODE    ((CsrUint8) 0x0F)
#define CMD_STATUS_TOTAL_LEN          ((CsrUint8) 0x04)
#define CMD_STATUS_FIELD              ((CsrUint8) 0x00)
#define CMD_STATUS_NHCP_FIELD         ((CsrUint8) 0x01)
#define CMD_STATUS_OPCODE_FIELD1      ((CsrUint8) 0x00)
#define CMD_STATUS_OPCODE_FIELD2      ((CsrUint8) 0x00)

static void sendSavedNop(CsrTmBlueCoreInstanceData *tmBcInst)
{
    if (!tmBcInst->savedNop)
    {
         CsrUint8* hciNop = CsrPmemAlloc(HCI_NOP_PAYLOAD_LEN);
         hciNop[0] = HCI_COMMAND_STATUS_EV_CODE;
         hciNop[1] = CMD_STATUS_TOTAL_LEN;
         hciNop[2] = CMD_STATUS_FIELD;
         hciNop[3] = CMD_STATUS_NHCP_FIELD;
         hciNop[4] = CMD_STATUS_OPCODE_FIELD1;
         hciNop[5] = CMD_STATUS_OPCODE_FIELD1;
         tmBcInst->savedNop = hciNop;
         tmBcInst->savedNopLength = HCI_NOP_PAYLOAD_LEN;
    }
...
static CsrBool IsHydraChip = FALSE;
...
#endif /* CSR_BLUECORE_PING_INTERVAL */
...
static CsrUint8 *csrTmBlueCoreBuildBootstrapPayload(CsrUint8 **array, CsrUint16 entries, CsrUint16 *payloadLength)
{
    ...
    if (length > 0)
    {
        ...
        {
            len = csrTmBlueCoreGetPayloadLength(array[i]);
            SynMemCpyS(&(bccmd[index]), len, array[i], len);
...
static void csrTmBlueCoreBccmdGetBuildIdCfm(CsrTmBlueCoreInstanceData *tmBcInst)
{
    ...
    if (prim->status == CSR_RESULT_SUCCESS)
    {
        ...
        if (IsHydraChip)
        {
            /* The Bootstrap procedure is not required for Hydra Chips */
            CsrTmBlueCoreActivate(tmBcInst);
            return;
        }
                
        array = CsrTmBlueCoreGetBootstrap(prim->buildId, &entries);
        if (array != NULL)
        {
...
#define BITMASK_CHIP_VERSION_MAJOR_NUMBER 0x00FF
#define AMBER_CHIP_HARDWARE_ID 0x44
#define A7CA_CHIP_HARDWARE_ID 0x60
static void csrTmBlueCoreBccmdGetChipVerCfm(CsrTmBlueCoreInstanceData *tmBcInst)
{
    CsrBccmdGetChipVerCfm *prim = (CsrBccmdGetChipVerCfm *) tmBcInst->recvMsgP;

    if (prim->status == CSR_RESULT_SUCCESS)
    {
        if( ((prim->chipver &  BITMASK_CHIP_VERSION_MAJOR_NUMBER ) == AMBER_CHIP_HARDWARE_ID) || 
            ((prim->chipver & BITMASK_CHIP_VERSION_MAJOR_NUMBER ) == A7CA_CHIP_HARDWARE_ID))
        {
            IsHydraChip = TRUE;
        }
        else
    ...
    {
        CSR_LOG_TEXT_ERROR((CsrTmBluecoreLto, 0,
                            "Failed to retrieve Chip Ver from device (0x%04X)", prim->status));
#ifdef CSR_BLUECORE_ONOFF
        tmBcInst->state = CSR_TM_BLUECORE_STATE_IDLE;
        CsrTmBlueCoreTransportStop(tmBcInst->blueCoreTransportHandle);
        discardSavedNop(tmBcInst);
...
static void csrTmBlueCoreActivateTransportReqHandler(CsrTmBlueCoreInstanceData *tmBcInst)
{
    ...
    switch (tmBcInst->state)
    {
        ...
        {
#ifdef CSR_USE_QCA_CHIP
#ifndef SKIP_PATCH_NVM_BUFFER_DOWNLOAD
            CsrHciRegisterVendorSpecificEventHandlerReqSend(CSR_TM_BLUECORE_IFACEQUEUE,
...
#else /* CSR_HYDRA_SSD */
            if (CsrTmBlueCoreTransportStart(tmBcInst->blueCoreTransportHandle,
                                            tmBcInst->service_handle))
#endif /* CSR_HYDRA_SSD */
            {
                tmBcInst->state = CSR_TM_BLUECORE_STATE_ACTIVATING;
#ifdef SKIP_PATCH_NVM_BUFFER_DOWNLOAD
                CsrTmBlueCoreActivate(tmBcInst);
#endif
#ifdef CSR_BLUECORE_ONOFF
                tmBcInst->numberOfForcedReset = 0;
                tmBcInst->timerId = CsrSchedTimerSet(CSR_BLUECORE_RESET_TIMEOUT,
                                                     resetTimeoutHandler,
                                                     0,
                                                     tmBcInst);
#endif /* CSR_BLUECORE_ONOFF */
...
#ifdef CSR_BLUECORE_ONOFF
                csrTmBlueCoreActivateTransportCfmSend(prim->phandle,
                                                      CSR_RESULT_FAILURE);
#endif /* CSR_BLUECORE_ONOFF */
            }
            break;
...
#endif /* CSR_BLUECORE_ONOFF */
            /* Get the chip version from the chip */
            CsrBccmdGetChipVerReqSend(CSR_TM_BLUECORE_IFACEQUEUE);
...
static void csrTmBlueCoreResetIndHandler(CsrTmBlueCoreInstanceData *tmBcInst)
{
    ...
    if (tmBcInst->state != CSR_TM_BLUECORE_STATE_RESTARTING)
    {
        ...
        tmBcInst->numberOfForcedReset = 0;
        CsrTmBlueCoreTransportDriverRestart(tmBcInst->blueCoreTransportHandle, CSR_UART_BAUD_RATE_IGNORE); /* No change in baudrate */
#else
...
#ifdef CSR_TARGET_PRODUCT_WEARABLE
static void csrTmBluecoreRegisterHciEventHandlerCfmSend(CsrSchedQid phandle)
{
    CsrTmBluecoreRegisterHciEventHandlerCfm *prim;
    prim = CsrPmemAlloc(sizeof(*prim));
    prim->type = CSR_TM_BLUECORE_REGISTER_HCI_EVENT_HANDLER_CFM;
    CsrSchedMessagePut(phandle, CSR_TM_BLUECORE_PRIM, prim);
}

static void csrTmBlueCoreRegisterHciEventHandlerReqHandler(CsrTmBlueCoreInstanceData *tmBcInst)
{
    CsrTmBluecoreRegisterHciEventHandlerReq *p = tmBcInst->recvMsgP;
    tmBcInst->nopHandler = p->hciEvtHandler;
    csrTmBluecoreRegisterHciEventHandlerCfmSend(p->phandle);
    sendSavedNop(tmBcInst);
}
#endif /* CSR_TARGET_PRODUCT_WEARABLE */
...
void CsrTmBlueCoreTmBlueCoreMsgHandler(CsrTmBlueCoreInstanceData *tmBcInst)
{
    ...
    switch (*primType)
    {
        ...
        case CSR_TM_BLUECORE_PATCH_SEGMENT_DOWNLOAD_REQ:
...
#endif
#ifdef CSR_TARGET_PRODUCT_WEARABLE
        case CSR_TM_BLUECORE_REGISTER_HCI_EVENT_HANDLER_REQ:
            csrTmBlueCoreRegisterHciEventHandlerReqHandler(tmBcInst);
            break;
#endif /* CSR_TARGET_PRODUCT_WEARABLE */
        default:
...
void CsrTmBlueCoreBccmdMsgHandler(CsrTmBlueCoreInstanceData *tmBcInst)
{
    ...
    switch (*primType)
    {
        ...
        case CSR_BCCMD_GET_CHIP_VER_CFM:
            csrTmBlueCoreBccmdGetChipVerCfm(tmBcInst);
            break;
#if defined(CSR_CHIP_MANAGER_ENABLE) || defined(CSR_BLUECORE_ONOFF) && defined(CSR_BLUECORE_PING_INTERVAL)
        case CSR_BCCMD_CFM:
...
void CsrTmBlueCoreSsdMsgHandler(CsrTmBlueCoreInstanceData *tmBcInst)
{
    ...
    switch (*primType)
    {
        case CSR_SSD_SERVICE_NOTIFICATION:
        {
            CsrSsdNotifyService* ssd_service = (CsrSsdNotifyService *) tmBcInst->recvMsgP;
            tmBcInst->service_handle = ssd_service->ssdinst;
            break;
        }
        default:
        CsrGeneralException(CsrTmBluecoreLto, 0, CSR_HCI_PRIM, *primType, tmBcInst->state, NULL);
            break;
    }
}
#endif /* CSR_HYDRA_SSD */
