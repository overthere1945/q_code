...
CsrMblk *CsrMblkMallocCreate(void **block_ptr, CsrMblkSize block_size)
{
    ...
    *block_ptr = NULL;
    mblk = CsrMblkPmallocNew(TRUE, 0, block_size, NULL);
    if (mblk)
    {
...
static CsrMblkSize mblk_pmalloc_read_head(CsrMblk *mblk, void *data, CsrMblkSize *data_size_ref)
{
    ...
    if (data != NULL)
    {
        SynMemCpyS(data, data_size, mblk->u.pmalloc.data, data_size);
    }
...
static CsrMblkSize mblk_pmalloc_read_tail(CsrMblk *mblk, void *data, CsrMblkSize *data_size_ref)
{
    ...
    if (data != NULL)
    {
        SynMemCpyS(data, data_size, mblk->u.pmalloc.data + mblk->data_size, data_size);
    }
    ...
    CsrUint8 *mem_ptr)
{
    ...
    SynMemCpyS(mem_ptr, mblk_size, mblk->u.pmalloc.data + mblk_offset, mblk_size);
