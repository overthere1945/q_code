...
/*! Procedure failed */
#define ATT_RESULT_OPERATION_FAILED             0xff86
/*! EATT is already registered */
#define ATT_RESULT_ALREADY_REGISTERED           0xff87
/*! Error returned when application is sending a primitive which violates MTU */
#define ATT_RESULT_MTU_VIOLATION                0xff88
/*! \} */


/*! Result code returned as part of enhanced credit based reconfigure response 
    Some of the locally generated error codes are also valid for reconfiguration
    e.g ATT_RESULT_INVALID_CID, ATT_RESULT_OPERATION_NOT_ALLOWED,
    ATT_RESULT_INVALID_MTU, ATT_RECONFIG_INVALID_MTU
 */
#define ATT_RECONFIG_SUCCESSFUL          0x0000
/*! Reconfiguration failed - reduction in size of MTU not allowed */
#define ATT_RECONFIG_INVALID_MTU         0x0001
/*! Reconfiguration failed - reduction in MPS not allowed for
    more than one channel at a time */
#define ATT_RECONFIG_INVALID_MPS         0x0002
/*! Reconfiguration failed - one or more CID invalid */
#define ATT_RECONFIG_INVALID_CID         0x0003
/*! Reconfiguration failed - other unacceptable parameters */
#define ATT_RECONFIG_UNACCEPTABLE_PARAM  0x0004
...
typedef enum att_prim_tag
{
    ...
    ENUM_ATT_READ_MULTI_VAR_REQ,
    ENUM_ATT_READ_MULTI_VAR_RSP,
    ENUM_ATT_MULTI_HANDLE_VALUE_NTF_REQ,
    ENUM_ATT_ENHANCED_REGISTER_REQ,
    ENUM_ATT_ENHANCED_UNREGISTER_REQ,
    ENUM_ATT_ENHANCED_CONNECT_REQ,
    ENUM_ATT_ENHANCED_CONNECT_RSP,
    ENUM_ATT_ENHANCED_RECONFIGURE_RSP,
    ...
    ENUM_ATT_READ_MULTI_VAR_CFM,
    ENUM_ATT_READ_MULTI_VAR_IND,
    ENUM_ATT_MULTI_HANDLE_VALUE_NTF_CFM,
    ENUM_ATT_MULTI_HANDLE_VALUE_NTF_IND,
    ENUM_ATT_ENHANCED_REGISTER_CFM,
    ENUM_ATT_ENHANCED_UNREGISTER_CFM,
    ENUM_ATT_ENHANCED_CONNECT_CFM,
    ENUM_ATT_ENHANCED_CONNECT_IND,
    ENUM_ATT_ENHANCED_RECONFIGURE_IND,
...
#define ATT_READ_MULTI_VAR_REQ  ((att_prim_t)ENUM_ATT_READ_MULTI_VAR_REQ)
#define ATT_READ_MULTI_VAR_RSP  ((att_prim_t)ENUM_ATT_READ_MULTI_VAR_RSP)
#define ATT_MULTI_HANDLE_VALUE_NTF_REQ  ((att_prim_t)ENUM_ATT_MULTI_HANDLE_VALUE_NTF_REQ)
#define ATT_ENHANCED_REGISTER_REQ   ((att_prim_t)ENUM_ATT_ENHANCED_REGISTER_REQ)
#define ATT_ENHANCED_UNREGISTER_REQ ((att_prim_t)ENUM_ATT_ENHANCED_UNREGISTER_REQ)
#define ATT_ENHANCED_CONNECT_REQ    ((att_prim_t)ENUM_ATT_ENHANCED_CONNECT_REQ)
#define ATT_ENHANCED_CONNECT_RSP    ((att_prim_t)ENUM_ATT_ENHANCED_CONNECT_RSP)
#define ATT_ENHANCED_RECONFIGURE_RSP ((att_prim_t)ENUM_ATT_ENHANCED_RECONFIGURE_RSP)
...
#define ATT_READ_MULTI_VAR_CFM  ((att_prim_t)ENUM_ATT_READ_MULTI_VAR_CFM)
#define ATT_READ_MULTI_VAR_IND  ((att_prim_t)ENUM_ATT_READ_MULTI_VAR_IND)
#define ATT_MULTI_HANDLE_VALUE_NTF_CFM ((att_prim_t)ENUM_ATT_MULTI_HANDLE_VALUE_NTF_CFM)
#define ATT_MULTI_HANDLE_VALUE_NTF_IND ((att_prim_t)ENUM_ATT_MULTI_HANDLE_VALUE_NTF_IND)
#define ATT_ENHANCED_REGISTER_CFM   ((att_prim_t)ENUM_ATT_ENHANCED_REGISTER_CFM)
#define ATT_ENHANCED_UNREGISTER_CFM ((att_prim_t)ENUM_ATT_ENHANCED_UNREGISTER_CFM)
#define ATT_ENHANCED_CONNECT_CFM    ((att_prim_t)ENUM_ATT_ENHANCED_CONNECT_CFM)
#define ATT_ENHANCED_CONNECT_IND    ((att_prim_t)ENUM_ATT_ENHANCED_CONNECT_IND)
#define ATT_ENHANCED_RECONFIGURE_IND ((att_prim_t)ENUM_ATT_ENHANCED_RECONFIGURE_IND)

#define ATT_DEBUG_IND           ((att_prim_t)ENUM_ATT_DEBUG_IND)
#define ATT_ERROR_IND           ((att_prim_t)ENUM_ATT_ERROR_IND)
/* \} */

/*! \brief Types for ATT */
typedef uint16_t                att_prim_t;
typedef uint16_t                att_result_t;
typedef uint16_t                att_rcfg_result_t;

/*! \brief Registration flags (bit map) used in ATT_ENHANCED_REGISTER_REQ
    Atleast one flag either ATT_REG_FLAG_EATT_BREDR_SUPPORT or 
    ATT_REG_FLAG_EATT_LE_SUPPORT should be set*/

#define ATT_REG_FLAG_EATT_BREDR_SUPPORT         0x00000001
#define ATT_REG_FLAG_EATT_LE_SUPPORT            0x00000002

typedef uint32_t                att_reg_flags_t;

/*!
    \brief Mode for the ATT connection.
           Use ATT_MODE_EATT for EATT operations, other values are reserved for future use
           Application request with values other than ATT_MODE_EATT will be rejected.
*/
#define ATT_MODE_EATT           0x0001
typedef uint16_t                att_mode_t;

/*!
    \brief Priority for EATT connection.
           bit mask - 2 bits per each bearer 0- high, 1-medium, 2 - low
*/
typedef uint16_t                att_priority_t;
...
typedef struct
{
    ...
    phandle_t           phandle;        /*!< Destination phandle */
#ifndef QBLUESTACK_RFCLI
    att_result_t        result;         /*!< Result code - uses ATT_RESULT range */
#else
    uint16_t            result;         /*!< Result code - uses ATT_RESULT range */
#endif
} ATT_REGISTER_CFM_T;
...
#else
    uint16_t            flags;          /*!< L2CAP connection flags */
#endif
} ATT_CONNECT_REQ_T;
...
#else
    uint16_t            result;         /*!< Result code - uses L2CA_CONNECT range */
#endif
} ATT_CONNECT_CFM_T;
...
/*! \brief Indication for ATT connection
    ...
    Note: This prim occurs when application has registered with ATT using 
          ATT_REGISTER_REQ and when a successful LE connection is established 
          with peer device.
 */
typedef struct
{
...
#else
    uint16_t            response;       /*!< Response from the application */
#endif
} ATT_CONNECT_RSP_T;

/*! \brief Sends disconnect request on ATT bearer

     For legacy ATT bearer:
     Sends request to force disconnect the ATT bearer on LE transport.
     ...
     For enhanced ATT(EATT) bearer:
     This request will simply bring down the L2CAP channel associated with
     the ATT bearer.
...
#else
    uint16_t            reason;         /*!< Reason code - uses L2CA_DISCONNECT range */
#endif
...
/*!  \brief Sends disconnect request on ATT bearer
    ...
    If the ATT bearer is for LE fixed channel, then this prim will disconnect 
    the LE ACL link when there are no other clients(eg LE L2CAP COC) or
    ...
    For enhanced ATT(EATT) bearer:
    This will bring down the L2CAP channel associated with the EATT bearer.
    flags is not applicable in this case.
...
/*!
    ...
    NOTE: This primitive is applicable only for LE fixed channel.
...
/*!
    \brief Register the local MTU, to be negotiated, for all future 
     BR/EDR legacy ATT bearers (both incoming and outgoing).
...
/*!
    ...
    Signed Write Command shall not be used for enhanced ATT(EATT) bearer and
    responded back with ATT_RESULT_SIGNED_DISALLOWED error code.
...
/*!
    ...
    \msc
        GATT,ATT,Air,"Remote Device";
        
                            --- [ label = "Prepare Write Req on Bearer A" ];
        
        "Remote Device"->Air    [ label = "ATT_PREPARE_WRITE_REQ(CID=A, Handle=X)" ];
        Air->ATT                [ label = "ATT_PREPARE_WRITE_REQ(CID=A, Handle=X)" ];
        ATT->GATT               [ label = "ATT_ACCESS_IND(CID=A, Handle=X, ATT_ACCESS_PERMISSION)" ];
        GATT->ATT               [ label = "ATT_ACCESS_RSP(CID=A, result)" ];
        ATT->Air                [ label = "ATT_PREPARE_WRITE_RSP(CID=A, Handle=X)" ];
        Air->"Remote Device"    [ label = "ATT_PREPARE_WRITE_RSP(CID=A, Handle=X)" ];
        "Remote Device"->Air    [ label = "ATT_PREPARE_WRITE_REQ(CID=A, Handle=X)" ];
        Air->ATT                [ label = "ATT_PREPARE_WRITE_REQ(CID=A, Handle=X)" ];
        ATT->Air                [ label = "ATT_PREPARE_WRITE_RSP(CID=A, Handle=X)" ];
        Air->"Remote Device"    [ label = "ATT_PREPARE_WRITE_RSP(CID=A, Handle=X)" ];
        
                            --- [ label = "Prepare Write Req on Bearer B"];
        
        "Remote Device"->Air    [ label = "ATT_PREPARE_WRITE_REQ(CID=B, Handle=Y)" ];
        Air->ATT                [ label = "ATT_PREPARE_WRITE_REQ(CID=B, Handle=Y)" ];
        ATT->GATT               [ label = "ATT_ACCESS_IND(CID=B, Handle=Y, ATT_ACCESS_PERMISSION)" ];
        GATT->ATT               [ label = "ATT_ACCESS_RSP(CID=B, result)" ];
        ATT->Air                [ label = "ATT_PREPARE_WRITE_RSP(CID=B, Handle=Y)" ];
        Air->"Remote Device"    [ label = "ATT_PREPARE_WRITE_RSP(CID=B, Handle=Y)" ];
        "Remote Device"->Air    [ label = "ATT_PREPARE_WRITE_REQ(CID=B, Handle=Y)" ];
        Air->ATT                [ label = "ATT_PREPARE_WRITE_REQ(CID=B, Handle=Y)" ];
        ATT->Air                [ label = "ATT_PREPARE_WRITE_RSP(CID=B, Handle=Y)" ];
        Air->"Remote Device"    [ label = "ATT_PREPARE_WRITE_RSP(CID=B, Handle=Y)" ];
        
                            --- [ label = "Execute Write Req on Bearer A"];
        
        "Remote Device"->Air    [ label = "ATT_EXECUTE_WRITE_REQ(CID=A)"];
        Air->ATT                [ label = "ATT_EXECUTE_WRITE_REQ(CID=A, ATT_EXECUTE_WRITE)" ];
        ATT->GATT               [ label = "ATT_ACCESS_IND(CID=A, Handle=X, ATT_ACCESS_WRITE)" ];
        ATT->GATT               [ label = "ATT_ACCESS_IND(CID=A, Handle=X, ATT_ACCESS_WRITE)" ];
        ATT->GATT               [ label = "ATT_ACCESS_IND(CID=B, Handle=Y, ATT_ACCESS_WRITE)" ];
        ATT->GATT               [ label = "ATT_ACCESS_IND(CID=B, Handle=Y, ATT_ACCESS_WRITE)" ];
        ATT->GATT               [ label = "ATT_ACCESS_IND(CID=A, Handle=0, ATT_ACCESS_WRITE_COMPLETE)" ];
        GATT->ATT               [ label = "ATT_ACCESS_RSP(CID=A, result)" ];
        ATT->Air                [ label = "ATT_EXECUTE_WRITE_RSP(CID=A)" ];
        Air->"Remote Device"    [ label = "ATT_EXECUTE_WRITE_RSP(CID=A)" ];
    \endmsc
*/
typedef struct 
{
...
/*! Indication of write cancelled, rsp mandatory.

    Inline with the specification, for execute-write-req (with flags=cancel), 
    ATT shall send across access-indication(write-cancelled) with the bearer 
    over which the cancel request was received. The application shall consider 
    this indication/cancellation for all bearers(on the LE-ACL link) on which 
    access indication for prepare-write was earlier notified for write operations 
    to proceed.
*/
#define ATT_ACCESS_WRITE_CANCELLED   0x0100

/*! This flag is set only for write command, applicable only when GATT offload 
    is enabled */
#define ATT_ACCESS_WRITE_CMD   0x0200
...
/*!
    ...
    NOTE: 
    1) Application is expected to store change aware information
    persistently for bonded device, and inform ATT module using
    ATT_ADD_ROBUST_CACHING_REQ on every power cycle initialization.

    2) Change aware is device specific and not bearer specific.
    For single bearer : Valid CID is included
    For multiple bearers : Single ATT_CHANGE_AWARE_IND is sent with 
    cid set to 0.
 */
typedef struct
{
...
typedef uint16_t        att_error_t;

#define ATT_ERROR_RESPONSE_TIMEOUT             ((att_error_t) 0x0000)
#define ATT_ERROR_PROTOCOL_VIOLATION           ((att_error_t) 0x0001)
#define ATT_ERROR_DB_OUT_OF_SYNC               ((att_error_t) 0x0002)
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_ERROR_IND */
    phandle_t           phandle;        /*!< Destination phandle */
    uint16_t            cid;
    att_error_t         error;
} ATT_ERROR_IND_T;
...
/*!
    \brief Send Read Multiple Variable Request to the server

    This is a client role primitive and it is used to request the 
    server to read two or more values of a set of attributes that
    have a variable or unknown value length and return their values.

    \return ATT_READ_MULTI_VAR_CFM_T
*/
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_READ_MULTI_VAR_REQ */
    phandle_t           phandle;        /*!< Destination phandle */
    uint16_t            cid;
    uint16_t            size_handles;   /*!< Number of handles */
    uint16_t            *handles;       /*!< A set of two or more attribute handles */
} ATT_READ_MULTI_VAR_REQ_T;

/*!
    \brief Confirmation of Read Multiple Variable Request

    For successful read operations, response contains the lengths and values
    of the attributes that have been read.
    Value is concatenation of Length Value Tuples for each of the attribute
    handles in the request in the order that they were requested.
    If the Length Value Tuple List is longer than (ATT_MTU-1), then only the
    first (ATT_MTU-1) octets shall be included in this response.

    Format of the Length Value Tuple : Length (2 bytes), Value (Length bytes). 
    Little-endian format shall be used for Length field. endianess for value is
    defined by GATT/higher layers.

    If Error response is sent from remote device, result contains the error code
    and error_handle contains the attribute handle in error.

    Note: If a Length Value Tuple is truncated, then the amount of Attribute 
    Value will be less than the value of the Value Length field. The client 
    must therefore not use the Value Length to determine the amount of the 
    Attribute Value actually included in the PDU.
*/
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_READ_MULTI_VAR_CFM */
    phandle_t           phandle;        /*!< Destination phandle */
    uint16_t            cid;
    att_result_t        result;         /*!< Result code - uses ATT_RESULT range */
    uint16_t            error_handle;   /*!< handle of the first attribute in error*/
    uint16_t            size_value;     /*!< Length of the value */
    uint8_t             *value;         /*!< Concatenated length value of requested attributes */
} ATT_READ_MULTI_VAR_CFM_T;

/*!
    \brief Indication sent on receiving Read Multiple Variable Request

    This primitive will be invoked if IRQ is set for all the handles in 
    Read Multiple Variable Request, otherwise ATT_ACCESS_IND is used
    for each attribute.
*/
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_READ_MULTI_VAR_IND */
    phandle_t           phandle;        /*!< Destination phandle */
    uint16_t            cid;
    uint16_t            flags;          /*!< Flags - uses ATT_ACCESS range */
    uint16_t            size_handles;   /*!< Number of handles */
    uint16_t            *handles;       /*!< A set of two or more attribute handles */
} ATT_READ_MULTI_VAR_IND_T;

/*!
    \brief This is a response to Read Multiple Variable Request

    The result shall be set to ATT_RESULT_SUCCESS, if size and value are valid.
    Value is concatenation of Length Value Tuples for each of the attribute
    handles in the request in the order that they were requested.
    If the Length Value Tuple List is longer than (ATT_MTU-1), then only the
    first (ATT_MTU-1) octets shall be included in this response.
    ...
    If the result is not ATT_RESULT_SUCCESS, error_handle contains the handle 
    of the first attribute causing the error.
*/
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_READ_MULTI_VAR_RSP */
    phandle_t           phandle;        /*!< Destination phandle */
    uint16_t            cid;
    att_result_t        result;         /*!< Result code - uses ATT_RESULT range */
    uint16_t            error_handle;   /*!< handle of the first attribute in error*/
    uint16_t            size_value;     /*!< Length of the value */
    uint8_t             *value;         /*!< A list of Length Value Tuples */
} ATT_READ_MULTI_VAR_RSP_T;

/*!
    \brief Send Multiple Handle Value Notification(MHVN) to the client.

    Application can use this primitive to send notification of two or more 
    attributes' values at any time. Value includes the concatenation of 
    Handle Length Value Tuples for each of the attributes being notified.

    Format of the Handle Length Value Tuple : Handle (2 bytes), 
    Length (2 bytes), Value (Length bytes). 
    Little-endian format shall be used for Handle and Length.
    endianess for value is defined by GATT/higher layers.

    A confirmation message would be generated to carry the same context
    passed in this API. Confirmation would be generated once the packet has 
    been queued to be sent to the remote device.

    The notification can be sent at any time and the client does not
    confirm receiving it.

    Flood defense is also applicable for MHVN, so application is not expected
    to flood with MHVN on prim interface.

    \return ATT_MULTI_HANDLE_VALUE_NTF_CFM_T
*/
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_MULTI_HANDLE_VALUE_NTF_REQ */
    phandle_t           phandle;        /*!< Destination phandle */
    context_t           context;        /*!< Application context returned in CFM */
    uint16_t            cid;
    uint16_t            size_value;     /*!< Length of the value */
    uint8_t             *value;         /*!< List of Handle Length Value Tuples */
} ATT_MULTI_HANDLE_VALUE_NTF_REQ_T;


/*!
    \brief Confirmation of Multiple Handle Value Notification along with CID and 
    application context received in the ATT_MULTI_HANDLE_VALUE_NTF_REQ_T.

    This event shall be generated to indicate ATT_RESULT_SUCCESS if notification
    PDU is lined up to be sent to the peer device or an error code for failure cases.
*/
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_MULTI_HANDLE_VALUE_NTF_CFM */
    phandle_t           phandle;        /*!< Destination phandle */
    context_t           context;        /*!< Application context returned in CFM */
    uint16_t            cid;
    att_result_t        result;         /*!< Result code - uses ATT_RESULT range */
} ATT_MULTI_HANDLE_VALUE_NTF_CFM_T;

/*!
    \brief Multiple Handle Value Notification from the server.

    The Handle Length Value Tuple List shall be a concatenation
    of Handle Length Value Tuples for each of the attributes being notified.
...
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_MULTI_HANDLE_VALUE_NTF_IND */
    phandle_t           phandle;        /*!< Destination phandle */
    uint16_t            cid;
    uint16_t            size_value;     /*!< Length of the value */
    uint8_t             *value;         /*!< Handle Length Value tuple list */
} ATT_MULTI_HANDLE_VALUE_NTF_IND_T;


#define EATT_MAX_CIDS (5)

/*! \brief Register with ATT subsystem for Enhanced ATT(EATT) support

    Application shall issue this request only if application supports EATT.
    EATT support shall be registered before performing any ATT operations.

    Application with EATT support shall only use ATT_ENHANCED_REGISTER_REQ 
    and not the ATT_REGISTER_REQ.

    NOTE:
    1. Application shall use ATT_CONNECT_XXX primitives for establishing
       legacy ATT bearer.
    2. Application shall use ATT_ENHANCED_CONNECT_XXX primitives for establishing
       EATT bearer(s).
*/
typedef struct
{
    att_prim_t          type;          /*!< Always ATT_ENHANCED_REGISTER_REQ */
    phandle_t           phandle;       /*!< Destination phandle */
    context_t           context;       /*!< Application context returned in CFM */
    att_reg_flags_t     flags;         /*!< bit field - for EATT */
} ATT_ENHANCED_REGISTER_REQ_T;


/*! \brief Enhanced ATT register confirmation */
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_ENHANCED_REGISTER_CFM */
    phandle_t           phandle;        /*!< Destination phandle */
    context_t           context;        /*!< Application context */
    att_result_t        result;         /*!< Result code - uses ATT_RESULT range */
} ATT_ENHANCED_REGISTER_CFM_T;


/*! \brief Unregister Enhanced ATT registration */
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_ENHANCED_UNREGISTER_REQ */
    phandle_t           phandle;        /*!< Destination phandle */
    context_t           context;        /*!< Application context returned in CFM */
} ATT_ENHANCED_UNREGISTER_REQ_T;


/*! \brief Enhanced ATT unregister confirmation */
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_ENHANCED_UNREGISTER_CFM */
    phandle_t           phandle;        /*!< Destination phandle */
    context_t           context;        /*!< Application context */
    att_result_t        result;         /*!< Result code - uses ATT_RESULT range */
} ATT_ENHANCED_UNREGISTER_CFM_T;


/*!
    \brief Request to establish ATT connection between a client and a server.

    Mode = ATT_MODE_EATT can be used to establish EATT bearer(s).
    * Application to use this primitive only if remote device supports EATT.
    * Application can request to create multiple EATT bearers (up to 5).
    * MTU requested and initial credits are applicable for all the requested bearers.

    This will return two ATT_ENHANCED_CONNECT_CFM, one intermediate and a final CFM.
    * Intermediate CFM(INITIATING) returns the list of local CID(s) based on num of
      requested bearers.
    * Final CFM is sent when a response is sent from remote device. This contains
      result and successful/failed CID list.

    NOTE : 
    1. When application has initiated the connection, it is expected to wait for final
       CFM before initiating a REQ for same device.
    2. Since number of bearers has impact on platform memory, application is expected
       to create only the necessary CID list based on its needs and memory requirements.

    \return ATT_ENHANCED_CONNECT_CFM_T

    App                        ATT                       ATT                          App
    |--ATT_ENHANCED_CONNECT_REQ->|                        |                            |
    |<-ATT_ENHANCED_CONNECT_CFM--|                        |                            |
    |  (initiating, CID list)    |                        |                            |
    |                            | (L2CAP CONNECT PDU)    |                            |
    |                            |                        |--ATT_ENHANCED_CONNECT_IND->|
    |                            |                        |<-ATT_ENHANCED_CONNECT_RSP--|
    |                            | (L2CAP CONNECT RSP PDU)|                            |
    |<-ATT_ENHANCED_CONNECT_CFM--|                        |--ATT_ENHANCED_CONNECT_CFM->|
    |  (result, CID list, MTU)   |                        |   (result, CID list, MTU)  |
*/

typedef struct
{
    att_prim_t          type;            /*!< Always ATT_ENHANCED_CONNECT_REQ */
    phandle_t           phandle;         /*!< Destination phandle */
    TP_BD_ADDR_T        tp_addrt;        /*!< Address of remote device including type and transport */
    l2ca_conflags_t     flags;           /*!< L2CAP connection flags */
    att_mode_t          mode;            /*!< ATT mode for connection */
    uint16_t            num_bearers;     /*!< number of EATT bearers requested, max 5*/
    uint16_t            mtu;             /*!< MTU requested, min 64 for EATT*/
    uint16_t            initial_credits; /*!< Initial credits in terms of MTU*/
    att_priority_t      priority;        /*!< priority for each bearer */
} ATT_ENHANCED_CONNECT_REQ_T;

/*!
    \brief Confirmation of ATT_ENHANCED_CONNECT_REQ

    Successfully created CID(s) is included in num_cid_success/cid_success,
    and failed CID(s) in num_cid_fail/cid_fail.

    For outgoing connection :
    * Result = INITIATING contains cid_success

    Both outgoing and incoming connection:
    * Result = L2CA_ALL_CONN_SUCCESSFUL contains cid_success.
    * Result = L2CA_SOME_CONN_REFUSED_XXX range, For multiple CIDs contains both 
      cid_success and cid_fail. For single CID contains cid_fail.
    * Any other result value, contains cid_fail.

    Data transfer is applicable only for successful CID list, final MTU is indicated 
    to the application. For failed CID list, application can free any resource 
    if already allocated and no explicit disconnect is required.
*/
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_ENHANCED_CONNECT_CFM */
    phandle_t           phandle;        /*!< Destination phandle */
    TP_BD_ADDR_T        tp_addrt;       /*!< Address of remote device including type and transport */
    l2ca_conflags_t     flags;          /*!< L2CAP connection flags */
    att_mode_t          mode;           /*!< ATT mode for connection */
    uint16_t            num_cid_success;/*!< Number of CIDs - successfully created */
    uint16_t            cid_success[EATT_MAX_CIDS]; /*!< List of local CID(s) successfully created */
    uint16_t            num_cid_fail;   /*!< Number of CIDs could not be established */
    uint16_t            cid_fail[EATT_MAX_CIDS];    /*!< List of local CID(s) failed */
    uint16_t            mtu;            /*!< Final MTU for successful connection(s) */
    l2ca_conn_result_t  result;         /*!< Result code - uses L2CA_RES_TABLE and 
                                             L2CA_ENHANCED_CREDIT_CONRES_TABLE range */
} ATT_ENHANCED_CONNECT_CFM_T;


/*! \brief Indication for incoming EATT connection

    Remote device can request to create multiple EATT bearers, this contains 
    the list of CIDs requested by remote device.

    Note this prim solicits a response - ATT_ENHANCED_CONNECT_RSP
 */
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_ENHANCED_CONNECT_IND */
    phandle_t           phandle;        /*!< Destination phandle */
    TP_BD_ADDR_T        tp_addrt;       /*!< Address of remote device including type and transport */
    l2ca_conflags_t     flags;          /*!< L2CAP connection flags */
    att_mode_t          mode;           /*!< ATT mode for connection */
    uint16_t            identifier;     /*!< Used to identify the signal, 
                                             same identifier is sent back in RSP */
    uint16_t            mtu;            /*!< 0 if mode is ATT_MODE_EATT, 
                                             final MTU is received in CFM*/
    uint16_t            num_cid;        /*!< number of CIDs */
    uint16_t            cid[EATT_MAX_CIDS]; /*!< requested local CIDs */
} ATT_ENHANCED_CONNECT_IND_T;

/*! \brief Response to ATT_ENHANCED_CONNECT_IND

    Application can respond to an incoming EATT connect request with
    * result = L2CA_ALL_CONN_SUCCESSFUL to inform all the requested CIDs 
      are successfully created. num_cid_success shall be set to number of CIDs in IND.
    * result = L2CA_SOME_CONN_REFUSED_XXX range to accept partial CID list.
      Application to set the num_cid_success, only the first num_cid_success CIDs 
      received in IND shall be accepted and others shall be rejected.
    * Any other result code to reject the request.num_cid_success is set to 0.
 */
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_ENHANCED_CONNECT_RSP */
    phandle_t           phandle;        /*!< Destination phandle */
    uint16_t            identifier;     /*!< Used to identify the signal, same as in IND */
    uint16_t            num_cid_success;/*!< Number of CIDs - accepted */
    l2ca_conn_result_t  response;       /*!< Response from the application - 
                                             uses L2CA_ENHANCED_CREDIT_CONRES_TABLE range*/
    uint16_t            mtu;            /*!< MTU requested from application */
    uint16_t            initial_credits;/*!< Initial credits in terms of MTU */
    att_priority_t      priority;       /*!< priority for each bearer */
} ATT_ENHANCED_CONNECT_RSP_T;


/*! \brief Indication for MTU reconfiguration on EATT bearer

    This is applicable only for EATT bearer.
    Remote device can reconfigure MTU for multiple EATT bearers indicated in num_cid.

    This primitive solicits a response - ATT_ENHANCED_RECONFIGURE_RSP.
    NOTE : Application not supporting MTU reconfiguration,can set the EATT MTU to 64
    which is minimum MTU value.
 */
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_ENHANCED_RECONFIGURE_IND */
    phandle_t           phandle;        /*!< Destination phandle */
    uint16_t            identifier;     /*!< Used to identify the signal, 
                                             same identifier is sent back in RSP */
    uint16_t            num_cid;        /*!< number of CIDs */
    uint16_t            cid[EATT_MAX_CIDS]; /*!< local CIDs */
    uint16_t            mtu[EATT_MAX_CIDS]; /*!< MTU for the connection */
} ATT_ENHANCED_RECONFIGURE_IND_T;

/*!
    \brief Respond to ATT_ENHANCED_RECONFIGURE_IND.

    Only when a successful response is sent, MTU for all the received CID(s)
    is updated

    NOTE: Initiating MTU reconfiguration primitive is not currently available
    to application, mainly due to
    1. Currently there are no valid usecase from application.
    2. Since ATT MTU is symmetric, reconfig may not always result in MTU change.
    3. There is also an erratam - 12971 w.r.t MPS reconfiguration in L2CAP.
*/
typedef struct
{
    att_prim_t          type;           /*!< Always ATT_ENHANCED_RECONFIGURE_RSP */
    phandle_t           phandle;        /*!< Destination phandle */
    uint16_t            identifier;     /*!< identifier - same as in IND */
    uint16_t            num_cid;        /*!< number of CIDs - same as in IND */
    uint16_t            cid[EATT_MAX_CIDS]; /*!< local CIDs - same as in IND */
    att_rcfg_result_t   result;         /*!< Result code - uses ATT_RECONFIG range */
} ATT_ENHANCED_RECONFIGURE_RSP_T;
...
typedef union
{
    ...
    ATT_READ_MULTI_VAR_REQ_T    att_read_multi_var_req;
    ATT_READ_MULTI_VAR_RSP_T    att_read_multi_var_rsp;
    ATT_ENHANCED_REGISTER_REQ_T     att_enhanced_register_req;
    ATT_ENHANCED_UNREGISTER_REQ_T   att_enhanced_unregister_req;
    ATT_ENHANCED_CONNECT_REQ_T      att_enhanced_connect_req;
    ATT_ENHANCED_CONNECT_RSP_T      att_enhanced_connect_rsp;
    ATT_ENHANCED_RECONFIGURE_RSP_T  att_enhanced_reconfigure_rsp;
    ATT_MULTI_HANDLE_VALUE_NTF_REQ_T att_multi_handle_value_ntf_req;
    ...
    ATT_READ_MULTI_VAR_CFM_T      att_read_multi_var_cfm;
    ATT_READ_MULTI_VAR_IND_T      att_read_multi_var_ind;
    ATT_ENHANCED_REGISTER_CFM_T       att_enhanced_register_cfm;
    ATT_ENHANCED_UNREGISTER_CFM_T     att_enhanced_unregister_cfm;
    ATT_ENHANCED_CONNECT_CFM_T        att_enhanced_connect_cfm;
    ATT_ENHANCED_CONNECT_IND_T        att_enhanced_connect_ind;
    ATT_ENHANCED_RECONFIGURE_IND_T    att_enhanced_reconfigure_ind;
    ATT_MULTI_HANDLE_VALUE_NTF_CFM_T  att_multi_handle_value_ntf_cfm;
    ATT_MULTI_HANDLE_VALUE_NTF_IND_T  att_multi_handle_value_ntf_ind;
