...
typedef enum dm_prim_tag
{
    ...
    ENUM_DM_ULP_EXT_SET_CONNECTION_PARAMS_REQ,
    ...
    ENUM_DM_ULP_EXT_SET_CONNECTION_PARAMS_CFM,
...
#ifndef EXCLUDE_VSC
#define DM_VS_REGISTER_REQ                                  ((dm_prim_t)ENUM_DM_VS_REGISTER_REQ)
#define DM_VS_REGISTER_CFM                                  ((dm_prim_t)ENUM_DM_VS_REGISTER_CFM)
#define DM_VS_COMMAND_REQ                                   ((dm_prim_t)ENUM_DM_VS_COMMAND_REQ)
#define DM_VS_COMMAND_CFM                                   ((dm_prim_t)ENUM_DM_VS_COMMAND_CFM)
#define DM_VS_EVENT_IND                                     ((dm_prim_t)ENUM_DM_VS_EVENT_IND)
#endif /* EXCLUDE_VSC */
...
#define DM_ULP_EXT_SET_CONNECTION_PARAMS_REQ         ((dm_prim_t)(ENUM_DM_ULP_EXT_SET_CONNECTION_PARAMS_REQ))
#define DM_ULP_EXT_SET_CONNECTION_PARAMS_CFM         ((dm_prim_t)(ENUM_DM_ULP_EXT_SET_CONNECTION_PARAMS_CFM))
...
#define BT_VERSION_MAX              BT_VERSION_5p4
...
/*---------------------------------------------------------------------------*
 *  PURPOSE
 *  Provides an API to allow an application to able to specify the connection 
 *  interval, slave latency, etc, that the local device uses when creating 
 *  extended connections in the central role.
 *
 *  Note: adv_handle and subevent are for future use for  
 *  Periodic Advertising With Response (PAwR) feature. These parameters shall be
 *  set to 0xFF if PAwR is not supported.
 *
 *  RETURNS
 *     DM_SET_EXT_CONNECTION_PARAMETERS_CFM_T
 *---------------------------------------------------------------------------*/

/* Number of initiating PHYs  */
#define DM_EXT_CONNECTION_NUM_INITIATING_PHYS (HCI_ULP_MAX_INITIATING_PHYS)
...
typedef enum
{
    ...
    DM_ULP_PHY_LE_CODED,
    DM_ULP_PHY_LE_MAX
} DM_ULP_PHY_LE_TYPE_T;

typedef struct
{
    DM_ULP_PHY_LE_TYPE_T    phy_type;
    ...
    uint16_t                min_ce_len; /* Minimum Connection events length */
    uint16_t                max_ce_len; /* Maximum Connection events length */
} DM_ULP_ES_INITIATING_PHY_T;

typedef struct
{
    dm_prim_t   type;           /* DM_ULP_EXT_SET_CONNECTION_PARAMS_REQ */
    phandle_t   phandle;        /* Where to send confirm to */
    uint8_t     adv_handle;     /* Reserved for future use. Shall be set to 0xFF for now */
    uint8_t     subevent;       /* Reserved for future use. Shall be set to 0xFF for now */
    uint16_t    conn_attempt_timeout; /* Equivalent of Page Timeout in BR/EDR */
    uint8_t     own_address_type; /* Own Address Type used in LE connection request by central device */
    uint8_t     phy_count; /* Number of entries in init_phys[]. Maximum DM_EXT_CONNECTION_NUM_INITIATING_PHYS
                              entries (LE 1M, 2M, LE Coded). Minimum = 1 */
    DM_ULP_ES_INITIATING_PHY_T   *init_phys[DM_EXT_CONNECTION_NUM_INITIATING_PHYS]; /* Initiating Phy parameters */
} DM_ULP_EXT_SET_CONNECTION_PARAMS_REQ_T;

typedef struct
{
    dm_prim_t type;     /* DM_ULP_EXT_SET_CONNECTION_PARAMS_CFM */
    phandle_t phandle;  /* destination phandle */
    uint8_t status;     /* Anything other than HCI_SUCCESS is a failure */
} DM_ULP_EXT_SET_CONNECTION_PARAMS_CFM_T;

#ifndef EXCLUDE_VSC
/*----------------------------------------------------------------------------*
 * PURPOSE
 *      Register request to Vendor Specific (VS) Interface, registering a 
 *      destination phandle for upstream VS primitives.
 *
 *----------------------------------------------------------------------------*/
typedef struct
{
     dm_prim_t    type;       /* DM_VS_REGISTER_REQ */
     phandle_t    phandle;    /* Protocol handle */
     uint16_t     flags;      /* Reserved - set to 0 */
} DM_VS_REGISTER_REQ_T;

/*----------------------------------------------------------------------------*
 * PURPOSE
 *      Confirmation that the registration request has been processed.
 *
 *  RETURNS
 *      phandle specified in DM_VS_REGISTER_REQ.
 *
 *----------------------------------------------------------------------------*/
typedef struct
{
     dm_prim_t    type;       /* DM_VS_REGISTER_CFM */
     phandle_t    phandle;    /* Destination phandle */
} DM_VS_REGISTER_CFM_T;

/*----------------------------------------------------------------------------*
 * PURPOSE
 *      Request to send vendor specific command to controller.
 *      Note:
 *          It is strongly suggested to avoid indiscriminate sending of VS
 *          commands but wait for the confirmation before sending subsequent
 *          ones. Application shall serialize VS command requests that have
 *          the same OCF.
 *
 * RETURNS
 *      DM_VS_COMMAND_CFM_T sent to phandle
 *
 *----------------------------------------------------------------------------*/

/* Flow control flags - not a bitmap */
/* When set, the application shall receive VS command confirm only when the
 * controller has responded with a command status event or a command
 * complete event for the VS command */
#define DM_VS_CMD_DEFAULT    0x0000

/* When set, the application shall receive VS command confirm once the
 * implementation has sent the command to lower controller.
 * The application is aware of the command being sent with this flag set and
 * controller shall NOT send any response for this command. When this flag is
 * set, credit_not_required will be ignored and HCI credit is not consumed while
 * sending this command.
 */
#define DM_VS_CMD_NO_RSP     0x0001

/* When set, the application shall receive VS command confirm once the 
 * implementation receives a command complete/status with NOP from 
 * controller */
#define DM_VS_CMD_NOP_AS_RSP 0x0002

typedef struct
{
     dm_prim_t    type;        /* DM_VS_COMMAND_REQ */
     phandle_t    phandle;     /* phandle for the response */

     /* An opaque value supplied by the requesting entity, and returned in the
      * VS command confirm primitive. It may be used to match a confirm to the
      * correct request.
      */
     uint32_t     context;

     /* Opcode Command Field (OCF) of the VS command. 
      * Valid Range: 0x0000 to 0x03FF
      * Opcode group field (OGF) will be appended as mentioned in the spec
      * before sending the command to the controller.
      */
     uint16_t     opcode_ocf;

     /* Total length of data packed in vs_data_part[] */
     uint8_t      data_length;

     /* Array of pointers containing VS command 
      * Users shall pack vendor specific command pdu using
      *     HCI_VS_DATA_BYTES_PER_PTR - bytes in a pointer at block.
      *     HCI_VS_DATA_BYTE_PACKET_PTRS - Number of pointer blocks.
      */
     uint8_t      *vs_data_part[HCI_VS_DATA_BYTE_PACKET_PTRS];

     /* flow control flags - for documentation refer to the above #defines */
     uint16_t     flow_control_flags;

     /* By default this should be set to FALSE.
      * It should be set to TRUE only when a particular VSC has been explicitly
      * documented indicating that host can send such commands even without HCI
      * command credits.
      */
...
/*----------------------------------------------------------------------------*
 * PURPOSE
 *      Confirmation that vendor specific command is sent to controller.
 *      This confirmation is generated when response from controller is 
 *      received for the VS command.
 *      If VS command doesn't have a response, i.e. when DM_VS_CMD_NO_RSP is 
 *      set, confirmation is sent when command is passed over the transport to 
 *      controller.
 *      This confirmation is sent to the phandle that was in DM_VS_COMMAND_REQ_T
 *
 * RETURNS
 *      phandle specified in DM_VS_COMMAND_REQ_T
 *      context from DM_VS_COMMAND_REQ
 *      ocf from DM_VS_COMMAND_REQ
 *      status of the request
 *      length of the return parameters
 *      return_param For all command complete events received from the 
 *                   controller, the array of pointers (each of size 
 *                   HCI_VS_DATA_BYTES_PER_PTR) will be populated based on the
 *                   data received from the controller. The data here excludes
 *                   status received from controller. In all other cases this
 *                   will be set to NULL and length as zero. 
 *
 *----------------------------------------------------------------------------*/
typedef struct
{
     dm_prim_t    type;       /* DM_VS_COMMAND_CFM*/
     phandle_t    phandle;    /* Destination phandle */
     uint32_t     context;    /* Context value supplied in the request */
     uint16_t     opcode_ocf; /* OCF from DM_VS_COMMAND_REQ */
     uint16_t     status;     /* Success or failure - See Note1 */
     uint8_t      length;     /* Length of the return parameters */
     uint8_t      *return_param[HCI_VS_DATA_BYTE_PACKET_PTRS];
} DM_VS_COMMAND_CFM_T;

/*----------------------------------------------------------------------------*
 * PURPOSE
 *      Indication used to send vendor specific event, received from controller,
 *      to the registered phandle in DM_VS_REGISTER_REQ
 *
 * RETURNS
 *      phandle specified in DM_VS_REGISTER_REQ
 *      length of the vs_data_part
 *      vs_data_part Array of pointers that point to a byte array of size 
 *                   HCI_VS_DATA_BYTES_PER_PTR. This is the event parameter(s)
 *                   received in the vendor specific event
 *
 *----------------------------------------------------------------------------*/
typedef struct
{
     dm_prim_t    type;        /* DM_VS_EVENT_IND */
     phandle_t    phandle;     /* Destination phandle */
 ...
 *
 *      SYNCHRONOUS CONNECTION INTERFACE PRIMITIVES
 *      These primitives are all sent to the ADV_SCAN_IFACEQUEUE input queue.
 *
 *-----------------------------------------------------------------------*/
 ...
 *
 *  Downstream primitives are sent to the ADV_SCAN_IFACEQUEUE, upstream primitives
...
#else
} DM_SM_APT_ROUTE_enum_T; /* autogen_makefile_ignore_this (DO NOT REMOVE THIS COMMENT) */
typedef uint16_t DM_SM_APT_ROUTE_T;
#endif
...
#else
} DM_SM_KEY_TYPE_enum_T;
typedef uint16_t DM_SM_KEY_TYPE_T;
#endif
...
typedef uint16_t DM_SM_TRUST_T;
#endif
...
#else
} DM_SM_PRIVACY_enum_T; /* autogen_makefile_ignore_this (DO NOT REMOVE THIS COMMENT) */
typedef uint16_t DM_SM_PRIVACY_T;
#endif
...
#else
} DM_SM_RANDOM_ADDRESS_enum_T; /* autogen_makefile_ignore_this (DO NOT REMOVE THIS COMMENT) */
typedef uint16_t DM_SM_RANDOM_ADDRESS_T;
#endif
...
typedef struct
{
    ...
    uint8_t         adv_handle;        /* 1 to MS_EXT_ADV_MAX_ADV_HANDLES - 1 */
...
typedef struct
{
    ...
    uint8_t                 encryption;     /* Encrypted BIS data if 1 */
...
typedef struct
{
    ...
    uint8_t         encryption;         /* Encryption mode of the BIG */
    uint8_t         broadcast_code[DM_BROADCAST_CODE_SIZE];
