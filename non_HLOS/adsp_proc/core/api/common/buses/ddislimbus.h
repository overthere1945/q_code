...
$Header: //components/rel/core.qdsp6/8.2.3/api/common/buses/ddislimbus.h#1 $

when       who     what, where, why
--------   ---     --------------------------------------------------------
08/10/21   RK      Added APIs to add and remove vote against island. 
06/17/21   RK      Added API to set clock source.
05/13/21   RK      Adding support for Island APIs.
01/04/18   MJS     Uncouple DALSYS from APIs.
...
===========================================================================
             Copyright (c) 2010-2018, 2021 QUALCOMM Technologies Incorporated.
                    All Rights Reserved.
...
#define SLIMBUS_EVENT_HANDLE_NON_BLOCKING  ((void*)0xffffffff)

#define SLIMBUS_EVENT_HANDLE_BATCHED_XFER  ((void*)0xfffffffe)
...
/** This enum specifies the event notification implementation. */
typedef enum
{
  SLIMBUS_NOTIFY_DALSYS,
  SLIMBUS_NOTIFY_QURT_SIGNAL,
  SLIMBUS_NOTIFY_CALLBACK
} SlimBusNotifyModeType;

/* Type definition for notification callback function. */
typedef void (*SlimBusCallbackType)(void *pUser);

/* Structure that defines the notification and callback handles 
   and parameters.
   For eMode==SLIMBUS_NOTIFY_DALSYS:      hEvent and pUser are valid
   For eMode==SLIMBUS_NOTIFY_QURT_SIGNAL: pSignal and sig_mask are valid
   For eMode==SLIMBUS_NOTIFY_CALLBACK:    pCb and pUser are valid
    */
typedef struct
{
  SlimBusNotifyModeType eMode;
  union {
    DALSYSEventHandle hEvent;
    qurt_anysignal_t *pSignal;
    SlimBusCallbackType pCb;
  } Notify;
  union {
    void *pUser;
    unsigned int sig_mask;
  } Data;
} SlimBusNotificationType;
...
typedef struct
{
   ...
   SlimBusNotificationType Notify;
} SlimBusBamRegisterEvent2Type;
...
typedef enum
{
  SLIMBUS_CLK_SRC_DEFAULT,
  SLIMBUS_CLK_SRC_XO = SLIMBUS_CLK_SRC_DEFAULT,
  SLIMBUS_CLK_SRC_RCO,
  SLIMBUS_CLK_SRC_MAX = SLIMBUS_CLK_SRC_RCO + 1,
  _PLACEHOLDER_SlimbusClksrc = 0x7fffffff
}SlimBusClkSrc;
...
struct DalSlimBus
{
   ...
   DALResult (*ReadValueElement)(uint32 ClientHandle, DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  uint8*  pucReadData,  uint32  uReadDataLen,  uint32*  puActualReadDataLen, SlimBusNotificationType *pNotify);
   DALResult (*WriteValueElement)(uint32 ClientHandle, DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  uint32  uByteSize, const uint8*  pucWriteData,  uint32  uWriteDataLen, SlimBusNotificationType *pNotify);
   DALResult (*ExchangeValueElement)(uint32 ClientHandle, DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  const uint8*  pucWriteData,  uint32  uWriteDataLen,  uint8*  pucReadData,  uint32  uReadDataLen,  uint32*  puActualReadDataLen, SlimBusNotificationType *pNotify);
   DALResult (*ReadInfoElement)(uint32 ClientHandle, DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  uint8*  pucReadData,  uint32  uReadDataLen,  uint32*  puActualReadDataLen, SlimBusNotificationType *pNotify);
   DALResult (*ClearInfoElement)(uint32 ClientHandle, DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr, uint32  uByteSize, const uint8*  pucClearMask,  uint32  uClearMaskLen, SlimBusNotificationType *pNotify);
   DALResult (*ReadAndClearInfoElement)(uint32 ClientHandle, DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  const uint8*  pucClearMask,  uint32  uClearMaskLen, uint8*  pucReadData,  uint32  uReadDataLen,  uint32*  puActualReadDataLen, SlimBusNotificationType *pNotify);
   ...
   DALResult (*RegisterBamEvent)(uint32 ClientHandle, DalDeviceHandle * _h, SlimBusResourceHandle  hPort, SlimBusBamTransferType  eTransferDir, SlimBusBamRegisterEvent2Type * pReg);
   ...
   DALResult (*RegisterPortEventEx)(uint32 ClientHandle, DalDeviceHandle * _h, SlimBusResourceHandle  hPort, SlimBusNotificationType *pNotify, SlimBusPortEventType eType);
   ...
   DALResult (*SendUserDefinedMessage)(uint32 ClientHandle, DalDeviceHandle * _h, SlimBusLogicalAddrType LA, uint32 MC, const uint8* pucPayload, uint32 uPayloadLen, SlimBusNotificationType *pNotify);
   ...
   DALResult (*RegisterClockGearEvent)(uint32 ClientHandle, DalDeviceHandle * _h, SlimBusNotificationType *pNotify);
   DALResult (*GetPortIndex)(DalDeviceHandle * _h, SlimBusResourceHandle  hPort, uint32 * puPgdIndex, uint32 * puPortIndex);
   DALResult (*GetClientId)(uint32 ClientHandle, DalDeviceHandle * _h, uint32 * pClientId);
   DALResult (*SetDeviceClockSrc)(uint32 ClientHandle, DalDeviceHandle * _h, SlimBusClkSrc *pClksrc);
   DALResult (*VoteAgainstIsland) (DalDeviceHandle * _h);
   DALResult (*RemoveIslandVote) (DalDeviceHandle * _h);
/* Use this interfance only while running the Slimbus test case.    
   These APIs are not exposed to any Slimbus clients */
#ifdef SLIMBUS_TEST_LPM_INTERFACE
   DALBOOL   (*IsLpmUsedForBamTrans)(DalDeviceHandle * _h);
   DALResult (*LpmMemAllocEx)(DalDeviceHandle * _h, uint32 uLen, DALSYSMemHandle *phMem);
   DALResult (*LpmMemInfoEx)(DalDeviceHandle * _h, DALSYSMemHandle hMem, size_t *puVirtAddr, size_t *puPhysAddr);
   DALResult (*LpmMemFreeEx)(DalDeviceHandle * _h, void *ptr);
#endif
};
...
/**
  ...
  @param[in] pNotify  Pointer to client notification structure
  ...
  */
static __inline DALResult
DalSlimBus_ReadValueElementEx(DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  uint8*  pucReadData,  uint32  uReadDataLen,  uint32*  puActualReadDataLen, SlimBusNotificationType *pNotify)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->ReadValueElement(DALDEVICE_LOCAL_CLIENTID, _h, LA, uByteAddr, pucReadData, uReadDataLen, puActualReadDataLen, pNotify);
}
...
{
  SlimBusNotificationType Notify = { SLIMBUS_NOTIFY_DALSYS };
  Notify.Notify.hEvent = hEvent;
  return DalSlimBus_ReadValueElementEx(_h, LA, uByteAddr, pucReadData, uReadDataLen, puActualReadDataLen, &Notify);
}
...
/**
  ...
  */
static __inline DALResult
DalSlimBus_WriteValueElementEx(DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr, uint32  uByteSize, const uint8*  pucWriteData,  uint32  uWriteDataLen, SlimBusNotificationType *pNotify)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->WriteValueElement(DALDEVICE_LOCAL_CLIENTID, _h, LA, uByteAddr, uByteSize, pucWriteData, uWriteDataLen, pNotify);
}
...
DalSlimBus_WriteValueElement(DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr, uint32  uByteSize, const uint8*  pucWriteData,  uint32  uWriteDataLen, DALSYSEventHandle  hEvent)
{
  SlimBusNotificationType Notify = { SLIMBUS_NOTIFY_DALSYS };
  Notify.Notify.hEvent = hEvent;
  return DalSlimBus_WriteValueElementEx(_h, LA, uByteAddr, uByteSize, pucWriteData, uWriteDataLen, &Notify);
}
...
/**
  ...
  */
static __inline DALResult
DalSlimBus_ExchangeValueElementEx(DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  const uint8*  pucWriteData,  uint32  uWriteDataLen,  uint8*  pucReadData,  uint32  uReadDataLen,  uint32*  puActualReadDataLen, SlimBusNotificationType *pNotify)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->ExchangeValueElement(DALDEVICE_LOCAL_CLIENTID, _h, LA, uByteAddr, pucWriteData, uWriteDataLen, pucReadData, uReadDataLen, puActualReadDataLen, pNotify);
}
...
DalSlimBus_ExchangeValueElement(DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  const uint8*  pucWriteData,  uint32  uWriteDataLen,  uint8*  pucReadData,  uint32  uReadDataLen,  uint32*  puActualReadDataLen, DALSYSEventHandle  hEvent)
{
  SlimBusNotificationType Notify = { SLIMBUS_NOTIFY_DALSYS };
  Notify.Notify.hEvent = hEvent;
  return DalSlimBus_ExchangeValueElementEx(_h, LA, uByteAddr, pucWriteData, uWriteDataLen, pucReadData, uReadDataLen, puActualReadDataLen, &Notify);
}
...
/**
  ...
  */
static __inline DALResult
DalSlimBus_ReadInfoElementEx(DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  uint8*  pucReadData,  uint32  uReadDataLen,  uint32*  puActualReadDataLen, SlimBusNotificationType *pNotify)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->ReadInfoElement(DALDEVICE_LOCAL_CLIENTID, _h, LA, uByteAddr, pucReadData, uReadDataLen, puActualReadDataLen, pNotify);
}
...
DalSlimBus_ReadInfoElement(DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  uint8*  pucReadData,  uint32  uReadDataLen,  uint32*  puActualReadDataLen, DALSYSEventHandle  hEvent)
{
  SlimBusNotificationType Notify = { SLIMBUS_NOTIFY_DALSYS };
  Notify.Notify.hEvent = hEvent;
  return DalSlimBus_ReadInfoElementEx(_h, LA, uByteAddr, pucReadData, uReadDataLen, puActualReadDataLen, &Notify);
}
...
/**
  ...
  */
static __inline DALResult
DalSlimBus_ClearInfoElementEx(DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr, uint32  uByteSize, const uint8*  pucClearMask,  uint32  uClearMaskLen, SlimBusNotificationType *pNotify)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->ClearInfoElement(DALDEVICE_LOCAL_CLIENTID, _h, LA, uByteAddr, uByteSize, pucClearMask, uClearMaskLen, pNotify);
}
...
DalSlimBus_ClearInfoElement(DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr, uint32  uByteSize, const uint8*  pucClearMask,  uint32  uClearMaskLen, DALSYSEventHandle  hEvent)
{
  SlimBusNotificationType Notify = { SLIMBUS_NOTIFY_DALSYS };
  Notify.Notify.hEvent = hEvent;
  return DalSlimBus_ClearInfoElementEx(_h, LA, uByteAddr, uByteSize, pucClearMask, uClearMaskLen, &Notify);
}
...
/**
  ...
  */
static __inline DALResult
DalSlimBus_ReadAndClearInfoElementEx(DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  const uint8*  pucClearMask,  uint32  uClearMaskLen, uint8*  pucReadData,  uint32  uReadDataLen,  uint32*  puActualReadDataLen, SlimBusNotificationType *pNotify)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->ReadAndClearInfoElement(DALDEVICE_LOCAL_CLIENTID, _h, LA, uByteAddr, pucClearMask, uClearMaskLen, pucReadData, uReadDataLen, puActualReadDataLen, pNotify);
}
...
DalSlimBus_ReadAndClearInfoElement(DalDeviceHandle * _h, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  const uint8*  pucClearMask,  uint32  uClearMaskLen, uint8*  pucReadData,  uint32  uReadDataLen,  uint32*  puActualReadDataLen, DALSYSEventHandle  hEvent)
{
  SlimBusNotificationType Notify = { SLIMBUS_NOTIFY_DALSYS };
  Notify.Notify.hEvent = hEvent;
  return DalSlimBus_ReadAndClearInfoElementEx(_h, LA, uByteAddr, pucClearMask, uClearMaskLen, pucReadData, uReadDataLen, puActualReadDataLen, &Notify);
}
 ...
 */
static __inline DALResult
DalSlimBus_SendUserDefinedMessageEx(DalDeviceHandle *_h, SlimBusLogicalAddrType LA, uint32 MC, const uint8* pucPayload, uint32 uPayloadLen, SlimBusNotificationType *pNotify)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->SendUserDefinedMessage(DALDEVICE_LOCAL_CLIENTID, _h, LA, MC, pucPayload, uPayloadLen, pNotify);
}
...
DalSlimBus_SendUserDefinedMessage(DalDeviceHandle *_h, SlimBusLogicalAddrType LA, uint32 MC, const uint8* pucPayload, uint32 uPayloadLen, DALSYSEventHandle hEvent)
{
  SlimBusNotificationType Notify = { SLIMBUS_NOTIFY_DALSYS };
  Notify.Notify.hEvent = hEvent;
  return DalSlimBus_SendUserDefinedMessageEx(_h, LA, MC, pucPayload, uPayloadLen, &Notify);
}
...
/**
  ...
  */
static __inline DALResult
DalSlimBus_RegisterClockGearEventEx(DalDeviceHandle * _h, SlimBusNotificationType *pNotify)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->RegisterClockGearEvent(DALDEVICE_LOCAL_CLIENTID, _h, pNotify);
}
...
DalSlimBus_RegisterClockGearEvent(DalDeviceHandle * _h, DALSYSEventHandle  hEvent)
{
  SlimBusNotificationType Notify = { SLIMBUS_NOTIFY_DALSYS };
  Notify.Notify.hEvent = hEvent;
  return DalSlimBus_RegisterClockGearEventEx(_h, &Notify);
}
...
/**
  ...
  */
static __inline DALResult
DalSlimBus_RegisterBamEventEx(DalDeviceHandle * _h, SlimBusResourceHandle  hPort, SlimBusBamTransferType  eTransferDir, SlimBusBamRegisterEvent2Type * pReg)
{
...
{
  SlimBusBamRegisterEvent2Type Reg2;
  Reg2.Notify.eMode = SLIMBUS_NOTIFY_DALSYS;
  if (NULL != pReg)
  {
    Reg2.eOptions = pReg->eOptions;
    Reg2.Notify.Notify.hEvent = pReg->hEvent;
    Reg2.Notify.Data.pUser = pReg->pUser;
  }
  return DalSlimBus_RegisterBamEventEx(_h, hPort, eTransferDir, &Reg2);
}
...
/**
  ...
  */
static __inline DALResult
DalSlimBus_RegisterPortEventEx2(DalDeviceHandle * _h, SlimBusResourceHandle  hPort, SlimBusNotificationType *pNotify, SlimBusPortEventType eType)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->RegisterPortEventEx(DALDEVICE_LOCAL_CLIENTID, _h, hPort, pNotify, eType);
}
...
DalSlimBus_RegisterPortEventEx(DalDeviceHandle * _h, SlimBusResourceHandle  hPort, DALSYSEventHandle  hEvent, void * pUser, SlimBusPortEventType eType)
{
  SlimBusNotificationType Notify = { SLIMBUS_NOTIFY_DALSYS };
  Notify.Notify.hEvent = hEvent;
  Notify.Data.pUser = pUser;
  return DalSlimBus_RegisterPortEventEx2(_h, hPort, &Notify, eType);
}
...
/**
  @brief Get Client index of a SLIMbus master device to use in slimbus island mode api
   
  This function retrieves the index of a SLIMbus master port. 
   
  @param[in] _h  Client handle to the SLIMbus driver 

  @param[out] puClientIndex  Pointer to the location to store the 
        port index

  @return  DAL_SUCCESS on success, an error code on error
  */
static __inline DALResult
DalSlimBus_GetClientId(DalDeviceHandle * _h, uint32 * puClientIndex)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->GetClientId( DALDEVICE_LOCAL_CLIENTID, _h, puClientIndex);
}
...
/**
  ...
  */
static __inline DALResult
DALSlimBus_SetDeviceClockSrc(DalDeviceHandle * _h, SlimBusClkSrc  *pClksrc)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->SetDeviceClockSrc(DALDEVICE_LOCAL_CLIENTID, _h, pClksrc);
}
...
/**
 * @brief Slimbus vote against ADSP ISLAND.
 *
 * This function vote against ADSP ISLAND.
 * 
 ...
 */
static __inline DALResult
DalSlimBus_VoteAgainstIsland(DalDeviceHandle * _h)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->VoteAgainstIsland(_h);
}

/**
 * @brief Slimbus remove vote against ADSP ISLAND.
 *
 * This function cancels vote against ADSP ISLAND.
 * 
 * @param[in] h  Client handle to the SLIMbus driver
 * 
 * @return SB_SUCCESS on success, error code on error
 */
static __inline DALResult
DalSlimBus_RemoveIslandVote(DalDeviceHandle * _h)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->RemoveIslandVote(_h);
}
...
/**
  @brief Returns whether the LPM is used for BAM transactions or not.
   
  This function returns whether the LPM is used for BAM transactions or not.
      
  @param[in] _h  Client handle to the SLIMbus driver 
   
  @return  DAL_SUCCESS on success, an error code on error   
  */
static __inline DALBOOL
DalSlimBus_IsLpmUsedForBamTrans(DalDeviceHandle * _h)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->IsLpmUsedForBamTrans(_h);
}


/**
  @brief Dynamically allocate memory from LPM.
   
  This function allocates memory from LPM, dynamically.
  Also updates the allocated memory pointers in a list.
  The memory will be freed automatically during the Slimbus Clock OFF,
  if not cleared/freed before that. 
   
  @param[in] _h  Client handle to the SLIMbus driver 
   
  @param[in] uLen  Length of the physical memory to allocate in bytes
   
  @param[out] phMem  Location to store the physical memory handle
   
  @return  DAL_SUCCESS on success, an error code on error   
  */
static __inline DALResult
DalSlimBus_LpmMemAllocEx(DalDeviceHandle * _h, uint32 uLen, DALSYSMemHandle *phMem)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->LpmMemAllocEx(_h, uLen, phMem);
}


/**
  @brief Retrive information about physical memory of LPM
   
  This function retrieves information about previously allocated physical memory from LPM
   
  @param[in] _h  Client handle to the SLIMbus driver 
   
  @param[in] hMem  Physical memory handle

  @param[out] puVirtAddr  Pointer to the location to store the virtual memory address
 
  @param[out] puPhysAddr  Pointer to the location to store the physical memory address
   
  @return  DAL_SUCCESS on success, an error code on error
   
  @see DalSlimBus_LpmMemAllocEx()       
  */
static __inline DALResult
DalSlimBus_LpmMemInfoEx(DalDeviceHandle * _h, DALSYSMemHandle hMem, size_t *puVirtAddr, size_t *puPhysAddr)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->LpmMemInfoEx(_h, hMem, puVirtAddr, puPhysAddr);
}


/**
  @brief Free memory previously allcated from LPM.
   
  This function Frees memory allocated from the LPM memory using SlimBus_LpmMemAlloc().
  Also free the pointer/pointers from the list.
   
  @param[in] _h  Client handle to the SLIMbus driver 
   
  @param[in] ptr  Pointer to the memory allcated from LPM by SlimBus_LpmMemAlloc()
  ...
  */
static __inline DALResult
DalSlimBus_LpmMemFreeEx(DalDeviceHandle * _h, void *ptr)
{
   return ((DalSlimBusHandle *)_h)->pVtbl->LpmMemFreeEx(_h, ptr);
}
