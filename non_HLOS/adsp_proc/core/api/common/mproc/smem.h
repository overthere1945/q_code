...
/*===========================================================================
Copyright (c) 2006-2013, 2018, 2022 Qualcomm Technologies, Inc.
All Rights Reserved.
...
$Header: //components/rel/core.qdsp6/8.2.3/api/common/mproc/smem.h#1 $
...
/** Flags for use with smem_create() in UserPDs */
#define SMEM_CREATE_FLAG_ALLOC          1
#define SMEM_CREATE_FLAG_GET_FD         2
...
/*===========================================================================
FUNCTION      smem_create
===========================================================================*/
/**
  Requests to allocate memory in shared memory. Upon success, an fd to the 
  SMEM item is returned. This is a UserPD API.

  @param[in] remote_host   Remote endpoint with which this SMEM item will be shared.
  @param[in] smem_type     Type of memory.
  @param[in] size          Size of the buffer requested.
  @param[in] flags         Flags for the allocation.
  
  @return
  file descriptor to the smem item, if the smem allocation was successful.
*/
/*=========================================================================*/
int smem_create
(
...
===========================================================================*/
/**
  Requests to read data in shared memory. Upon success, data is copied into the
  buffer. This is a UserPD API.

  @param[in]  fd            File descriptor associated with the smem type.
  @param[in]  offset        Offset from which the data needs to be read.
  @param[in]  size          Size of data to be copied into buffer.
  @param[out] buffer        buffer into which the data is copied.
  
  @return
  SMEM_STATUS_SUCCESS          - Read was successful
  SMEM_STATUS_FAILURE          - General failure
*/
/*=========================================================================*/
int32 smem_read
...
/**
  ...
  @param[in]  fd            File descriptor associated with the smem type.
  @param[in]  offset        Offset at which the data needs to be written.
  @param[in]  size          Size of data to be written into smem.
  @param[in] buffer         buffer to copy into the smem region
  
  @return
  SMEM_STATUS_SUCCESS          - Write was successful
  SMEM_STATUS_FAILURE          - General failure
*/
/*=========================================================================*/
int32 smem_write
