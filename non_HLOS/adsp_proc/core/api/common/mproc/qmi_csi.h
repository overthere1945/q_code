...
#define QMI_CSI_OPTIONS_DATA_LIMITED_TRANSMIT_VALID (1024)
...
/*!
@brief
  Sets max outstanding bytes in tx_q before a QMI_CONN_BUSY error is returned

@param[in]  opt                 Service options object
@param[in]  max_outstanding_data_in_bytes Max outstanding bytes                
*/
/*=========================================================================*/
#define QMI_CSI_OPTIONS_SET_MAX_OUTSTANDING_DATA_BYTES(opt, max_outstanding_data_in_bytes) \
  do { \
    (opt).data_limit_for_tx = max_outstanding_data_in_bytes; \
    (opt).options_set |= QMI_CSI_OPTIONS_DATA_LIMITED_TRANSMIT_VALID; \
  } while(0)
