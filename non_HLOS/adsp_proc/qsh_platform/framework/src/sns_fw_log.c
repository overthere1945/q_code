...
/* CM Sensor Data Type to be used for logging */
static const char cm_data_type[] DIAG_ISLAND_RODATA = "client_manager";

/*=============================================================================
  Public Function Definitions
  ===========================================================================*/

DIAG_ISLAND_TEXT sns_rc sns_fw_log_encode_sensor_api_cb(void *log,
                                                        size_t log_size,
                                                        ...
                                                        size_t *bytes_written)
{
  ...
  if(!pb_encode(&stream, sns_diag_sensor_api_log_fields, sensor_api_log))
  {
    SNS_SPRINTF(ERROR, sns_fw_printf,
                "Error encoding sensor api log header : %s",
                PB_GET_ERROR(&stream));
    rc = SNS_RC_FAILED;
  }
  else
  {
    ...
    case SNS_LOG_SENSOR_API_REQUEST:
    {
                        ...
                        sns_diag_sensor_api_log_request_payload_tag))
      {
        rc = SNS_RC_FAILED;
        SNS_SPRINTF(ERROR, sns_fw_printf,
                    "Error encoding sensor api req payload"
                    "field tag: %s",
                    PB_GET_ERROR(&stream));
      }
    ...
    default:
    {
                        ...
                        sns_diag_sensor_api_log_opaque_payload_tag))
      {
        rc = SNS_RC_FAILED;
        SNS_SPRINTF(ERROR, sns_fw_printf,
                    "Error encoding sensor api event "
                    "payload field tag: %s",
                    PB_GET_ERROR(&stream));
      }
      else if(!pb_encode_varint(
                  &stream, (log_info->payload_len + tag_size + length_size)))
      {
        rc = SNS_RC_FAILED;
        SNS_SPRINTF(ERROR, sns_fw_printf,
                    "Error encoding sensor api event payload"
                    "string: %s",
                    PB_GET_ERROR(&stream));
      }
                             ...
                             sns_diag_opaque_payload_payload_tag))
      {
        rc = SNS_RC_FAILED;
        SNS_SPRINTF(ERROR, sns_fw_printf,
                    "Error encoding sensor api event log"
                    "payload field tag: %s",
                    PB_GET_ERROR(&stream));
      }
      else if(!pb_encode_string(&stream, (pb_byte_t *)log_info->payload,
...
DIAG_ISLAND_TEXT static sns_rc sns_fw_log_sensor_inst_api_msg(
    sns_fw_sensor_instance *instance, sns_sensor_uid const *sensor_uid,
    sns_fw_diag_service_log_id log_id, uint32_t message_id, uint64_t timestamp,
    void *payload, size_t payload_len)
{
  sns_rc rc = SNS_RC_SUCCESS;
  sns_diag_instance_config *diag_config =
      sns_diag_find_instance_config(instance, sensor_uid);
  if(NULL == diag_config)
  {
  ...
  else
  {
    ...
    log_info = (sensor_api_log_info *)sns_diag_log_alloc(log_info_size, log_id);
    ...
    if(NULL != log_info)
    {
      ...
      sns_memscpy(log_info->payload, payload_len, payload, payload_len);
    }
    ...
    rc = sns_diag_publish_sensor_log(
        (uintptr_t)instance, diag_config->suid,
        (char const *)diag_config->config->datatype, log_id, log_info_size,
        log_info, encoded_sensor_api_hdr_size + payload_len,
...
sns_rc sns_fw_log_sensor_inst_api_event(sns_sensor_instance *instance,
                                        ...
                                        sns_sensor_event *event)
{
  ...
  if(NULL == event)
  {
    SNS_PRINTF(ERROR, sns_fw_printf, "NULL event");
    return SNS_RC_INVALID_VALUE;
  }
  SNS_NON_ISLAND_DIAG_CHECK(
      if(SNS_ISLAND_STATE_IN_ISLAND != sns_island_get_island_state()))
  {
    ...
    sns_diag_instance_config *diag_config =
        sns_diag_find_instance_config(fw_instance, sensor_uid);
    ...
    if(NULL != diag_config)
    {
      ...
      rc = sns_fw_log_sensor_inst_api_msg(
          fw_instance, sensor_uid, SNS_LOG_SENSOR_API_EVENT, event->message_id,
          event->timestamp, event->event, event->event_len);
    }
  }
  return rc;
}
...
sns_fw_log_sensor_api_msg(sns_fw_sensor *sensor,
                          sns_fw_diag_service_log_id log_id,
                          uint32_t message_id, uint64_t timestamp,
                          uint64_t stream_id, void *payload, size_t payload_len)
{
  sns_rc rc = SNS_RC_SUCCESS;
  sns_diag_sensor_config *diag_config = &sensor->diag_config;
  sensor_api_log_info *log_info = NULL;
  size_t log_info_size = sizeof(sensor_api_log_info) + payload_len;
  size_t encoded_sensor_api_hdr_size =
      sns_diag_get_preencoded_size(SNS_DIAG_ENC_SIZE_SENSOR_API_HDR);
  ...
  rc = sns_diag_publish_sensor_log(
      (uintptr_t)0, diag_config->suid, diag_config->config->datatype, log_id,
      log_info_size, log_info, encoded_sensor_api_hdr_size + payload_len,
      sns_fw_log_encode_sensor_api_cb);
...
SNS_SECTION(".text.sns")
sns_rc sns_fw_log_sensor_api_event(sns_fw_data_stream *stream,
                                   sns_sensor_event *event)
{
  sns_rc rc = SNS_RC_FAILED;
  SNS_NON_ISLAND_DIAG_CHECK(
      if(SNS_ISLAND_STATE_IN_ISLAND != sns_island_get_island_state()))
  {
    if(NULL != stream && NULL != event && NULL != stream->data_source)
    {
      ...
      rc = sns_fw_log_sensor_api_msg(
          fw_sensor, SNS_LOG_SENSOR_API_EVENT, event->message_id,
          event->timestamp, (size_t)stream, event->event, event->event_len);
    }
  }
  return rc;
}
...
SNS_SECTION(".text.sns")
sns_rc sns_fw_log_sensor_api_req(sns_sensor *sensor, sns_fw_request const *req)
{
  sns_rc rc = SNS_RC_FAILED;
  if(NULL == req)
  {
    SNS_PRINTF(ERROR, sns_fw_printf, "NULL request");
    return SNS_RC_INVALID_VALUE;
  }
      ...
      if(SNS_ISLAND_STATE_IN_ISLAND != sns_island_get_island_state()))
  {
    ...
    rc = sns_fw_log_sensor_api_msg(
        fw_sensor, SNS_LOG_SENSOR_API_REQUEST, req->request.message_id,
        sns_get_system_time(), (size_t)req->stream, req->request.request,
        req->request.request_len);
  }
  return rc;
}

DIAG_ISLAND_TEXT sns_rc sns_fw_log_encode_inst_map_cb(void *log,
                                                      size_t log_size,
                                                      size_t encoded_log_size,
                                                      void *encoded_log,
                                                      size_t *bytes_written)
{
  ...
  for(int i = 0; i < log_info->num_dest_id; i++)
  {
    ...
    else if(!pb_encode_fixed64(&stream,
                               (pb_byte_t *)(&log_info->dest_instance_id[i])))
    {
  ...
  for(uint_fast8_t i = 0; i < log_info->num_dest_id; i++)
  {
                      ...
                      sns_diag_instance_map_log_stream_id_tag))
    {
      rc = SNS_RC_FAILED;
      SNS_SPRINTF(ERROR, sns_fw_printf,
                  "Error encoding sns_diag_instance_map_log_stream_id_tag: %s",
                  PB_GET_ERROR(&stream));
    }
...
sns_fw_log_inst_map_internal(sns_sensor_instance *instance)
{
      ...
      sns_isafe_list_iter_advance(&req_list_iter))
  {
    ...
    for(sns_isafe_list_iter_init(&client_iter, &req_list->client_requests,
                                 true);
        NULL != sns_isafe_list_iter_curr(&client_iter);
        sns_isafe_list_iter_advance(&client_iter))
    {
      sns_fw_data_stream *fw_stream =
          (sns_fw_data_stream *)sns_isafe_list_iter_get_curr_data(&client_iter);
  ...
  rc = sns_diag_publish_fw_log(SNS_LOG_INSTANCE_MAP, sizeof(inst_map_log_info),
                               log_info, encoded_log_size,
                               sns_fw_log_encode_inst_map_cb);

  return rc;
}

SNS_SECTION(".text.sns")
sns_rc sns_fw_log_inst_map(sns_sensor_instance *instance)
{
  sns_rc rc = SNS_RC_FAILED;
  SNS_NON_ISLAND_DIAG_CHECK(
      if(SNS_ISLAND_STATE_IN_ISLAND != sns_island_get_island_state()))
...
DIAG_ISLAND_TEXT sns_rc sns_fw_log_encode_client_api_cb(void *log,
                                                        size_t log_size,
                                                        size_t encoded_log_size,
                                                        void *encoded_log,
                                                        size_t *bytes_written)
{
  ...
  hdr.client_id = log_info->client_connect_id;
  ...
  {
    SNS_SPRINTF(ERROR, sns_fw_printf,
                "Error encoding client api req header : %s",
                PB_GET_ERROR(&stream));
    rc = SNS_RC_FAILED;
  }
  else
  {
    ...
    case SNS_LOG_QSOCKET_CLIENT_API_REQ:
    // Fallthrough
    case SNS_LOG_QSOCKET_CLIENT_API_RESP:
    // Fallthrough
    case SNS_LOG_QSOCKET_CLIENT_API_EVENT:
    {
      field_tag = sns_diag_client_api_log_qcm_payload_tag;
      break;
    }
    default:
    {
    ...
    if(!pb_encode_tag(&stream, PB_WT_STRING, field_tag))
    {
      rc = SNS_RC_FAILED;
      SNS_SPRINTF(ERROR, sns_fw_printf,
                  "Error encoding client api payload"
                  "field tag: %s",
                  PB_GET_ERROR(&stream));
    }
    else if(!pb_encode_string(&stream, (pb_byte_t *)log_info->payload,
...
DIAG_ISLAND_TEXT sns_rc sns_fw_log_client_api_msg(
    intptr_t instance, sns_sensor_uid const *sensor_uid,
    sns_fw_diag_service_log_id log_id, uint64_t client_connect_id,
    sns_diag_datatype_config *diag_config_ptr, void const *payload,
    size_t payload_len)
{
  sns_rc rc = SNS_RC_SUCCESS;
  ...
  if(!sns_island_is_island_ptr((intptr_t)diag_config_ptr))
    SNS_ISLAND_EXIT();

  if(!diag_config_ptr->enabled)
  {
    return SNS_RC_NOT_SUPPORTED;
  }
  ...
  rc = sns_diag_publish_sensor_log(
      (uintptr_t)instance, sensor_uid, cm_data_type, log_id, log_info_size,
      log_info, encoded_client_api_hdr_size + payload_len,
...
SNS_SECTION(".text.sns")
sns_rc sns_fw_log_client_api_req(sns_sensor_instance *instance,
                                 ...
                                 size_t payload_len)
{
  ...
  if(NULL == payload)
  {
    SNS_SPRINTF(ERROR, sns_fw_printf, "NULL payload");
    return SNS_RC_INVALID_VALUE;
  }
  ...
  SNS_NON_ISLAND_DIAG_CHECK(
      if(SNS_ISLAND_STATE_IN_ISLAND != sns_island_get_island_state()))
  {
    rc = sns_fw_log_client_api_msg(
        (intptr_t)instance, sensor_uid, SNS_LOG_CLIENT_API_REQUEST,
        client_connect_id, diag_config_ptr, payload, payload_len);
  }
...
SNS_SECTION(".text.sns")
sns_rc sns_fw_log_client_api_resp(sns_sensor_instance *instance,
                                  sns_diag_datatype_config *diag_config_ptr,
                                  sns_sensor_uid const *sensor_uid,
                                  uint64_t client_connect_id, uint64_t error)
{
      ...
      if(SNS_ISLAND_STATE_IN_ISLAND != sns_island_get_island_state()))
  {
    ...
    if(!pb_encode(&stream, sns_diag_client_resp_msg_fields, &resp_msg))
    {
      rc = SNS_RC_FAILED;
      SNS_SPRINTF(ERROR, sns_fw_printf,
                  "Error encoding client resp msg log: %s",
                  PB_GET_ERROR(&stream));
    }
    else
    {
      rc = sns_fw_log_client_api_msg(
          (intptr_t)instance, sensor_uid, SNS_LOG_CLIENT_API_RESPONSE,
          client_connect_id, diag_config_ptr, buffer, stream.bytes_written);
    }
  }
  return rc;
}
...
SNS_SECTION(".text.sns")
sns_rc sns_fw_log_client_api_ind(sns_sensor_instance *instance,
                                 sns_diag_datatype_config *diag_config_ptr,
                                 sns_sensor_uid const *sensor_uid,
                                 uint64_t client_connect_id, void *payload,
                                 size_t payload_len)
{
      ...
      if(SNS_ISLAND_STATE_IN_ISLAND != sns_island_get_island_state()))
  {
    rc = sns_fw_log_client_api_msg((intptr_t)instance, sensor_uid,
                                   SNS_LOG_CLIENT_API_EVENT, client_connect_id,
                                   diag_config_ptr, payload, payload_len);
  }
  return rc;
}

#else /* SNS_LOG_DISABLED */
...
SNS_SECTION(".text.sns")
inline sns_rc sns_fw_log_sensor_api_event(sns_fw_data_stream *stream,
                                          sns_sensor_event *event)
{
  UNUSED_VAR(stream);
  UNUSED_VAR(event);
...
SNS_SECTION(".text.sns")
inline sns_rc sns_fw_log_sensor_api_req(sns_sensor *sensor,
                                        sns_fw_request const *req)
{
  UNUSED_VAR(sensor);
  UNUSED_VAR(req);
...
{
  UNUSED_VAR(instance);
  UNUSED_VAR(diag_config_ptr);
  UNUSED_VAR(sensor_uid);
  UNUSED_VAR(client_connect_id);
  UNUSED_VAR(payload);
  UNUSED_VAR(payload_len);
