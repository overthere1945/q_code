...
#define PDTSW_UTILITY_GLINK_AM_Q6_CHANNEL_NAME    "am-q6-utility"

#define PDTSW_UTILITY_GLINK_WM_REMOTE_SS          "swm"
#define PDTSW_UTILITY_GLINK_Q6_REMOTE_SS          "lpass"
...
typedef enum {
    ...
    PDTSW_UTILITY_GLINK_AM_Q6_LINK,       /**< AM-Q6 link */
...
static pdtsw_utility_glink_ctx_t pdtsw_utility_glink_ctx = 
{
    ...
    .link_info[PDTSW_UTILITY_GLINK_AM_Q6_LINK] = {NULL, GLINK_LINK_STATE_DOWN},
    .ch_info[PDTSW_UTILITY_GLINK_AM_WM_CHANNEL] = {NULL, GLINK_LOCAL_DISCONNECTED},
    .ch_info[PDTSW_UTILITY_GLINK_AM_Q6_CHANNEL] = {NULL, GLINK_LOCAL_DISCONNECTED}
};
    ...
    pdtsw_utility_glink_channel_t channel, uint32_t remote_signal)
{
    ...
    switch(signal.id)
    {
        ...
        {
            /* If the signal is for PSRAM, process it using the PSRAM arbiter */
            pdtsw_psram_arbiter_process_remote_signal(channel, signal);
            break;
        }
#endif
        default:
...
/**
 * @brief Callback for AM-Q6 GLINK link state changes
 *
 * Handles link state changes and submits the event to the work queue
 *
 * @param[in] link_info Pointer to the link information structure
 ...
 * @return void
 */
static void pdtsw_utility_glink_am_q6_link_state_cb(glink_link_info_type *link_info, void *priv)
{
    /* Check if link_info is NULL */
    if(link_info == NULL)
    {
    ...
    /* Populate the work queue message with event details */
    args->evt = PDTSW_UTILITY_GLINK_EVT_LINK_STATE;
    args->channel = PDTSW_UTILITY_GLINK_AM_Q6_CHANNEL;
    args->size = sizeof(glink_link_state_type);
    args->data = (void *)((uint8_t *)args + sizeof(pdtsw_utility_glink_workq_msg_t));
    memscpy(args->data, args->size, (void *)&link_info->link_state, sizeof(glink_link_state_type));
...
static glink_err_type pdtsw_utility_glink_open(pdtsw_utility_glink_channel_t glink_ch)
{
    ...
    if(glink_ch == PDTSW_UTILITY_GLINK_AM_WM_CHANNEL)
    {
        ...
        open_cfg.remote_ss = PDTSW_UTILITY_GLINK_WM_REMOTE_SS;
    }
    else
    {
        open_cfg.name = PDTSW_UTILITY_GLINK_AM_Q6_CHANNEL_NAME;
        open_cfg.remote_ss = PDTSW_UTILITY_GLINK_Q6_REMOTE_SS;
    }
...
static void pdtsw_utility_glink_link_state_handler(pdtsw_utility_glink_workq_msg_t *p_msg)
{
    ...
    pdtsw_utility_glink_link_t link = (p_msg->channel == PDTSW_UTILITY_GLINK_AM_WM_CHANNEL)
        ? PDTSW_UTILITY_GLINK_AM_WM_LINK
        : PDTSW_UTILITY_GLINK_AM_Q6_LINK;
...
static void pdtsw_utility_glink_chnl_state_handler(pdtsw_utility_glink_workq_msg_t *p_msg)
{
    ...
    switch (ch_evt)
    {
...
#ifdef CONFIG_QC_PDTSW_PSRAM_ARBITER_ENABLE
            /* Ask PSRAM arbiter to send PSRAM availability status to the respective channel */
            pdtsw_psram_arbiter_send_avail_sts(p_msg->channel);
#endif
            break;
        }
        ...
        case GLINK_REMOTE_DISCONNECTED:
        {
            ...
            UTIL_GLINK_DBG("Channel:%d Remote Disconnect\n", p_msg->channel);
    ...
    pdtsw_utility_glink_channel_t channel, pdtsw_utility_glink_signal_t signal)
{
    ...
    UTIL_GLINK_DBG("Success for ch: %d, sig: %d\n", channel, signal.raw_value);
...
int pdtsw_utility_glink_init()
{
    ...
    link_info.remote_ss = PDTSW_UTILITY_GLINK_WM_REMOTE_SS;
    ...
    /* Register AM-Q6 link state callback */
    link_info.remote_ss = PDTSW_UTILITY_GLINK_Q6_REMOTE_SS;
    link_info.link_notifier = pdtsw_utility_glink_am_q6_link_state_cb;
    glink_err = glink_register_link_state_cb(&link_info, NULL);
    if(GLINK_STATUS_SUCCESS != glink_err)
    {
        UTIL_GLINK_DBG("AM<->Q6 Link State Cb Reg Failed\n");
        return -1;
    }

    pdtsw_utility_glink_ctx.link_info[PDTSW_UTILITY_GLINK_AM_Q6_LINK].handle = link_info.handle;


    UTIL_GLINK_INF("AM: PDTSW Util Glink Init\n");
