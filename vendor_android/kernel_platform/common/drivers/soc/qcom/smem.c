				   ...
				   size_t size)
{
	...
	/* add 2026.05.04 hoyoun.lee@lge.com for himaxDSP jpeg image buffer */
	phys_addr = part->phys_base + ((void *)hdr - part->virt_base);

	if (phys_addr >= 0x81EC0000) {
		return -ENOSPC;
	}
	/* ednd 2026.05.04 hoyoun.lee@lge.com */
				  ...
				  size_t size)
{
	...
	/* add 2026.05.04 hoyoun.lee@lge.com for himaxDSP jpeg image buffer */
	phys_addr = smem->regions[0].aux_base + le32_to_cpu(header->free_offset);

	if (phys_addr >= 0x81EC0000) {
		return -ENOMEM;
	}
	/* end 2026.05.04 hoyoun.lee@lge.com  */
