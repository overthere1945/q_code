				   ...
				   size_t size)
{
	...
	phys_addr_t my_phys_addr;
	my_phys_addr = part->phys_base + ((void *)hdr - part->virt_base);

	if (my_phys_addr >= 0x81EC0000) {
		return -ENOSPC;
	}
				  ...
				  size_t size)
{
	...
	/* add 2026.05.04 hoyoun.lee@lge.com for himaxDSP jpeg image buffer */
	phys_addr_t my_phys_addr;
	my_phys_addr = smem->regions[0].aux_base + le32_to_cpu(header->free_offset);
