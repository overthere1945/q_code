/*============================================================================
  ...
  Copyright (c) 2013, 2015-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
using android::hardware::sensors::V2_1::subhal::implementation::sensors_hw_module;
using android::hardware::sensors::V2_0::implementation::IScopedWakelockRefCounter;
...
struct sensor_t *sensors_list = NULL;
int sensor_count;
...
static const char *handle_to_string( struct sensor_t const* sensors_list, int handle )
{
  const struct sensor_t *list_p = sensors_list;
  if(list_p!=nullptr){
      while( list_p->name ) {
...
inline void convertToSensorEventV2_1(const Event& src, sensors_event_t* dst) {
    switch (src.sensorType) {
        case SensorType::HINGE_ANGLE:
            ...
            *dst = {.version = sizeof(sensors_event_t),
            ...
            break;
        default:
            convertToSensorEvent(reinterpret_cast<const EventV1_0&>(src), dst);
            break;
    }
}

// Print out the usage statement in interactive mode
void print_usage_msg( struct sensor_t const *sensors_list )
{
  int count = 0;
  struct sensor_t const *list_p = sensors_list;

  fprintf(stderr,"Sensors HAL TEST APP, version 1\n");
  fprintf(stderr,"Usage:\n");
  fprintf(stderr,"\tChoose a sensor to interact with by inputting the sensor's type and whether or not is the wakeup version as shown in the 'Sensors sensors_list'\n");
  ...
  fprintf(stderr,"Sensors sensors_list:\n");
...
void print_sensor_list( struct sensor_t const* sensors_list )
{
  struct sensor_t const* list_p = sensors_list;
  fprintf(stderr, "Sensor sensors_list:\n");
  if(list_p!=nullptr){
      while ( list_p->name )
      {
        print_sensor(list_p);
...
int find_sensor_by_type(int sensor_type, char wakeup_flag, struct sensor_t const* sensors_list, const struct sensor_t **ret)
{
  const struct sensor_t *list_p = sensors_list;
  while( list_p->name ) {
...
static void print_sns_event(sensors_event_t buffer){
    ...
    if( buffer.type == SENSOR_TYPE_META_DATA ) {
        printf("\n%" PRIu64 ".%06" PRIu64 ", %s/%s, %" PRId64 ".%06" PRId64 ", %d latency: %" PRIu64 ".%06" PRIu64 "\n\n",
        ...
        if( buffer.type == SENSOR_TYPE_ADDITIONAL_INFO) {
            printf("%" PRIu64 ".%06" PRIu64 ", %s/%s, %" PRId64 ".%06" PRId64 ", TYPE_ADDITIONAL_INFO type:%d, value: %f \n\n",
            now_ns/NSEC_PER_MS, now_ns%NSEC_PER_MS,
            type_to_string( buffer.type ), handle_to_string(sensors_list, buffer.sensor),
            buffer.timestamp/NSEC_PER_MS, buffer.timestamp%NSEC_PER_MS,
            buffer.additional_info.type,
            buffer.additional_info.data_float[0]);
        }else{
            printf("%" PRIu64 ".%06" PRIu64 ", %s/%s, %" PRId64 ".%06" PRId64 ", %f, %f, %f, %f, %" PRIu64 ", latency(ms): %" PRIu64 ".%06" PRIu64"\n",
            now_ns/NSEC_PER_MS, now_ns%NSEC_PER_MS,
            type_to_string( buffer.type ), handle_to_string(sensors_list, buffer.sensor),
            buffer.timestamp/NSEC_PER_MS, buffer.timestamp%NSEC_PER_MS,
            buffer.data[0], buffer.data[1], buffer.data[2], buffer.data[3],
            buffer.u64.step_counter,
            (now_ns - buffer.timestamp) / NSEC_PER_MS,
            (now_ns - buffer.timestamp) % NSEC_PER_MS);
        }
    }
...
class ScopedWakelockRefCounter : public IScopedWakelockRefCounter {
  public:
    bool incrementRefCountAndMaybeAcquireWakelock(size_t delta,
                                                        int64_t* timeoutStart /* = nullptr */) {return true;};
    void decrementRefCountAndMaybeReleaseWakelock(size_t delta,
                                                        int64_t timeoutStart /* = -1 */) {};
};

ScopedWakelockRefCounter mScopedWakelockRefCounter;

ScopedWakelock::ScopedWakelock(IScopedWakelockRefCounter* refCounter, bool locked)
    : mRefCounter(refCounter), mLocked(locked) {
    if (mLocked) {
        int64_t time = 0;
        mLocked = mRefCounter->incrementRefCountAndMaybeAcquireWakelock(1, &time);
    }
}
...
ScopedWakelock::~ScopedWakelock() {
    if (mLocked) {
        int64_t time = 0;
        mRefCounter->decrementRefCountAndMaybeReleaseWakelock(1, time);
    }
}



class HalProxyCallbackBase : public IHalProxyCallback {
...
void HalProxyCallbackBase::postEvents(const std::vector<Event>& events, ScopedWakelock wakelock) {
    if (events.empty()) {
        ALOGE("empty event\n");
    }
    ...
    for(int i = 0; i <  events.size() ; i++) {
        convertToSensorEventV2_1(events[i], &buffer);
        print_sns_event(buffer);
    }
}

ScopedWakelock HalProxyCallbackBase::createScopedWakelock(bool lock) {
    ScopedWakelock wakelock(&mScopedWakelockRefCounter, lock);
    return wakelock;
}
...
int main( int argc, char * const argv[] )
{
  ...
  sp<HalProxyCallbackBase> subHalCallback = new HalProxyCallbackBase();
  sensor_module->initialize(subHalCallback);
  ...
  sensor_count=_hal->get_sensors_list( (struct sensor_t const**)&sensors_list );
  ...
  if (list_sensors)
  {
    print_sensor_list(sensors_list);
    delete sensor_module;
    sensor_module = nullptr;
    exit(1);
  }
  ...
  if(!is_interactive)
  {
    ...
    error = find_sensor_by_type(sensor_type, wakeup_flag, sensors_list, &sensor_to_use);
    if ( error ) {
