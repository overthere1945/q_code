...
public class SensorReqMessage {
  ...
  private Map streamingStatusTable = new HashMap();

  SensorReqMessage() {
    streamingStatusTable = new HashMap();
  }
  ...
  public boolean isSensorStreaming(int clientConnectId) {
    if(streamingStatusTable.containsKey(clientConnectId) == true)
      return (boolean) streamingStatusTable.get(clientConnectId);
    else
      return false;
  }

  public void setSensorStreaming(boolean sensorStreaming, int clientConnectId) {
    streamingStatusTable.put(clientConnectId, sensorStreaming);
  }
}
