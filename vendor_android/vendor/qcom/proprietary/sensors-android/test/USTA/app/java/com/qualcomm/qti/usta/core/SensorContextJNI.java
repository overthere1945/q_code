...
public class SensorContextJNI {
  ...
  private static native int sendRequest(ModeType inModeType, int inSensorHandle, ReqMsgPayload inReqMsg, SendReqStdFields sendReqStdFieldObj, String logFileName, int clientConnectID);
  ...
  private static native String[] getResamplerType(ModeType inModeType);

  private static native String[] getThresholdType(ModeType inModeType);
  ...
  public static String[] getResamplerTypeNativeWrapper(ModeType inModeType) {

    if (Build.MODEL.contains("Android SDK")) {

      return new String[]{"SNS_RESAMPLER_RATE_FIXED", "SNS_RESAMPLER_RATE_MINIMUM"};
    } else {

      return getResamplerType(inModeType);
    }
  }

  public static String[] getThresholdTypeNativeWrapper(ModeType inModeType) {

    if (Build.MODEL.contains("Android SDK")) {

      return new String[]{"SNS_THRESHOLD_TYPE_RELATIVE_VALUE", "SNS_THRESHOLD_TYPE_RELATIVE_PERCENT", "SNS_THRESHOLD_TYPE_ABSOLUTE" , "SNS_THRESHOLD_TYPE_ANGLE"};
    } else {

      return getThresholdType(inModeType);
    }
  }
  public static int sendRequestNativeWrapper(ModeType inModeType, int inSensorHandle, ReqMsgPayload inReqMsg, SendReqStdFields sendReqStdFieldObj, String logFileName , int clientConnectID) {

    if (!Build.MODEL.contains("Android SDK")) {
      USTALog.i("sendRequestNativeWrapper clientConnectID " + clientConnectID);
      USTALog.i("sendRequestNativeWrapper getResamplerType " + sendReqStdFieldObj.getResamplerType());
      USTALog.i("sendRequestNativeWrapper getResamplerFilter " + sendReqStdFieldObj.getResamplerFilter());
      USTALog.i("sendRequestNativeWrapper getThresholdType " + sendReqStdFieldObj.getThresholdType());
      USTALog.i("sendRequestNativeWrapper getThresholdValueX " + sendReqStdFieldObj.getThresholdValueX());
      USTALog.i("sendRequestNativeWrapper getThresholdValueY " + sendReqStdFieldObj.getThresholdValueY());
      USTALog.i("sendRequestNativeWrapper getThresholdValueZ " + sendReqStdFieldObj.getThresholdValueZ());

      return(sendRequest(inModeType, inSensorHandle, inReqMsg, sendReqStdFieldObj, logFileName, clientConnectID));
    }
    else {
