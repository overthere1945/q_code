...
public class SensorPayloadFragment extends Fragment {
  ...
  private static boolean isDirectChannelDynamicLayout = false;

  public static SensorPayloadFragment CreateSensorPayloadFragmentInstance(int sensorHandle, int reqMsgID, SensorContext sensorContextInstance, boolean isDirectChannel) {
    ...
    isDirectChannelDynamicLayout = isDirectChannel;
  ...
  public void onViewCreated(View view, Bundle savedInstanceState){
    if(sensorContext.getScriptUtil().getScriptDataTable() != null){
      assignPayLoadDataFromScript(sensorContext.getScriptUtil().getScriptDataTable());
    }
  }
  ...
  private View createPayloadViewLayout(Vector<SensorPayloadField> sensorPayloadFields) {
    ...
    if(false == isDirectChannelDynamicLayout) {
      //Request Button
      ...
      if(clientConnectID == -1)
        addButton(level_2_Layout, "Flush Request", RIDFlushReq, flushReqBtnListener(), false);
      else
      addButton(level_2_Layout, "Flush Request", RIDFlushReq, flushReqBtnListener(), sensorContext.getSensors().get(sensorHandle).isSensorStreaming(sensorReqHandle, sensorContext.getSensors().get(sensorHandle).getSensorStdFields(sensorReqHandle).getClientConnectID()));
      ...
      USTALog.i("before creating stop button ");
      USTALog.i("sensor Handle " + sensorHandle + " sensor Request handle " + sensorReqHandle + " client Connect ID " + sensorContext.getSensors().get(sensorHandle).getSensorStdFields(sensorReqHandle).getClientConnectID());
      if(clientConnectID == -1)
        addButton(level_2_Layout, stopStreaming , RIDStopReq, stopReqBtnListener(), false);
      else
      addButton(level_2_Layout, stopStreaming , RIDStopReq, stopReqBtnListener(), sensorContext.getSensors().get(sensorHandle).isSensorStreaming(sensorReqHandle, sensorContext.getSensors().get(sensorHandle).getSensorStdFields(sensorReqHandle).getClientConnectID()));
    }
...
//      updateRepeatedFields();
    ...
    return new View.OnClickListener() {
      ...
      public void onClick(View v) {
        if(sensorContext.isScriptGenerationEnabled == true) {
          ...
          if (sensorContext.sendRequest(sensorHandle, sensorReqHandle, getActivity(), "sensorLog.txt", false) > 0) {
  ...
  private View.OnClickListener stopReqBtnListener() {
    ...
    return new View.OnClickListener() {
      ...
      @Override
      public void onClick(View v) {
        USTALog.i("Stop Button Pressed ");
        int clinetConnectId = sensorContext.sendRequest(sensorHandle, sensorReqHandle, getContext(), null, true);
        USTALog.i("Stop Completed with " + clinetConnectId);
        if(clinetConnectId > 0) {
          if(false == sensorContext.getSensors().get(sensorHandle).isSensorStreaming(sensorReqHandle, clinetConnectId)) {
            USTALog.i(" sensor is not streaming - So disabling it ");
...
//    }
      ...
      while (it.hasNext()) {
        ...
        if (currentMapEntry.getValue() != null) {
          ...
          if (currentPayLoadFieldFromHashTable.getFieldName().equals(payloadField.getFieldName()) == true) {
            updateFieldValuefromScript(payloadField.getDataType(), (payLoadFieldValuesFromScript != null && payLoadFieldValuesFromScript.size() != 0) ? payLoadFieldValuesFromScript.elementAt(0) : null, (int) currentMapEntry.getKey(), currentPayLoadFieldFromHashTable);
  ...
  private void updateFieldValuefromScript(DataType fieldDataType, String fieldValue , int widgetID, SensorPayloadField currentPayLoadFieldFromHashTable){
    switch (fieldDataType){
      case ENUM:
        Spinner spinner = (Spinner) getView().findViewById(widgetID);
      ...
      case UNSIGNED_INTEGER64:
        EditText editText = (EditText) getView().findViewById(widgetID);
        if(fieldValue == null || fieldValue.length() == 0 || fieldValue.equals("null")){
        ...
        break;
      case BOOLEAN:
        CheckBox checkBox = (CheckBox) getView().findViewById(widgetID);
        if(fieldValue == null || fieldValue.length() == 0 || fieldValue.equals("null")){
          checkBox.setChecked(false);
          currentPayLoadFieldFromHashTable.addValueToWidgetIDValueTable(null ,widgetID );
        } else{
  ...
  public void assignPayLoadDataFromScript(Map scriptDataTable) {
