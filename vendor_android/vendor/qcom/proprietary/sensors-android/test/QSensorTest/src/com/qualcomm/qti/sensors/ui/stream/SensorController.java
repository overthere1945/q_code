  ...
  public void onClick(View view) {
   ...
   if(R.id.delay_button_game == view.getId()){
    ...
    if(R.id.delay_button_submit == view.getId()){
      ...
      try{
        reportRate = Integer.parseInt(reportField.getText().toString());
      ...
      try{
        ...
        this.sensorAdapter.isUserDefined = true;
      }
      catch(NumberFormatException e){
        Log.i (QSensorContext.TAG," sample field value is in google pre-defined format");
        if(rate == -1)
          Toast.makeText(TabControl.getContext(), "Invalid number entry", Toast.LENGTH_SHORT).show();
      }
      catch(Exception e){
        this.sensorAdapter.isDelayButtonSubmit = false;
        Toast.makeText(TabControl.getContext(), "Invalid number entry", Toast.LENGTH_SHORT).show();
      }

      this.sensorAdapter.isDelayButtonSubmit = true;
      this.sensorAdapter.streamRateIs(rate, reportRate, true);
