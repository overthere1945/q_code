...
Copyright (c) 2013-2018,2020, 2022-2023 Qualcomm Technologies, Inc.
All Rights Reserved.
...
public class SensorAdapter extends QSensor {
  ...
  public boolean isDelayButtonSubmit = false;
  public boolean isUserDefined = false;
  public static boolean isFlushRequest = false;
  static {
  ...
  public synchronized void streamRateIs(int streamRate, int reportRate, boolean clear) {
    ...
    Log.d(QSensorContext.TAG, " streamRateIs " +streamRate+ " Sensor:" + this.sensor().getName());
    if(this.sensor().isAdditionalInfoSupported()){
      if(null != this.addlInfoVerifier) {
        this.sensorManager.unregisterListener(addlInfoVerifier, this.sensor());
        this.addlInfoVerifier = null;
      }
    }else{
      if(null != this.sensorListener) {
        this.sensorManager.unregisterListener(sensorListener, this.sensor());
        this.sensorListener = null;
      }
    }
      ...
      if(this.sensor().isAdditionalInfoSupported()) {
          ...
          if(-1 == reportRate) {
              ...
              if(!sensorManager.registerListener(addlInfoVerifier,
                    this.sensor(), isUserDefined == true ? streamRate * 1000: streamRate, handler)) {
                  streamRate = -1;
              ...
              if(!sensorManager.registerListener(addlInfoVerifier, this.sensor(),
                isUserDefined == true ? streamRate * 1000: streamRate, reportRate * 1000, handler)) {
                  streamRate = -1;
                  reportRate = -1;
                  isDelayButtonSubmit = false;
                  Log.e(QSensorContext.TAG, "Unable to register new listener using addinfoHandler");
          ...
          if(-1 == reportRate) {
            if(!sensorManager.registerListener(this.sensorListener,
             ...
             if(!sensorManager.registerListener(this.sensorListener,
                this.sensor(), isUserDefined == true ? streamRate * 1000 : streamRate, reportRate * 1000, handler)) {
              streamRate = -1;
              reportRate = -1;
              isDelayButtonSubmit = false;
              Log.e(QSensorContext.TAG, "Unable to register new listener");
    ...
    if(this.sensor().isAdditionalInfoSupported()){
      if(null == this.addlInfoVerifier) {
        ...
        Toast.makeText(QSensorContext.getContext(), "Cannot flush inactive sensor:\n" + this.sensor().getName(),
          ...
          sensorManager.flush(this.addlInfoVerifier);
        }
    }else{
      if(null == this.sensorListener) {
        Log.e(QSensorContext.TAG, "Cannot flush inactive sensor");
  ...
  public synchronized String accuracyString() {
    ...
    outputString = "Acc:" +
        SensorAdapter.sensorAccuracy(accuracy) +
  ...
  public synchronized String streamRateString() {
    ...
    outputString += "Rate: " + streamRateString(this.streamRate());
  ...
  public synchronized String reportRateString() {
    String outputString = "";
    outputString += "Batch: " + reportRateString(this.reportRate());
   ...
   */
  public String streamRateString(int streamRate){
    String rateStr="";
    if(isUserDefined){
      if(streamRate == -1)
        rateStr = "Off";
  ...
  public String reportRateString(int reportRate){
    String rateStr="";
    return reportRate == -1 ? "Off" : String.valueOf(reportRate);
  }
