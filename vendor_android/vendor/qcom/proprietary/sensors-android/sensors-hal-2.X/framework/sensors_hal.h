...
class sensors_hal_event_callback {
  public:
    virtual ~sensors_hal_event_callback(){};
    virtual void post_events(const std::vector<Event>& events, bool wakeup) = 0;
};
...
public:
    ...
    void register_event_callback(sensors_hal_event_callback* callback) { _event_callback = callback; }

    bool is_direct_channel_supported() { return _is_direct_channel_supported; }
     ...
     *
     */
    int unregister_direct_channel(int channel_handle);
    /**
...
private:
    ...
    ~sensors_hal(){};
    /*
     * discovers suids of sensors present on QSH and creates sensor
    ...
    sensors_hal_event_callback* _event_callback;

    std::list<direct_channel*> open_direct_channels;
    std::mutex open_direct_channels_mutex;
