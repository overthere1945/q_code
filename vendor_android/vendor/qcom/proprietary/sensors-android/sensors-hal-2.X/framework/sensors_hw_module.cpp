...
Return<Result> sensors_hw_module::injectSensorData_2_1(const Event &data)
{
    return Result::INVALID_OPERATION;
}
...
Return<Result> sensors_hw_module::unregisterDirectChannel(int32_t channelHandle)
{
    if(_is_direct_channel_supported == false) {
      // HAL does not support
        ...
        int ret = _hal->unregister_direct_channel((int)channelHandle);
        if(ret != 0) {
          sns_loge("%s failed for channelhandle[%d]" , __FUNCTION__ , (int)channelHandle);
          return Result::BAD_VALUE;
        }
...
Return<void> sensors_hw_module::getSensorsList_2_1(getSensorsList_2_1_cb _hidl_cb)
{
    sensor_t const *list;
...
ISensorsSubHal* sensorsHalGetSubHal_2_1(uint32_t* version) {
    static ::android::hardware::sensors::V2_1::subhal::implementation::sensors_hw_module subHal;
