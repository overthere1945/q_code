...
static const char *QSH_DATATYPE_ROTV = "rotv";

class rotv : public qsh_sensor
{
public:
    rotv(suid sensor_uid, sensor_wakeup_type wakeup);
    static const char* qsh_datatype() { return QSH_DATATYPE_ROTV; }
private:
    virtual void handle_sns_std_sensor_event(
        const sns_client_event_msg_sns_client_event& pb_event) override;
};

rotv::rotv(suid sensor_uid, sensor_wakeup_type wakeup):
...
static bool rotv_module_init()
{
    ...
    sensor_factory::register_sensor(
        ...
        qsh_sensor::get_available_sensors<rotv>);
    sensor_factory::request_datatype(QSH_DATATYPE_ROTV);
    return true;
}
