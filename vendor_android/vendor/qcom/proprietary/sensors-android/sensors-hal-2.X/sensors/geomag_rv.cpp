...
static const char *QSH_DATATYPE_GEOMAG_RV = "geomag_rv";

class geomag_rv : public qsh_sensor
{
public:
    geomag_rv(suid sensor_uid, sensor_wakeup_type wakeup);
    static const char* qsh_datatype() { return QSH_DATATYPE_GEOMAG_RV; }
private:
    virtual void handle_sns_std_sensor_event(
        const sns_client_event_msg_sns_client_event& pb_event) override;
};

geomag_rv::geomag_rv(suid sensor_uid, sensor_wakeup_type wakeup):
...
void geomag_rv::handle_sns_std_sensor_event(
    ...
    sns_logv("geomag_rv_event: ts=%llu, x = %f, y=%f, z = %f, w = %f, acc = %f",
...
static bool geomag_rv_module_init()
{
    ...
    sensor_factory::register_sensor(SENSOR_TYPE_GEOMAGNETIC_ROTATION_VECTOR,
        qsh_sensor::get_available_sensors<geomag_rv>);
    sensor_factory::request_datatype(QSH_DATATYPE_GEOMAG_RV);
    return true;
}
