...
static const char *QSH_DATATYPE_TILT_DETECTOR = "tilt";

class tilt : public qsh_sensor
{
public:
    tilt(suid sensor_uid, sensor_wakeup_type wakeup);
    static const char* qsh_datatype() { return QSH_DATATYPE_TILT_DETECTOR; }
...
tilt::tilt(suid sensor_uid, sensor_wakeup_type wakeup):
...
static bool tilt_module_init()
{
    ...
    sensor_factory::register_sensor(SENSOR_TYPE_TILT_DETECTOR,
        qsh_sensor::get_available_wakeup_sensors<tilt>);
    sensor_factory::request_datatype(QSH_DATATYPE_TILT_DETECTOR);
    return true;
}
