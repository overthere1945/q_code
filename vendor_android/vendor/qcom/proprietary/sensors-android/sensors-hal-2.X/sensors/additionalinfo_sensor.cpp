...
additionalinfo_sensor::additionalinfo_sensor(suid sensor_uid,
...
void additionalinfo_sensor::send_additional_info(int64_t timestamp)
{
    ...
    hal_event.u.additional.type =
        (AdditionalInfoType)AINFO_BEGIN;
    events.push_back(hal_event);

    hal_event.timestamp++;
    hal_event.u.additional.type =
            (AdditionalInfoType)AINFO_INTERNAL_TEMPERATURE;
