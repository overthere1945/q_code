...
#define ALIGN_16(x) ((x + 0x0F)&0xFFFFFFF0)
...
pthread_t stop_sreaming_thread;
...
/**
 * @brief    open direct channel session
 * @return   remote_handle64 handle for communication to DSP
 **/
static remote_handle64 direct_channel_open(){
  int nErr = AEE_SUCCESS;
  char *uri = sns_direct_channel_URI "&_dom=adsp";
  remote_handle64 handle_l;

  if (AEE_SUCCESS == (nErr = sns_direct_channel_open(uri, &handle_l))) {
    printf("sns_direct_channel_open success for sensorspd - handle_l is %ud\n" , (unsigned int)handle_l);
  } else {
    printf("sns_direct_channel_open failed for sensorspd - handle_l is %u\n" , (unsigned int)handle_l);
  }
  return handle_l;
}
...
/**
 * @brief   Allocates via ION a buffer of specified size
 *          RPCMEM_HEAP_ID_SYSTEM  -  heap id to use
 *          RPCMEM_DEFAULT_FLAGS  -  ion flags to use to when allocating
 *          DIRECT_CHANNEL_SHARED_MEMORY_SIZE  -  the buffer size to allocate
 *
 * @return  fd to allocated buffer
 **/
...
/**
 * @brief   Deletes the sns_direct_channel_stream_id "channel_handle"
 *          on the current direct channel session "fastRPC_remote_handle"
 **/
...
/**
 * @brief   Creates a new sns_direct_channel_stream_id "channel_handle"
 *          on the current direct channel session "fastRPC_remote_handle"
 **/
...
/**
 * @brief   Sends configuration request for the current active
 *          sns_direct_channel_stream_id "channel_handle" in
 *          pb-encoded format
 **/
...
/**
 *  @brief  Suspends execution for specified amount of time
 */
void sr_based_sleep() {
...
/**
 *  @brief  Reads event received from DSP.
 *          Multiplexed data event is received in the Android defined structure format.
 */
void* read_mux_data_event(void *) {
  ...
  while(1) {
    ...
    if(previous_counter >= data.reserved0) {
      ...
      printf("ts_nsec[" PRId64 "]," ,data.timestamp);
...
/**
 *  @brief  Read events received from DSP.
 *          Generic data event is the received in pb-encoded format
 */
    ...
    while(1) {
      ...
      if(pckt_header_end_ptr > end_ptr) {
        ...
        if(0 == event.timestamp) {
          ...
          char *pckt_payload_end_ptr = pckt_header_end_ptr +   ALIGN_16(event.event_len);
          ...
          if(pckt_payload_end_ptr > end_ptr) {
            ...
            pckt_payload_end_ptr = pckt_payload_start_ptr + ALIGN_16(event.event_len);
          }
//          /*SUID Event decoding*/
          ...
          if(event_type == SNS_GENERIC_SAMPLE) {
            printf("time_stamp %" PRId64 " with msg_ID %d\t" , event.timestamp, (int)event.message_id);
          ...
          }else if( SNS_DIRECT_CHANNEL_MSGID_SNS_GENERIC_CHANNEL_LATENCY_MSG == event.message_id ){
            if(event_type == SNS_GENERIC_SAMPLE) {
              sns_generic_channel_latency_msg sns_latency_data;
              sns_latency_data.ParseFromArray(pckt_payload_start_ptr, event.event_len);
              printf("\n\n-------------------Latency Packet Data------------------------\n");
              if(sns_latency_data.has_sample_count())
                printf("Sample Count =%" PRIu64 "\n", sns_latency_data.sample_count());
              if(sns_latency_data.has_max_latency_ts())
                printf("Max Latency TS =%" PRIu64 "\n", sns_latency_data.max_latency_ts());
              if(sns_latency_data.has_max_latency())
                printf("Max Latency =%u Micro Seconds\n", sns_latency_data.max_latency());
              if(sns_latency_data.has_avg_latency())
                printf("Avg Latency =%u Micro Seconds\n", sns_latency_data.avg_latency());
              printf("----------------------------------------------------------------\n");
           }
          }
            printf("\n");
...
/**
 *  @brief  Performs suid discovery for user-specified datatype.
 *          A new fRPC channel is opened, config request is sent,
 *          pb-encoded suid event received from DSP
 *          is decoded and after this fRPC channel is closed.
 */
void get_suid() {
...
/**
 *  @brief  Query and save attributes for user-specified datatype.
 *          A new fRPC channel is opened, config request is sent,
 *          pb-encoded attribute event received from DSP
 *          is decoded and after this fRPC channel is closed.
 */
void get_attributes() {
...
/**
 *  @brief  Sends time offset value between client time domain &
 *          direct channel time domain, in pb-encoded format
 */
void set_ts_offset(){
...
/**
 *  @brief  Activates streaming on the channel for the requested datatype
 *          through specified channel type
 */
...
/**
 *  @brief  Removes existing requests on the channel,
 *          and deletes the direct channel session
 */
void *stop_sensor_streaming(void *) {
...
/**
 * @brief   Calculates and adjusts the sample rate according the supported sample rates in hz
 */
...
int main(int argc, char *argv[])
{
   ...
   * direct channel APIs to send/receive from DSP side.
   *
   * For the clients to be able to get sensor data via direct channel,
   * create a fRPC handle ->  start direct channel session  ->  get suid and attribute events for the requested datatype  ->
   * activate the streaming activity  ->  close direct channel session  ->  close fRPC handle
   *
   * Generic channel - transfers pb-encoded data, multiplexing is NOT supported
   * Mux channel -  transfers data in Android defined struct, supports multiplexing, ensures concurrency
   *
   * In case generic channel is used, only one streaming request will be served at a time.
   * For multiple streaming requests to be executed parallely, multiple direct channel sessions have to be created, each serving one request at a time.
   *
   * ====================================================================*/
  ...
  // open direct channel session
  fastRPC_remote_handle = direct_channel_open();

  // perform suid discovery for requested datatype
  get_suid();

  // get attributes for requested datatype
  get_attributes();

  // adjust the sample rate acc to the supported rate
  caluclate_adjusted_sample_rate();

  // stop the streaming activity for the specified dataype once test duration expires
  pthread_create(&stop_sreaming_thread ,NULL, stop_sensor_streaming, NULL);

  // activate sensor streaming
  start_sensor_streaming();

  // close direct channel session
  direct_channel_close();

  // wait for the exixting requests to complete
  (void)pthread_join(stop_sreaming_thread, NULL);
  printf("Main Ended \n");
  return 0;
}
