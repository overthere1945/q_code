...
bool is_direct_channel_supported(){
  char stats_debug[PROP_VALUE_MAX];
...
{
  sns_logi("get_fastRPC_remote_handle Start");
  if(fastRPC_remote_handle == -1) {
    ...
    if (AEE_SUCCESS == (nErr = sns_direct_channel_open(uri.c_str(), &handle_l))) {
      sns_logi("sns_direct_channel_open success for sensorspd - handle_l is %ud" , (unsigned int)handle_l);
      fastRPC_remote_handle = handle_l;
    } else {
      fastRPC_remote_handle = -1;
    }
  }
  sns_logi("get_fastRPC_remote_handle End");
}
...
{
  sns_logi("sensor_direct_channel start");
  ...
  if(0 == ret) {
    sns_logi("sensor_direct_channel sns_direct_channel_create success , and channel_handle %d ", channel_handle);
  } else {
    sns_loge("sensor_direct_channel sns_direct_channel_create failed");
  }
  ...
  sns_logi("sensor_direct_channel End");
}

sensor_direct_channel::~sensor_direct_channel() {
  sns_logi("sensor_direct_channel ~ Start ");
  if(sns_direct_channel_delete(fastRPC_remote_handle, channel_handle) == 0) {
    sns_logi("sns_direct_channel_delete Pass ");
  } else {
    sns_logi("sns_direct_channel_delete failed");
  }
  fd_ptr = -1;
  sns_logi("sensor_direct_channel ~ End ");
}
...
{
  sns_logi("sensor_direct_channel update_offset Start");
  ...
  if(0 == ret) {
    sns_logi("sensor_direct_channel update_offset success");
  } else {
    sns_loge("sensor_direct_channel update_offset failed");
  }
  current_offset = offset;
  sns_logi("sensor_direct_channel update_offset End");
  return ret;
}
    ...
    )
{
  ...
  if(true == stream_info.has_calibrated) {
    ...
    sns_logi("sensor_direct_channel::send_request set_calibrated %d " , (int)stream_info.is_calibrated);
  }

  if(true == stream_info.has_fixed_rate) {
    stream_id_msg->set_resampled(stream_info.is_resampled);
    sns_logi("sensor_direct_channel::send_request is_resampled %d " , (int)stream_info.is_resampled);
  }
  ...
  if(true == std_req_fields_info.has_batch_period) {
    ...
    sns_logi("sensor_direct_channel::send_request has_batch_period is false - Omiting batch_spec as it is optional");
  }
  ...
  sns_logi("pb_encoded_direct_channel_config_msg string is %s and size is %zu" , pb_encoded_direct_channel_config_msg.c_str() , pb_encoded_direct_channel_config_msg.size());

  int ret =sns_direct_channel_config(fastRPC_remote_handle, channel_handle, (const unsigned char*)pb_encoded_direct_channel_config_msg.c_str(), pb_encoded_direct_channel_config_msg.size());
  if(0 == ret) {
    sns_logi("sensor_direct_channel send_request success");
  } else {
    sns_loge("sensor_direct_channel send_request failed");
  }
...
{
  sns_logi("sensor_direct_channel stop_request Start");
  ...
  suid->set_suid_low(stream_info.suid_low);
  suid->set_suid_high(stream_info.suid_high);
  sns_logi("channel_handle[%d], suid_low[%ld] , suid_high[%ld]" , channel_handle, stream_info.suid_low, stream_info.suid_high);
  if(true == stream_info.has_calibrated) {
    stream_id_msg->set_calibrated(stream_info.is_calibrated);
    sns_logi("is_calibrated[%d]" , stream_info.is_calibrated);
  }

  if(true == stream_info.has_fixed_rate) {
    stream_id_msg->set_resampled(stream_info.is_resampled);
    sns_logi("is_resampled[%d]" , stream_info.is_resampled);
  }
  config_msg.SerializeToString(&pb_encoded_direct_channel_config_msg);

  sns_logi("sensor_direct_channel stop_request before sending to DSP ");
  int ret = sns_direct_channel_config(fastRPC_remote_handle, channel_handle, (const unsigned char*)pb_encoded_direct_channel_config_msg.c_str(), pb_encoded_direct_channel_config_msg.size());
  if(0 == ret) {
    sns_logi("sensor_direct_channel stop_request success");
  } else {
    sns_loge("sensor_direct_channel stop_request failed");
  }

  sns_logi("sensor_direct_channel stop_request end ");
  return ret;
}
...
void sensor_direct_channel::dump_mux_channel_data(char *start_ptr, FILE *fp)
{
  ...
  while(1) {
    ...
    if(previous_counter >= data.reserved0) {
      ...
      if(temp >= end_ptr) {
        sns_logi("end of circular buffer ");
        temp = start_ptr;
      }
    }
  }
...
void sensor_direct_channel::dump_generic_channel_data(char *start_ptr, FILE *fp)
{
  ...
  while(1) {
    ...
    if(pckt_header_end_ptr > end_ptr) {
      sns_logi("Circular buffer overflow scenario - header not fit");
      temp = start_ptr;
      continue;
    } else {
      ...
      if(0 == event.timestamp) {
        ...
        if(pckt_payload_start_ptr == NULL || pckt_payload_end_ptr == NULL)
          break;
        ...
        if(pckt_payload_end_ptr > end_ptr) {
          sns_logi("Circular buffer overflow scenario - Headers fits but not payload");
        ...
        sns_logi("ts_nsec[%" PRId64 "]\t msg ID[%" PRId32 "]\t status[%d]\t", event.timestamp, event.message_id , (int)std_sensor_data.status());
        ...
        if( SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_EVENT == event.message_id ) {
          for(int i = 0 ; i < std_sensor_data.data_size() ; i++) {
           ...
           fprintf(fp, "Sample Count =%" PRIu64, sns_latency_data.sample_count());
           fprintf(fp, "Max Latency TS =%" PRIu64, sns_latency_data.max_latency_ts());
           fprintf(fp, "Max Latency =%u", sns_latency_data.max_latency());
           fprintf(fp, "Avg Latency =%u", sns_latency_data.avg_latency());
        }
        fprintf(fp, "\n");
