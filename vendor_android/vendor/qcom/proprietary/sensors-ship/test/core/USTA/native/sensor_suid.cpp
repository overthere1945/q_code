...
SuidSensor::SuidSensor(handle_suid_event_cb suid_event_cb, ISession* conn):
    suid_event_cb(suid_event_cb)
{
  ...
  {
    sns_loge("SuidSensor::SuidSensor sns_suid_sensor is Not found");
  ...
  sensor_uid.low = suid_low_desc->default_value_uint64();
  sensor_uid.high = suid_high_desc->default_value_uint64();

  // SUID nows opens a connections to QSH client
  _suid_event_cb=[this](const uint8_t* msg , int msgLength, uint64_t time_stamp)
                { this->sensor_event_cb(msg, msgLength, time_stamp); };

  _conn = conn;
  int ret = _conn->setCallBacks(sensor_uid,_suid_resp_cb,_suid_error_cb,_suid_event_cb);
  if(0 != ret)
  ...
  {
    sns_loge("Message parser instance creation failed "
...
vector<string> SuidSensor::get_resampler_type_list()
{
  vector<string> resampler_type;
  const EnumDescriptor* enumDescriptor = pool.FindEnumTypeByName("sns_resampler_rate");
  if(enumDescriptor == NULL)
  {
    sns_loge("SuidSensor::get_resampler_type_list sns_resampler_rate is Not found");
  } else{
    for (int enum_index = 0; enum_index < enumDescriptor->value_count(); enum_index++)
    {
      ...
      resampler_type.push_back(enum_name);
    }
  }
  return resampler_type;
}
vector<string> SuidSensor::get_threshold_type_list()
{
  vector<string> threshold_type;
  const EnumDescriptor* enumDescriptor = pool.FindEnumTypeByName("sns_threshold_type");
  if(enumDescriptor == NULL)
  {
    sns_loge("SuidSensor::get_threshold_type_list sns_threshold_type is Not found");
  } else{
    for (int enum_index = 0; enum_index < enumDescriptor->value_count(); enum_index++)
    {
      const EnumValueDescriptor* enum_value_des = enumDescriptor->value(enum_index);
      ...
      //So skipping the current enum value and proceeding further.
      string enum_name = enum_value_des->name();
      threshold_type.push_back(enum_name);
    }
  }
  return threshold_type;
}

vector<string> SuidSensor::get_wakeup_delivery_list()
{
  vector<string> wakeup_delivery_type;
  const EnumDescriptor* enumDescriptor = pool.FindEnumTypeByName("sns_client_delivery");
  if(enumDescriptor == NULL)
  {
    sns_loge("SuidSensor::get_wakeup_delivery_list sns_client_delivery is Not found");
  } else{
    for (int enum_index = 0; enum_index < enumDescriptor->value_count(); enum_index++)
    {
      const EnumValueDescriptor* enum_value_des = enumDescriptor->value(enum_index);
      ...
      //So skipping the current enum value and proceeding further.
      string enum_name = enum_value_des->name();
      wakeup_delivery_type.push_back(enum_name);
    }
  }
  return wakeup_delivery_type;
}

vector<string> SuidSensor::get_client_processor_list()
{
  vector<string> client_processor;
  const EnumDescriptor* enumDescriptor = pool.FindEnumTypeByName("sns_std_client_processor");
  if(enumDescriptor == NULL)
  {
    sns_loge("SuidSensor::get_client_processors_list sns_std_client_processor is Not found");
  } else{
    for (int enum_index = 0; enum_index < enumDescriptor->value_count(); enum_index++)
    {
      const EnumValueDescriptor* enum_value_des = enumDescriptor->value(enum_index);
      ...
      //So skipping the current enum value and proceeding further.
      string enum_name = enum_value_des->name();
      client_processor.push_back(enum_name);
    }
  }
...
void SuidSensor::sensor_event_cb(const uint8_t *encoded_data_string,
    ...
    uint64_t time_stamp) // receiving response from qsh_client
{
  ...
  if(message_descriptor == NULL)
  {
    sns_loge("SuidSensor::sensor_event_cb sns_client_event_msg is Not found");
  ...
  if(prototype == NULL)
  {
    sns_loge("SuidSensor::sensor_event_cb prototype is Null");
  ...
  if(message == NULL)
  {
    sns_loge("SuidSensor::sensor_event_cb message is Null");
  ...
  if(reflection == NULL)
  {
    sns_loge("SuidSensor::sensor_event_cb reflection is Null");
  ...
  {
    sns_loge("SuidSensor::sensor_event_cb suid_field_desc is Null");
  ...
  {
    sns_loge("SuidSensor::sensor_event_cb sub_message / suid_msg_desc is Null");
  ...
  {
    sns_loge("SuidSensor::sensor_event_cb events_field_desc is Null");
    return;
  }
  ...
  {
    sns_logd("Number of events are not equal to 1");
    return ;
  }
  ...
  {
    sns_loge("SuidSensor::sensor_event_cb events_msg_desc is Null");
  ...
  {
    sns_loge("SuidSensor::sensor_event_cb data_type is not found");
    return; // Occurs when there is a mismatch between SLPI and HLOS protos.
    // Avoid force crash to un-block dev and I&T team
  }

  sns_logd(" suid event received for data type %s", reflection->GetString(*message, fd_data_type).c_str());

  const FieldDescriptor* fd_suid = message_descriptor->FindFieldByName("suid");
  if(fd_suid == NULL)
  {
    sns_loge("SuidSensor::sensor_event_cb suid is not found");
    return; // Occurs when there is a mismatch between SLPI and HLOS protos.
    // Avoid force crash to un-block dev and I&T team
  }
  int num_suids = reflection->FieldSize(*message, fd_suid);

  sns_logd("\nNumber of suids recevied is %d", num_suids);
  ...
  for (int j=0;j<num_suids;j++)
  {
    ...
    if(sub_des == NULL)
    {
      sns_loge("SuidSensor::sensor_event_cb sub_des is NULL");
    ...
    if(fd1 == NULL)
    {
      sns_loge("SuidSensor::sensor_event_cb suid_low is not found");
    ...
    if(fd1 == NULL)
    {
      sns_loge("SuidSensor::sensor_event_cb suid_high is not found");
      continue;//fd1 can be NULL if proto mismatch between SLPI and HLOS
    }
    suid_received.high =
        sub_message->GetReflection()->GetUInt64(*sub_message, fd1);
    ...
    suid_event_cb(suid_received.low,suid_received.high,
...
usta_rc SuidSensor::send_request(string& data_type)
{
  ...
  if(NULL == fileStreamPtr)
  {
    ...
    sns_logd("logFile Name for sensor list: is %s", logfile_completepath.c_str());
    fileStreamPtr = fopen(logfile_completepath.c_str() , "w");

    if(NULL == fileStreamPtr)
    {
      sns_loge("logFile creation failed for %s error is %d", logfile_name.c_str(), errno);
  ...
  {
    sns_loge("parse request message is failed for SUID Class");
    return rc;
  }
  ...
  send_req_field_info suid_field;
  {
    ...
    send_req_nested_msg_info suid_msg;
    {
      ...
      suid_low_field.value.push_back(to_string(sensor_uid.low));

      send_req_field_info suid_high_field;
      suid_high_field.name = "suid_high";
      suid_high_field.value.push_back(to_string(sensor_uid.high));
  ...
  {
    sns_loge("Error!! encoding main request failed for SUID class sensor");
    return rc;
  }
  ...
  sns_logd("sending request through new interface");
  if(nullptr != _conn){
    int ret = _conn->sendRequest(sensor_uid, main_request_encoded);
    if(0 != ret){
