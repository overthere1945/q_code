...
void MessageParser::get_protos_from_api(string protoName)
{
  ...
  {
    sns_loge(" Error! requested proto file (%s) not found in descriptor pool", protoName.c_str());
    return;
  }
...
usta_rc MessageParser::set_api_info(vector<string>& api)
{
  ...
  {
    protos.push_back(api[index]);
    get_protos_from_api(api[index]);
  }
  ...
  for(auto it=protos.begin();it!=protos.end();it++)
  {
    ...
    if (NULL == file_desc)
    {
      sns_loge(" Error! requested proto file (%s) not found in descriptor pool", (*it).c_str());
    }
    else
    {
      protos_found |= true;
  ...
  if(! protos_found)
  {
    sns_loge(" Error None of the proto files found, returning");
...
usta_rc MessageParser::get_request_msg(vector<msg_info> &final_request_msg_out)
{
  if(!protos_found)
  {
    sns_loge("Error!! no proto file found for get_request_msg");
    return USTA_RC_NO_PROTO;
  }
  if(request_msgs.size()==0)
  {
    sns_loge("Error!! No request message found");
...
usta_rc MessageParser::formulate_request_msg(send_req_msg_info& req_msg,string& request_string)
{
  ...
  {
    sns_loge(" Error!! message %s not found in descriptor pool,cannot formulate request", req_msg.msg_name.c_str());
  ...
  if(message == NULL ) {
    sns_loge(" failed to create profo buffer memory for message");
    return USTA_RC_NO_MEMORY;
  }

  if(USTA_RC_SUCCESS !=(rc=InputMessage(message,req_msg.fields)))
  {
    sns_loge(" formulation for message %s failed", req_msg.msg_name.c_str());
    return rc;
  }

  message->SerializeToString(&request_string);
  sns_logd(" encoded input message for %s is generated", req_msg.msg_name.c_str());
...
usta_rc MessageParser::InputMessage(Message* message,vector<send_req_field_info>& fields)
{
  ...
  if(NULL == descriptor)
  {
    sns_logd("descriptor was not found ");
    ...
    vector<send_req_field_info>& user_defined_fields)
{
  ...
  case FieldDescriptor::CPPTYPE_ENUM:
  {
    ...
    if(field->is_repeated())
    {
      for(unsigned int repeat_index = 0 ; repeat_index < input_str.size();repeat_index++)
      {
        const EnumValueDescriptor *enum_value = ed->FindValueByName(input_str[repeat_index]);
        if (enum_value == NULL)
        {
          sns_loge("Error!! enum name %s in not member of enum %s", input_str[repeat_index].c_str(), ed->name().c_str());
          return USTA_RC_FAILED;
          break;
        }
    ...
    {
      const EnumValueDescriptor *enum_value = ed->FindValueByName(input_str[0]);
      if (enum_value == NULL)
      {
        sns_loge("Error!! enum name %s in not member of enum %s", input_str[0].c_str(), ed->name().c_str());
        return USTA_RC_FAILED;
        break;
      }
  ...
  default:
    sns_loge("Error!! type not valid for request message");
    break;
  }
...
usta_rc MessageParser::	generate_request_msg_list()
{
  ...
  {
    sns_logd(" Error !! msg_look_up size is 0.."
  ...
  for (auto it = msg_look_up.begin(); it != msg_look_up.end(); it++)
  {
    if((it->first>=512) && (it->first<=767))
    {
      ...
      sns_logd("msg ID is %s", to_string(it->first).c_str());
    ...
    const string& message,int index)
{
  ...
  if (NULL == desc)
  {
    sns_logd(" Error ! message %s not found in descriptor pool "
        "parse_request_msg_fields failed", message.c_str());
    return USTA_RC_DESC_FAILED;
  }
...
usta_rc MessageParser::generate_msg_look_up()
{
  ...
  for (auto it = protos.begin(); it != protos.end(); it++)
  {
    ...
    {
      sns_logd("Check!! No enum found in %s", (*it).c_str());
      continue;
    }
    for (int e = 0; e < enumDescriptor->value_count(); e++)
    {
      ...
      if (pos != 0)
      {
        sns_logd(" convention not followed for enum");
      }
      else
      {
        ...
        {
          sns_logd(" Error!! Duplicate id %s found. overwriting", to_string(id).c_str());
        }
    ...
    int data_size)
{
  ...
  if (NULL == message_descriptor)
  {
    sns_loge("message %s not found in decriptor pool", message_name.c_str());
    return USTA_RC_DESC_FAILED;
  }
  DynamicMessageFactory dmf;
  ...
  if(ret != true)
  {
    sns_logd("Required fields are missing at encoding level");
...
void MessageParser::OutputMessage(Message* message)
{
  ...
  if(NULL == reflection)
  {
    sns_loge("Error!! reflection is Null");
    return;
  }
  ...
  {
      bool is_payload_field = (bool)!((*field_it)->name().compare("payload"));
      bool is_empty_payload = false;

      if (is_payload_field){
          std::string str_payload = reflection->GetString(*message, *field_it);
          if (str_payload.empty()){
              is_empty_payload = true;
          }
      }
      if (is_empty_payload){
          sns_logd(" Don't add Field Separator to JSON Format, as payload is empty.  msg_id:%s", to_string(current_msg_id).c_str());
      }else{
          if (i > 0)
              OutputFieldSeparator();
      }
      if (is_empty_payload){
          sns_logd(" Don't add payload field to JSON Format, as payload is empty.  msg_id: %s", to_string(current_msg_id).c_str());
      }else{
    ...
    const Reflection* reflection, const FieldDescriptor* field)
{
  ...
  if (field->is_repeated())
  {
    ...
    if(message->GetTypeName().compare("sns_std_attr_event") == 0)
      sns_logd("Number of attributes is %s", to_string(count).c_str());
  }
    ...
    int index , string &out_value)
{
  if(NULL == message)
  {
    sns_loge("MessageParser::OutputFieldValue message is NULL ");
    return;
  }
  if(NULL == reflection)
  {
    sns_loge("MessageParser::OutputFieldValue reflection is NULL ");
    return;
  }
  if(NULL == field)
  {
    sns_loge("MessageParser::OutputFieldValue field is NULL ");
    return ;
  }
  ...
  case FieldDescriptor::CPPTYPE_INT32:
  {
    ...
    if (message->GetTypeName().compare("sns_std_attr") == 0)
    {
      ...
      if(ed == NULL)
      {
        sns_loge("MessageParser::OutputFieldValue sns_std_sensor_attr_id not found ");
        return;
      }
      ...
      {
        sns_logd("Skipping enum ID %s", to_string(msg).c_str());
      }
    }
    else
    {
  ...
  case FieldDescriptor::CPPTYPE_UINT64:
  {
            ...
            (field->name().compare("suid_high") == 0)))
    {
      ...
      output_str += "\"";
      if(field->name().compare("suid_low") == 0){
        suid_low = ss.str();
      }
      if((field->name().compare("suid_high") == 0)){
        suid_high = ss.str();
      }
    }
    else
  ...
  case FieldDescriptor::CPPTYPE_STRING: {
    ...
    if (field->name().compare("payload") == 0)
    {
      ...
      if (attrib_event_evd != NULL && current_msg_id == attrib_event_evd->number())
      {
      ...
      else
      {
        ...
        if(512 == current_msg_id){
          if( "abababababababab" == suid_low && "abababababababab" == suid_high ){
            message_type = "sns_suid_req";
          }
        }
      }
  ...
  case FieldDescriptor::CPPTYPE_ENUM:
  {
    ...
    if(enum_desc == NULL)
    {
      sns_loge("MessageParser::OutputFieldValue enum_desc is NULL ");
      return;
    }
    ...
    vector<string> &repeated_values)
{
  if(message == NULL)
  {
    sns_loge("MessageParser::OutputRepeatedField message is NULL");
    return;
  }
  if(reflection == NULL)
  {
    sns_loge("MessageParser::OutputRepeatedField reflection is NULL");
    return;
  }
  if(field == NULL)
  {
    sns_loge("MessageParser::OutputRepeatedField field is NULL");
    return;
  }
    ...
    const string& value)
{
  ...
  if (NULL == message_descriptor)
  {
    sns_loge("Error!! message %s not found in descriptor"
        " pool cannot parse..", message_name.c_str());
  ...
  if(prototype == NULL)
  {
    sns_loge("MessageParser::OutputMessageOpaquePayload prototype is NULL");
    return;
  }
  ...
  if(message == NULL)
  {
    sns_loge("MessageParser::OutputMessageOpaquePayload message is NULL");
    return;
  }
...
void MessageParser::OutputFieldName(const FieldDescriptor* field)
{
  if(field == NULL)
  {
    sns_loge("MessageParser::OutputFieldName field is NULL");
    return;
  }
