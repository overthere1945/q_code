...
bool sensors_diag_log::diag_flag = false;

sensors_diag_log::sensors_diag_log()
{
  is_client_connect_id_updated = false;
  client_connect_id = 0;
  if(false == diag_flag) {
...
sensors_diag_log::~sensors_diag_log()
{
  if(0 != pending_req_msg.size())
  {
    pending_req_msg.clear();
  }
}
    ...
    char *logging_module_type)
{
  ...
  if(false == is_client_connect_id_updated)
  {
    sns_diag_req_mesg_data current_req_msg;
    current_req_msg.request_msg.assign(request_msg);
    current_req_msg.src_sensor_data_type.assign(src_sensor_data_type);
    current_req_msg.logging_module_type.assign(logging_module_type);
    current_req_msg.log_time_stamp = sensors_timeutil::get_instance().qtimer_get_ticks();
    lock_guard<mutex> lk(_mutex_pending_req_msg);
    pending_req_msg.push_back(current_req_msg);
    return return_code;
  }
  else
  {
    log_request_msg(request_msg,src_sensor_data_type,logging_module_type,sensors_timeutil::get_instance().qtimer_get_ticks());
  }
    ...
    uint64_t log_time_stamp)
{
  ...
  client_api_log_req_msg.set_client_id(client_connect_id);
    ...
    char *logging_module_type)
{
  ...
  client_api_log_event_msg.set_client_id(client_connect_id);
...
void sensors_diag_log::update_client_connect_id(uint64_t in_client_connect_id)
{
  if(false == is_client_connect_id_updated)
  {
    lock_guard<mutex> lk(_mutex_pending_req_msg);
    client_connect_id = in_client_connect_id;
    for(size_t pending_req_index  = 0; pending_req_index < pending_req_msg.size();pending_req_index++)
    {
      sns_diag_req_mesg_data current_req_msg;
      current_req_msg.request_msg.assign(pending_req_msg.at(pending_req_index).request_msg);
      current_req_msg.src_sensor_data_type.assign(pending_req_msg.at(pending_req_index).src_sensor_data_type);
      current_req_msg.logging_module_type.assign(pending_req_msg.at(pending_req_index).logging_module_type);
      current_req_msg.log_time_stamp = pending_req_msg.at(pending_req_index).log_time_stamp;

      log_request_msg(
          current_req_msg.request_msg,
          current_req_msg.src_sensor_data_type,
          const_cast<char *>(current_req_msg.logging_module_type.c_str()),
          current_req_msg.log_time_stamp
          );
    }
    pending_req_msg.clear();
    is_client_connect_id_updated = true;
  }
