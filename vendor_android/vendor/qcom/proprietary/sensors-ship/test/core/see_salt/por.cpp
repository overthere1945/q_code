...
/**
 * @brief - parse the por_spec.txt file located in data folder 
 *        and set the SNS_STD_SENSOR_ATTRID_POR in matching
 *        sensors
 */
int parse_por( see_salt *psalt)
{
    string json_fn = LOG_PATH + string("sensors/por_spec.txt");
    cout << "por file: " << json_fn;

    ifstream in_file;
    in_file.open( json_fn.c_str());
    if (in_file.fail()) {
       cout << " not found." << endl;
       return 4;
    }
