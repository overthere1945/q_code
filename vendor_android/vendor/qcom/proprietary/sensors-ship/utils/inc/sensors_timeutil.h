...
public:
    ...
    bool recalculate_offset(bool force_update)
    {
       ...
       return update_qtimer_to_realtime_offset((uint64_t)(time_elapsed.tv_sec*NSEC_PER_SEC+time_elapsed.tv_nsec), force_update);
#endif
    }
...
private:
...
#else
                struct timespec _curr_android_ts;
                clock_gettime(CLOCK_REALTIME, &_curr_android_ts);
                int64_t curr_android_ts = (int64_t)(_curr_android_ts.tv_sec*NSEC_PER_SEC +_curr_android_ts.tv_nsec);
#endif
        ...
        _qtimer_freq(qtimer_get_freq())
    {
        ...
        struct timespec time_elapsed;
        clock_gettime(CLOCK_REALTIME, &time_elapsed);
        _last_realtime_ns = (int64_t)(time_elapsed.tv_sec*NSEC_PER_SEC+time_elapsed.tv_nsec);
#endif
