...
#define COMMUNICATION_TYPE_USB 1
#define COMMUNICATION_TYPE_IP  2
#define BT_FTM_SOUND_CARD_NUM (100)

extern void DiagPrint(int client, unsigned char *response, int length);
extern int remote_com_type;

#define BT_HCI_PROTO_BYTE (0)
...
void ftm_log_send_msg(const uint8 *pEventBuf,int event_bytes)
{
  ...
  if (strcasecmp(transport_type, "smd") || ((pEventBuf != NULL) && (pEventBuf[0] == FTM_BT_CMD_NV_READ)))
  {
    ...
    if (remote_com_type == COMMUNICATION_TYPE_USB) {
...
#endif
	if (remote_com_type == COMMUNICATION_TYPE_IP) {
	  printf("ftm_log_send_msg: sending packet using COMMUNICATION_TYPE_IP\n");
	  printf("client num %d\n", current_client);
      unsigned char DataToSendasLogPacketFor1366[15 + event_bytes];
      int DataToSendasLogPacketFor1366_Length = 15 + event_bytes;
      memset(DataToSendasLogPacketFor1366, 0, DataToSendasLogPacketFor1366_Length);
	  unsigned char DataToSendasLogPacketHeaderFor1366[] = { 0x10, 00, 0x12, 00, 0x12, 00, 0x66, 0x13, 0xE5, 0x62, 0x2A, 0xC5, 0xFF, 01, 00 };
	  //for 0x1366
      for (i = 0; i < 15; i++)
      {
      ...
      for (i = 0; i < event_bytes; i++) {
        DataToSendasLogPacketFor1366[15 + i] = pEventBuf[i];
      }
      ...
      DataToSendasLogPacketFor1366[1] = (lengthOfDiagPacketFor1366 >> 8);
      DataToSendasLogPacketFor1366[2] = (lengthOfDiagPacketFor1366 & 0x00ff);
      DataToSendasLogPacketFor1366[3] = (lengthOfDiagPacketFor1366 >> 8);
      DataToSendasLogPacketFor1366[4] = (lengthOfDiagPacketFor1366 & 0x00ff);
      printf ("%s: sending event log packet back to QRCT\n", __func__);
	  DiagPrint(current_client, DataToSendasLogPacketFor1366, DataToSendasLogPacketFor1366_Length);
      return;
	}
...
void ftm_bt_dispatch(void *ftm_bt_pkt ,int cmd_len )
{
  ...
  /* For ACL Data, no need to wait for command complete event */
  if (bt_ftm_buffer[BT_HCI_PROTO_BYTE] == BT_HCI_ACL_PKT)
    sem_post(&semaphore_cmd_complete);
...
static boolean filter_diag_events(uint8 * event_buf_ptr) {
  boolean retval = FALSE;

  if (event_buf_ptr[0] == BT_HCI_EVT_PKT) {
    if (event_buf_ptr[1] == BT_HCI_CONN_CMPLT_EVT &&
        event_buf_ptr[2] == HCI_CONN_CMPLT_EVT_LEN &&
        is_slim_bus_test == 1) {
      retval = TRUE;
    } else if (event_buf_ptr[1] == BT_HCI_CMD_CMPLT_EVT &&
               event_buf_ptr[2] > 4 &&
               (*(uint16 *) (event_buf_ptr + 4) == BT_QSOC_EDL_CMD_OPCODE) &&
               event_buf_ptr[7] == BT_QSOC_EDL_APPVER_CMD_SUBOPCODE &&
               global_state == FTM_SOC_READ_APP_VER) { // APP VER event
      retval = TRUE;
    } else if (event_buf_ptr[1] == BT_HCI_CMD_CMPLT_EVT &&
               event_buf_ptr[2] == 4 &&
               (*(uint16 *) (event_buf_ptr + 4) == 0x0c03) &&
               global_state == FTM_SOC_RESET) { // HCI RESET event
      retval = TRUE;
    }
  }
...
boolean ftm_bt_hci_hal_read_event (uint8 * event_buf_ptr)
{
  ...
  */
  ...
  if ( event_buf_ptr[0] == BT_HCI_EVT_PKT &&
       event_buf_ptr[1] == BT_HCI_CMD_CMPLT_EVT &&
       event_buf_ptr[2] == 0x04 &&
       ...
       is_slim_bus_test == 1)
  {
    printf("\nInitializing soundwire bus for pin-connectivity\n");
    mxr = mixer_open(BT_FTM_SOUND_CARD_NUM);
    if (!mxr)
    {
      printf("Could not open mixer for %s: %d\n", "BT_FTM_SOUND_CARD_NUM", BT_FTM_SOUND_CARD_NUM);
      return FALSE;
    }

    ctl = mixer_get_ctl_by_name(mxr, ctlname);
    if (!ctl)
    {
      printf("Could not get mixer ctrl for %s: %d\n", "BT_FTM_SOUND_CARD_NUM", BT_FTM_SOUND_CARD_NUM);
      mixer_close(mxr);
      return FALSE;
    }

    ret_val = mixer_ctl_set_value(ctl, 0, 1);
    if (ret_val < 0)
    {
      printf("Could not set mixer ctrl for %s: %d\n", "BT_FTM_SOUND_CARD_NUM", BT_FTM_SOUND_CARD_NUM);
      mixer_close(mxr);
      return FALSE;
    }

    /* Sleep required for hardware to stabilize after control change */
    usleep(500);
    ret_val = mixer_ctl_set_value(ctl, 0, 0);
    if (ret_val < 0)
    {
      printf("Could not reset mixer ctrl for %s: %d\n", "BT_FTM_SOUND_CARD_NUM", BT_FTM_SOUND_CARD_NUM);
      mixer_close(mxr);
      return FALSE;
    }
    mixer_close(mxr);
    ...
    if (ret_val < 0)
    {
      ...
      printf("\nFailed to initialise soundwire bus %d\n", ret_val);
      status = FALSE;
    } else
    {
      event_buf_ptr[PIN_CON_EVT_STATUS_BIT] = 0;
      printf("\nSoundwire bus initiazed succesfully\n");
    }
    if (verbose == 1)
    {
  ...
  if (!filter_diag_events(event_buf_ptr))
  {
      ftm_log_send_msg(event_buf_ptr,event_bytes);
  } else
  {
