...
{
   ALOGE("STEREO: %d", (uint8)stereo);
   fm_global_params.stype = stereo;
}

void fm_rds_avail_status_cb(bool rds_avl)
{
   ALOGE("fm_rds_avail_status_cb: %d", (uint8)rds_avl);
...
struct fm_interface_t *vendor_interface;
...
)
{
 int status;
  ...
  vendor_interface = (struct fm_interface_t *)dlsym(lib_handle, FM_LIBRARY_SYMBOL_NAME);
  if (!vendor_interface) {
