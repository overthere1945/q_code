...
request_status ftm_bt_hci_pfal_deinit_transport()
{
  ...
  if(!isLatestTarget() || bt_ftm_flg != TRUE)
  {
    close(fd_transport);
    fd_transport = -1;
  }
...
request_status ftm_bt_hci_pfal_nwrite(uint8 *buf, int size)
{
  ...
  uint8 loop_back_cmd[] = {0x1, 0x67, 0xFC, 0x03, 0x01, 0x6c, 0x00};
  ...
  {
     is_slim_bus_test = 1;
     printf("\nPinConnectivityTest: Sending loopback command to SOC before initiasing soundwire\n");
     buf = loop_back_cmd;
     buf_size = size = sizeof (loop_back_cmd);
     printf("Send loopback command (size: %u)\n", size);
  }
  do
  {
    nwrite = write(fd_transport, (buf + tx_bytes), (size - tx_bytes));
