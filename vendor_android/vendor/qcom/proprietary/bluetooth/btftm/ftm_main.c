...
int remote_com_type = 0;
int keep_tool_alive = 0;
#define SHOW_PRIO 1
#define SHOW_TIME 2
...
bool bt_ftm_flg = TRUE;


int soc_type;
struct _Socket *_ListenSocket;
#ifdef SUPPORT_USB_INTERFACE
...
static int get_bt_soc_type(void)
{
    ...
    if (ret != 0) {
        ...
        else if (!strncasecmp(bt_soc_type,"cologne",sizeof("cologne"))) {
            return BT_SOC_COLOGNE;
        }
        else if (!strncasecmp(bt_soc_type, "congo", sizeof("congo"))) {
            return BT_SOC_CONGO;
        }
        else {
...
static void usage(void)
{
    fprintf(stderr, "\nusage: %s [options] \n"
            ...
            "   -f, --fm-passthrough   FM passthrough mode \n"
            "   -p, --port-no       port number for TCP based communication \n"
            "   -c, --remote-comm-type   remote communication type [usb or ip] \n"
...
#ifdef CONFIG_FTM_BT
/*===========================================================================
FUNCTION   signal_cb_handler

DESCRIPTION
  Exiting the ftmdaemon process and deinitializing the Diag for BT FTM
...
void signal_cb_handler(int signo) {
    bt_ftm_flg = FALSE;
    int ret = ftm_bt_hci_pfal_deinit_transport();
    if(ret == STATUS_SUCCESS){
        DPRINTF(FTM_DBG_TRACE, "\n FTMDaemon Deinit the transport layer\n");
    }
    sleep(1);
...
int main(int argc, char *argv[])
{
    ...
    static struct option options[] =
    {
        ...
        {"fm-passthrough", no_argument, NULL, 'f'},
        {"uart-baudrate", no_argument, NULL, 'u'},
        {"sibs", no_argument, NULL, 's'},
        {"debug", no_argument, NULL, 'd'},
        {"port-no", required_argument, NULL, 'p'},
        {"remote-comm-type", required_argument, NULL, 'c'},
        {"tool-should-persistent", no_argument, NULL, 'a'},
        {0, 0, 0, 0}
    };
    int daemonize = 1;
    block_bt = false;
    ...
    while (1)
    {
        c = getopt_long(argc, argv, "hb:nsfusdap:c:", options, NULL);
        ...
        case 'p':
            ...
            break;
        case 'a':
            /* when connection is terminated by client. when this
             * flag is set, wait instead of aborting the tool.
             */
            keep_tool_alive = 1;
            break;
        case 'c':
#ifdef SUPPORT_USB_INTERFACE
            if (!strncasecmp(optarg, "usb", sizeof("usb"))) {
              remote_com_type = COMMUNICATION_TYPE_USB;
              printf("remote_com_type : USB\n");
              break;
            }
#endif
            if (!strncasecmp(optarg, "ip", sizeof("ip"))) {
              remote_com_type = COMMUNICATION_TYPE_IP;
              printf("remote_com_type : IP\n");
              break;
            }
    ...
    if (remote_com_type == COMMUNICATION_TYPE_USB) {
        DPRINTF(FTM_DBG_INFO, "Selected USB, initializing FTM Daemon\n");
...
#endif  // SUPPORT_USB_INTERFACE
#ifdef CONFIG_FTM_BT
    if (remote_com_type == COMMUNICATION_TYPE_IP) {
        DPRINTF(FTM_DBG_INFO, "Selected IP, initializing server..\n");
        Start_Server(port);
        if (_ListenSocket->sockfd <= 0) {
            DPRINTF(FTM_DBG_ERROR, "Failed to create server\n");
            exit(EXIT_FAILURE);
        }  else {
            DPRINTF(FTM_DBG_INFO, "Socked created:%d\n", _ListenSocket->sockfd);
            DPRINTF(FTM_DBG_INFO, "Starting read thread for clients...\n");
            pthread_create(&thread_id, NULL, StartReadThread, NULL);
        }
    }
