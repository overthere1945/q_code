...
using Module = aidl::android::hardware::audio::core::Module;
...
int32_t registerIModuleBluetoothSWQti() {
    ALOGE("registerIModuleBluetoothSWQti");

    std::unique_ptr<Configuration>&& config = aidl::android::hardware::audio::core::internal::getConfiguration(Module::Type::BLUETOOTH);
    auto gModuleBluetoothQti = ndk::SharedRefBase::make<::aidl::android::hardware::audio::core::ModuleBluetooth>(std::move(config));
    const std::string kServiceName =
            std::string(gModuleBluetoothQti->descriptor).append("/").append("bluetooth");
    ALOGE("registerIModuleBluetoothSWQti, asBinder().get() = %p, kServiceName = %s", gModuleBluetoothQti->asBinder().get(), kServiceName.c_str());
    binder_exception_t status = AServiceManager_addService(gModuleBluetoothQti->asBinder().get(), kServiceName.c_str());
    if (status != EX_NONE) {
        LOG(ERROR) << __func__ << " failed to register " << kServiceName << " ret:" << status;
        return status;
    }
    //AIBinder_setMinSchedulerPolicy(gModuleBluetoothQti->asBinder().get(), SCHED_NORMAL, ANDROID_PRIORITY_AUDIO);
    return 0;
}
