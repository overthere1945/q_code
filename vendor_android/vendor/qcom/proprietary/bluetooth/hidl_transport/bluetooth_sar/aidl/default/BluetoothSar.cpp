...
#define LOG_TAG "vendor.qti.hardware.bluetooth_sar.aidl_service"
...
ndk::ScopedAStatus BluetoothSar::setBluetoothTxPowerCap(int8_t cap) {
...
std::vector<uint8_t> buildBTSarTechBasedVSCommand(int8_t br_cap, int8_t edr_cap, int8_t ble_cap) {
   ALOGD("%s:",__func__);
  if(br_cap < 0 ) {
    ALOGE("%s: cap= %d, Tx output power is negative",__func__, br_cap);
    br_cap = 0;
  }
  else if(br_cap > 80) {
     ALOGE("%s: cap= %d, Tx output power is out of range",__func__, br_cap);
     br_cap = 80;
  }
  if( edr_cap < 0 ) {
    ALOGE("%s: edr_cap= %d, Tx output power is negative",__func__, edr_cap);
    edr_cap = 0;
  }
  else if(edr_cap > 80) {
      ALOGE("%s: ble_cap= %d, Tx output power is out of range",__func__, br_cap);
      edr_cap = 80;
  }
  if( ble_cap < 0) {
    ALOGE("%s: ble_cap= %d, Tx output power is negative",__func__, ble_cap);
    ble_cap = 0;
  }
  else if(ble_cap > 80) {
    ALOGE("%s: cap= %d, Tx output power is out of range",__func__, br_cap);
  ...
  data.push_back((uint8_t)opcode);
  data.push_back((uint8_t)(opcode >> 8));
  data.push_back(8);//length of remaning  data
  data.push_back(14);//sub opcode
  data.push_back(3);
  data.push_back(0);
  data.push_back(br_cap);
  data.push_back(1);
  data.push_back(edr_cap);
  data.push_back(2);
  data.push_back(ble_cap);
  return data;
}

ndk::ScopedAStatus BluetoothSar::setBluetoothTechBasedTxPowerCap(int8_t br_cap, int8_t edr_cap, int8_t ble_cap) {
  ALOGI("BluetoothSar:setBluetoothTechBasedTxPowerCap");
  std::vector<uint8_t> cmd_data = buildBTSarTechBasedVSCommand(br_cap, edr_cap, ble_cap);
  sendDataToController(cmd_data);
  return ndk::ScopedAStatus::ok();
}

ndk::ScopedAStatus BluetoothSar::setBluetoothModeBasedTxPowerCap(const std::array<uint8_t, 3>& chain0Cap, const std::array<uint8_t, 3>& chain1Cap, const std::array<uint8_t, 6>& beamformingCap){
  ALOGI("BluetoothSar:setBluetoothModeBasedTxPowerCap, Implementation is pending!");
  return ndk::ScopedAStatus::ok();
}

ndk::ScopedAStatus BluetoothSar::setBluetoothModeBasedTxPowerCapPlusHR(const std::array<uint8_t, 4>& chain0Cap, const std::array<uint8_t, 4>& chain1Cap, const std::array<uint8_t, 8>& beamformingCap){
  ALOGI("BluetoothSar:setBluetoothModeBasedTxPowerCapPlusHR, Implementation is pending!");
  return ndk::ScopedAStatus::ok();
}

ndk::ScopedAStatus BluetoothSar::setBluetoothAreaCode(const std::array<uint8_t, 3>& areaCode){
  ALOGI("BluetoothSar:setBluetoothAreaCode, Implementation is pending!");
  return ndk::ScopedAStatus::ok();
}
