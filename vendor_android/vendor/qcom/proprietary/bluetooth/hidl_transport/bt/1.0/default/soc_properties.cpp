...
std::string setVendorPropertiesDefault() {
  ...
  if (ret == 0) {
    fd_btdev = open("/dev/spibt", O_RDWR | O_NONBLOCK);
    if (fd_btdev < 0) {
    ...
    if (strstr(compatable_chipset_id, "qca6490")) {
      ...
      ret_string = "ganges";
    } else if (strstr(compatable_chipset_id, "wcn786x")) {
      ret = property_set("persist.vendor.qcom.bluetooth.soc", "ganges");
      ret = property_set("persist.vendor.qcom.bluetooth.sec_soc", "brahma");
      ret_string = "ganges";
      ...
      ret_string = "evros";
    } else if(strstr(compatable_chipset_id, "wcn7760")){
      ret = property_set("persist.vendor.qcom.bluetooth.soc", "cologne");
      ret_string = "cologne";
    }
    if (ret < 0) {
    ...
    if (!strncasecmp(soc_name, "themisto", sizeof("themisto")))
      ALOGD("%s:vendor properties set : soc name  : themisto\n", __func__);
    else
