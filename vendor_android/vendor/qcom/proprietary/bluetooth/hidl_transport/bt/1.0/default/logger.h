...
#define PERI_SOC_DUMP_PREFIX "ramdump_bt_peri_fw_crashdump_"
#define UWB_SOC_DUMP_PREFIX "ramdump_bt_uwb_fw_crashdump_"
#define SOC_DUMP_PATH LOG_COLLECTION_DIR SOC_DUMP_PREFIX LOGS_TS FW_DUMP_EXTN
#define PERI_SOC_DUMP_PATH LOG_COLLECTION_DIR PERI_SOC_DUMP_PREFIX LOGS_TS FW_DUMP_EXTN
#define UWB_SOC_DUMP_PATH LOG_COLLECTION_DIR UWB_SOC_DUMP_PREFIX LOGS_TS FW_DUMP_EXTN
...
#define UART_LOGS_DEST_PATH LOG_COLLECTION_DIR UART_LOG_PREFIX LOGS_TS LOGS_EXTN
#define BT_STATE_FILE_PATH LOG_COLLECTION_DIR BT_STATE_FILE_NAME_PREFIX LOGS_TS LOGS_EXTN

#define SPI_IPC_TX_LOGS_DEST_PATH  LOG_COLLECTION_DIR SPI_IPC_TX_LOG_PREFIX LOGS_TS LOGS_EXTN
#define SPI_IPC_RX_LOGS_DEST_PATH LOG_COLLECTION_DIR SPI_IPC_RX_LOG_PREFIX LOGS_TS LOGS_EXTN
#define SPI_IPC_STATE_LOGS_DEST_PATH LOG_COLLECTION_DIR SPI_IPC_STATE_LOG_PREFIX LOGS_TS LOGS_EXTN
#define SPI_IPC_PWR_LOGS_DEST_PATH LOG_COLLECTION_DIR SPI_IPC_PWR_LOG_PREFIX LOGS_TS LOGS_EXTN
#define SPI_LOGS_DEST_PATH LOG_COLLECTION_DIR SPI_LOG_PREFIX LOGS_TS LOGS_EXTN
...
#define LOG_PERI_EVT_VENDOR_SPECIFIC 0xFF
#define LOG_BT_EVT_CMD_CMPLT         0x0E
#define LOG_PERI_EVT_PACKET_TYPE     0X34
...
#define LOG_HCI_PERI_CRASH_DUMP_MEMDUMP     0x04
#define LOG_HCI_PERI_CRASH_DUMP_INFORMATION 0x05
#define LOG_HCI_PERI_HARDWARE_ERROR         0x06
#define LOG_HCI_PERI_BUFF_DUMP              0x07
#define LOG_PERI_CRASH_DUMP_SUB_ID          0x00
#define LOG_BT_CRASH_DUMP_SUB_ID            0x01
...
/*offset related to peri crash dump */
#define MINI_DUMP_ARM_REG_SIZE_OFFSET_PERI   11
#define MINI_DUMP_CALL_STK_SIZE_OFFSET_PERI  88
#define HCI_BUFFER_DUMP_SIZE_OFFSET_PERI     5
#define DUMP_PKT_SIZE_OFFSET_PERI            2

/* Peri related commands*/
...
#define UART_INIT_IDEAL_TIME_DELAY_CHE_BASED  300
...
#define MAX_COUNT_WLAN_BT_EN_PIN_PULLED_DOWN      5
...
#define DUMP_IPC_LOGS          (1)
#define DUMP_FTRACE_LOGS         (2)
#define DUMP_FTRACE_AND_IPC_LOGS (3)
...
typedef enum {
  ...
  BT_HOST_REASON_BT_CAL_CMD_CC_NOT_RCVD          = 0x4A,
  BT_HOST_REASON_BT_PWR_VOTE_FAILED              = 0x4B,
  BT_HOST_REASON_SPI_INIT_STUCK                  = 0x4C,
  BT_HOST_REASON_SPI_INIT_FAILED                 = 0x4D,
  ...
  // TME-L Crash Reasons
  BT_HOST_REASON_TME_GETVER_CMD_FAILED      =  0xB6,
  BT_HOST_REASON_TME_ARBITRATION_NTF_STUCK  =  0xB7,
  BT_HOST_REASON_TME_FILE_SYSTEM_CALL_STUCK =  0xB8,
  BT_HOST_REASON_TME_GETVER_SEND_STUCK      =  0xB9,
  BT_HOST_REASON_TME_ARBITRATION_CMD_STUCK  =  0xBA,
#endif
  // BT SOC Calibration Reasons
  BT_HOST_REASON_ERR_INVALID_HCI_CAL_CMD_PARAM             =  0xBB,
  BT_HOST_REASON_ERR_NO_VALID_CAL_DATA                     =  0xBC,
  BT_HOST_REASON_CALIBRATION_STATUS_IDLE                   =  0xBD,
  BT_HOST_REASON_FAILED_RECV_RSP_FOR_READ_CAL_NVM_DATA_CMD =  0xBE,
  BT_HOST_REASON_CALIBRATION_SIZE_MISMATCH                 =  0xBF,
  BT_HOST_REASON_FILE_OPEN_FAILED_TO_STORE_CAL_DATA        =  0xC0,
  BT_HOST_REASON_FAILED_TO_CREATE_HEAP_MEMORY_FOR_CAL      =  0xC1,

  BT_HOST_REASON_PATCHING_FAILED            =  0xC2,
#ifdef IS_PERI_ENABLED
  PERI_SOC_REASON_DEFAULT                   =  0xC3,
  TME_SOC_REASON_DEFAULT                    =  0xC4,
#endif
  // License File Reasons
  BT_HOST_REASON_LICENSE_READ_STUCK         =  0xC5,
  BT_HOST_REASON_LICENSE_DNLD_STUCK         =  0xC6,
  BT_HOST_REASON_LICENSE_DOWNLOAD_FAILED    =  0xC7,
#ifdef XPAN_SUPPORTED
  BT_HOST_REASON_XPAN_THREAD_START_STUCK    =  0xC8,
#endif
#ifdef PASSTHROUGH_ENABLED
  BT_HOST_HCI_PERI_BT_SPI_TXP_SWITCH_RSP_NOT_RCVD = 0xC9,
#endif
} SecondaryReasonCode;

#ifdef IS_PERI_ENABLED
enum FwDownlodState {
  DEFAULT_DWNLD_STATE = 0,
  PRE_FW_DWNLD,
  PERI_DWNLD_STARTED,
  PERI_DWNLD_CMPL,
  PERI_DWNLD_FAILED,
  ...
  BT_DWNLD_FAILED,
  STARTING_RX_THREAD,
  RX_THREAD_START_FAILED,
  RX_THREAD_STARTED,
  FW_DWNLD_SUCCESS,
  FW_DWNLD_FAILED,
};

static inline char *FwDownlodStateToString(enum FwDownlodState state)
{
  if (state == DEFAULT_DWNLD_STATE) {
    return "DEFAULT_DWNLD_STATE";
  } else if (state == PRE_FW_DWNLD) {
    return "PRE_FW_DWNLD";
  } else if (state == PERI_DWNLD_STARTED) {
    return "PERI_DWNLD_STARTED";
  } else if (state == PERI_DWNLD_CMPL) {
    return "PERI_DWNLD_CMPL";
  } else if (state == PERI_DWNLD_FAILED) {
    return "PERI_DWNLD_FAILED";
  } else if (state == BT_DWNLD_STARTED) {
    return "BT_DWNLD_STARTED";
  } else if (state == BT_DWNLD_CMPL) {
    return "BT_DWNLD_CMPL";
  } else if (state == BT_DWNLD_FAILED) {
    return "BT_DWNLD_FAILED";
  } else if (state == STARTING_RX_THREAD) {
    return "STARTING_RX_THREAD";
  } else if (state == RX_THREAD_START_FAILED) {
    return "RX_THREAD_START_FAILED";
  } else if (state == RX_THREAD_STARTED) {
    return "RX_THREAD_STARTED";
  } else if (state == FW_DWNLD_SUCCESS) {
    return "FW_DWNLD_SUCCESS";
  } else if (state == FW_DWNLD_FAILED) {
    return "FW_DWNLD_FAILED";
  } else {
    return "INVALID";
  }
}
...
static SecondaryReasonMap secReasonMap[] = {
  ...
  { BT_HOST_REASON_BT_CAL_CMD_CC_NOT_RCVD          ,  "BT Calibration cc not rcvd"},
  { BT_HOST_REASON_BT_PWR_VOTE_FAILED              ,  "BT soc power voting failed"},
  { BT_HOST_REASON_SPI_INIT_STUCK                  ,  "BT host spi init stuck"},
  { BT_HOST_REASON_SPI_INIT_FAILED                 ,  "BT host spi init failed"},
  ...
  { BT_HOST_REASON_PERI_NVM_FILE_NOT_FOUND         ,  "Peri NVM file not found"},
  ...
  { BT_HOST_REASON_TME_GETVER_CMD_FAILED           ,  "TME-L Get Ver cmd failed"},
  { BT_HOST_REASON_TME_ARBITRATION_NTF_STUCK       ,  "TME-L SS Activate ntf stuck"},
  { BT_HOST_REASON_TME_FILE_SYSTEM_CALL_STUCK      ,  "TME-L File system call stuck"},
  { BT_HOST_REASON_TME_GETVER_SEND_STUCK           ,  "TME-L Get Ver Send Stuck"},
  { BT_HOST_REASON_TME_ARBITRATION_CMD_STUCK       ,  "TME-L arbitration cmd stuck"},
  { BT_HOST_REASON_LICENSE_READ_STUCK	           ,  "License file read stuck"},
  { BT_HOST_REASON_LICENSE_DNLD_STUCK	           ,  "License file download stuck"},
  { BT_HOST_REASON_LICENSE_DOWNLOAD_FAILED	   ,  "License file download failed"},
  { TME_SOC_REASON_DEFAULT                         ,  "TME soc reason default"},
  { PERI_SOC_REASON_DEFAULT                        ,  "PERI soc reason default"},
#endif
  { BT_HOST_REASON_ERR_INVALID_HCI_CAL_CMD_PARAM   ,  "Invalid Calibration Cmd Parameter"},
  { BT_HOST_REASON_ERR_NO_VALID_CAL_DATA           ,  "SOC don't have valid Calibration Data"},
  { BT_HOST_REASON_CALIBRATION_STATUS_IDLE         ,  "Calibration status is set to Default"},
  { BT_HOST_REASON_FAILED_RECV_RSP_FOR_READ_CAL_NVM_DATA_CMD ,  "No rsp received for read NVM CAL Cmd"},
  { BT_HOST_REASON_CALIBRATION_SIZE_MISMATCH                 ,  "Mismatch in Calibration data recived size"},
  { BT_HOST_REASON_FILE_OPEN_FAILED_TO_STORE_CAL_DATA        ,  "Failed to open calibration.bin File"},
  { BT_HOST_REASON_FAILED_TO_CREATE_HEAP_MEMORY_FOR_CAL      ,  "Failed to create Heap memory for Cal"},
#ifdef XPAN_SUPPORTED
  { BT_HOST_REASON_XPAN_THREAD_START_STUCK         ,  "XPAN thread start stuck"},
#endif
  { BT_HOST_REASON_PATCHING_FAILED                 ,  "BT SOC patching failed"}
};
...
class Logger
{
 private:
...
#ifdef IS_PERI_ENABLED
  size_t peri_mini_dump_size_;
  char peri_mini_dump_buff_[HAL_DUMP_FILE_SIZE];
  bool isPeriDumpSetCrashReason;
#endif
  int cal_fd;
  char *cal_buf;
#ifdef DUMP_IPC_LOG
  ...
  void SaveSocCalibrationData(uint8_t *eventBuf, uint16_t packet_len, bool isframeTypeIsAcl);
  void CalibrationCleanUpHandler(bool cal_ssr);
  ...
  void SetPrimaryCrashReasonAsDiag();

  void DumpTransportLogs(void) {
    transport_log_.DumpLogs();
  };
  ...
  SecondaryReasonCode dump_crash_reason;
#ifdef IS_PERI_ENABLED
  void SaveSocPeriMemDump(uint8_t *, uint16_t, PrimaryReasonCode);
  bool HandlePeriEvent(HciPacketType, uint8_t *, uint16_t);
  void DecodeHwErrorPeriEvt(uint8_t *buff);
  bool isValidCrashReason(uint32_t reason);
#endif
 public:
  Logger();
  enum {
  ...
  void GetCrashDumpFileName(char*, uint8_t);
...
#ifdef DUMP_IPC_LOG
  void GetTransportDumpFilename(short int, char*);
  void DumpTransportIpcLogs(void) {
    ipc_log_.DumpTransportLogs();
  };
#endif
  void GetTransportLogFilename(char*);
  ...
  void CollectDumps(bool is_transport_ipc_enabled, bool is_state_log_enabled);
  ...
  void UpdateLicPatchIdealDelay(int size);
  ...
  static std::mutex cal_last_evnt_lock;

#ifdef IS_PERI_ENABLED
  static std::mutex fwdownload_mtx;
  enum FwDownlodState fwdownload_state;
  enum FwDownlodState GetFwDownloadState();
  void SetFwDownloadState(enum FwDownlodState);
#endif
};
extern std::mutex gThermalInstanceMtx;
} // namespace implementation
} // namespace V1_0
} // namespace bluetooth
} // namespace hardware
