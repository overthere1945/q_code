...
#ifdef BT_VER_1_1
#define LOG_TAG "vendor.qti.bluetooth@1.1-bluetooth_address"
#else
...
bt_address bt_address_ins;
char modem_type[PROPERTY_VALUE_MAX] = "Not set";
...
bool BluetoothAddress::QmiDmsInit()
{
  ...
  if (strcmp(modem_type, "Not set") == 0) {
    /* Find out the modem type */
    memset(modem_type, 0x00, sizeof(modem_type));
    Logger :: Get()->PropertyGet(QMI_UIM_PROPERTY_BASEBAND, modem_type, "");
  }
  /*check if it's APQ/SDA only device */
  if (strcmp(modem_type, QMI_UIM_PROP_BASEBAND_VALUE_APQ) == 0 ||
          strcmp(modem_type, QMI_UIM_PROP_BASEBAND_VALUE_SDA) == 0) {
...
bool BluetoothAddress::GetLocalAddress(uint8_t *local_addr)
{
  ...
  BtState :: Get()->AddLogTag(dst_buff, tv, (char *)"Fetching vendor BD Address");
  BtState :: Get()->SetPreFetchLocalAddressTS(dst_buff);
  // Get addr from vendor location
  if (fetch_vendor_addr(local_addr)) {
    valid_bda = true;
  }
  gettimeofday(&tv, NULL);
  BtState :: Get()->AddLogTag(dst_buff, tv, (char *)"Done fetching vendor BD Address");
  BtState :: Get()->SetPostFetchLocalAddressTS(dst_buff);

  // Get local bdaddr storage path from a system property.
  if (!valid_bda) {
    if (strcmp(bt_address_ins.bt_bdaddr_path, "Not set") == 0) {
      if (Logger :: Get()->PropertyGet(PROPERTY_BT_BDADDR_PATH, property, NULL)) {
        strlcpy(bt_address_ins.bt_bdaddr_path, property,
                sizeof(bt_address_ins.bt_bdaddr_path));
      }
    }
    if (strcmp(bt_address_ins.bt_bdaddr_path, "Not set") != 0) {
      int addr_fd;
      ALOGD("%s: Trying %s", __func__, property);
      addr_fd = open(bt_address_ins.bt_bdaddr_path, O_RDONLY);
      if (addr_fd != -1) {
  ...
  if (!valid_bda) {
    if (memcmp(bt_address_ins.factory_bt_bdaddr, zero_bdaddr, kBytes) == 0) {
      if (Logger :: Get()->PropertyGet(FACTORY_BDADDR_PROPERTY, property, NULL) &&
        StringToBytes(property, local_addr)) {
        valid_bda = true;
        le2bd(local_addr);
        memcpy(bt_address_ins.factory_bt_bdaddr, local_addr,  kBytes);
      }
    }
    else {
      valid_bda = true;
      memcpy(local_addr, bt_address_ins.factory_bt_bdaddr, kBytes);
    }
  }

  // Check if address is stored @ Modem NV
  if ((!valid_bda)&& Logger :: Get()->PropertyGet("persist.vendor.bluetooth.modem_nv_support",
                   property, "false") && (strcmp(property, "true") == 0))
 {
    ALOGD("%s: QMI initialization START ",__func__);
      ...
      while (retry < QMI_INIT_MAX_TRY) {
        ...
        if (QmiDmsInit() == true) {
          if (QmiDmsGetAddress(modem_bdaddr_) == true) {
            ...
            if (Logger :: Get()->PropertySet(PERSIST_BDADDR_PROPERTY, (char*)bdstr) < 0) {
  ...
  if (!valid_bda && Logger :: Get()->PropertyGet(PERSIST_BDADDR_PROPERTY, property, NULL) &&
      StringToBytes(property, local_addr)) {
    valid_bda = true;
    le2bd(local_addr);
  }
