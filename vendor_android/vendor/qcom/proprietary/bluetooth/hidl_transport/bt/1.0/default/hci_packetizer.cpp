...
#ifdef BT_VER_1_1
#define LOG_TAG "vendor.qti.bluetooth@1.1-hci_packetizer"
#else
...
size_t GetPreambleSizeForType(HciPacketType type)
{
  ...
  switch (type) {
    ...
    case HCI_PACKET_TYPE_ISO_DATA:
      header_len = HCI_ISO_PREAMBLE_SIZE;
      break;
    ...
    case HCI_PACKET_TYPE_PERI_EVT:
      header_len = HCI_EVENT_PREAMBLE_SIZE + 1;
      break;
    default:
      break;
  }
...
size_t HciGetPacketLengthForType(HciPacketType type, const uint8_t* preamble)
{
  ...
  if (type == HCI_PACKET_TYPE_ACL_DATA)
    ...
    return ((((preamble[offset + 1]) & HCI_PACKET_ISO_PAYLOAD) << 8) | preamble[offset]);
  else
