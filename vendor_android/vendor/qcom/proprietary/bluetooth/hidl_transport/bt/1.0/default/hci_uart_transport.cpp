...
#ifdef BT_VER_1_1
#define LOG_TAG "vendor.qti.bluetooth@1.1-uart_transport"
#else
...
static tUSERIAL_CFG wakeup_cfg =
{
  (USERIAL_DATABITS_8 | USERIAL_PARITY_NONE | USERIAL_STOPBITS_1),
  USERIAL_BAUD_38400
};
...
uint8_t userial_to_tcio_baud(uint8_t cfg_baud, uint32_t *baud)
{
  if (cfg_baud == USERIAL_BAUD_115200)
    *baud = B115200;
  else if (cfg_baud == USERIAL_BAUD_8M)
    *baud = (uint32_t)UART_BAUDRATE_8_M;
...
int userial_tcio_baud_to_int(uint32_t baud, uint8_t soc_type)
{
  ...
  switch (baud) {
    ...
    case BOTHER:
#endif
      if ((soc_type == BT_SOC_GANGES) ||
          (soc_type == BT_SOC_ORNE) ||
          (soc_type == BT_SOC_COLOGNE) ) {
        baud_rate = 8000000;
        ALOGE("%s: baudrate is set to %d", __func__, baud_rate);
      } else {
        baud_rate = 3200000;
      }
      break;
...
bool HciUartTransport::InitTransport(tUSERIAL_CFG *p_cfg)
{
  ...
  if (soc_type_ == BT_SOC_CHEROKEE) {
    ...
    if (baud == BOTHER) {
      ...
      term2.c_ospeed = term2.c_ispeed = (speed_t)userial_tcio_baud_to_int(baud, soc_type_);
      term2.c_cflag &= ~CBAUD;
    ...
    ALOGI("%s: device fd = %d open @%d bps", __func__, ctrl_fd_,
          userial_tcio_baud_to_int(baud, soc_type_));
...
uint8_t HciUartTransport::find_baud_rate_code(int baud_rate)
{
  switch (baud_rate) {
    case UART_BAUDRATE_300:
      return USERIAL_BAUD_300;
    case UART_BAUDRATE_600:
      return USERIAL_BAUD_600;
    case UART_BAUDRATE_2400:
      return USERIAL_BAUD_2400;
    case UART_BAUDRATE_9600:
      return USERIAL_BAUD_9600;
    case UART_BAUDRATE_19200:
      return USERIAL_BAUD_19200;
    case UART_BAUDRATE_38400:
      return USERIAL_BAUD_38400;
    case UART_BAUDRATE_57600:
      return USERIAL_BAUD_57600;
    case UART_BAUDRATE_115200:
      return USERIAL_BAUD_115200;
    case UART_BAUDRATE_230400:
      return USERIAL_BAUD_230400;
    case UART_BAUDRATE_460800:
      return USERIAL_BAUD_460800;
    case UART_BAUDRATE_921600:
      return USERIAL_BAUD_921600;
    case UART_BAUDRATE_1000000:
      return USERIAL_BAUD_1M;
    case UART_BAUDRATE_1500000:
      return USERIAL_BAUD_1_5M;
    case UART_BAUDRATE_2000000:
      return USERIAL_BAUD_2M;
    case UART_BAUDRATE_3000000:
      return USERIAL_BAUD_3M;
    case UART_BAUDRATE_3200000:
      return USERIAL_BAUD_3_2M;
    case UART_BAUDRATE_4000000:
      return USERIAL_BAUD_4M;
    case UART_BAUDRATE_8000000:
      return USERIAL_BAUD_8M;
  }
  return BAUDRATE_Reserved;
}
...
int HciUartTransport::Ioctl(userial_vendor_ioctl_op_t op, int *p_data)
{
  ...
  switch (op) {
    ...
    case USERIAL_GET_ERR_CODE:
      err = ioctl(ctrl_fd_, MSM_GENI_SERIAL_TIOCFAULT, NULL);
      break;
...
int HciUartTransport::Write(const uint8_t *buf, int len)
{
...
void HciUartTransport::SetBaudRate(uint8_t userial_baud)
{
  ...
  gettimeofday(&tv, NULL);
  Logger::Get()->SetCurrentactivityStartTime(tv, BT_HOST_REASON_UART_IOCTL_STUCK, "SET UART BAUDRATE");

  userial_to_tcio_baud(userial_baud, &tcio_baud);

#ifndef B3200000
  if (tcio_baud == BOTHER || userial_baud == USERIAL_BAUD_8M) {
    struct termios2 term2;
    if (ioctl(ctrl_fd_, TCGETS2, &term2) == -1) {
    ...
    if (USERIAL_BAUD_8M == userial_baud) {
        term2.c_ospeed = term2.c_ispeed = (speed_t)UART_BAUDRATE_8_M;
    } else {
        term2.c_ospeed = term2.c_ispeed = (speed_t)userial_tcio_baud_to_int(tcio_baud, soc_type_);
    }
    term2.c_cflag &= ~CBAUD;
    term2.c_cflag |= BOTHER;
...
int HciUartTransport::GetBaudRate(void)
{
  ...
  gettimeofday(&tv, NULL);
  Logger::Get()->SetCurrentactivityStartTime(tv, BT_HOST_REASON_UART_IOCTL_STUCK, "GET UART BAUDRATE");

  tcgetattr(ctrl_fd_, &termios);

  baud_rate = userial_tcio_baud_to_int(cfgetispeed(&termios), soc_type_);
...
uint8_t HciUartTransport::GetMaxBaudrate()
{
  switch (soc_type_) {
    ...
    case BT_SOC_COLOGNE:
    case BT_SOC_GANGES:
      return (uint8_t) USERIAL_BAUD_8M;
...
bool HciUartTransport::SendPowerOnPulse(bool need_reload)
{
  bool status = true;
  char reset_val = 0xFC;
  int len;

  if (!need_reload || soc_type_ != BT_SOC_CHEROKEE)
    return true;

  ALOGD("%s: Opening port for sending power ON pulse", __func__);
  ...
  /* Controller completes it bootup sequence in 58ms after HOST
   * sent FC. Added a buffer of 42ms to wait here before proceeding
   * to next sequence.
  */
  usleep(100 * 1000);
error:
...
bool HciUartTransport::Init(BluetoothSocType soc_type, bool need_reload)
{
  ...
  switch (soc_type) {
    ...
    case BT_SOC_ROME: {
      ...
      status = InitTransport(&userial_init_cfg);
    }
    break;
    ...
    case BT_SOC_GANGES:
      if (!need_reload)
        /* Keep baudrate consistent with firmware, i.e. 3M, 3.2M or 8M */
        normal_cfg.baud = GetMaxBaudrate();
      else {
        if (soc_type_ == BT_SOC_GANGES) {
	  ALOGI("%s: power_state_ %d", __func__, power_state_);
          if (power_state_ == (enum power_states)ALL_CLIENTS_ON) {
            ctrl_fd_ = open(BT_HS_UART_DEVICE, O_RDWR);
	    if (ctrl_fd_ == -1) {
		    ALOGE("%s: failed to open %s err %s", __func__, BT_HS_UART_DEVICE, strerror(errno));
		    return false;
	    }
            int current_bd = GetBaudRate();
	    close(ctrl_fd_);
	    ALOGI("%s: current_bd %d bps", __func__, current_bd);
	    if (current_bd != 115200 && current_bd != 9600 && current_bd != 2400) {
              normal_cfg.baud = GetMaxBaudrate();
	      ALOGI("%s: current_bd %d bps and resetting bd to %d bps", __func__,
                    current_bd, normal_cfg.baud);
	    }
	  }
	}
      }
    ...
    case BT_SOC_CHEROKEE: {
      ...
      DeInitTransport();
      if (!(status = SendWakeupSignal(true)))
	      break;

      // TODO to remove it and proper fix
      CleanUp();
      if (!(status = SendPowerOnPulse(true)))
        break;
...
void HciUartTransport::Disconnect(bool need_reload, bool wakeup_needed) {
  if (ctrl_fd_ > 0) {
    ...
    gettimeofday(&tv, NULL);
    BtState::Get()->AddLogTag(dst_buff, tv, (char *)"Wakelock :: Acquiring during cleanup");
    BtState::Get()->SetWakeLockAcqTimeName(dst_buff);
    Wakelock :: Acquire();
#endif
    DeInitTransport();
    SendWakeupSignal(wakeup_needed);
    CleanUp(need_reload);
#ifdef WAKE_LOCK_ENABLED
    gettimeofday(&tv, NULL);
    BtState::Get()->AddLogTag(dst_buff, tv, (char *)"Wakelock :: Releasing after cleanup");
...
void HciUartTransport::Disconnect(bool wakeup_needed) {
  return Disconnect(true, wakeup_needed);
}

bool HciUartTransport::GetUartCtsStatus() {
  int status = 0;
  ioctl(ctrl_fd_, TIOCMGET, &status);
  // If status is 1 that means UART CTS line is low
  return (status & TIOCM_CTS);
}

void HciUartTransport::SocRxDWakeup() {
  int err = 0;
  ...
  ALOGD("%s: Flow off->Change UART baudrate to 38.4kbs->send 0x00->Change UART baudrate to max->Flow on", __func__);
  // Pull RFR line high
  if ((err = Ioctl(USERIAL_OP_FLOW_OFF, &flags)) < 0) {
    ALOGE("%s: HW Flow-off error: 0x%x\n", __func__, err);
    return;
  }

  // Change UART baudrate to 38400 bps
  tcdrain(ctrl_fd_);
  userial_to_tcio_baud(USERIAL_BAUD_38400, &tcio_baud);
  tcgetattr(ctrl_fd_, &termios);
  cfmakeraw(&termios);
  termios.c_cflag &= ~(CRTSCTS); /* Clear CTS/RTS flow control bit */
  cfsetospeed(&termios, tcio_baud);
  ...
  /* 0x00 @38400 bps gives SoC around 234usec low pulse on RxD line.
   * The requirement is minimum of 150usec for SoC to detect wakeup signal.
   */
  int len = Write(&wake_val, 1);
  if (len != 1)
    ALOGE("%s: Send failed with ret value: %d", __func__, len);

  /* Send out pending data on UART TX side to controller
   * to ensure SoC receive wakeup signal.
   */
  tcdrain(ctrl_fd_);

  /* Change Back to Max baudrate */
  {
    struct termios termios;
    struct timeval tv;
    uint8_t userial_baud;

    userial_baud = GetMaxBaudrate();
    ALOGI("## userial_vendor_set_baud: %d", userial_baud);
  ...
  // Pull RFR line low
  if ((err = Ioctl(USERIAL_OP_FLOW_ON, &flags)) < 0) {
    ALOGE("%s: HW Flow-on error: 0x%x \n", __func__, err);
  }
}

bool HciUartTransport::SendWakeupSignal(bool wakeup_needed)
{
  bool status = true;
  char wakeup_singal = 0x00;
  int len, i;

  if (!wakeup_needed || soc_type_ != BT_SOC_CHEROKEE)
    return true;

  ALOGD("%s: Opening port for sending wakeup signal", __func__);
  if (!InitTransport(&wakeup_cfg)) {
    return false;
  }
  ...
  /* Sending 0x00 @38400 bps gives SoC around 234usec low pulse
   * on RxD line. This acts as a wakeup signal for SoC.
   * HW team asked to send 0x00 for two times.
   */
  for (i =0; i < 2; i++) {
    len = write(ctrl_fd_, &wakeup_singal, 1);
    if (len != 1 ) {
      ALOGE("%s: Send failed with ret value: %d", __func__, len);
      status = false;
      goto error;
    }
    // 2ms delay has to be given before next wakeup byte sent.
    usleep(2 * 1000);
  }

  tcdrain(ctrl_fd_);
  ALOGI("%s:sent out wakeup signal", __func__);
  /* Controller completes it bootup sequence in 58ms after HOST
   * sent FC. Added a buffer of 42ms to wait here before proceeding
   * to next sequence.
   */
...
int HciUartTransport :: CheckForUartFailureCode() {
    int status = 0;
    /* Call get mctrl so that UART can check RFR line status
     * before returning err code.
     */
    ioctl(ctrl_fd_, TIOCMGET, &status);
    int uart_err = Ioctl(USERIAL_GET_ERR_CODE, NULL);
    if (uart_err < 0) {
      ALOGE("%s: MSM_GENI_SERIAL_TIOCFAULT IOCTL not supported", __func__);
      return UART_REASON_DEFAULT;
    }
    uart_err += UART_REASON_OFFSET;
    if (uart_err < UART_ERR_CODE_LOWER_BOUND || uart_err > UART_ERR_CODE_UPPER_BOUND) {
      ALOGE("%s: UART driver returns invalid err code = 0x%x", __func__, uart_err);
      return UART_REASON_DEFAULT;
    }
    ALOGE("%s: UART driver returns err code = 0x%x", __func__, uart_err);
    return uart_err;
}

int HciUartTransport::SetSocToCrash(uint8_t crash_code)
{
  int err = 0;
  int flags;
  uint32_t tcio_baud;
  struct termios termios;
  struct timeval tv;
  ALOGD("%s: Change UART baudrate to 2400kbs->send crash byte :%x", __func__, crash_code);
  gettimeofday(&tv, NULL);
  Logger::Get()->SetCurrentactivityStartTime(tv, BT_HOST_REASON_UART_IOCTL_STUCK, "SET UART BAUDRATE");

  tcdrain(ctrl_fd_);
  userial_to_tcio_baud(USERIAL_BAUD_2400, &tcio_baud);
  tcgetattr(ctrl_fd_, &termios);
  cfmakeraw(&termios);
  termios.c_cflag &= ~(CRTSCTS); /* Clear CTS/RTS flow control bit */
  cfsetospeed(&termios, tcio_baud);
  ...
  /* Send out pending data on UART TX side to controller
   * to ensure SoC receive crash byte.
   */
  tcdrain(ctrl_fd_);
  return err;
}

void HciUartTransport::ResetHostBaudRate(void)
{
  int flag;
  uint32_t tcio_baud;
  struct termios termios;
  ...
  tcgetattr(ctrl_fd_, &termios);
  cfmakeraw(&termios);
  userial_to_tcio_baud(USERIAL_BAUD_115200, &tcio_baud);
  cfsetospeed(&termios, tcio_baud);
  cfsetispeed(&termios, tcio_baud);
  tcsetattr(ctrl_fd_, TCSANOW, &termios);
}
