...
#ifdef BT_VER_1_1
#define LOG_TAG "vendor.qti.bluetooth@1.1-mct_transport"
#else
...
int InitTransport (int channel_id )
{
  ...
  if (tcflush(fd, TCIOFLUSH) < 0) {
    ALOGW("%s: can't flush %s\n", __func__,DevSmdNodes[channel_id]);
  }

  if (tcgetattr(fd, &term) < 0) {
    ALOGW("%s: can't get attributes\n", __func__);
  } else {
    cfmakeraw(&term);
    term.c_cflag |= (CRTSCTS | CLOCAL);
    if (tcsetattr(fd, TCSANOW, &term) < 0) {
      ALOGW("%s: can't set attributes\n", __func__);
    }
  }
...
int HciMctTransport::Write(const uint8_t *buf, int len){
  return WriteSafely(data_fd_,buf, len);
}
