...
#ifdef BT_VER_1_1
#define LOG_TAG "vendor.qti.bluetooth@1.1-patch_dl_manager"
#else
...
#define HCI_LC3_OVER_HFP_SUPPORT(x) ((x)[4] & 0x20)
#define MULTI_NVM_HEADER_SIZE 0x04
#define ACL_HEADER_SIZE 5

#define EN_BCS_CAL_IN_NVM  0x03
...
bool nvm_download = false;

bool is_bcs_cal_dnl_disabled_ = false;
bool bcs_cal_property_read_ = false;

#ifdef BT_CP_CONNECTED
CoPVerSupported cop_ver_supported = {0};
#endif

#ifdef IS_PERI_ENABLED
extern bool tmel_patching_successful;
#endif

extern bool isCalPropertyRead;
bool is_soc_cal_enabled;

PatchDLManager::PatchDLManager(BluetoothSocType soc_type, BluetoothSecSocType sec_soc_type, HciTransport* transport, PowerManager* power_manager) :
  soc_type_(soc_type), sec_soc_type_(sec_soc_type),  transport_(transport), power_manager_(power_manager), dnld_fd_in_progress_(-1)
{
  ALOGD("%s", __func__);
  bt_logger_ = Logger::Get();
  fd_transport_ = transport_->GetCtrlFd(),
  wait_vsc_evt_ = true;
  ...
  LoadPatchMaptable();
  //Conditionally load BcscalMaptable based on property
  if (IsBcsCalDownLoadDisabled()) {
    ALOGI("%s: BCS CAL disabled by property", __func__);
  } else {
    LoadBcscalMaptable();
  }
  file_operation_failed = false;
  tlv_dwn_type_ = PATCH_THROUGH_CMD;
  board_id_cmd_status = -1;
}
...
PatchDLManager::~PatchDLManager()
{
  ...
  for(auto & element: BcscalInfoMap_)
    delete element.second;
  BcscalInfoMap_.clear();
}

bool PatchDLManager::IsBcsCalDownLoadDisabled() {

  if (bcs_cal_property_read_)
    return is_bcs_cal_dnl_disabled_;

  char value[PROPERTY_VALUE_MAX] = {'\0'};

  // Read the property. Default is "false".
  bt_logger_->PropertyGet("persist.vendor.qcom.bluetooth.bcs_cal_en", value, "false");
  if (strcmp(value, "true") == 0) {
    is_bcs_cal_dnl_disabled_ = true;
    ALOGI("%s: 'persist.vendor.qcom.bluetooth.bcs_cal_en' set to true", __func__);
  } else {
    is_bcs_cal_dnl_disabled_ = false;
    ALOGI("%s: 'persist.vendor.qcom.bluetooth.bcs_cal_en' set to false.", __func__);
  }

  bcs_cal_property_read_ = true; // Mark as read
  return is_bcs_cal_dnl_disabled_;
}
...
int PatchDLManager::PerformChipInit()
{
  ...
  gettimeofday(&tv, NULL);
    if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_UART){
      HciUartTransport* uart_transport = static_cast<HciUartTransport*>(transport_);
      ...
      err = uart_transport->Ioctl(USERIAL_OP_FLOW_ON, &flags);
    gettimeofday(&tv, NULL);
...
void PatchDLManager::ReadSocDump()
{
  int tot_len = 0;
  ...
  while (1) {
    ...
    if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
      tot_len = ReadNewHciEvent_SPI(buf, HCI_MAX_EVENT_SIZE);
    }else{
      tot_len = ReadNewHciEvent(buf, HCI_MAX_EVENT_SIZE);
    }
#else
    tot_len = ReadNewHciEvent(buf, HCI_MAX_EVENT_SIZE);
#endif
...
FILE *PatchDLManager::CheckAndUpdateBcsCalFile(void)
{
  FILE *bcsFile = NULL;
  std::string nvm_name;
  char bcs_nvm_file_path_bid[256] = { 0, };
  char bcs_nvm_alt_file_path_bid[256] = { 0, };
  int bcs_nvm_file_path_len = strlen(bcs_nvm_file_path);
  int bcs_nvm_alt_file_path_len = strlen(bcs_nvm_alt_file_path);

  // Check if BCS cal is enabled before proceeding
  if (IsBcsCalDownLoadDisabled()) {
    ALOGI("%s: BCS calibration is disabled by property. Skipping file check.", __func__);
    return NULL;
  }

  if (bcs_nvm_file_path_len == 0  && bcs_nvm_alt_file_path_len == 0) {
    ALOGI("%s: no bcs cal file to download", __func__);
    return NULL;
  }

  if (board_id_cmd_status  != -1) {
    if (bcs_nvm_file_path_len != 0) {
      memcpy(bcs_nvm_file_path_bid, bcs_nvm_file_path, bcs_nvm_file_path_len - 2);
      strlcat(bcs_nvm_file_path_bid, (char*)board_id_, sizeof(bcs_nvm_file_path_bid));
    }
    if (bcs_nvm_alt_file_path_len != 0){
      memcpy(bcs_nvm_alt_file_path_bid, bcs_nvm_alt_file_path, bcs_nvm_alt_file_path_len - 2);
      strlcat(bcs_nvm_alt_file_path_bid, (char*)board_id_, sizeof(bcs_nvm_alt_file_path_bid));
    }
    bcsFile = OpenPatchFile(bcs_nvm_file_path_bid, bcs_nvm_alt_file_path_bid);
  }

  if (bcsFile == NULL) {
    ALOGI("%s: Trying to open default bin instead of board id specific bin", __func__);
    bcsFile = OpenPatchFile(bcs_nvm_file_path, bcs_nvm_alt_file_path);
    if (bcsFile == NULL) {
      ALOGI("%s: might be BCS calibration is not supported", __func__);
      return NULL;
    }
  }
...
int PatchDLManager::GetTlvFile(const char *file_path, const char* alt_file_path)
{
  FILE * pFile = NULL;
  FILE * cFile = NULL;
  int p_fileSize, c_fileSize = 0, bcs_calfile_size = 0;
  int readSize, fileSize = 0;
  bool cal_file_avl = false, is_bcs_calfileavb = false;
  int offset;
  FILE *bcsFile = NULL;
  ...
  /* Get NVM file Size */
  fseek(pFile, 0, SEEK_END);
  p_fileSize = ftell(pFile);
  rewind(pFile);

  if (nvm_download) {
    cFile = OpenCalFile(BT_FW_CAL_DEFAULT_PATH);
    if (cFile != NULL) {
      // CAL file available
      unsigned char first_byte = 0;
      fread(&first_byte, 1, 1, cFile);
      rewind(cFile);

      if(first_byte == 0x02) {

        cal_file_avl = true;
        ALOGI("%s: %s file is available, and open succeeded", __func__, BT_FW_CAL_DEFAULT_PATH);

        /* Get CAL file Size */
        fseek(cFile, 0, SEEK_END);
        c_fileSize = ftell(cFile);
        rewind(cFile);

      } else {
        ALOGI("%s: %s file is available but corrupted, not appending with NVM data", __func__, BT_FW_CAL_DEFAULT_PATH);
      }
    }

    bcsFile = CheckAndUpdateBcsCalFile();
    if (bcsFile  != NULL) {
      /* Get BCS cal file Size */
      fseek(bcsFile, 0, SEEK_END);
      bcs_calfile_size = ftell(bcsFile);
      rewind(bcsFile);
      is_bcs_calfileavb = true;
    }
  }

  if (cal_file_avl || is_bcs_calfileavb) {
    fileSize = (p_fileSize + c_fileSize + bcs_calfile_size + MULTI_NVM_HEADER_SIZE);
  } else {
    fileSize = p_fileSize;
  }
  ...
  if (cal_file_avl || is_bcs_calfileavb) {
    // Multi NVM header
    int size = fileSize - MULTI_NVM_HEADER_SIZE;
    pdata_buffer_[0] = 0x04;
    pdata_buffer_[1] = (size & 0xFF);
    pdata_buffer_[2] = ((size >> 8) & 0xFF);
    pdata_buffer_[3] = ((size >> 16) & 0xFF);
    // Copy NVM file into buffer
    readSize = fread((pdata_buffer_ + MULTI_NVM_HEADER_SIZE), 1, p_fileSize, pFile);
    // Close NVM File
    fclose(pFile);
    if (readSize != p_fileSize) {
      ALOGE("Read Patch file size(%d) not matched with actual file size (%ld bytes)", readSize, p_fileSize);
      delete []pdata_buffer_;
      return -1;
    }
    offset = MULTI_NVM_HEADER_SIZE + p_fileSize ;
  }

  if (cal_file_avl) {
    readSize = fread((pdata_buffer_ + offset), 1, c_fileSize, cFile);
    fclose(cFile);
    if (readSize != c_fileSize) {
      ALOGE("Read CAL file size(%d) not matched with actual file size (%ld bytes)", readSize, c_fileSize);
      delete []pdata_buffer_;
      return -1;
    }
    ALOGE("Appending Calibration data (%d) with NVM", c_fileSize);
    offset += c_fileSize;
  }

  if (is_bcs_calfileavb) {
    readSize = fread((pdata_buffer_ + offset), 1, bcs_calfile_size, bcsFile);
    fclose(bcsFile);
    if (readSize != bcs_calfile_size) {
      ALOGE("Read BCS CAL file size(%d) not matched with actual file size (%ld bytes)", readSize, bcs_calfile_size);
      delete []pdata_buffer_;
      return -1;
    }
    ALOGE("Appending BCS Calibration data (%d) with NVM", bcs_calfile_size);
  }
  ...
  ALOGI("%s: fileSize %d p_fileSize %d c_fileSize %d bcs_calfile_size %d", __func__, fileSize,
       p_fileSize, c_fileSize, bcs_calfile_size);
  if (ReadTlvInfo())
    return fileSize;
  else
    return -1;
      ...
      if (ptlv_header->tlv_type == TLV_TYPE_BT_NVM) {
        ...
        if (IS_NEWER_THAN_CHEROKEE_2_1(chipset_ver_)) {
          ...
          if (nvm_ptr->tag_id == 17) {
            if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_UART){
              HciUartTransport* uart_transport = static_cast<HciUartTransport*>(transport_);
              *(nvm_byte_ptr + 2) = uart_transport->GetMaxBaudrate();
            }
          }
        }
        //code changes to enable/disable calibration for RTT in the NVM.
        if ((nvm_ptr->tag_id == 283) && (IsBcsCalDownLoadDisabled())) {
          ALOGD("%s: Enabled BCS Calibration mode in NVM \n", __func__);
          *(nvm_byte_ptr + 20) = EN_BCS_CAL_IN_NVM;
        }
      }
...
FILE* PatchDLManager::OpenCalFile(const char *file_path) {
...
/**
 * TlvDnldFrameThroughACL - Download the TLV through ACL packet format
 * @index: Frame index to be downloaded
 * @frame_size: Size of each ACL frame
 * @wait_cc_evt: Waiting status for command complete,
 *               true: wait for command complete
 *               false: Do not wait for command complete
 *
 * @Returns:
 *          1. Size of bytes successfully downloaded, if wait_cc_evt is false
 *          2. Status of Command Complete received from SoC (0: success, -1: Fail)
 */
int PatchDLManager::TlvDnldFrameThroughACL(int index, int frame_size, unsigned char wait_cc_evt)
{
  int err = -1;
  unsigned char cmd[ACL_MAX_CMD_SIZE];
  unsigned char rsp[HCI_MAX_EVENT_SIZE];
  HciUartTransport* uart_transport = static_cast<HciUartTransport*>(transport_);

  ALOGV("%s: Downloading TLV Patch segment no:%d, size:%d", __func__, index, frame_size);

  /* Frame the ACL CMD PKT to be sent to controller */
  FrameACLPkt(cmd, index, frame_size);

  bt_logger_->ProcessTx(HCI_PACKET_TYPE_ACL_DATA, &cmd[1], frame_size+4);

  /* Send ACL command packet to controller */
  err = uart_transport->Write(cmd, frame_size+5);
  if (err != (frame_size + ACL_HEADER_SIZE)) {
  ...
  if (wait_cc_evt) {
    err = ReadHciEvent(rsp, HCI_MAX_EVENT_SIZE);
    if (err < 0) {
      ALOGE("%s: Failed to send the patch payload to the controller ! 0x%x",__func__, err);
      return err;
    }
  }
...
int PatchDLManager::TlvDnldSegment(int index, int seg_size, unsigned char wait_cc_evt)
{
  ...
  if ( err != size) {
    ALOGE("Failed to send the patch payload to the Controller! 0x%x", err);
    ALOGE("Retrying the failed segment again");
    err = HciSendVsCmd((unsigned char*)cmd, rsp, size);
    if(err != size){
      return err;
    }
...
int PatchDLManager::getTlvDnldStatus() {
  return tlv_dl_status;
}
#ifdef IS_PERI_ENABLED
int PatchDLManager::ReadNewACLEvent_SPI(void) {
  int tot_len, err;
  unsigned char protocol_byte;
  unsigned char host_id_byte;
  uint16_t size;
  unsigned char rsp[ACL_MAX_CMD_SIZE];
  HciSpiTransport* spi_transport = static_cast<HciSpiTransport*> (transport_);
  while (1) {

    tot_len = spi_transport->Read((uint8_t *)&protocol_byte,(uint8_t *)&host_id_byte, rsp, ACL_MAX_CMD_SIZE);
    if (tot_len < 0) {
   ...
   if (rsp[0] == 0xFD) {
      ...
      rsp[0] = 0xFC;
      err = spi_transport->Write(rsp, 1);
      if (err != 1) {
        ALOGE("%s: Send failed with ret value: %d", __func__, err);
        return -1;
      }
      ALOGI("%s: Sent IBS_WAKE_ACK", __func__);
      rsp[0] = 0;
      continue;
    } else if (rsp[0] == 0xFC) {
      ...
      ALOGI("%s: Sent IBS_WAKE_IND", __func__);
      rsp[0] = 0;
      continue;
    } else if (rsp[0] == 0x02) {
      //tot_len = spi_transport->Read((rsp+1), 4);
      //if (tot_len < 0) {
      //  ALOGE("%s: Failed to read the frame", __func__);
      //  return -1;
      //}
	  if(tot_len < 5) {
		  ALOGE("%s: Failed to read the frame", __func__);
      ...
      ALOGE("%s: Ignore the ACL event = (0x%x%x) received", __func__, rsp[2], rsp[1]);

      size = ((rsp[4] << 8) | rsp[3]);

      if (tot_len < sizeof(rsp)) {
		  ALOGE("%s: Failed to read the frame", __func__);
		  return -1;
	  }else{
		  ALOGE("%s: Failed to read entire frame", __func__);
	  }
      /*if ((size + 5) < sizeof(rsp)) {
        tot_len = spi_transport->Read((rsp+5), size);
        if (tot_len < 0) {
          ALOGE("%s: Failed to read the frame", __func__);
          return -1;
        }
      ...
      //tot_len = spi_transport->Read((rsp+1), 7);
      //if (tot_len < 0) {
      //  ALOGE("%s: Failed to read the frame", __func__);
      //  return -1;
      //}
      ...
      ALOGV("HCI-RSP: \t0x%x \t0x%x \t0x%x \t0x%x \t0x%x \t0x%x \t0x%x \t0x%x",
        rsp[0], rsp[1], rsp[2], rsp[3], rsp[4], rsp[5], rsp[6], rsp[7]);

      if ((((rsp[5] << 8) | rsp[4]) == HCI_VS_DOWNLOAD_COMMAND) && ( rsp[6] == 0x00)) {
        ALOGV("%s : Commad Credit received\n", __func__);
        return 0;
      } else {
        ALOGE("%s: Failed to receive CC, Received an (0x%x%x) event with the status = (0x%x)",
          __func__, rsp[5], rsp[4], rsp[6]);
        return -1;
      }
    }
...
/**
 * TlvDnldReqACL - TLV Download Request through ACL
 * @tlv_size: Size of TLV/NVM file
 *
 * @Returns:
 *          1. Number of bytes downloaded per frame.
 *          2. Returns -1 in case of failure.
 */
int PatchDLManager::TlvDnldReqACL(int tlv_size) {
  int total_frames, remain_size, i, err = -1;
  unsigned char wait_cc_evt = true;
  bool last_frame = false;
  int frame_download_len = MAX_SIZE_PER_TLV_SEGMENT_FOR_ACL;

  total_frames = tlv_size / MAX_SIZE_PER_TLV_SEGMENT_FOR_ACL;
  remain_size = tlv_size % MAX_SIZE_PER_TLV_SEGMENT_FOR_ACL;

  ALOGI("%s: TLV size: %d, Total Seg num: %d, Remain size: %d",
        __func__, tlv_size, total_frames, remain_size);

  if (tlv_type_ == TLV_TYPE_PATCH || tlv_type_ == ELF_TYPE_PATCH
      || tlv_type_ == TLV_TYPE_BT_NVM || tlv_type_ == TLV_TYPE_BT_FM_NVM) {
  ...
  for (i = 0; i <= total_frames && !last_frame; i++) {
    /* check for last frame based on the remaining size
     * and total number of frames
     */
    if ((remain_size && i == total_frames)
          || (!remain_size && (i + 1) == total_frames)) {
      last_frame = true;
      // Update segment download len if last segment is being downloaded
      if (remain_size)
        frame_download_len = remain_size;
      ALOGD("%s: Updating frame len to %d as last frame", __func__, frame_download_len);
    }

    if ((tlv_type_ == TLV_TYPE_PATCH || tlv_type_ == ELF_TYPE_PATCH
          || tlv_type_ == TLV_TYPE_BT_NVM || tlv_type_ == TLV_TYPE_BT_FM_NVM)
          && last_frame) {
      /* If tlv_dwn_cfg_ is SKIP_EVT_VSE_CC, then wait for CCE */
      wait_cc_evt = true;
    }

    patch_dnld_pending_ = true;
    if ((err = TlvDnldFrameThroughACL(i, frame_download_len, wait_cc_evt)) < 0) {
      bt_logger_->SetInitFailureReason(BT_HOST_REASON_TLV_DOWNLOAD_FAILED);
      goto error;
    }
    patch_dnld_pending_ = false;
...
bool PatchDLManager::getFileOperationStatus(void) {
...
int PatchDLManager::DownloadTlvFile()
{
  ...
  if ((tlv_size = GetTlvFile(rampatch_file_path, rampatch_alt_file_path)) > 0) {
    ...
    bt_logger_->SetCurrentactivityStartTime(tv, BT_HOST_REASON_PATCH_DNLD_STUCK,
                                              "RAM PATCH DOWNLOAD");
    if (soc_type_ == BT_SOC_ORNE && isPatchThroughACL())
      err = TlvDnldReqACL(tlv_size);
    else
  ...
  if (IsXmemDownload(&tlv_size)) {
    ...
    if (soc_type_ == BT_SOC_ORNE && tlv_dwn_type_ == PATCH_THROUGH_ACL) {
      ReadNVMConfig();
      if (strcmp(nvm_config_, "no_value") != 0) {
        err = PatchConfigReq();
        if (err < 0) {
          ALOGE("%s: Patch config NVM CMD Failed", __func__);
          bt_logger_->SetInitFailureReason(BT_HOST_REASON_PATCH_CONFIG_FAILED);
          delete []pdata_buffer_;
          return err;
        }
    ...
    bt_logger_->SetCurrentactivityStartTime(tv, BT_HOST_REASON_NVM_DNLD_STUCK,
                                              "NVM file download");
#ifdef  IS_PERI_ENABLED
    if (isPatchThroughACL())
      err = TlvDnldReqACL(tlv_size);
    else
      err = TlvDnldReq(tlv_size);
#else
    if (tlv_dwn_type_ == PATCH_THROUGH_ACL)
      err = TlvDnldReqACL(tlv_size);
    else
      err = TlvDnldReq(tlv_size);
#endif
...
#ifdef BT_CP_CONNECTED
int PatchDLManager::GetCoPVersion(void)
{
  int size, err = 0;
  unsigned char cmd[HCI_MAX_CMD_SIZE] = {0x01, 0x17, 0xFC, 0x01, 0x19};;
  unsigned char rsp[HCI_MAX_EVENT_SIZE];
  unsigned char temp_wait_vsc_evt;
  uint8_t num_of_scenarios;
  uint8_t len;

  /* Total length of the packet to be sent to the Controller */
  size = (HCI_CMD_IND + HCI_COMMAND_HDR_SIZE + 1);

  ALOGD("%s: Sending Get COP version command", __func__);
  ALOGD("HCI-CMD: \t0x%x \t0x%x \t0x%x \t0x%x \t0x%x", cmd[0],
		  cmd[1], cmd[2], cmd[3], cmd[4]);

  // Command Status not expected for CoP Version
  ...
  err = ReadHciEvent(rsp, HCI_MAX_EVENT_SIZE);
   if (err < 0) {
      ALOGE("%s: Failed to CoP Version", __func__);
      goto error;
  }
  num_of_scenarios = rsp[8];
  len = (num_of_scenarios * 2) + sizeof(uint8_t);

  memset(&cop_ver_supported, 0, sizeof(CoPVerSupported));
  cop_ver_supported.len = len;
  if (!num_of_scenarios || len == 1) {
    ALOGE("%s: CoP pkt is not supported or full formed", __func__);
    goto error;
  }

  memcpy(cop_ver_supported.payload, &rsp[8], len);
...
bool PatchDLManager::isSocCalibrationEnabled(void) {
  char value[PROPERTY_VALUE_MAX] = {'\0'};

  if (isCalPropertyRead)
    return is_soc_cal_enabled;

  isCalPropertyRead = true;

  bt_logger_->PropertyGet("persist.vendor.service.bdroid.is_soc_cal_enabled", value, "true");

  if (strcmp(value, "true") == 0) {
    is_soc_cal_enabled = true;
  } else {
    is_soc_cal_enabled = false;
  }
...
SocAddOnFeatures_t* PatchDLManager::GetAddOnFeatureList()
{
  ...
  if ((param_len > HCI_EVENT_PARAMS)  && (cmd_status == 0x00)) {
    ...
    gettimeofday(&tv, NULL);
    bt_logger_->CheckAndAddToDelayList(&tv);
    if (isSocCalibrationEnabled()) {
#ifdef IS_PERI_ENABLED
      if ((GetChipVersion() == GANGES_VER_2_0) || (GetChipVersion() == BRAHMA_VER_2_0) ||
          (GetChipVersion() == THEMISTO_VER_1_0) || (GetChipVersion() == THEMISTO_VER_2_0) || 
          (GetChipVersion() == ORNE_BT_VER_1_0)) {
#else
      if (GetChipVersion() == ORNE_BT_VER_1_0) {
#endif
        DataHandler::Get()->updateSocCalibrationSupportStatus(rsp[HCI_ADDON_CMD_CALIBRATION_BYTE]);
      } else {
        DataHandler::Get()->isSocSupportCal = false;
        ALOGE("%s: Soc calibration loging disabled based on persist property", __func__);
      }
    }
    return &add_on_features;
  }
...
#ifdef IS_PERI_ENABLED
    if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
      ret = ReadNewHciEvent_SPI(rsp, HCI_MAX_EVENT_SIZE);
    }else{
...
void PatchDLManager::EnableEnhControllerLog()
{
  ...
  unsigned char cmd[ENH_LOG_ENABLE_CMD_LEN] = { 0x01, 0x17, 0xFC, 0x02, 0x14, 0x00};
  unsigned char ext_cmd[EXT_ENH_LOG_ENABLE_CMD_LEN]
                   = { 0x01, 0x17, 0xFC, 0x06, 0x17, 0x04, 0x00, 0x00, 0x00, 0x00 };
  unsigned char temp_wait_vsc_evt;
  unsigned char rsp[HCI_MAX_EVENT_SIZE];
  char value[PROPERTY_VALUE_MAX] = { '\0' };
  uint8_t cmdLen;
  ...
  /* Based on the property value, if value <= 255, host sends the cmd (sub-opcode 0x14) with 1 byte.
     If value > 255, host sends the cmd (sub-opcode 0x17) with 4 bytes log mask */
  if (logMask <= 255) {
    cmdLen = ENH_LOG_ENABLE_CMD_LEN;
    cmd[5] = (unsigned char)logMask;
   } else {
    cmdLen = EXT_ENH_LOG_ENABLE_CMD_LEN;
    ext_cmd[6] = (unsigned char)logMask;
    ext_cmd[7] = (unsigned char)(logMask >> 8);
    ext_cmd[8] = (unsigned char)(logMask >> 16);
    ext_cmd[9] = (unsigned char)(logMask >> 24);
   }
  ...
  if (cmdLen == ENH_LOG_ENABLE_CMD_LEN)
    ret = HciSendVsCmd((unsigned char*)cmd, rsp, cmdLen);
  else
    ret = HciSendVsCmd((unsigned char*)ext_cmd, rsp, cmdLen);
  if (ret != cmdLen) {
    ALOGE("%s: command failed", __func__);
  }
  wait_vsc_evt_ = temp_wait_vsc_evt;
...
int PatchDLManager::SetBaudRateReq()
{
  ...
  int flags;
  HciUartTransport* uart_transport = static_cast<HciUartTransport*>(transport_);
  uint8_t bt_baud_rate = uart_transport->GetMaxBaudrate();
  struct timeval tv;
  ...
  /* Send the HCI command packet to UART for transmission */
  err = uart_transport->Write(cmd, size);
  if (err != size) {
    ALOGE("%s: Send failed with ret value: %d", __func__, err);
    err  = -1;
    goto error;
  ...
  uart_transport->SetBaudRate(bt_baud_rate);
...
int PatchDLManager::SetUartBuadRate(uint8_t bt_baud_rate)
{
   ...
   baud_rate_value = userial_tcio_baud_to_int(baud_rate_tcio, soc_type_);
   if (!status) {
...
int PatchDLManager::SocInit(uint8_t *bdaddr, bool is_emb_wp_mode)
{
  ...
  if ( QCA_VER_PROD_ID(chipset_ver_) == PROD_ID_BT_SOC_EVROS ) {
    map_chipset_ver =  QCA_CHIPSET_VER_NO_SOC_ID(chipset_ver_);
  } else {
    map_chipset_ver = chipset_ver_;
  }
  auto itr = PatchPathInfoMap_.find(map_chipset_ver);
  ...
  if (chipset_ver_ == MOSELLE_VER_1_2 ||
      chipset_ver_ == MOSELLE_VER_1_1 ||
      chipset_ver_ == MOSELLE_VER_1_0) {
    GetAppVerCmd();
...
#ifdef IS_PERI_ENABLED
  if (chipset_ver_ == GANGES_VER_1_0 ||
      chipset_ver_ == GANGES_VER_2_0 ||
      chipset_ver_ == BRAHMA_VER_1_0 ||
      chipset_ver_ == BRAHMA_VER_2_0) {
        if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_UART){
            /* Checking Baud rate */
            HciUartTransport* uart_transport = static_cast<HciUartTransport*>(transport_);
            int baud_rate_set = uart_transport->GetBaudRate();
  ...
  if (getTlvDnldStatus() != HCI_CMD_SUCCESS) {

    ALOGE("%s: Patching failed might because of corrupted %s file", __func__,
          BT_FW_CAL_DEFAULT_PATH);

    if (remove(BT_FW_CAL_DEFAULT_PATH)) {
      ALOGE("%s: Failed to delete %s File !!!", __func__,
        BT_FW_CAL_DEFAULT_PATH);
    } else {
      ALOGE("%s: %s File deleted Successfully !!!", __func__,
        BT_FW_CAL_DEFAULT_PATH);
    }
  ...
  if (patch_through_acl)
    patch_through_acl = false;
  ...
  if (soc_type_ == BT_SOC_GANGES) {
  	if (tmel_patching_successful) {
  /* Download License file here */
	ALOGI("%s: TME-L supported, start downloading license file\n", __func__);
  err = DownloadLicenseFile();
  if (err < 0) {
	ALOGE("%s: License download failed,"
			"but basic features will be supported.\n", __func__);
    } else {
      ALOGI("%s: Download License file successfully.", __func__);
  }
  } else
      ALOGI("%s: TME-L unpatched, skipping license file download.", __func__);
  }
#endif
...
void PatchDLManager::SetBcsCalRegularPaths(std::string& nvm_name)
{
  snprintf(bcs_nvm_file_path, sizeof(bcs_nvm_file_path), "%s%s%s", BT_FW_REGULAR_PATH,
           alt_path_for_fw, nvm_name.c_str());
  snprintf(bcs_nvm_alt_path_qti_check, sizeof(bcs_nvm_alt_path_qti_check), "%s%s%s",
           BT_FW_QTI_ALT_PATH, alt_path_for_fw, nvm_name.c_str());
  snprintf(bcs_nvm_alt_path_check, sizeof(bcs_nvm_alt_path_check), "%s%s%s",
           BT_FW_ALT_PATH, alt_path_for_fw, nvm_name.c_str());
}
...
void PatchDLManager::FormRegularPaths(PatchPathManager * info, bool is_xmem)
{
  ...
  std::string tlv_name;
  std::string nvm_name;
  std::string bcs_nvm_name;
  XmemPatchPathManager * xmem_info = NULL;
  Bcscal *bcs_info = NULL;
  bool bcs_cal_file_avb = false;
  char dst_buff[MAX_BUFF_SIZE];
  struct timeval tv;
  ...
  if (is_xmem && info && info->GetPatchType() == "XMEM" &&
  ...
  //Conditionally get BCS cal info
  if (!IsBcsCalDownLoadDisabled()) {
      auto itr_bcs = BcscalInfoMap_.find(chipset_ver_);
      if (itr_bcs != BcscalInfoMap_.end())
        bcs_info = itr_bcs->second;

      if (bcs_info && !bcs_info->GetChipName().empty()) {
        bcs_nvm_name = bcs_info->GetNVMName();
        SetBcsCalRegularPaths(bcs_nvm_name);
        bcs_cal_file_avb = true;
      }
  }
  ...
  if (CheckAndSetProperRampatchAltPath() == -1) {
    ...
    if (info && info->GetPatchType() == "ELF" && stat(rampatch_file_path, &sts) == -1){
  ...
  if (!stat(nvm_alt_path_qti_check, &sts) && sts.st_size > 0) {
    ...
    if (bcs_cal_file_avb)
      strlcpy(bcs_nvm_alt_file_path, bcs_nvm_alt_path_qti_check, sizeof(bcs_nvm_alt_file_path));
    ...
    memset(bcs_nvm_alt_file_path, 0, sizeof(bcs_nvm_alt_file_path));
  }
...
int PatchDLManager::HciReset()
{
  ...
  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
    HciSpiTransport* spi_transport = static_cast<HciSpiTransport*> (transport_);
    err = spi_transport->Write(HCI_PACKET_TYPE_CMD_FROM_HAL, cmd, size);
  }else{
    err = transport_->Write(cmd, size);
  }
  if (err != size) {
...
int PatchDLManager::ReadNewHciEvent(unsigned char* buf, int size)
{
  ...
  do {
    ...
    retval = transport_->Read(&protocol_byte, 1);
    if (retval < 0) {
      ALOGE("%s: read error", __func__);
      return -1;
    }
    ...
    if (protocol_byte == LOG_BT_EVT_PACKET_TYPE) {
      ...
      retval = transport_->Read(hdr, BT_EVT_HDR_SIZE);
      if (retval < 0) {
      ...
      retval = transport_->Read(buf + BT_EVT_HDR_SIZE + 1, packet_len);
      if (retval < 0) {
      ...
      retval = transport_->Read(&host_id_byte, 1);
      if (retval < 0) {
      ...
      retval = transport_->Read(hdr, PERI_EVT_HDR_SIZE);
      if (retval < 0) {
      ...
      retval = transport_->Read(buf + PERI_EVT_HDR_SIZE + 2, packet_len);
      if (retval < 0) {
      ...
      retval = transport_->Read(buf, BT_ACL_HDR_SIZE);
      if (retval < 0) {
        ALOGE("%s: read error", __func__);
        return -1;
      }
      ...
      retval = transport_->Read(tmp_buf + BT_ACL_HDR_SIZE, tot_len);
      if (retval < 0) {
        ALOGE("%s: read error", __func__);
        if (tmp_buf != NULL) {
          delete [] tmp_buf;
      ...
      transport_->Write(&wake_byte, 1);
    } else if (protocol_byte == 0xFE) {
      ALOGI("%s: Received sleep indication 0xFE", __func__);
    } else {
...
#ifdef IS_PERI_ENABLED
int PatchDLManager::ReadNewHciEvent_SPI(unsigned char* buf, int size){
  int retval;
  unsigned char protocol_byte;
  unsigned char hdr[BT_EVT_HDR_SIZE];
  unsigned char packet_len;
  ...
  unsigned char* buff = new (std::nothrow) unsigned char[size];
...
    return -1;
  ...
  do {
    retval = spi_transport->Read((uint8_t *)&protocol_byte,(uint8_t *)&host_id_byte, buff, size);
    ...
    if (protocol_byte == LOG_BT_EVT_PACKET_TYPE) {
        ...
        if(retval >= 2){
          ...
          ALOGE("%s: size is too less for host_id_byte", __func__);
          delete[] buff;
          return -1;
        }
        buf[0] = protocol_byte;
        buf[1] = host_id_byte;
        memcpy(buf+2, buff, retval);
        tot_len = retval;
        break;
    } else if (protocol_byte == LOG_BT_ACL_PACKET_TYPE) {
      ALOGI("%s: Got ACL Packet, Ignore",__func__);
    } else if (protocol_byte == 0xFD) {
      ALOGI("%s: Got FD , responding with FC", __func__);
      uint8_t wake_byte = 0xFC;
      spi_transport->Write(HCI_PACKET_TYPE_COMMAND, &wake_byte, 1);
    } else if (protocol_byte == 0xFE) {
      ALOGI("%s: Received sleep indication 0xFE", __func__);
    } else {
      ALOGI("%s: Got an invalid proto byte: %d", __func__, protocol_byte);
...
#ifdef IS_PERI_ENABLED
    if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
      tot_len = ReadNewHciEvent_SPI(buf, size);
    }else{
...
int PatchDLManager::ReadHciEvent(unsigned char* buf, int size)
{
  ...
  do {
    ...
    if (buf[1] == LOG_BT_EVT_VENDOR_SPECIFIC) {
      if (buf[3] == LOG_BT_CONTROLLER_LOG) {
        /* if it is RAMDUMP event OR if normal logging event and diag is enabled */
    ...
    } else if ((buf[2] == PERI_EVT_CMD_COMPLETE) &&
             (buf[4] == HCI_PERI_EVT_CLASS) &&
        ...
        if (!collecting_peri_ram_dump) {
          ALOGE("%s: It is RAMDUMP OR HCI_BUFFER_DUMP, keep looping to get RAMDUMP events",
               __func__);
          collecting_peri_ram_dump = true;
        } else if (buf[5] != LOG_HCI_PERI_BUFF_DUMP) {
  ...
  } while (collecting_peri_ram_dump);
  return tot_len;
}
...
int PatchDLManager::ReadVsHciEvent(unsigned char* buf, int size)
{
  ...
  } while (collecting_ram_dump || collecting_peri_ram_dump);
...
int PatchDLManager::GetVsHciEvent(unsigned char *rsp)
{
  ...
  if((rsp[EVENTCODE_OFFSET] == VSEVENT_CODE) || (rsp[EVENTCODE_OFFSET] == EVT_CMD_COMPLETE)){
    ALOGV("%s: Received BT HCI-Vendor Specific event", __func__);
  } else {
  ...
  switch ( ocf ) {
    ...
    case HCI_VS_STRAY_EVT:
      if (unified_hci && (subOpcode == HCI_VS_HOST_LOG_ENH_ENABLE_SUBID ||
                           subOpcode == HCI_VS_HOST_LOG_ENH_ENABLE_EXT_SUBID) ) {
      ...
      if (unified_hci && (subOpcode == HCI_VS_HOST_SUB_GET_COP_VER)) {
        if (status != HCI_CMD_SUCCESS) {
          ALOGE("%s: CoPVersion failed with status: 0x%x", __func__, status);
          err = -1;
	}
        break;
      }
  ...
  switch (subOpcode) {
    ...
    case EDL_APP_VER_RES_EVT:
      if (!unified_hci) {
        ...
        if (productid == PROD_ID_GANGES && sec_soc_type_ == BT_SEC_SOC_BRAHMA) {
          productid = PROD_ID_BRAHMA;
        }
...
int PatchDLManager::HciSendVsCmd(unsigned char *cmd, unsigned char *rsp, int size)
{
  ...
  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
    HciSpiTransport* spi_transport = static_cast<HciSpiTransport*> (transport_);
    ret = spi_transport->Write(HCI_PACKET_TYPE_CMD_FROM_HAL, cmd, size);
  }else{
    ret = transport_->Write(cmd, size);
  }
  if (ret != size) {
...
/**
* FrameACLPkt - Form a ACL Packet
* @cmd: Empty char array to be filled with ACL header and payload.
* @pktNo: Packet number to be sent.
* @size: Size of ACL Packet to be sent.
*
* @Returns: None
*/
void PatchDLManager::FrameACLPkt(unsigned char *cmd, int pktNo, int size)
{
  int offset = 0;

  memset(cmd, 0, ACL_MAX_CMD_SIZE);

  // ACL Identifier
  cmd[0] = HCI_PACKET_TYPE_ACL_DATA;

  // Data Handler
  cmd[1] = (char) ACL_DATA_HANDLE;
  cmd[2] = (char) (ACL_DATA_HANDLE >> 8);

  // Size of ACL frame
  cmd[3] = (char) size;
  cmd[4] = (char) (size >> 8);

  offset = (pktNo * MAX_SIZE_PER_TLV_SEGMENT_FOR_ACL);
  memcpy(cmd+5, (pdata_buffer_ + offset), size);

  ALOGV("%s: Sending ACL_PATCH_TLV_REQ_CMD", __func__);
  ALOGV("HCI-CMD1: \t0x%x \t0x%x \t0x%x \t0x%x \t0x%x", cmd[0], cmd[1], cmd[2], cmd[3], cmd[4]);
...
)
{
  ...
  switch (edl_cmd) {
    ...
    case EDL_TLV_PATCH_CONFIG: {
      ...
      if (strcmp(elf_config_, "no_value") != 0) {
        unsigned long ul;
        /* converting hex in string to real hex number and
         * send the Patch Config, if SoC is ORNE and tlv_type is NVM.
         */
        if ((soc_type_ == BT_SOC_ORNE) && (tlv_type_ == TLV_TYPE_BT_NVM || tlv_type_ == TLV_TYPE_BT_FM_NVM))
          ul = strtoul (nvm_config_, NULL, 16);
        else
        ul = strtoul (elf_config_, NULL, 16);
        /* patch config size is 4 */
        memcpy(&cmd[5], &ul, sizeof(uint32_t));
        /* Checking BIT 0, BIT 1, BIT 2 and BIT 4:
         * 1) BIT 0 and BIT 2: Whether SoC needs to respond for every
         *     download command (0) or only at the end (1).
         * 2) BIT 1: Whether to perform Parallel (1) or Sequential
         *     validation (0)
         * 3) BIT 4: Whether Patch needs to download via ACL frame(1)
         * 	   or via EDL packet(0).
         */
        if (soc_type_ == BT_SOC_ORNE) {
          if (tlv_type_ == ELF_TYPE_PATCH) {
            /* Check the BIT 0 for Patch download optimization */
            if ((cmd[5] & 1) == 0) {
            ...
            /* Check the BIT 2 for NVM download optimization */
            if ((cmd[5] & 4) == 0) {
              tlv_dwn_cfg_ = SKIP_EVT_NONE;
            }
            else {
          ...
          /* Check the BIT 4 for ACL download or EDL download */
          if ((cmd[5] & 16) == 0) {
            tlv_dwn_type_ = PATCH_THROUGH_CMD;
            ALOGI("%s: Download patch through EDL", __func__);
          } else {
            tlv_dwn_type_ = PATCH_THROUGH_ACL;
            ALOGI("%s: Download patch through ACL", __func__);
          }
        } else {
          if ((cmd[5] & 1) == 0) {
            tlv_dwn_cfg_ = SKIP_EVT_NONE;
...
unsigned char PatchDLManager::IsSobsEnabled()
{
  /* Check whether OBS is enabled through the macro 'WCNSS_OBS_ENABLED'
     and the property 'persist.vendor.service.bdroid.sobs'. */
#ifdef WCNSS_OBS_ENABLED
  return ObsHandler::IsEnabled();
#else
  return false;
#endif
}
...
void PatchDLManager::UpdateNewNvmFormat(tlv_nvm_hdr *nvm)
{
  ...
  if (nvm->tag_id == TAG_NUM_17) {
    if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_UART){
		HciUartTransport* uart_transport = static_cast<HciUartTransport*>(transport_);
        uint8_t baudrate = uart_transport->GetMaxBaudrate();
  ...
  /* Update Tag#155: sensor uart configs */
  if (nvm->tag_id == TAG_NUM_155 && soc_type_ == BT_SOC_HAMILTON) {
    soc_info_v0_1 msm_soc_info = {MSM_CPU_UNKNOWN, MSM_CPU_UNKNOWN};
    get_soc_info(&msm_soc_info);
    ALOGI("%s: msm_cpu=%d", __func__, msm_soc_info.msm_cpu);
    if (msm_soc_info.msm_cpu != MSM_CPU_UNKNOWN &&
        msm_soc_info.msm_cpu != MSM_CPU_KALAMA &&
        msm_soc_info.msm_cpu != MSM_CPU_PINEAPPLE) {
      ALOGI("%s: enable inband wakeup on sensor uart", __func__);
      *(nvm_byte_ptr + 14) = 0xFF;
    }
  }

#ifdef QTI_BT_FMD_SW_WAR_DISABLE
/* Update Tag#279: FMD config to enable/disable FMD SW WAR */
  if (nvm->tag_id == TAG_NUM_279) {
    /* Reset 8th byte of FMD tag 279 to disable FMD SW WAR */
    *(nvm_byte_ptr + 8) = 0x00;
    ALOGI("%s: disabled FMD SW WAR", __func__);
  }
#endif /* QTI_BT_FMD_SUPPORTED */
...
void PatchDLManager::LoadBcscalMaptable() {
  BcscalInfoMap_.insert(std::make_pair<uint64_t, Bcscal*>(GANGES_VER_1_0,
                          new Bcscal("GAN1_0", "gngbcscal10.bin")));
  BcscalInfoMap_.insert(std::make_pair<uint64_t, Bcscal*>(GANGES_VER_2_0,
                          new Bcscal("GAN2_0", "gngbcscal20.bin")));
  BcscalInfoMap_.insert(std::make_pair<uint64_t, Bcscal*>(BRAHMA_VER_2_0,
                           new Bcscal("BRAHMA_VER_2_0", "brhbcscal20.bin")));
  BcscalInfoMap_.insert(std::make_pair<uint64_t, Bcscal*>(THEMISTO_VER_1_0,
                           new Bcscal("THEMISTO_VER_1_0", "tmobcscal10.bin")));
  BcscalInfoMap_.insert(std::make_pair<uint64_t, Bcscal*>(THEMISTO_VER_2_0,
                           new Bcscal("THEMISTO_VER_2_0", "tmobcscal20.bin")));
  BcscalInfoMap_.insert(std::make_pair<uint64_t, Bcscal*>(ORNE_BT_VER_1_0,
                          new Bcscal("ORN_1_1", "ornbcscal10.bin")));
  BcscalInfoMap_.insert(std::make_pair<uint64_t, Bcscal*>(COLOGNE_BT_VER_1_0,
                          new Bcscal("COLOGNE_1_1", "clnbcscal10.bin")));
  ALOGI("%s: BcscalMaptable loaded successfully.", __func__);
}
...
void PatchDLManager::LoadPatchMaptable() {
  ...
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(MOSELLE_VER_1_1,
                          new ELFPatchPathManager("MOS1_1", "msbtfw11.mbn", "msnv11.bin",
                                                  "msbtfw11.tlv")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(MOSELLE_VER_1_2,
                          new ELFPatchPathManager("MOS1_2", "msbtfw12.mbn", "msnv12.bin",
                                                  "msbtfw12.tlv")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(HAMILTON_VER_1_0,
                          new PatchPathManager("HAM1_0", "hmtbtfw10.tlv", "hmtnv10.bin")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(HAMILTON_VER_1_0_1,
                          new PatchPathManager("HAM1_0_1", "hmtbtfw10.tlv", "hmtnv10.bin")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(HAMILTON_VER_2_0,
                          new PatchPathManager("HAM2_0", "hmtbtfw20.tlv", "hmtnv20.bin")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(GANGES_VER_1_0,
                        new PatchPathManager("GAN1_0", "gngbtfw10.tlv", "gngbtnv10.bin")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(GANGES_VER_2_0,
                        new ELFPatchPathManager("GAN2_0", "gngbtfw20.mbn", "gngbtnv20.bin",
                                                  "gngbtfw20.tlv")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(BRAHMA_VER_1_0,
                        new PatchPathManager("BRAHMA_VER_1_0", "brhbtfw10.tlv",
                                                  "brhbtnv10.bin")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(BRAHMA_VER_2_0,
                        new ELFPatchPathManager("BRAHMA_VER_2_0", "brhbtfw20.mbn",
                                                  "brhbtnv20.bin", "brhbtfw20.tlv")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(ORNE_BT_VER_1_0,
                          new ELFPatchPathManager("ORN_1_1", "ornbtfw10.mbn", "ornnv10.bin",
                                                  "ornbtfw10.tlv")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(COLOGNE_BT_VER_1_0,
                         new ELFPatchPathManager("COLOGNE_1_1", "clnbtfw10.mbn",
                                                  "clnbtnv10.bin", "clnbtfw10.tlv")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(EVROS_BT_VER_1,
                        new PatchPathManager("EVROS_BT1", "evrbtfw10.tlv", "evrnv10.bin")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(EVROS_BT_VER_2,
                        new PatchPathManager("EVROS_BT2", "evrbtfw20.tlv", "evrnv20.bin")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(THEMISTO_VER_1_0,
                        new ELFPatchPathManager("THEMISTO1_0","tmobtfw10.mbn", "tmonv10.bin", "tmobtfw10.tlv")));
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(THEMISTO_VER_2_0,
                        new ELFPatchPathManager("THEMISTO2_0","tmobtfw20.mbn", "tmonv20.bin", "tmobtfw20.tlv")));
}
...
void PatchDLManager::ReadELFConfig() {
  if (!elf_config_read_) {
    ...
    if (soc_type_ == BT_SOC_MOSELLE) {
      default_value = "0x01";
    } else if ((soc_type_ == BT_SOC_ORNE) || (soc_type_ == BT_SOC_COLOGNE)) {
      if (isPatchThroughACL()) {
        if (isPatchOptEnable()) {
          // Download patch through ACL and expect CC for last downloaded packet
          ALOGI("%s: Patch through ACL and optimization is enable", __func__);
          default_value = "0x11";
        } else {
          // Download the patch through ACL but don't optimize the download process
          ALOGI("%s: Patch through ACL and optimization is disable", __func__);
          default_value = "0x10";
        }
      } else {
        ALOGI("%s: Patch through EDL and optimization is enable", __func__);
        default_value = "0x01";
      }
    }
...
/* Read the NVM download configuration */
void PatchDLManager::ReadNVMConfig() {
  if (!nvm_config_read_) {
    const char* default_value = "no_value";
    if (isPatchOptEnable()) {
      // Download patch through ACL and expect CC for last download packet
      default_value = "0x14";
      ALOGI("%s: Download NVM through ACL and optimization is enable", __func__);
    } else {
      // Download the patch through ACL but don't optimize the download process
      default_value = "0x10";
      ALOGI("%s: Download NVM through ACL and optimization is disable", __func__);
    }
    bt_logger_->PropertyGet("persist.vendor.bluetooth.nvm_config", nvm_config_, default_value);
    ALOGI("%s: NVM patch config: %s", __func__, nvm_config_);
    nvm_config_read_ = true;
  }
}

/* Check for TLV/NVM Optimization is enabled */
bool PatchDLManager::isPatchOptEnable()
{
  if (nvm_optim_read)
    return tlv_optim;;

  char value[PROPERTY_VALUE_MAX] = {'\0'};
  bt_logger_->PropertyGet("persist.vendor.service.bdroid.patch_optimization", value, "true");
  nvm_optim_read = true;
  if (strcmp(value, "true") == 0) {
    tlv_optim = true;
    ALOGI("%s: TLV Download Optimization is enabled.", __func__);
  } else {
    tlv_optim = false;
    ALOGI("%s: TLV Download Optimization is disabled.", __func__);
  }

  return tlv_optim;
}

/* Check for ACL Paching is enabled */
bool PatchDLManager::isPatchThroughACL()
{
  if(!acl_prop_read_) {
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    if (DataHandler::Get()->GetSocType() == BT_SOC_ORNE) {
      bt_logger_->PropertyGet("persist.vendor.service.bdroid.patch_through_acl",
        value, "true");
    } else {
      bt_logger_->PropertyGet("persist.vendor.service.bdroid.patch_through_acl",
        value, "false");
    ...
    acl_prop_read_ = true;
    if (strcmp(value, "true") == 0)
      patch_through_acl = true;
    else
      patch_through_acl = false;
  }

  return patch_through_acl;
}

void PatchDLManager::EnableLc3Swb()
{
  static bool lc3_swb_property_already_set = false;

  if (add_on_features.feat_mask_len == SOC_ADD_ON_FEATURE_MASK_INVALID_LENGTH ||
      add_on_features.feat_mask_len < 5) {
    ALOGI("%s: does not have lc3 over hfp bit, len=%d", __func__, add_on_features.feat_mask_len);
    return;
  }

  if (lc3_swb_property_already_set) {
    return;
  }
  lc3_swb_property_already_set = true;

  char value[PROPERTY_VALUE_MAX] = { '\0' };
  bt_logger_->PropertyGet("bluetooth.hfp.swb.supported", value, "");
  ALOGI("%s: current property value=%s", __func__, value);
  if (strcmp(value, "") != 0) {
    return;
  }

  if (HCI_LC3_OVER_HFP_SUPPORT(add_on_features.features)) {
    bt_logger_->PropertySet("bluetooth.hfp.swb.supported", "true");
    ALOGI("%s: enabled", __func__);
  } else {
    bt_logger_->PropertySet("bluetooth.hfp.swb.supported", "false");
    ALOGI("%s: disabled", __func__);
  }
}

#ifdef IS_PERI_ENABLED
int PatchDLManager::DownloadLicenseFile()
{
  int license_file_size = 0;
  int err = -1;
  int license_file_len = strlen(LICENSE_FILE_PATH);
  struct timeval tv;
  ALOGI("%s: Entry", __func__);

  /* License file downloading */
  gettimeofday(&tv, NULL);
  bt_logger_->SetCurrentactivityStartTime(tv ,BT_HOST_REASON_LICENSE_READ_STUCK,
											"LICENSE PATCH OPEN & READ");

  if ((license_file_size = GetLicenseFile(LICENSE_FILE_PATH)) > 0) {
	gettimeofday(&tv, NULL);
	bt_logger_->CheckAndAddToDelayList(&tv);

	bt_logger_->UpdateLicPatchIdealDelay(license_file_size);
	bt_logger_->SetSecondaryCrashReason(BT_HOST_REASON_LICENSE_DNLD_STUCK);
	gettimeofday(&tv, NULL);
	bt_logger_->SetCurrentactivityStartTime(tv, BT_HOST_REASON_LICENSE_DNLD_STUCK,
											"LICENSE DOWNLOAD");
	err = LicenseDnldReq(license_file_size);
	gettimeofday(&tv, NULL);
	bt_logger_->CheckAndAddToDelayList(&tv);
...
int PatchDLManager::GetLicenseFile(const char *file_path)
{
  int err;
  FILE *pFile = NULL;
  int file_size, file_size_total, read_size;
  unsigned char file_len[3];

  ALOGI("%s: Entry", __func__);
  if (pFile == NULL) {
	if((pFile = OpenPatchFile(file_path, NULL)) == NULL) {
		ALOGE("%s: License file open failed.", __func__);
		return -1;
	}
  }
  ...
  /* Adding license file header size */
  file_size_total = file_size + LICENSE_FILE_HDR_SZ;

  pdata_buffer_ = (unsigned char*)new char[file_size_total];
  if (pdata_buffer_ == NULL) {
  ...
  /* Copy file into allocated buffer */
  read_size = fread(pdata_buffer_ + LICENSE_DATA_OFFSET, 1, file_size, pFile);

  /* File Close */
  fclose(pFile);

  if (read_size != file_size) {
	ALOGE("Read file size(%d) not matched with actual file size (%ld bytes)", read_size, file_size);
	delete []pdata_buffer_;
    return -1;
  }

  /* Copy the length of file */
  memcpy(file_len, &file_size, LICENSE_HDR_LEGNTH_BYTES);

  /* License file Header formation */
  if (FormLicenseHeader(file_len)) {
	ALOGE("%s: Memory allocation for license header failed.", __func__);
	return -1;
  }
  ALOGI("%s: Memory allocation for license header successful", __func__);

  lic_dwn_cfg_ = license_head->uid;

  if (memcpy(pdata_buffer_, license_head, LICENSE_FILE_HDR_SZ) != NULL) {
	return file_size_total;
  }
  ALOGE("%s: Failed: copying of license data into buffer.", __func__);
  return -1;
}

int PatchDLManager::LicenseDnldReq(int license_size)
{
  int total_segment, remain_size, i , err = -1;
  unsigned char wait_cc_evt = true;
  bool is_last_seg = false;
  ...
  total_segment = license_size / MAX_SIZE_PER_TLV_SEGMENT;
  remain_size = (license_size < MAX_SIZE_PER_TLV_SEGMENT) ? \
    license_size : (license_size % MAX_SIZE_PER_TLV_SEGMENT);

  ALOGI("%s: License File Size: %d, Total Seg num: %d, remain size: %d",
        __func__, license_size, total_segment, remain_size);

  /* Based on the license UID, the event handling mechanism will be decided
   * If UID == 0x86, mechanism changed to SKIP_EVT_VSE_SS,
   * If UID == 0x06, mechanism changed to SKIP_EVET_NONE.
   */
  switch (lic_dwn_cfg_)
  {
    case SKIP_LIC_EVT_NONE:
	wait_vsc_evt_ = true;
	wait_cc_evt = true;
	ALOGI("Event handling type: SKIP_LIC_EVET_NONE");
    break;
    case SKIP_LIC_EVT_CC:
    wait_vsc_evt_ = false;
	wait_cc_evt = false;
	ALOGI("Event handling type: SKIP_LIC_EVET_NONE");
    break;
    default:
	ALOGE("%s: Unsupported Event handling: %d", __func__, lic_dwn_cfg_);
  }
  ...
  for (i = 0; i <= total_segment && !is_last_seg; i++) {
	...
	patch_dnld_pending_ = true;
	if ((err = TlvDnldSegment(i, segment_download_len, wait_cc_evt)) < 0) {
		bt_logger_->SetInitFailureReason(BT_HOST_REASON_LICENSE_DOWNLOAD_FAILED);
		goto error;
	}
    patch_dnld_pending_ = false;
  }
...
int PatchDLManager::FormLicenseHeader(unsigned char *file_size)
{
  int err = -1;

  /* Allocate the memory for license header */
  license_head = (license_header*)new license_header;
  if (license_head == NULL) {
    ALOGE("%s: Memory allocation failed for license header.",
			__func__);
    return err;
  }

  license_head->uid = SKIP_LIC_EVT_NONE;
  if (memcpy(license_head->length, file_size, LICENSE_HDR_LEGNTH_BYTES) != NULL)
    return 0;
