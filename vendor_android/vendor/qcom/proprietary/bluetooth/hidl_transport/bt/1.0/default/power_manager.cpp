...
{
  int ret = -1;
  if (bt_soc_type_ == BT_SOC_THEMISTO) {
    ALOGE("SetPower is not handled for soc type Themisto. bailing out...");
    return ret;
  }
...
bool PowerManager :: SendSocVersion(unsigned int soc_id)
{
  ...
  ALOGE("bt soc version  (%d)\n", soc_id);

  err = ioctl(DataHandler::Get()->GetSpiDriverFd(),
...
int PowerManager :: GetAndLogPwrRsrcStates(struct power_buff_struct *power_buff) {
  ...
  int ret = ioctl(DataHandler::Get()->GetSpiDriverFd(), get_bt_pwr_srcs_state_cmd_, &pwr_state);
