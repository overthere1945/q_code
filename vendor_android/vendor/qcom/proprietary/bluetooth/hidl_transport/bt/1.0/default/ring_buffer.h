...
enum RxThreadEventType {
  ...
  RX_PRE_STACK_ISO_CALL_BACK = 6,
  RX_POST_STACK_ISO_CALL_BACK = 7,
  RX_PRE_STACK_SCO_CALL_BACK = 8,
  RX_POST_STACK_SCO_CALL_BACK = 9,
};
...
enum PrimaryReasonCode  {
  ...
  BT_HOST_REASON_PERI_SOC_CRASHED_ON_OTHER_SUB_SYSTEM = 0x3A,  //Peripheral core crash deteced in UWB SS
  BT_HOST_REASON_PERI_SUB_SYSTEM_FAILED_UPDATE_SSR_COMPLETE = 0x3B, // UWB FAILED TO UPDATE THE SSR COMPLETE STATUS
#endif
  BT_HOST_REASON_SOC_CALIBRATION_DATA_RECV_TIMEOUT = 0x3C,     // SOC CALIBRATION DATA RECV TIMEOUT
  BT_HOST_REASON_STORE_SOC_CALIBRATION_FAILED = 0x3D,          // STORE SOC CALIBRATION DATA FAILED
  BT_HOST_REASON_THERMAL_THROTTLE_IS_SEVERE = 0x3E             // HOST THERMAL THOTTLE IS SEVERE
};
...
{
 private:
  const static long int max_buff_size_ = 10 * 1024 * 1024;
...
class RingBuffer
{
 private:
  ...
  struct timeval time_pre_stack_iso_call_back;
  struct timeval time_post_stack_iso_call_back;
  struct timeval time_pre_stack_sco_call_back;
  struct timeval time_post_stack_sco_call_back;
