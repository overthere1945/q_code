...
#ifdef BT_VER_1_1
#define LOG_TAG "vendor.qti.bluetooth@1.1-logger"
#else
...
#define BT_NVM_HEADER_IDENTIFIER 0x02

//BYTE[0]
#define BT_VND_SPECIFIC_EVENT    0xFF
#define HCI_PERI_EVT_CODE        0xF0
...
#define PERI_DUMP_REASON_OFFSET           0x09
#define HCI_PERI_CRASH_DUMP_MEMDUMP       0x04
#define HCI_PERI_CRASH_DUMP_INFORMATION   0x05
#define DUMP_INFO_OFFSET 0x10000
#define PERI_DUMP_INFO_OFFSET  0x22C
#else
#define DUMP_INFO_OFFSET 0xFEE0
#endif

#define DIAG_SSR_BT_CMD     0x0007

#define LAST_SEQUENCE_NUM  0xFFFF
#define FIRST_SEQUENCE_NUM 0x0000
...
#ifdef IS_PERI_ENABLED
std::mutex Logger :: fwdownload_mtx;
#endif
std::mutex Logger :: cal_last_evnt_lock;
static unsigned short seq_num_cnt = 0;
static uint16_t total_cal_len = 0;
static bool dump_process_running = false;
char *SubSystems[] = {"Peri Subsytem", "BT SubSystem", "UWB SubSystem"};
Logger * Logger :: Get()
{
  if (!instance_) {
...
#ifdef IS_PERI_ENABLED
  peri_mini_dump_size_ = 0;
  memset(peri_mini_dump_buff_, 0, sizeof(peri_mini_dump_buff_));
  isPeriDumpSetCrashReason = false;
  fwdownload_state = DEFAULT_DWNLD_STATE;
#endif
  cal_fd = -1;
  cal_buf = nullptr;
  dump_crash_reason = BT_SOC_REASON_DEFAULT;
}
...
bool Logger :: ProcessRx(HciPacketType type, const uint8_t *buff, uint16_t len)
{
  ...
  switch (type) {
    ...
    {
      uint16_t handle = buff[0]|(buff[1]<<8);
      if (handle == CAL_ACL_HANDLE) {
#ifdef DUMP_RINGBUF_LOG
        snoop_buff_.AddLog(RingBuffer::SOC_TO_HOST, type, (uint8_t*)buff, len);
#endif
        SaveSocCalibrationData((uint8_t*)buff, len, DataHandler::Get()->getFarmeType());
      }
    }
    ...
    case HCI_PACKET_TYPE_PERI_EVT:
      ret =  HandlePeriEvent(type, (uint8_t*)buff, len);
      ...
      if (diag_interface_.GetDiagInitStatus() && IsSnoopLogEnabled()) {
        ...
        // need to check the LOG_PERI_HCI_EV_C
...
bool Logger :: ProcessTx(HciPacketType type, const uint8_t *buff, uint16_t len)
{
  ...
  BtState::Get()->SetLastTxPacket((uint8_t*)buff, len, type);
#endif
  ...
  if (diag_interface_.GetDiagInitStatus() && IsSnoopLogEnabled()) {
    ...
    } else if (type == HCI_PACKET_TYPE_ACL_DATA || type == HCI_PACKET_TYPE_ISO_DATA) {
      gettimeofday(&tv, NULL);
...
#ifdef IS_PERI_ENABLED
    } else if (type == HCI_PACKET_TYPE_PERI_CMD) {
      gettimeofday(&tv, NULL);
      BtState::Get()->AddLogTag(dst_buff, tv, (char *)"HCI TX packet going for diag logging");
      BtState::Get()->SetPreTsDiagTxHciCmd(dst_buff, tv);
      // need to check for peri
      diag_interface_.SendLogs(buff, len, LOG_BT_HCI_CMD_C);
      gettimeofday(&tv, NULL);
      BtState::Get()->AddLogTag(dst_buff, tv, (char *)"Diag logging done for HCI TX packet");
      BtState::Get()->SetPostTsDiagTxHciCmd(dst_buff, tv);
...
void Logger :: SetPrimaryCrashReasonAsDiag () {
  if (isDiagSsrTriggered()) {
#ifdef BT_GANGES_SUPPORTED
   if (diag_interface_.isDiagSsrTriggeredForPeri()) {
     dump_reason_ = BT_HOST_REASON_PERI_SOC_CRASHED_DIAG_SSR;
   } else {
...
#ifdef IS_PERI_ENABLED
void Logger :: DecodeHwErrorPeriEvt(uint8_t *buff)
{
   ...
   */
  SecondaryReasonCode reason = (SecondaryReasonCode)(buff[BT_HW_ERR_EVT_BYTE+1] | BT_SOC_ERR_MASK);
  ALOGE("Rx Peri VSE HW Dump Info event::%s\n", get_reset_reason_str(reason));
  peri_mini_dump_size_ = buff[DUMP_PKT_SIZE_OFFSET_PERI] - (MINI_DUMP_ARM_REG_SIZE_OFFSET_PERI -
                                    DUMP_PKT_SIZE_OFFSET_PERI - 1);

  // Update minidump data if arm reg. and call stack data are present.
  if (peri_mini_dump_size_ > 0) {
    memcpy(peri_mini_dump_buff_, buff + MINI_DUMP_ARM_REG_SIZE_OFFSET_PERI, peri_mini_dump_size_);
  } else {
    peri_mini_dump_size_ = 0;
  }
  ALOGD("%s mini_dump_size = %d", __func__, peri_mini_dump_size_);

  SetPrimaryCrashReasonAsDiag();
...
void Logger :: DecodeHwErrorEvt(uint8_t *buff)
{
  ...
  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
    SpiController * instance = (SpiController *)DataHandler::Get()->controller_;
    instance->setModulesInvolvedInSSR(SpiController::NON_CHAINED_BT);
  }
}

#ifdef IS_PERI_ENABLED
bool Logger :: HandlePeriEvent(HciPacketType type, uint8_t *buff, uint16_t len)
{
  bool ret = true;

  byte evtCode, evtClass, msgType, dumpReason, periHwErrorReason;

  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
    evtCode = buff[0];
    evtClass = buff[2];
    msgType = buff[3];
    periHwErrorReason = buff[4];
    dumpReason = buff[9];
  }else{
    evtCode = buff[1];
    evtClass = buff[3];
    msgType = buff[4];
    periHwErrorReason = buff[5];
    dumpReason = buff[10];
  }
  if(evtCode == BT_VND_SPECIFIC_EVENT) {
    if(evtClass == HCI_PERI_EVT_CODE) {
      switch(msgType) {
        case LOG_HCI_PERI_CRASH_DUMP_MEMDUMP:
        {
          SetPrimaryCrashReasonAsDiag();
          if (SetSsrTriggeredFlag()) {
            DataHandler * instance = DataHandler::Get();
            SetPrimaryCrashReason(BT_HOST_REASON_PERI_SOC_CRASHED);
            ...
            if (instance) {
              ...
              if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
                // Set Relevant Secondary Crash Reason
                //SetSecondaryCrashReason();
              }else{
          ...
          SaveSocPeriMemDump(buff, len, dump_reason_);
          break;
        }
        case LOG_HCI_PERI_CRASH_DUMP_INFORMATION:
        {
          if (dumpReason == BT_HW_ERR_EVT_VALID) {
            if (SetSsrTriggeredFlag()) {
              DataHandler * instance = DataHandler::Get();
              SetPrimaryCrashReason(BT_HOST_REASON_PERI_SOC_CRASHED);
              if (instance) {
          ...
          DecodeHwErrorPeriEvt(buff);
#ifdef DUMP_RINGBUF_LOG
          snoop_buff_.AddLog(RingBuffer::SOC_TO_HOST, type, buff, len);
#endif
        } else {
          ALOGE("Peri Rx HW error event with wrong payload");
        }
        break;
     }
     case LOG_HCI_PERI_HARDWARE_ERROR:
     {
       SecondaryReasonCode reason = (SecondaryReasonCode) periHwErrorReason;

       ALOGE("Rx Peri VSE HW error event::%s\n", get_reset_reason_str(reason));

       if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
        SpiController * instance = (SpiController *)DataHandler::Get()->controller_;
        instance->setModulesInvolvedInSSR(SpiController::CHAINED_PERI_AND_BT);
       }
       dump_process_running = false;

       if (crash_dump_status_cb_)
         crash_dump_status_cb_(dump_process_running);
...
// For Themisto, Zealis Debug Policy is Non-Chained by default.
// No need to close UWB HAL for Standalone BT Crash
#if 0
#ifdef  IS_PERI_ENABLED
          /* notify to Power Driver about BT SSR */
        NotifySignal *notify_signal_ = NotifySignal::Get();
...
// No need to close UWB HAL for Standalone BT Crash
      ...
      default:
        ...
        (buff[2] == VND_EVT_CLASS_VS_RADIO_CAL_NVM_DATA) &&
        (buff[3] == MSG_TYPE_VS_RADIO_CAL_NVM_DATA)) {
#ifdef DUMP_RINGBUF_LOG
    snoop_buff_.AddLog(RingBuffer::SOC_TO_HOST, type, buff, len);
#endif
    SaveSocCalibrationData(buff, len, DataHandler::Get()->getFarmeType());
...
#ifdef  IS_PERI_ENABLED
bool Logger::isValidCrashReason(uint32_t reason) {
  if (BT_SOC_REASON_UNKNOWN        == reason ||
      BT_SOC_REASON_SW_REQUESTED   == reason ||
      BT_SOC_REASON_STACK_OVERFLOW == reason ||
      BT_SOC_REASON_EXCEPTION      == reason ||
      BT_SOC_REASON_ASSERT         == reason ||
      BT_SOC_REASON_TRAP           == reason ||
      BT_SOC_REASON_OS_FATAL       == reason ||
      BT_SOC_REASON_HCI_RESET      == reason ||
      BT_SOC_REASON_PATCH_RESET    == reason ||
      BT_SOC_REASON_ABT            == reason ||
      BT_SOC_REASON_RAMMASK        == reason ||
      BT_SOC_REASON_PREBARK        == reason ||
      BT_SOC_REASON_BUSERROR       == reason ||
      BT_SOC_REASON_IO_FATAL       == reason ||
      BT_SOC_REASON_SSR_CMD        == reason ||
      BT_SOC_REASON_POWERON        == reason ||
      BT_SOC_REASON_WATCHDOG       == reason ||
      BT_SOC_REASON_RAMMASK_RGN1   == reason ||
      BT_SOC_REASON_RAMMASK_RGN0   == reason ||
      BT_SOC_REASON_Q6_WATCHDOG    == reason ||
      BT_SOC_REASON_ZEALIS_RAM_MASK_RGN0 ==  reason ||
      BT_SOC_REASON_ZEALIS_RAM_MASK_RGN1 ==  reason ||
      BT_SOC_REASON_APSS_RESET           ==  reason ||
      BT_SOC_REASON_TIME_RESET           ==  reason ||
      BT_SOC_REASON_AUDIOSS_RESET        ==  reason ||
      BT_SOC_REASON_HOST_WARMRESET       ==  reason ||
      BT_SOC_REASON_HOST_NMI_INIT        ==  reason ||
      BT_SOC_REASON_PANIC_FAULT          ==  reason ||
      BT_SOC_REASON_EARLY_TRAP           ==  reason ||
      BT_SOC_REASON_INSTR_ADDR_MISALIGN  ==  reason ||
      BT_SOC_REASON_INSTR_ACCESS_FAULT   ==  reason ||
      BT_SOC_REASON_ILLEGAL_INSTR        ==  reason ||
      BT_SOC_REASON_BREAKPOINT_EXCEPTION ==  reason ||
      BT_SOC_REASON_LOAD_ADDR_MISALIGN   ==  reason ||
      BT_SOC_REASON_LOAD_ACCESS_FAULT    ==  reason ||
      BT_SOC_REASON_STORE_ADDR_MISALIGN  ==  reason ||
      BT_SOC_REASON_STORE_ACCESS_FAULT   ==  reason ||
      BT_SOC_REASON_ECALL_UMODE          ==  reason ||
      BT_SOC_REASON_ECALL_MMODE          ==  reason ||
      BT_SOC_REASON_STACK_UNDERFLOW      ==  reason ||
      BT_SOC_REASON_MACHINE_EXT_INT      ==  reason ||
      BT_SOC_REASON_PERF_MONITOR_OVERFLOW==  reason ||
      BT_SOC_REASON_EXT_SUBSYS_RESET     ==  reason ||
      BT_SOC_REASON_IPC_STALL            ==  reason ||
      BT_SOC_REASON_PEER_CPU0_NMI        ==  reason ||
      BT_SOC_REASON_PEER_CPU1_NMI        ==  reason ||
      BT_SOC_REASON_PEER_CPU2_NMI        ==  reason ||
      BT_SOC_REASON_TX_RX_INVALID_PKT_FATAL == reason ||
      BT_SOC_REASON_TX_RX_INVALID_LEN_FATAL == reason ||
      BT_SOC_REASON_INVALID_STACK          ==  reason ||
      BT_SOC_REASON_INVALID_MCI_MSG_RCVD   ==  reason)
    return true;
  return false;
}
#endif
...
void Logger::FrameCrashEvent(hidl_vec<uint8_t>*bt_packet_)
{
  ...
  if (sec_crash_reason.reason == BT_SOC_REASON_DEFAULT &&
      ...
      timeptr = localtime(&time_rx_call_back);
      if (timeptr)
        snprintf(crash_ts, MAX_CRASH_BUFF_SIZE, "%s", asctime(timeptr));
  } else {
      timeptr = localtime(&sec_crash_reason.time_instance);
      if (timeptr)
        snprintf(crash_ts, MAX_CRASH_BUFF_SIZE, "%s", asctime(timeptr));
...
#ifdef  IS_PERI_ENABLED
  if ((pri_crash_reason == BT_HOST_REASON_SOC_CRASHED && GetUartErrCode() == UART_REASON_DEFAULT) ||
      (pri_crash_reason == BT_HOST_REASON_PERI_SOC_CRASHED && GetUartErrCode() == UART_REASON_DEFAULT) ||
      (pri_crash_reason == BT_HOST_REASON_PERI_SOC_CRASHED_DIAG_SSR) ||
      (pri_crash_reason == BT_HOST_REASON_SOC_CRASHED_DIAG_SSR)) {
#else
...
void Logger :: CalibrationCleanUpHandler(bool cal_ssr) {
  ...
  if (cal_ssr) {
    ...
    if (data_handler && data_handler->controller_ != nullptr) {
      ...
      instance->SsrCleanup(BT_HOST_REASON_STORE_SOC_CALIBRATION_FAILED);
    }
  }
...
void Logger :: SaveSocCalibrationData(uint8_t *eventBuf, uint16_t packet_len, bool isframeTypeIsAcl)
{
  ...
  if (isframeTypeIsAcl) {
    seq_num = eventBuf[6] | (eventBuf[7] << 8);

    // Removing the cal size stored byte from length count.
    packet_len -= 9;

    //Removing the header size
    dump_ptr = eventBuf + 9;
  } else {
  ...
  if ((seq_num != LAST_SEQUENCE_NUM) && (seq_num != seq_num_cnt)) {
    ALOGE("%s: Expected calibration event no = 0x%x but received = 0x%x", __func__, seq_num_cnt, seq_num);
    return;
  }

  if (seq_num == FIRST_SEQUENCE_NUM) {

    if (isframeTypeIsAcl) {
      if (eventBuf[13] != BT_NVM_HEADER_IDENTIFIER) {
        ALOGE("%s: Calibration data comming with wrong NVM formate", __func__);
        return;
      }

      // Updating the Calibration Data size
      dump_size = eventBuf[9] | (eventBuf[10] << 8) |
                 (eventBuf[11] << 16) | (eventBuf[12] << 24);
    } else {
      if (eventBuf[11] != BT_NVM_HEADER_IDENTIFIER) {
        ALOGE("%s: Calibration data comming with wrong NVM formate", __func__);
        return;
      }

      // Updating the Calibration Data size
      dump_size = eventBuf[7] | (eventBuf[8] << 8) |
                 (eventBuf[9] << 16) | (eventBuf[10] << 24);
	}
    ALOGI("%s: Calibration data size = %d bytes", __func__, dump_size);

    // Alocating Heap memory to store calibration data
    cal_buf = (char*)malloc(dump_size);

    if (cal_buf != NULL) {
      // Initialize the buffer to default value
      memset(cal_buf, 0, dump_size);
    } else {
      ALOGE("%s: Failed to creat heap memory for calibration data to log", __func__);
      SetSecondaryCrashReason(BT_HOST_REASON_FILE_OPEN_FAILED_TO_STORE_CAL_DATA);
      CalibrationCleanUpHandler(true);
      return;
    }

    // Removing the Calibration Data size.
    packet_len -= 4;

    if (isframeTypeIsAcl) {
      //Removing the Calibration Data size BYTES
      dump_ptr = eventBuf + 13;
    } else {
      //Removing the Calibration Data size BYTES
  ...
  if (packet_len && cal_buf != NULL) {
    // Copy the calibration data to buffer
    memcpy((cal_buf + total_cal_len), dump_ptr, packet_len);
    total_cal_len += packet_len;
  }

  if (isframeTypeIsAcl) {
    ALOGI("%s: Received ACL event (%d), length = %d. Total CAL data = %d",
          __func__, seq_num_cnt, packet_len, total_cal_len);
  } else {
    ALOGI("%s: Received HCI event (%d), length = %d. Total CAL data = %d",
          __func__, seq_num_cnt, packet_len, total_cal_len);
  }

  if (seq_num == LAST_SEQUENCE_NUM) {

    ALOGE("%s: Last calibration data event received ", __func__);

    DataHandler *data_handler = DataHandler::Get();
    if (data_handler && data_handler->controller_ != nullptr) {
      data_handler->controller_->SignalCalDataCollectFinish();
    } else {
      ALOGE("%s: data_handler or data_handler->controller_ is destroyed, not logging CAL data, returning from here", __func__);
      return;
    }

    if (total_cal_len != dump_size) {
      ALOGE("%s: Error in receive calibration data, received: %ld", __func__, total_cal_len);
      SetSecondaryCrashReason(BT_HOST_REASON_CALIBRATION_SIZE_MISMATCH);
      CalibrationCleanUpHandler(true);
      return;
    }

    // Opening the calibration file
    if ((cal_fd = open(BT_FW_CAL_DEFAULT_PATH, O_CREAT | O_SYNC | O_WRONLY,  S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)) < 0) {
      ALOGE("%s: File open (%s) failed: errno: %d", __func__, BT_FW_CAL_DEFAULT_PATH, errno);
      SetSecondaryCrashReason(BT_HOST_REASON_FAILED_TO_CREATE_HEAP_MEMORY_FOR_CAL);
      CalibrationCleanUpHandler(true);
      return;
    }

    // Dumping the calibration data Received
    bytes_written = write(cal_fd, cal_buf, dump_size);

    if (bytes_written != dump_size) {
      ALOGE("%s: Failed to write Calibration data to %s file, bytes_written: %ld (err = %s)",
            __func__,  BT_FW_CAL_DEFAULT_PATH, bytes_written, strerror(errno));
      SetSecondaryCrashReason(BT_HOST_REASON_CALIBRATION_SIZE_MISMATCH);
      CalibrationCleanUpHandler(true);
      return;
    } else {
      ALOGE("%s: Calibration data logged in %s file successfully\n", __func__, BT_FW_CAL_DEFAULT_PATH);
    }
    CalibrationCleanUpHandler(false);
  }
...
void Logger :: SaveSocMemDump(uint8_t *eventBuf, uint16_t packet_len, PrimaryReasonCode reason)
{
  ...
  packet_len -= 7;
  long bytes_written = -1;
  static BluetoothSocType soc_type = DataHandler::Get()->GetSocType();

  ALOGV("%s: LOG_BT_MESSAGE_TYPE_MEM_DUMP (%d) ", __func__, seq_num);

  if ((seq_num != seq_num_cnt) && seq_num != SOC_DUMP_LAST_PKT) {
  ...
  if (seq_num == SOC_DUMP_FIRST_PKT) {
    ...
    /* Get file path based on the soc type crash */
    GetCrashDumpFileName(path, BT_SS);
    ...
    ALOGI("%s: File Open (%s) successfully ", __func__, path);
  }

  if (dump_size < (total_size + packet_len)) {
    ALOGE("%s: Total dump size updated from first event is %d,but received dump data size is %d higher."
      " Discarding the upcoming dump event", __func__, dump_size, (total_size + packet_len));
    if (p != NULL) {
      bytes_written = write(dump_fd, p, total_size);
      if (bytes_written)
        ALOGE("%s: Written %d bytes into the %s file", __func__, bytes_written, path);
      else
        ALOGE("%s: Error writing crash dump: %s", __func__, strerror(errno));
      free(p);
      temp_buf = NULL;
      p = NULL;
      close(dump_fd);
  ...
  if (dump_fd >= 0) {
    for (; (seq_num > seq_num_cnt) && (seq_num != SOC_DUMP_LAST_PKT); seq_num_cnt++) {
  ...
  seq_num_cnt++;
  if (seq_num == SOC_DUMP_LAST_PKT && p != NULL) {
    char dst_buff[MAX_BUFF_SIZE] = {'\0'};
    ...
    if (dump_fd >= 0) {
      ...
      if (-1 != (dump_fd = open(path, O_RDONLY))) {
        if (NULL != (dumpinfo = (unsigned char*)malloc(SOC_DUMP_SIGNATURE_SIZE))) {
          tempptr = dumpinfo;
          lseek(dump_fd, DUMP_INFO_OFFSET, SEEK_SET);
...
#ifdef IS_PERI_ENABLED
          if (isValidCrashReason(uResetReason))
            uResetReason = GetSecondaryCrashReasonCode();
#endif
          if (0 != ucFilename) {
          ...
          ALOGE("PC Addr::0x%x\n", uPCAddr);
#ifdef IS_PERI_ENABLED
          if (isPeriDumpSetCrashReason == false) {
            pc_address_ = uPCAddr;
          }
#else
          pc_address_ = uPCAddr;
  ...
  // if (((0 == seq_num) || SOC_DUMP_LAST_PKT == seq_num) && crash_dump_status_cb_) {
  //   crash_dump_status_cb_(is_crash_dump_in_progress_);
  // }
}
#ifdef IS_PERI_ENABLED
void Logger :: SaveSocPeriMemDump(uint8_t *eventBuf, uint16_t packet_len, PrimaryReasonCode reason)
{
  static unsigned int dump_size = 0, total_size = 0;
  unsigned short seq_num = 0;
  static unsigned short seq_num_cnt = 0;
  ...
  long bytes_written = -1;
  static bool decode_dump = false;
  static BluetoothSocType soc_type = DataHandler::Get()->GetSocType();
  bool log_sub_system_dump = false;

  ALOGV("%s: PERI SOC MEM DUMP (%d) ", __func__, seq_num);
  ...
  if (seq_num == SOC_DUMP_FIRST_PKT) {
    ...
    dump_process_running = true;

    if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
      dump_size = (unsigned int)
                (eventBuf[7] | (eventBuf[8] << 8) |
                (eventBuf[9] << 16) | (eventBuf[10] << 24));
    }else{
      dump_size = (unsigned int)
                (eventBuf[8] | (eventBuf[9] << 8) |
                (eventBuf[10] << 16) | (eventBuf[11] << 24));
    }
    ...
    if (log_sub_system_dump) {
      ALOGE("LAST Sub Failed to allocate mem for the crash dump size: %d", dump_size);
    } else {
      log_sub_system_dump = true;
      is_crash_dump_in_progress_ = true; 
      if (crash_dump_status_cb_)
        crash_dump_status_cb_(log_sub_system_dump);
    }
    ...
    ALOGD("%s: %s Crash Dump Collection Start", __func__, SubSystems[ss_type]);

    /* Get file path based on the soc type crash */
    GetCrashDumpFileName(path, ss_type);
    dump_fd = open(path, O_CREAT | O_SYNC | O_WRONLY,  S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
    if (dump_fd < 0) {
      ALOGE("%s: File open (%s) failed: errno: %d", __func__, path, errno);
      seq_num_cnt++;
  ...
  if (seq_num == SOC_DUMP_LAST_PKT && p != NULL) {
    ...
    ALOGE("Writing crash dump of size %d bytes in to the files", total_size);
    gettimeofday(&tv, NULL);
    snprintf(dst_buff, sizeof(dst_buff), "Last sequence num %d rcvd from SOC", seq_num_cnt);
    BtState ::Get()->AddLogTag(seq_buff, tv, dst_buff);
    BtState ::Get()->SetLastSeqTS(seq_buff);
    ...
    if (dump_fd >= 0) {
      ...
      if (!decode_dump) {
        ...
        if (-1 != (dump_fd = open(path, O_RDONLY))) {
          if (NULL != (dumpinfo = (unsigned char*)malloc(SOC_DUMP_SIGNATURE_SIZE))) {
            tempptr = dumpinfo;
            lseek(dump_fd, PERI_DUMP_INFO_OFFSET, SEEK_SET);
            readsize = SOC_DUMP_SIGNATURE_SIZE;
            while (readsize) {
              cntr = read(dump_fd, (void*)tempptr, readsize);
              tempptr += cntr;
             ...
             */
            if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
              // Set Appropriate Secondary Crash Reason
            }else{
              if(host_crash_during_init == false && GetUartErrCode() == UART_REASON_DEFAULT) {
                SetSecondaryCrashReason((SecondaryReasonCode)uResetReason);
              }
...
void Logger :: GetCrashDumpFileName(char* path, uint8_t type)
{
  if (type == BT_SS)
    snprintf(path, SOC_DUMP_PATH_BUF_SIZE, SOC_DUMP_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);
  else if (type == UWB_SS)
    snprintf(path, SOC_DUMP_PATH_BUF_SIZE, UWB_SOC_DUMP_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);
  else
    snprintf(path, SOC_DUMP_PATH_BUF_SIZE, PERI_SOC_DUMP_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);

  dump_files.push_back(std::string(path));
}

#ifdef DUMP_IPC_LOG
void Logger :: GetTransportDumpFilename(short int uart_dump, char* path)
{
  switch (uart_dump){
  case IpcLogs::TX_LOG:
    if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
      snprintf(path, TRANSPORT_IPC_PATH_BUF_SIZE, SPI_IPC_TX_LOGS_DEST_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);
    }else{
      snprintf(path, TRANSPORT_IPC_PATH_BUF_SIZE, UART_IPC_TX_LOGS_DEST_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);
    }
    dump_files.push_back(std::string(path));
    break;
  case IpcLogs::RX_LOG:
    if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
      snprintf(path, TRANSPORT_IPC_PATH_BUF_SIZE, SPI_IPC_RX_LOGS_DEST_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);
    }else{
      snprintf(path, TRANSPORT_IPC_PATH_BUF_SIZE, UART_IPC_RX_LOGS_DEST_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);
    }
    dump_files.push_back(std::string(path));
    break;
  case IpcLogs::STATE_LOG:
    if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
      snprintf(path, TRANSPORT_IPC_PATH_BUF_SIZE, SPI_IPC_STATE_LOGS_DEST_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);
    }else{
      snprintf(path, TRANSPORT_IPC_PATH_BUF_SIZE, UART_IPC_STATE_LOGS_DEST_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);
    }
    dump_files.push_back(std::string(path));
    break;
  case IpcLogs::POWER_LOG:
    if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
      snprintf(path, TRANSPORT_IPC_PATH_BUF_SIZE, SPI_IPC_PWR_LOGS_DEST_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);
    }else{
      snprintf(path, TRANSPORT_IPC_PATH_BUF_SIZE, UART_IPC_PWR_LOGS_DEST_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);
    }
    dump_files.push_back(std::string(path));
    break;
  }
...
void Logger::GetTransportLogFilename(char* path) {
  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
    snprintf(path, TRANSPORT_LOG_PATH_BUF_SIZE, SPI_LOGS_DEST_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);
  }else{
    snprintf(path, TRANSPORT_LOG_PATH_BUF_SIZE, UART_LOGS_DEST_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);
  }
  dump_files.push_back(std::string(path));
}
...
void Logger :: DeleteDumpsIfRequired()
{
  ...
  char *ipc_look_for;
  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
    ipc_look_for = (char *)"spi_ipc_rx";
  }else{
    ipc_look_for = (char *)"uart_ipc_rx";
  }
#endif
  ...
  // delete UART logs
  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
    look_for = (char *)"spi_ftrace";
  }else{
    look_for = (char *)"uart_ftrace";
  }
  count = 0;
  while ((p_dirent = readdir(p_dir)) != NULL) {
    if (strstr(p_dirent->d_name, look_for)) {
  ...
  count = 0;
  while ((p_dirent = readdir(p_dir)) != NULL) {
    if (strstr(p_dirent->d_name, ipc_look_for)) {
      count++;
      if(count == 1) {
        first_entry = p_dirent->d_name;
      }
...
void Logger :: DeleteDumps(char *first_entry)
{
  ...
  char *look_for = (char *)"fw_crashdump";
  char *uart_look_for = (char *)"uart_ftrace";

  char timestamp[HAL_DUMP_TIMESTAMP_LENGTH];
  char path[HAL_DUMP_SIZE];
  char oldest_file[HAL_DUMP_FILE_SIZE];

  char uart_timestamp[HAL_DUMP_TIMESTAMP_LENGTH];
  char uart_oldest_file[HAL_DUMP_FILE_SIZE];

#ifdef DUMP_IPC_LOG
  char ipc_oldest_file[HAL_DUMP_FILE_SIZE];
  char ipc_timestamp[HAL_DUMP_TIMESTAMP_LENGTH];
  char *ipc_look_for = (char *)"uart_ipc_rx";
  strlcpy(ipc_oldest_file, first_entry, HAL_DUMP_FILE_SIZE);
  ...
  while ((p_dirent = readdir(p_dir)) != NULL) {
    if (strstr(p_dirent->d_name, ipc_look_for) ) {
      if(strcmp(p_dirent->d_name, ipc_oldest_file) < 0) {
        strlcpy(ipc_oldest_file, p_dirent->d_name, HAL_DUMP_FILE_SIZE);
      }
    }
  }
  ...
  strlcpy(uart_oldest_file, first_entry, HAL_DUMP_FILE_SIZE);
  ...
  while ((p_dirent = readdir(p_dir)) != NULL) {
    if (strstr(p_dirent->d_name, uart_look_for) ) {
      if(strcmp(p_dirent->d_name, uart_oldest_file) < 0) {
        strlcpy(uart_oldest_file, p_dirent->d_name, HAL_DUMP_FILE_SIZE);
      }
    }
  }
  closedir(p_dir);
...
#ifdef DUMP_IPC_LOG
  strlcpy(ipc_timestamp, ipc_oldest_file + HAL_DUMP_TIMESTAMP_OFFSET, HAL_DUMP_TIMESTAMP_LENGTH);
  p_dir = opendir(LOG_COLLECTION_DIR);
  if (p_dir == NULL) {
    ALOGE("%s: Cannot open dump location %s", __func__, LOG_COLLECTION_DIR);
    return;
  ...
  while ((p_dirent = readdir(p_dir)) != NULL) {
   if (strstr(p_dirent->d_name, ipc_timestamp)) {
  ...
  strlcpy(uart_timestamp, uart_oldest_file + HAL_DUMP_TIMESTAMP_OFFSET + 1, HAL_DUMP_TIMESTAMP_LENGTH);
  ...
  while ((p_dirent = readdir(p_dir)) != NULL) {
   if (strstr(p_dirent->d_name, uart_timestamp)) {
     strlcpy(path, LOG_COLLECTION_DIR, HAL_DUMP_SIZE);
     strlcat(path, p_dirent->d_name, HAL_DUMP_SIZE);
     ALOGE("%s: Deleting oldest dump file: %s", __func__, path);
     ret = remove(path);
...
void Logger::DumpLogsInBugreport(int fd)
{
  ...
  current_file = LastestLogFileName(UART_LOG_PREFIX, LOG_COLLECTION_DIR);
  ALOGD("%s: log file returned %s", __func__, current_file.name.c_str());
  if (current_file.name.length() != 0) {
    if (isLastestLogFileName(current_file, latest_file)) {
      latest_file = current_file;
...
#ifdef IS_PERI_ENABLED
  current_file = LastestLogFileName(PERI_SOC_DUMP_PREFIX, LOG_COLLECTION_DIR);
  ALOGD("%s: log file returned %s", __func__, current_file.name.c_str());
  if (current_file.name.length() != 0) {
    if (isLastestLogFileName(current_file, latest_file)) {
      latest_file = current_file;
  ...
  WriteToFd(fd, latest_file.ts, UART_LOG_PREFIX, LOG_COLLECTION_DIR, LOGS_EXTN);
  WriteToFd(fd, latest_file.ts, SOC_DUMP_PREFIX, LOG_COLLECTION_DIR, FW_DUMP_EXTN);
#ifdef IS_PERI_ENABLED
  WriteToFd(fd, latest_file.ts, PERI_SOC_DUMP_PREFIX, LOG_COLLECTION_DIR, FW_DUMP_EXTN);
#endif
}
...
void Logger::CollectDumps(bool is_transport_ipc_enabled, bool is_state_log_enabled) {
  std::unique_lock<std::recursive_mutex> guard(bugreport_mutex);
  int dump_transport_logs = DUMP_FTRACE_AND_IPC_LOGS;

  char value[PROPERTY_VALUE_MAX] = "false";
  property_get("persist.vendor.service.bdroid.dump_uartlogs", value, "3");
  dump_transport_logs = atoi(value);
  {
    DataHandler *data_handler = DataHandler::Get();
    if (is_transport_ipc_enabled && data_handler
        && data_handler->GetSocType() != BT_SOC_SMD) {
#ifdef DUMP_IPC_LOG
      if (dump_transport_logs == DUMP_IPC_LOGS || dump_transport_logs == DUMP_FTRACE_AND_IPC_LOGS)
        DumpTransportIpcLogs();
#endif
      if (dump_transport_logs == DUMP_FTRACE_LOGS || dump_transport_logs == DUMP_FTRACE_AND_IPC_LOGS)
        DumpTransportLogs();
    }
  }
...
#ifdef DUMP_RINGBUF_LOG
  {
    ...
    char value[PROPERTY_VALUE_MAX] = "false";
    if (!is_transport_ipc_enabled)
      PropertyGet("persist.vendor.service.bdroid.dump_ringbuff", value, "false");
    if (is_transport_ipc_enabled || (strcmp(value, "true") == 0))
      DumpRingBuffers();
  }
#endif
...
#ifdef IS_PERI_ENABLED
void Logger :: UpdateLicPatchIdealDelay(int size) {
	ideal_time_mapper_[BT_HOST_REASON_LICENSE_DNLD_STUCK] = size/NVM_DOWNLOAD_RATE + MAX_PATCH_DOWNLOAD_VARIABLE_DELAY;
}
#endif
...
#ifdef IS_PERI_ENABLED
    ideal_time_mapper_[BT_HOST_REASON_PERI_NVM_DNLD_STUCK] = size/NVM_DOWNLOAD_RATE + MAX_PATCH_DOWNLOAD_VARIABLE_DELAY;
#endif
}
...
#ifdef IS_PERI_ENABLED
      ideal_time_mapper_[BT_HOST_REASON_PERI_PATCH_DNLD_STUCK] = size/RAM_PATCH_DOWNLOAD_RATE;
#endif
    } else {
      ideal_time_mapper_[BT_HOST_REASON_PATCH_DNLD_STUCK] = size/RAM_PATCH_DOWNLOAD_RATE_WITH_CC;
#ifdef IS_PERI_ENABLED
      ideal_time_mapper_[BT_HOST_REASON_PERI_PATCH_DNLD_STUCK] = size/RAM_PATCH_DOWNLOAD_RATE_WITH_CC;
#endif
    }
    ideal_time_mapper_[BT_HOST_REASON_PATCH_DNLD_STUCK] += MAX_PATCH_DOWNLOAD_VARIABLE_DELAY;
#ifdef IS_PERI_ENABLED
    ideal_time_mapper_[BT_HOST_REASON_PERI_PATCH_DNLD_STUCK] += MAX_PATCH_DOWNLOAD_VARIABLE_DELAY;
#endif
}
...
#ifdef IS_PERI_ENABLED
      ideal_time_mapper_[BT_HOST_REASON_PERI_PATCH_DNLD_STUCK] = size/ELF_PATCH_SEQUENTIAL_DOWNLOAD_RATE;
#endif
    } else {
      ideal_time_mapper_[BT_HOST_REASON_PATCH_DNLD_STUCK] = size/ELF_PATCH_SEQUENTIAL_DOWNLOAD_RATE_CC;
#ifdef IS_PERI_ENABLED
      ideal_time_mapper_[BT_HOST_REASON_PERI_PATCH_DNLD_STUCK] = size/ELF_PATCH_SEQUENTIAL_DOWNLOAD_RATE_CC;
#endif
    }
    ideal_time_mapper_[BT_HOST_REASON_PATCH_DNLD_STUCK] += MAX_PATCH_DOWNLOAD_VARIABLE_DELAY;
#ifdef IS_PERI_ENABLED
...
bool Logger :: CheckActivityCodeForCrashCmd(SecondaryReasonCode activity_code) {
  ...
  if (activity_code != BT_HOST_REASON_FILE_SYSTEM_CALL_STUCK &&
      ...
      activity_code != BT_HOST_REASON_XPAN_THREAD_START_STUCK &&
#endif
...
#ifdef IS_PERI_ENABLED
      activity_code != BT_HOST_REASON_PERI_RAM_PATCH_READ_STUCK &&
      activity_code != BT_HOST_REASON_PERI_NVM_PATCH_READ_STUCK &&
      activity_code != BT_HOST_REASON_PERI_POWER_IOCTL_STUCK &&
      activity_code != BT_HOST_REASON_PERI_FILE_SYSTEM_CALL_STUCK &&
#endif
...
void Logger :: CheckDelayListAndSetCrashReason() {
    ...
    if (sec_crash_reason.reason == BT_HOST_REASON_SOC_NAME_UNKOWN) {
      //Ignore UART err code in this case as it can be false positive.
      uart_err_code_ = UART_REASON_DEFAULT;
    } else if (uart_err_code_ != UART_REASON_DEFAULT) {
      /* Setting appropriate secondary crash reason */
...
void Logger :: InitialIdealTimeTable() {
  ...
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                            (BT_HOST_REASON_SOC_NAME_UNKOWN, FILE_SYSTEM_CALL_IDEAL_TIME_DELAY));
#ifdef IS_PERI_ENABLED
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                            (BT_HOST_REASON_PERI_RAM_PATCH_READ_STUCK, FILE_SYSTEM_CALL_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                            (BT_HOST_REASON_PERI_NVM_PATCH_READ_STUCK, FILE_SYSTEM_CALL_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                            (BT_HOST_REASON_PERI_PATCH_DNLD_STUCK, DEFAULT_RAM_PATCH_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                            (BT_HOST_REASON_PERI_NVM_DNLD_STUCK, DEFAULT_NVM_PATCH_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                            (BT_HOST_REASON_PERI_GETVER_NO_RSP_RCVD, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                            (BT_HOST_REASON_PERI_GETBOARDID_CMD_STUCK, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                            (BT_HOST_REASON_PERI_GETBLDINFO_CMD_STUCK, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                            (BT_HOST_REASON_PERI_RESET_STUCK, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                            (BT_HOST_REASON_PERI_PATCH_CONFIG_CMD_STUCK, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                            (BT_HOST_REASON_PERI_FILE_SYSTEM_CALL_STUCK, FILE_SYSTEM_CALL_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                            (BT_HOST_REASON_PERI_POWER_IOCTL_STUCK, POWER_IOCTL_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_PERI_ARBITRATION_CMD_STUCK, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_PERI_ARBITRATION_NTF_STUCK, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_BT_ACTIVATE_NTF_STUCK, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_PERI_GETVER_SEND_STUCK, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_PERI_RESET_CMD_FAILED, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_BT_ACTIVATE_CMD_STUCK, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_BT_ACTIVATE_CMD_FAILED, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_PERI_GETVER_CMD_FAILED, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_PERI_PATCH_CONFIG_FAILED, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_PERI_TLV_DOWNLOAD_FAILED, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_PERI_NVM_FILE_NOT_FOUND, FILE_SYSTEM_CALL_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_PERI_GETBLDINFO_CMD_FAILED, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_PERI_ARB_NOTIFY_FAILED, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_PERI_SETBAUDRATE_CMD_STUCK, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_PERI_SETBAUD_CMD_FAILED, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_MEMORY_ALLOCATION_FAILED_PERI, FILE_SYSTEM_CALL_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_HCI_ACTIVATE_CC_NOT_RCVD, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_PERI_ACCESS_STUCK, POWER_ON_REGS_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (PERI_SOC_REASON_DEFAULT, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_LICENSE_READ_STUCK, FILE_SYSTEM_CALL_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_LICENSE_DNLD_STUCK, DEFAULT_NVM_PATCH_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_LICENSE_DOWNLOAD_FAILED, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_TME_ARBITRATION_CMD_STUCK, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_TME_GETVER_CMD_FAILED, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_TME_GETVER_SEND_STUCK, CMD_RSP_IDEAL_TIME_DELAY));
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                             (BT_HOST_REASON_TME_FILE_SYSTEM_CALL_STUCK, FILE_SYSTEM_CALL_IDEAL_TIME_DELAY));
#endif
#ifdef XPAN_SUPPORTED
  ideal_time_mapper_.insert(std::make_pair<SecondaryReasonCode, int>
                            (BT_HOST_REASON_XPAN_THREAD_START_STUCK, RX_THREAD_START_IDEAL_TIME_DELAY));
#endif
}
...
#ifdef IS_PERI_ENABLED
void Logger:: SetFwDownloadState(enum FwDownlodState state)
{
  std::lock_guard<std::mutex> lock(fwdownload_mtx);
  fwdownload_state = state;
}

enum FwDownlodState Logger:: GetFwDownloadState(void)
{
  std::lock_guard<std::mutex> lock(fwdownload_mtx);
  return fwdownload_state;
}
#endif
} // namespace implementation
} // namespace V1_0
