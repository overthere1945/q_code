...
#ifdef BT_VER_1_1
#define LOG_TAG "vendor.qti.bluetooth@1.1-async_fd_watcher"
#else
...
void AsyncFdWatcher::ThreadRoutine()
{
  int ret;
  pid_t tid;
  struct sched_param rt_params,ret_rt_params;

  tid = gettid();
  sched_getparam(tid, &ret_rt_params);
  ALOGD("%s: default priority:%d ", __func__,ret_rt_params.sched_priority);
  rt_params.sched_priority = sched_get_priority_max(SCHED_FIFO);
  ALOGD("%s: max priority:%d & min priority %d for SCHED_FIFO policy for Rx. Setting Rx prio to :%d\n",
  __func__, sched_get_priority_max(SCHED_FIFO), sched_get_priority_min(SCHED_FIFO), rt_params.sched_priority);
  // Setting RX thread scheduling policy to Real time policy
  ret = sched_setscheduler(tid, SCHED_FIFO, &rt_params);
  if (ret != 0) {
    ALOGE("%s unable to set SCHED_FIFO policy %d for tid %d, error %s",
          __func__, sched_get_priority_max(SCHED_FIFO), tid, strerror(errno));
  } else {
    ALOGD("%s: Successfully set SCHED_FIFO policy for Rx ", __func__);
  }
  sched_getparam(tid, &ret_rt_params);
  ALOGD("%s: final priority:%d ", __func__,ret_rt_params.sched_priority);
