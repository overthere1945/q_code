...
class UartController : public Controller {
 public:
  ...
  virtual CoPVerSupported GetCoPVersion() override;
#endif
  ...
  UartController(BluetoothSocType soc_type, BluetoothSecSocType sec_soc_type);
  static void SocCrashWaitTimeout(union sigval sig);
  virtual void WaitforCrashdumpFinish();
  static void calDataLogTimeOut(union sigval sig);
