...
**  Constants & Macros
******************************************************************************/
#define ACL_MAX_CMD_SIZE                     1019

#define HCI_MAX_CMD_SIZE                     260
#define HCI_MAX_EVENT_SIZE                   260
#define HCI_MAX_OTP_EVENT_SIZE               267
...
#define ENH_LOG_ENABLE_CMD_LEN               6
#define EXT_ENH_LOG_ENABLE_CMD_LEN           10
...
// HCI_VS_PERSIST_ACCESS_OTP_GET config type
#define HCI_VS_PERSIST_ACCESS_OTP_GET_CONFIG_TYPE 0x0A
#define OTP_GET_CMD_CONFIG_TYPE_OFFSET       (8)
#define OTP_GET_CMD_DATA_LEN_OFFSET          (9)
#define OTP_GET_CMD_DATA_OFFSET              (10)
...
#define MAX_SIZE_PER_TLV_SEGMENT_FOR_ACL    (1014)
/* Unified VSC offset*/
...
#define HCI_VS_PERSIST_ACCESS_COMMAND        (0x09)

/* VS Commands */
#define VSC_SET_BAUDRATE_REQ_LEN             (1)
#define EDL_PATCH_CMD_LEN                    (1)
#define EDL_PATCH_CONFIG_CMD_LEN             (5)
#define EDL_PATCH_CMD_REQ_LEN                (1)
#define EDL_SECURE_BRIDGE_CMD_LEN            (1)
#define HCI_VS_PERSIST_ACCESS_CMD_LEN        (2)
...
#define EDL_SECURE_BRIDGE_CMD                (0x29)
#define HCI_VSC_GET_SCRAMBLING_FREQS         (0x11)
#define EDL_TLV_PATCH_CONFIG                 (0x28)
#define HCI_VS_PERSIST_ACCESS_OTP_GET        (0x00)
#define EDL_GETAPPVER_REQ                    (0x06)
...
#define HCI_VS_HOST_LOG_ENH_ENABLE_EXT_SUBID (0x17)
#define HCI_VS_HOST_LOG_CTRL_SUBID           (0x00)
#define HCI_VS_HOST_SUB_GET_COP_VER          (0x19)

#ifdef IS_PERI_ENABLED
/* Event class */
#define HCI_VS_DOWNLOAD_EVENT_CLASS                (0xF0)
...
#define HCI_VS_SENSOR_NOTIFICATION                 (0x50)
...
/* Error Status Codes for TLV_DOWNLOAD_COMMAND */
#define LENGTH_ERROR                     (0x1)
#define VERSION_ERROR                    (0x2)
#define SIGNATURE_ERROR                  (0x3)
#define NO_MEMORY_ERROR                  (0x4)
#define NVM_TAG_ERROR                    (0x5)
#define NVM_LENGTH_ERROR                 (0x6)
#define BT_MODE_ERRO                     (0x7)
#define UNKOWN_ERROR                     (0xE)
#define TYPE_ERROR                       (0x10)
#define IMAGE_TYPE_ERROR                 (0x11)
#define CRC_ERROR                        (0x13)
#define ID_ERROR                         (0x14)
#define WRONG_VER_ERROR                  (0x15)
#define SMALL_VER_ERROR                  (0x16)
#define REGION_ERROR                     (0x17)
#define NO_PATCH_ERROR                   (0x18)
#define WRONG_ANTI_ROLLBACK_ERROR        (0x19)
#define OTP_WR_ERROR                     (0x20)
#define INVALID_PATCH_PER_OTP_ERROR      (0x21)
#define INVALID_PATCH_HEADER_ERROR       (0x22)
#define INVALID_RSA_KEY_ERROR            (0x23)
#define INVALID_RSA_SIGNATURE_ERROR      (0x24)
#define INVALID_SERIAL_NUMBER_ERROR      (0x25)
...
#define GETAPPVER_REQ_INVALID_CMD            (0x10)
...
#define TLV_TYPE_PATCH_XMEM                  (5)
#define ELF_TYPE_PATCH                       (6)
...
/* 155 : sensor uart configs */
#define TAG_NUM_155                          155

/* 279 : FMD config */
#define TAG_NUM_279                          279
...
// ACL data Handle
#define ACL_DATA_HANDLE                      (0x0EDD)
#define HCI_VS_DOWNLOAD_COMMAND              (0xFC00)
...
#define BT_FW_XMEM_DEFAULT_PATH                 "/data/vendor/bluetooth/"
#define BT_FW_CAL_DEFAULT_PATH                  "/data/vendor/bluetooth/calibration.bin"
...
#ifdef IS_PERI_ENABLED
/* License file path */
#define LICENSE_FILE_PATH		"/vendor/bt_firmware/image/cnss_softsku_peach.pfm"
#define LICENSE_FILE_HDR_SZ		4
#define LICENSE_HDR_LEGNTH_BYTES 3
#define LICENSE_DATA_OFFSET		4

/* This header value in license file decides event handling mechanism in the HOST */
#define SKIP_LIC_EVT_NONE     0x06
#define SKIP_LIC_EVT_CC       0x86

/* License File Header */
typedef struct {
  unsigned char uid;
  unsigned char length[3];
} __attribute__ ((packed)) license_header;
...
#define ELF_FLAG     0x7f

#define FILE_PATH_LEN (64)

#define HCI_ADDON_CMD_CALIBRATION_BYTE   15
#define HCI_ADDON_CMD_CALIBRATION_BIT  0x40
#define PATCH_THROUGH_ACL    0x00
#define PATCH_THROUGH_CMD    0x01
...
#define QCA_BT_VER(s, p, b) (((uint64_t)(s) << 32) | ((uint64_t)(p & 0xffff) << 16) \
                            | ((uint64_t)(b & 0xffff)))
#define QCA_BT_VER_NO_SOC_ID(p, b) (((uint64_t)(p & 0xffff) << 16) \
                            | ((uint64_t)(b & 0xffff)))
#define QCA_CHIPSET_VER_NO_SOC_ID(a) ((uint64_t)(a) & ~((uint64_t)0xFFFFFFFF << 32))
#define QCA_VER_PROD_ID(a) (((uint64_t)(a) & (0xFF << 16)) >> 16)
#ifdef USER_DEBUG
#define BUILD_LABEL_LENGTH 255
#endif
...
#ifdef USER_DEBUG
typedef struct {
  uint64_t chipset_ver;
  unsigned short patchversion;
  char build_label[BUILD_LABEL_LENGTH];
} version_info;
#endif
...
enum {
  ...
  PROD_ID_HAMILTON = 0x19,
  PROD_ID_GANGES = 0x1e,
  PROD_ID_BRAHMA = 0x21,
  PROD_ID_ORNE_BT = 0x20,
  PROD_ID_BT_SOC_EVROS = 0x1D,
  PROD_ID_BT_SOC_COLOGNE = 0x20,
  PROD_ID_THEMISTO = 0x23,
};
...
enum {
    ...
    COMANCHE_BUILD_VER_0130   = 0X0103,
};
...
enum {
    ...
    COMANCHE_VER_1_2_UMC   = QCA_BT_VER(QCA_COMANCHE_SOC_ID_0120, PROD_ID_COMANCHE, COMANCHE_BUILD_VER_0120),
    COMANCHE_VER_1_2       = QCA_BT_VER(QCA_COMANCHE_SOC_ID_5120, PROD_ID_COMANCHE, COMANCHE_BUILD_VER_0120),
    COMANCHE_VER_1_3_UMC   = QCA_BT_VER(QCA_COMANCHE_SOC_ID_0130, PROD_ID_COMANCHE, COMANCHE_BUILD_VER_0130),
    COMANCHE_VER_1_3_TSMC  = QCA_BT_VER(QCA_COMANCHE_SOC_ID_4130, PROD_ID_COMANCHE, COMANCHE_BUILD_VER_0130),
    COMANCHE_VER_1_3       = QCA_BT_VER(QCA_COMANCHE_SOC_ID_5130, PROD_ID_COMANCHE, COMANCHE_BUILD_VER_0130),
};
...
// EVROS BT build details
enum {
  EVROS_BT_BUILD_VER_0100 = 0x0100
};

enum {
  EVROS_BT_BUILD_VER_0200 = 0x0200
};

enum {
 EVROS_BT_VER_1  = QCA_BT_VER_NO_SOC_ID(PROD_ID_BT_SOC_EVROS, EVROS_BT_BUILD_VER_0100),
 EVROS_BT_VER_2  = QCA_BT_VER_NO_SOC_ID(PROD_ID_BT_SOC_EVROS, EVROS_BT_BUILD_VER_0200),
};
...
enum {
  ...
  MOSELLE_BUILD_VER_0110 = 0x0101,
  MOSELLE_BUILD_VER_0120 = 0x0102,
};
...
enum {
  ...
  MOSELLE_VER_1_1   = QCA_BT_VER(QCA_MOSELLE_SOC_ID_0110, PROD_ID_MOSELLE, MOSELLE_BUILD_VER_0110),
  MOSELLE_VER_1_2   = QCA_BT_VER(QCA_MOSELLE_SOC_ID_0120, PROD_ID_MOSELLE, MOSELLE_BUILD_VER_0120),
};
...
enum {
  ...
  HSP_VER_2_0_G = QCA_BT_VER(QCA_HSP_SOC_ID_1201, PROD_ID_HSP, HSP_BUILD_VER_0200),
  HSP_VER_2_1   = QCA_BT_VER(QCA_HSP_SOC_ID_0210, PROD_ID_HSP, HSP_BUILD_VER_0201),
  HSP_VER_2_1_G = QCA_BT_VER(QCA_HSP_SOC_ID_1211, PROD_ID_HSP, HSP_BUILD_VER_0201),
};
...
enum {
  HAMILTON_BUILD_VER_0100 = 0x0100,
  HAMILTON_BUILD_VER_0200 = 0x0200,
};
...
enum {
  HAMILTON_VER_1_0   = QCA_BT_VER(QCA_HAMILTON_SOC_ID_0100, PROD_ID_HAMILTON, HAMILTON_BUILD_VER_0100),
  HAMILTON_VER_1_0_1   = QCA_BT_VER(QCA_HAMILTON_SOC_ID_0101, PROD_ID_HAMILTON, HAMILTON_BUILD_VER_0100),
  HAMILTON_VER_2_0   = QCA_BT_VER(QCA_HAMILTON_SOC_ID_0200, PROD_ID_HAMILTON, HAMILTON_BUILD_VER_0200),
};

enum {
  GANGES_BUILD_VER_0100 = 0x0100,
  GANGES_BUILD_VER_0200 = 0x0200
};

enum {
  GANGES_SOC_ID_0100 = 0x40210100,
  GANGES_SOC_ID_0200 = 0x40210200
};

enum {
  GANGES_VER_1_0   = QCA_BT_VER(GANGES_SOC_ID_0100, PROD_ID_GANGES, GANGES_BUILD_VER_0100),
  GANGES_VER_2_0   = QCA_BT_VER(GANGES_SOC_ID_0200, PROD_ID_GANGES, GANGES_BUILD_VER_0200)
};

enum {
  BRAHMA_BUILD_VER_0100 = 0x0100,
  BRAHMA_BUILD_VER_0200 = 0x0200
};

enum {
  BRAHMA_SOC_ID_0100 = 0x40210100,
  BRAHMA_SOC_ID_0200 = 0x40210200
};

enum {
  BRAHMA_VER_1_0   = QCA_BT_VER(BRAHMA_SOC_ID_0100, PROD_ID_BRAHMA, BRAHMA_BUILD_VER_0100),
  BRAHMA_VER_2_0   = QCA_BT_VER(BRAHMA_SOC_ID_0200, PROD_ID_BRAHMA, BRAHMA_BUILD_VER_0200)
};

// Orne BT build details
enum {
  ORNE_BT_BUILD_VER_0100 = 0x0100
};

enum {
  QCA_ORNE_BT_SOC_ID_0100 = 0x40262100
};

enum {
  ORNE_BT_VER_1_0   = QCA_BT_VER(QCA_ORNE_BT_SOC_ID_0100, PROD_ID_ORNE_BT, ORNE_BT_BUILD_VER_0100),
};

//Cologne BT build details
enum {
  COLOGNE_BT_BUILD_VER_0100 = 0x0101
};

enum {
  QCA_COLOGNE_BT_SOC_ID_0100 = 0x40292100  //need to change not available in confluence
};

enum {
  COLOGNE_BT_VER_1_0 = QCA_BT_VER(QCA_COLOGNE_BT_SOC_ID_0100, PROD_ID_BT_SOC_COLOGNE, COLOGNE_BT_BUILD_VER_0100),
};

enum {
  THEMISTO_BUILD_VER_0100 = 0x0100,
  THEMISTO_BUILD_VER_0200 = 0x0200
};

enum {
  THEMISTO_SOC_ID_0100 = 0x402c0100,
  THEMISTO_SOC_ID_0200 = 0x402c0200
};

enum {
  THEMISTO_VER_1_0   = QCA_BT_VER(THEMISTO_SOC_ID_0100, PROD_ID_THEMISTO, THEMISTO_BUILD_VER_0100),
  THEMISTO_VER_2_0   = QCA_BT_VER(THEMISTO_SOC_ID_0200, PROD_ID_THEMISTO, THEMISTO_BUILD_VER_0200)
};
...
class PatchPathManager {
 ...
 public:
  ...
  virtual const std::string GetPatchType() const {
    return "TLV";
  }

  virtual ~PatchPathManager() {}

};

class XmemPatchPathManager: public PatchPathManager {
  private:
    std::string xmem_tlv_name_;
    std::string xmem_nvm_name_;

  public:
    XmemPatchPathManager(std::string chip_name, std::string tlv_name, std::string nvm_name,
                          std::string xmem_tlv_name, std::string xmem_nvm_name) :
                            PatchPathManager(chip_name, tlv_name, nvm_name),
                              xmem_tlv_name_(xmem_tlv_name), xmem_nvm_name_(xmem_nvm_name){}

  const std::string& GetXmemTLVName() const {
    return xmem_tlv_name_;
  }

  const std::string& GetXmemNVMName() const {
    return xmem_nvm_name_;
  }

  const std::string GetPatchType() const {
    return "XMEM";
  }

};

class ELFPatchPathManager: public PatchPathManager {
  private:
    std::string alternate_tlv_name_;

  public:
    ELFPatchPathManager(std::string chip_name, std::string tlv_name, std::string nvm_name,
                          std::string alternate_tlv_name) :
                            PatchPathManager(chip_name, tlv_name, nvm_name),
                              alternate_tlv_name_(alternate_tlv_name) {}

  const std::string& GetAlternatePatchFileName() const {
    return alternate_tlv_name_;
  }
...
class  Bcscal {
 ...
 public:
  ...
  Bcscal(std::string chip_name, std::string nvm_name) :
    chip_name_(chip_name), nvm_name_(nvm_name) {
  }
...
class PatchDLManager {
 public:
  PatchDLManager(BluetoothSocType soc_type, BluetoothSecSocType sec_soc_type, HciTransport* transport, PowerManager* power_manager);
  int PerformChipInit();
  int SetBaudRateReq();
  unsigned char IsSobsEnabled();
  unsigned char IsSibsEnabled();
  SocAddOnFeatures_t* GetAddOnFeatureList();
 ...
 protected:
  ...
  void FrameHciCmdPkt(unsigned char *cmd, int edl_cmd, unsigned int p_base_addr,
                         int segtNo, int size);
  void FrameACLPkt(unsigned char *cmd, int pktNo, int size);
  ...
  int GetXmemTlvFile(const char *file_path, const char* alt_file_path1, const char* alt_file_path2);
  bool ReadTlvInfo();
  int TlvDnldSegment(int index, int seg_size, unsigned char wait_cc_evt);
  int TlvDnldFrameThroughACL(int index, int frame_size, unsigned char wait_cc_evt);
  int DisableInternalLdo();
  void EnableControllerLog();
  void EnableEnhControllerLog();
  int ReadNewHciEvent(unsigned char* buf, int size);
  int ReadNewHciEvent_SPI(unsigned char* buf, int size);
  ...
  bool IsXmemDownload(int* tlv_size);
  int  EnableSecureBridge();
  int  SetUartBuadRate(uint8_t bt_baud_rate);
  void SetRampatchRegularPaths(std::string& tlv_name);
  void SetNVMRegularPaths(std::string& nvm_name);
  int CheckAndSetProperRampatchAltPath();
  void FormRegularPaths(PatchPathManager* info, bool is_xmem);
 ...
 int ReadNewACLEvent_SPI(void);
  FILE* OpenCalFile(const char *file_path);
  int tlv_dl_status;
  int getTlvDnldStatus();
#ifdef DEBUG_CRASH_SOC_WHILE_DLD
  int SendCrashCommand();
#endif
  void ReadELFConfig();
  void ReadNVMConfig();
  void GetOTPBytesInfo();
  int GetCoPVersion(void);
#ifdef IS_PERI_ENABLED
  FILE *OpenLicenseFile(const char *file_path);
  int DownloadLicenseFile();
  int GetLicenseFile(const char *file_path);
  int LicenseDnldReq(int license_size);
  int LicenseDnldInit(void);
  int FormLicenseHeader(unsigned char *file_size);
#endif
 private:
  void CheckForWakeupMechanism(unsigned char *nvm_byte_ptr, int offset);
  void EdlModeChange();
  void GetAppVerCmd();
  bool isPatchOptEnable();
  bool isPatchThroughACL();
  int TlvDnldReqACL(int tlv_size);
  void EnableLc3Swb();
  ...
  std::map<uint64_t, Bcscal *> BcscalInfoMap_;
  SocAddOnFeatures_t add_on_features;
  ...
  char xmem_rampatch_file_path[FILE_PATH_LEN] = {'\0'};
  char xmem_nvm_file_path[FILE_PATH_LEN] = {'\0'};
  char elf_config_[PROPERTY_VALUE_MAX] = "no_value";
  char nvm_config_[PROPERTY_VALUE_MAX] = "no_value";
  bool elf_config_read_ = false;
  bool nvm_optim_read = false;
  bool nvm_config_read_ = false;
  bool acl_prop_read_ = false;
  bool tlv_optim = false;
  bool patch_through_acl = false;
#ifdef IS_PERI_ENABLED
  unsigned char lic_dwn_cfg_;
  license_header *license_head;
#endif
  bool isSocCalibrationEnabled();
  int board_id_cmd_status = -1;
  char bcs_nvm_alt_path_qti_check[FILE_PATH_LEN] = {'\0'};
  char bcs_nvm_alt_path_check[FILE_PATH_LEN] = {'\0'};
  char bcs_nvm_file_path[FILE_PATH_LEN] = {'\0'};
  char bcs_nvm_alt_file_path[FILE_PATH_LEN] = {'\0'};
  FILE *CheckAndUpdateBcsCalFile(void);
  void SetBcsCalRegularPaths(std::string&);
  void LoadBcscalMaptable(void);
  bool IsBcsCalDownLoadDisabled(void);
};
