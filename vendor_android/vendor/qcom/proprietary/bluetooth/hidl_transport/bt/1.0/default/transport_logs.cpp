...
#define SPI_LOG_COLLECTION_DIR  "/sys/kernel/tracing/instances/spi_cnss"

#define TRANSPORT_LOG_FILE_NAME    "/trace"
...
void TransportLogs::DumpLogs() {
  int fd;
  bool ret;
  char dpath[TRANSPORT_LOG_PATH_BUF_SIZE];
  char spath[TRANSPORT_LOG_PATH_BUF_SIZE];
  struct DIR* p_dir;
  ...
  static_cast<Logger*>(logger_)->GetTransportLogFilename(dpath);

  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
    strlcpy(spath, SPI_LOG_COLLECTION_DIR, sizeof(spath));
  }else{
  ...
  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
    p_dir = opendir(SPI_LOG_COLLECTION_DIR);
    if (p_dir == NULL) {
    ALOGE("%s: Unable to open the Dir %s err: %s (%d)", __func__, SPI_LOG_COLLECTION_DIR,
  ...
  closedir(p_dir);
  ALOGI("%s: found Transport source log location: %s", __func__, spath);
  strlcat(spath, TRANSPORT_LOG_FILE_NAME, sizeof(spath));

  fd = open(spath, O_RDONLY | O_NONBLOCK);

  if (fd >= 0) {
    close(fd);
    ret = DumpTransportLogs(spath, dpath, TRANSPORT_LOG_MAX_SIZE);
    ALOGD("%s: Transport logs written at %s", __func__, dpath);
  } else {
    ALOGE("%s: Error opening source file (%s) error (%s)", __func__, spath, strerror(errno) );
  }
}

bool TransportLogs :: DumpTransportLogs(const char *spath, const char *dpath, long log_limit)
{
  int src_fd = -1;
  int dest_fd = -1;
  long logs_written = 0;
  ...
  buff = new (std::nothrow)char[TRANSPORT_LOG_MAX_READ_PER_ITERATION];
  if (buff == NULL) {
    ALOGE("%s: Error allocating memory %ld bytes for log buffer", __func__,
             TRANSPORT_LOG_MAX_READ_PER_ITERATION);
    return false;
  }
  ...
  do {
    ret = read(src_fd, buff, TRANSPORT_LOG_MAX_READ_PER_ITERATION);
    if (ret <= 0) {
    ...
    logs_written += ret;
    if ((log_limit != COMPLETE_TRANSPORT_LOGS) && (logs_written >= log_limit)) {
