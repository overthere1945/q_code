...
UartController::UartController(BluetoothSocType soc_type, BluetoothSecSocType sec_soc_type)
  : soc_crashed(false), soc_type_(soc_type),  sec_soc_type_(sec_soc_type),
...
bool UartController::Init(PacketReadCallback pkt_read_cb)
{
  ...
  if (soc_need_reload_patch) {
    ...
    if (power_manager_.SetPower(false) < 0) {
      logger_->SetInitFailureReason(BT_HOST_REASON_BT_PWR_VOTE_FAILED);
      ALOGD("%s: Failed to power off the soc", __func__);
      goto error;
    }

    // power on the chip using power manager
    if (power_manager_.SetPower(true) < 0) {
      logger_->SetInitFailureReason(BT_HOST_REASON_BT_PWR_VOTE_FAILED);
      ALOGD("%s: Failed to power on the soc", __func__);
      goto error;
    }
  ...
  patch_dl_manager = new (std::nothrow)PatchDLManager(soc_type_, sec_soc_type_, uart_transport, &power_manager_);
  if (!patch_dl_manager) {
    logger_->SetInitFailureReason(BT_HOST_REASON_MEMORY_ALLOCATION_FAILED);
    goto error;
  }
  ...
  if (soc_type_ == BT_SOC_GANGES) {
    ...
    peri_patch_dl_manager = new (std::nothrow)PeriPatchDLManager(soc_type_, sec_soc_type_, uart_transport);
	...
	((peri_patch_status = peri_patch_dl_manager->PerformChipInit())< 0)) {
      ...
      if ((peri_patch_status == -2) && !peri_patch_dl_manager->getFileOperationStatus()) {
...
#ifdef IS_PERI_ENABLED
        if (!patch_dl_manager->getFileOperationStatus()) {
          logger_->SetSecondaryCrashReason(BT_HOST_REASON_PATCHING_FAILED);
          StopRecoveryStuckTimer();
          DataHandler::Get()->StopInitTimer();
          SsrCleanup(BT_HOST_REASON_INIT_FAILED);
  ...
  if (soc_type_ == BT_SOC_HASTINGS || soc_type_ == BT_SOC_MOSELLE ||
      ...
      soc_type_ == BT_SOC_ORNE || soc_type_ == BT_SOC_EVROS ||
...
void UartController::SsrCleanup(PrimaryReasonCode reason)
{
  ...
  NotifySignal *IS_PERI_ENABLED = NotifySignal::Get();
  ...
  if (reason == BT_HOST_REASON_SSR_CMD_TIMEDOUT ||
      ...
      reason == BT_HOST_REASON_THERMAL_THROTTLE_IS_SEVERE||
    ...
    if (force_special_byte_enabled_ && !logger_->IsCrashDumpStarted()) {
      ...
      if (IsSoCCrashNotNeeded(reason) || logger_->isThisFpissue()) {
        ...
        if ((chipset_ver_ == HAMILTON_VER_2_0) ||
            ...
            (soc_type_ == BT_SOC_ORNE) || soc_type_ == BT_SOC_COLOGNE)) {
...
#ifdef IS_PERI_ENABLED
    if (NotifySignal::Get()->GetSubSystemSsrStatus() == SSR_ON_UWB) {
      ALOGE("%s: UWB Sub-system Failed to notify SSR COMPLETE status, Doing silent recovery", __func__);
              ...
              bool cleanupSocCrashWaitTimer, bool cleanupIbs)
{
  ...
  if (isDiagSsr) {
    ...
    if (kill_needed) {
      kill(getpid(), SIGKILL);
     } else if (reason == BT_HOST_REASON_THERMAL_THROTTLE_IS_SEVERE) {
      ALOGW("%s: Killing daemon, as system thermal sensors throttle is severe",
        __func__);
      kill(getpid(), SIGKILL);
    } else {
...
void UartController::SendSpecialBuffer(PrimaryReasonCode reason)
{
  ...
  if (reason != BT_HOST_REASON_SSR_INVALID_BYTES_RCVD) {
    retval = uart_transport->Write(buf, LEN_PACKET_TO_SOC);
    if (retval <= 0) {
    ...
    for(int i = 0; i < NUM_MS_FOR_SENDING_CRASH_BYTES; i++) {
      retval = uart_transport->Write(buf,
...
// TO-DO: set cflag to clear ctsrts bits
  ...
  if(crash_code == NMI_INTERRUPT_BYTE) {
    ...
    if ((reason == BT_HOST_REASON_SOC_CALIBRATION_DATA_RECV_TIMEOUT)||
       ((reason == BT_HOST_REASON_INIT_FAILED) &&
       ((sec_reason == BT_HOST_REASON_GETVER_NO_RSP_RCVD) ||
...
bool UartController::Cleanup(void)
{
  ...
  fmd_flag = DataHandler::Get()->IsFmdModeEnabledSendToSoc();
#endif
...
void UartController::OnPacketReady(hidl_vec<uint8_t> *data)
{
  ...
  if (read_cb_) {
    read_cb_(DataHandler::Get()->GetProtocol(hci_packet_type_), hci_packet_type_, data);
  }
...
void UartController::RxThreadTimeOut(union sigval sig)
{
...

  {
    ...
    if (DataHandler::Get()->isBtTpiEventFlag) {
      ALOGE("%s: Rx thread is blocked while posting BtTpi event", __func__);
    }
...
bool UartController::IsSoCCrashNotNeeded(PrimaryReasonCode reason) {
  ...
  return logger_->DiagInitOnGoing() ||
         ...
         sec_reason == BT_HOST_REASON_XPAN_THREAD_START_STUCK ||
#endif
