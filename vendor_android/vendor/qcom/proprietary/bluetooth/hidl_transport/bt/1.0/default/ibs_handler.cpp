...
# Copyright (c) 2013-2014, 2016-2017, 2023 Qualcomm Technologies, Inc.
# All Rights Reserved.
...
#ifdef BT_VER_1_1
#define LOG_TAG "vendor.qti.bluetooth@1.1-ibs_handler"
#else
...
#define DEFAULT_LPM_IDLE_TIMEOUT (250)
...
#else /* DETECT_SPURIOUS_WAKE */
#define UART_SPURIOUS_WAKE_SURPRESS_DELAY      (3*1000)
...
bool IbsHandler::rx_data_received = false;

IbsHandler *IbsHandler::Init(HciTransport* theTransport, HealthInfoLog* theHealthInfo,
...
void IbsHandler::InitInt()
{
  ...
  tty_fd_ = hci_transport_->GetCtrlFd();
  ...
  if (status == 0) {
    ...
    ALOGE("%s: Failed to create wake Timer", __func__);
  }
...
IbsHandler::IbsHandler(HciTransport* theTransport,
                      HealthInfoLog* theHealthInfo,
                      Controller* theController)
{
  ALOGD("%s", __func__);
  hci_transport_ = theTransport;
  ...
  cleanup_hidl_in_progress = false;
  IbsHandler::rx_data_received = false;
...
bool IbsHandler::CheckCleanupHidlProgress()
{
  std::unique_lock<std::mutex> guard(critical_section_lock);
  return cleanup_hidl_in_progress;
}

void IbsHandler::SetCleanupHidlProgress()
{
  std::unique_lock<std::mutex> guard(critical_section_lock);
  cleanup_hidl_in_progress = true;
}

void IbsHandler::WriteIbsCmd(uint8_t ibs_data)
{
  int ret;
  if (CheckCleanupHidlProgress()) {
    ALOGE("%s: Cleanup is in progress, Failed to write IBS data", __func__);
    return;
  }

  ret = hci_transport_->Write(&ibs_data, 1);
  if (ret < 0) {
...
void IbsHandler::WakeRetransTimeout(union sigval /* sig */)
{
  ...
  uint8_t tx_ibs_state = instance_->state_machine_.tx_ibs_state;
  if (tx_ibs_state != IBS_TX_WAKING) {
    ALOGI("%s: tx_ibs_state(%d) is not waking, exit", __func__, tx_ibs_state);
    instance_->num_try_ = 1;
    return;
  }
...
void IbsHandler::SerialClockVote(uint8_t vote)
{
  ...
  if (new_vote != old_vote) {
    ...
    if (new_vote) {
      ...
      if ((IbsHandler::rxd_wakeup_enabled) && (vote == IBS_TX_VOTE_CLOCK_ON) && (!CheckCleanupHidlProgress())) {
...
void IbsHandler::DeleteWackTimer()
{
  ALOGV("%s", __func__);
...
void IbsHandler::SetRxDataReceived()
{
    IbsHandler::rx_data_received = true;
}

void IbsHandler::HandleSpuriousErrorGracefully()
{
    if (!instance_) {
      ALOGD("%s: IBS handler has been destroyed ", __func__);
      return;
    ...
    if (Logger::Get()->isSsrTriggered()) {
      ALOGE("SsrCleanup in progress, not allowing to %s", __func__);
      return;
    }

    ALOGE("%s:rx_ibs_state = %d",__func__, instance_->state_machine_.rx_ibs_state);
    if (instance_->state_machine_.rx_ibs_state == IBS_RX_AWAKE) {
        ALOGE("%s: RX path is awake and spurious condition detected, Vote Off UART", __func__);
        instance_->UpdateRxState(IBS_RX_ASLEEP);
        ALOGE("%s: UART RX Vote Off", __func__);
        instance_->SerialClockVote(IBS_RX_VOTE_CLOCK_OFF);
    }
}
...
#ifdef DETECT_SPURIOUS_WAKE
  if (IbsHandler::rx_data_received) {
      UartController* uart_controller = static_cast<UartController*>(instance_->controller);
      uart_controller->SsrCleanup(BT_HOST_REASON_SSR_SPURIOUS_WAKEUP);
  } else {
      IbsHandler::HandleSpuriousErrorGracefully();
  }
#else /* DETECT_SPURIOUS_WAKE */
  IbsHandler::HandleSpuriousErrorGracefully();
#endif /* DETECT_SPURIOUS_WAKE */
...
{
  if (CheckCleanupHidlProgress()) {
    ALOGE("%s: Cleanup is in progress, Failed to vote on clock", __func__);
    return;
  }
  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_UART){
    HciUartTransport* uart_transport = static_cast<HciUartTransport*>(hci_transport_);
...
void IbsHandler::VoteOffClock()
{
  StopIdleTimer();
  if (CheckCleanupHidlProgress()) {
    ALOGE("%s: Cleanup is in progress, Failed to vote off clock", __func__);
    return;
 }
  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_UART){
    HciUartTransport* uart_transport = static_cast<HciUartTransport*>(hci_transport_);
...
void IbsHandler::SocRxDWakeup() {
  if (CheckCleanupHidlProgress()) {
    ALOGE("%s: Cleanup is in progress,Soc RxD Wakeup Failed", __func__);
    return;
  }
  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_UART){
    HciUartTransport* uart_transport = static_cast<HciUartTransport*>(hci_transport_);
