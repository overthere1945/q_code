...
#ifdef BT_VER_1_1
#define LOG_TAG "vendor.qti.bluetooth@1.1-mct_controller"
#else
...
bool MctController::Init(PacketReadCallback pkt_read_cb)
{
  ...
  if (!mct_transport->Init(soc_type_)) {
    ALOGE("%s: failed to intiailize Bluetooth SMD transport", __func__);
    return false;
  }
...
SocAddOnFeatures_t* MctController:: GetAddOnFeatureList()
{
  return NULL;
}

uint64_t MctController:: GetChipVersion()
{
  return INVALID_CHIP_VERSION;
}

bool MctController::GetCleanupStatus(void)
{
  return false;
}

void MctController::SetCleanupStatusDuringSSR() {

}

#ifdef BT_CP_CONNECTED
CoPVerSupported MctController::GetCoPVersion()
{
  return {0};
}
#endif
