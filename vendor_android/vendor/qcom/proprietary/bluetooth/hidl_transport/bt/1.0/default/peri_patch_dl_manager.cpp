...
bool tmel_patching_successful = false;
...
PeriPatchDLManager::PeriPatchDLManager(BluetoothSocType soc_type, BluetoothSecSocType sec_soc_type, HciTransport* transport) :
  soc_type_(soc_type), sec_soc_type_(sec_soc_type), transport_(transport), dnld_fd_in_progress_(-1)
{
  ALOGD("%s", __func__);
  bool tmel = false;
...
void PeriPatchDLManager::ReadSocDump()
{
  ...
  while (1) {
    ...
    if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
      tot_len = ReadHciPeriEvent_SPI(buf, HCI_MAX_EVENT_SIZE);
    }else{
...
bool PeriPatchDLManager::getFileOperationStatus(void) {
...
int PeriPatchDLManager::PerformChipInit()
{
  ...
  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_UART){
    ...
    */
    ret = uart_transport->Ioctl(USERIAL_OP_FLOW_ON, &flags);
  }
  gettimeofday(&tv, NULL);
...
int PeriPatchDLManager::TlvDnldSegment(int index, int seg_size, unsigned char wait_cc_evt)
{
  ...
  if (tmel) {
    ...
    if (err != size) {
      ALOGE("Failed to send the peri patch payload to the Controller! 0x%x for segment : %d", err, index);
      ALOGE("Retrying the failed segment : %d again",index);
      err = HciSendVsCmd((unsigned char*)cmd, rsp, size);
      if(err != size){
        return err;
      }
...
int PeriPatchDLManager::DownloadTlvFile()
{
  ...
  if ((tlv_size = GetTlvFile(rampatch_file_path)) > 0) {
    ...
    /*
    This is a SW Workaround added to read Command Complete after Peri Patch last segment download
    */
    usleep(50000);
...
int PeriPatchDLManager::WakeUpSoC(void)
{
  ...
  for (i = 0; i < WAKE_IND_RETRIES; i++) {
    ...
    if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
        ret = transport_->Write(HCI_PACKET_TYPE_COMMAND, &wakeup_byte, 1);
        if(ret != 1){
          ALOGE("%s: Failed to send WAKE_IND: %d", __func__, err);
          break;
        }
    }else{
        ret = transport_->Write(&wakeup_byte, 1);
        if (ret != 1) {
          ALOGE("%s: Failed to send WAKE_IND: %d", __func__, err);
          break;
        }
...
int PeriPatchDLManager::PeriSocInit(void)
{
  ...
  ALOGI("%s: get peri patch version ", __func__);
...
ALOGI("%s: get baudrate ", __func__);
  ...
  ALOGI("%s: set arbitration ", __func__);
...
void PeriPatchDLManager::FormRegularPaths(PatchPathManager * info)
{
  ...
  if (!tmel && info && info->GetPatchType() == "ELF" && (stat(rampatch_file_path, &sts) == -1)) {
    std::string tlv_name;
    ALOGI("%s: mbn file is missing in %s going with tlv now", __func__, rampatch_file_path);
    ...
    SetPeriRampatchRegularPaths(tlv_name);
  }
  if (!tmel) {
...
int PeriPatchDLManager::ReadHciPeriEvent_SPI(unsigned char* buf, int size){
  ...
  do{
    retval = spi_transport->Read((uint8_t *)&hci_packet_type_,(uint8_t *)&host_id_byte, buff, size);
    if (retval < 0) {
      ALOGE("%s: read error", __func__);
      return -1;
    }
    if(retval >= 1){
        protocol_byte = hci_packet_type_;
    }else{
      ALOGE("%s: size is too less for protocol_byte", __func__);
      return -1;
    }

    if (protocol_byte == LOG_BT_EVT_PACKET_TYPE) {
      memcpy(buf, &buff, retval);
      tot_len = retval;
      ALOGV("%s: read successfully bt_evt_pkt_type: tot_len: %d", __func__, tot_len);
      break;
    } else if (protocol_byte == LOG_PERI_EVT_PACKET_TYPE) {
      if(retval >= 2){
        if (host_id_byte != BT_HOST_ID) {
      ...
      buf[0] = hci_packet_type_;
      buf[1] = host_id_byte;
      memcpy(buf+2, &buff, retval);
      //Adding Protocol_Byte and Host_Id_Byte to Total Len
      tot_len = retval + 2;
      break;
    } else if (protocol_byte == LOG_BT_ACL_PACKET_TYPE) {
      if(retval >= 5){
        PTR_TO_UINT16(tot_len, &buff[BT_ACL_HDR_LEN_OFFSET - 1]);
        ALOGD("%s: Number of bytes of ACL data(header + payload) read = %d",
            __func__, tot_len + BT_ACL_HDR_SIZE);
        bt_logger_->ProcessRx(HCI_PACKET_TYPE_ACL_DATA, tmp_buf, tot_len + BT_ACL_HDR_SIZE);
      }else{
        ALOGE("%s: size is too less for acl_header", __func__);
        return -1;
      }
    } else if (protocol_byte == 0xFD) {
      ALOGI("%s: Got FD , responding with FC", __func__);
      uint8_t wake_byte = 0xFC;
      spi_transport->Write(HCI_PACKET_TYPE_CMD_FROM_HAL, &wake_byte, 1);
    } else if (protocol_byte == 0xFE) {
      ALOGI("%s: Sleep Byte Recieved ", __func__);
    } else if (protocol_byte == 0xFC) {
      if (soc_wakeup_in_progress) {
...
int PeriPatchDLManager::ReadHciEvent(unsigned char* buf, int size)
{
  ...
  do {
    read_again = false;
    if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
      tot_len = ReadHciPeriEvent_SPI(buf, size);
    }else{
    ...
    if (ss.str().length() > 0) {
      ...
      params += ss.str();
      ALOGV("%s: %s", __func__, params.c_str());
    }
    ...
    if(buf[0] == HCI_PACKET_TYPE_PERI_EVT && buf[2] == PERI_EVT_CMD_COMPLETE) {
      ...
      if (!collecting_ram_dump) {
        ALOGD("%s: It is RAMDUMP OR HCI_BUFFER_DUMP, keep looping to get RAMDUMP events", __func__);
      ...
      ALOGV("%s: BT Evt with type %02x", __func__, buf[1]);
...
int PeriPatchDLManager::ProcessPeriEdlPatchEvt(unsigned char *rsp, unsigned char paramlen)
{
  ...
  char dst_buff[MAX_BUFF_SIZE];
#endif
  static bool isProductIdSet = false;
  ...
  if ((!tmel) && (!isProductIdSet)) {
    std::string str = std::to_string(productid);
    Logger::Get()->PropertySet("persist.vendor.qcom.bluetooth.soc.productid", str.c_str());
    isProductIdSet = true;
  }
...
int PeriPatchDLManager::GetVsHciEvent(unsigned char *rsp)
{
  ...
  if(opcode == HCI_VS_DNLD_OPCODE_PERI) {
	...
	peri_activate_action_ = (HciActivate)status;
        ...
        break;
      case HCI_PERI_RESET:
        ALOGD("%s: HCI_PERI_RESET Response received", __func__);
...
int PeriPatchDLManager::HciSendVsCmd(unsigned char *cmd, unsigned char *rsp, int size) {
  return HciSendVsCmdWithOptions(cmd, rsp, size, SPI_DEFAULT_SLEEP_BIT);
}

int PeriPatchDLManager::HciSendVsCmdWithOptions(unsigned char *cmd, unsigned char *rsp,
  int size, uint32_t options)
{
  int ret = 0;
  char dst_buff[MAX_BUFF_SIZE] = {'\0'};
  struct timeval tv;
  ...
  if(DataHandler::Get()->getTransportType() == BT_TRANSPORT_TYPE_SPI){
    HciSpiTransport* spi_transport = static_cast<HciSpiTransport*> (transport_);
    ret = spi_transport->WriteWithOptions(HCI_PACKET_TYPE_PERI_CMD, cmd, size, options);
  }else{
...
void PeriPatchDLManager::LoadTMELPatchMaptable() {
  TMEPatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(GANGES_VER_1_0,
            new PatchPathManager("GAN_PERI1_0", "tmel_peach_10.elf", "")));
  TMEPatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(BRAHMA_VER_1_0,
            new PatchPathManager("BRAHMA_VER_1_0", "tmel_peach_10.elf", "")));
  TMEPatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(GANGES_VER_2_0,
            new PatchPathManager("GAN_PERI2_0", "tmel_peach_20.elf", "")));
  TMEPatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(BRAHMA_VER_2_0,
            new PatchPathManager("BRAHMA_VER_2_0", "tmel_peach_20.elf", "")));
}

void PeriPatchDLManager::LoadPatchMaptable() {
  PeriPatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(GANGES_VER_1_0,
            new PatchPathManager("GAN_PERI1_0", "gngperifw10.tlv", "gngperinv10.bin")));
  PeriPatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(GANGES_VER_2_0,
            new ELFPatchPathManager("GAN_PERI2_0", "gngperifw20.mbn", "gngperinv20.bin", "gngperifw20.tlv")));
  PeriPatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(BRAHMA_VER_1_0,
            new PatchPathManager("BRAHMA_VER_1_0", "brhperifw10.tlv", "brhperinv10.bin")));
  PeriPatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(BRAHMA_VER_2_0,
            new ELFPatchPathManager("BRAHMA_VER_2_0", "brhperifw20.mbn", "brhperinv20.bin", "brhperifw20.tlv")));
  PeriPatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(THEMISTO_VER_1_0,
                        new PatchPathManager("THEMISTO_PERI1_0", "tmoperifw10.mbn", "tmoperinv10.bin")));
  PeriPatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(THEMISTO_VER_2_0,
                        new PatchPathManager("THEMISTO_PERI2_0", "tmoperifw20.mbn", "tmoperinv20.bin")));
}
...
int PeriPatchDLManager::TMEArbitrate()
{
  ...
  BtState :: Get()->AddLogTag(dst_buff, tv, (char*)"Sending Arbitration CMD to PeriSoC (Ssid: TMEL)");
  BtState :: Get()->SetPeriArbitReqTS(dst_buff);
  ...
  gettimeofday(&tv, NULL);
  snprintf(res_buff, sizeof(res_buff), "Peri SS Arbitration rsp(Ssid:TMEL) = %d", err);
  dst_buff[0] = '\0';
  BtState :: Get()->AddLogTag(dst_buff, tv, res_buff);
  BtState :: Get()->SetPeriArbitEvtTS(dst_buff);
...
int PeriPatchDLManager::TMEPatchVerReq()
{
  ...
  gettimeofday(&tv, NULL);
  BtState :: Get()->AddLogTag(dst_buff, tv, (char*)"Sending Get Version CMD To Peri SoC (Ssid:TMEL)");
  BtState :: Get()->SetPeriGetVerReqTS(dst_buff, SEND_CMD_TS);
  ...
  gettimeofday(&tv, NULL);
  snprintf(res_buff, sizeof(res_buff), "Peri Get Version rsp rcvd(Ssid:TMEL), num bytes in last rsp = %d", err);
  BtState :: Get()->AddLogTag(dst_buff, tv, res_buff);
  BtState :: Get()->SetPeriGetVerResEvtTS(dst_buff);
  bt_logger_->SetSecondaryCrashReason(TME_SOC_REASON_DEFAULT);
...
int PeriPatchDLManager::ConfigureTMESoC(void)
{
  ...
  switch (tme_patch_status_) {
	...
	if (version_info_ins_tme.chipset_ver != chipset_tme_ver_ ||
		...
		BtState :: Get()->AddLogTag(tme_dst_buff, tv, (char*)"Writing tme version info. in file");
		BtState :: Get()->SetPrePeriVersioInfoTS(tme_dst_buff);
		if (NULL != (tmeversionfile = fopen(PERI_VERSION_FILEPATH, "ab+"))) {
		...
		ALOGD("%s: Done writing TME-L Version Info. in %s", __func__, PERI_VERSION_FILEPATH);
		gettimeofday(&tv, NULL);
		BtState :: Get()->AddLogTag(tme_dst_buff, tv, (char*)"Done writing version info. in file");
		BtState :: Get()->SetPostPeriVersioInfoTS(tme_dst_buff);
		bt_logger_->CheckAndAddToDelayList(&tv);
	}
#endif
	return err;
