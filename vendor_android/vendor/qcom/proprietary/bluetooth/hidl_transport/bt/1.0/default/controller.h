...
#define UWB_SSR_TIMEOUT                (60000) // for uwb ssr to complete
#define CALIBRATION_DATA_LOG_TIME      ( 3000) // This time is to collect SOC calibration data
#define SSR_TIMEOUT                    (60000) // SOC sends first packet after 4 sec once watchdog ssr is triggred
...
class Controller {
 public:
  ...
  virtual bool GetCleanupStatus(void) { return false;};
  virtual void SetCleanupStatusDuringSSR() {};
  void updateCalEvntRecvStatus(bool status);
  void waitForCalDataToCollect();
  void SignalCalDataCollectFinish();
