...
class PeriPatchDLManager {
 public:
  ...
  PeriPatchDLManager(BluetoothSocType soc_type, BluetoothSecSocType sec_soc_type, HciTransport* transport);
  int PerformChipInit();
  int SetBaudRateReq();
  ...
  void FramePeriHciPkt_SPI(unsigned char *cmd, unsigned short opcode, byte subOpcode, unsigned int p_base_addr, int segtNo, uint8_t size);
  ~PeriPatchDLManager();
  bool file_operation_failed;
  bool getFileOperationStatus(void);
 ...
 protected:
  ...
  int ReadHciPeriEvent_SPI(unsigned char* buf, int size);
  ...
  int TMEReset(void);
  int HciSendVsCmdWithOptions(unsigned char *cmd, unsigned char *rsp,
