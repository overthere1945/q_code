...
#ifdef BT_VER_1_1
#define LOG_TAG "vendor.qti.bluetooth@1.1-ring_buffer"
#else
...
void RingBuffer::UpdateRxEventTag(RxThreadEventType rx_event)
{
  switch (rx_event) {
    ...
    case RX_POST_SELECT_CALL_BACK:
      ...
      break;
    case RX_PRE_STACK_ISO_CALL_BACK:
      gettimeofday(&time_pre_stack_iso_call_back, NULL);
      break;
    case RX_POST_STACK_ISO_CALL_BACK:
      gettimeofday(&time_post_stack_iso_call_back, NULL);
      break;
    case RX_PRE_STACK_SCO_CALL_BACK:
      gettimeofday(&time_post_stack_sco_call_back, NULL);
      break;
    case RX_POST_STACK_SCO_CALL_BACK:
      gettimeofday(&time_post_stack_sco_call_back, NULL);
      break;
    default:
...
void RingBuffer::DumpData()
{
  ...
  AddRxTagEvent(tag_event_call_back,time_pre_stack_sco_call_back);
  snprintf(dst_buff, MAX_BUFF_SIZE, "TS of posting SCO packet to stack %s", tag_event_call_back);
  BtState::Get()->SetPreStackIso(dst_buff);
  ALOGE("%s", dst_buff);

  AddRxTagEvent(tag_event_call_back, time_post_stack_sco_call_back);
  snprintf(dst_buff, MAX_BUFF_SIZE, "TS of SCO packet posted to stack %s", tag_event_call_back);
  BtState::Get()->SetPostStackIso(dst_buff);
  ALOGE("%s", dst_buff);
...
PacketBuff :: PacketBuff()
{
  ...
  map_code_reason_[BT_HOST_REASON_DEFAULT_NONE] =  (char *)"Invalid reason";
  map_code_reason_[BT_HOST_REASON_SOC_CRASHED] = (char *)"SOC crashed";
  map_code_reason_[BT_HOST_REASON_SOC_CRASHED_DIAG_SSR] =  (char *)"SOC crashed with diag initiated SSR";
  map_code_reason_[BT_HOST_REASON_INIT_FAILED] =  (char *)"Init failed";
  map_code_reason_[BT_HOST_REASON_CLOSE_RCVD_DURING_INIT] =  (char *)"Close received from stack during SOC init";
  map_code_reason_[BT_HOST_REASON_ERROR_READING_DATA_FROM_UART] =  (char *)"Error reading data from UART";
  map_code_reason_[BT_HOST_REASON_WRITE_FAIL_SPCL_BUFF_CRASH_SOC] =  (char *)"Failed to write special bytes to crash SOC";
  map_code_reason_[BT_HOST_REASON_RX_THREAD_STUCK] =  (char *)"Rx Thread Stuck";
  map_code_reason_[BT_HOST_REASON_SSR_CMD_TIMEDOUT] =  (char *)"SSR due to command timed out";
  map_code_reason_[BT_HOST_REASON_SSR_SPURIOUS_WAKEUP] =  (char *)"SSR due to spurious wakeup";
  map_code_reason_[BT_HOST_REASON_SSR_INVALID_BYTES_RCVD] =  (char *)"Invalid HCI cmd type received";
  map_code_reason_[BT_HOST_REASON_SSR_RCVD_LARGE_PKT_FROM_SOC] =  (char *)"Large packet received from SOC";
  map_code_reason_[BT_HOST_REASON_SSR_UNABLE_TO_WAKEUP_SOC] =  (char *)"Unable to wake SOC";
  map_code_reason_[BT_HOST_REASON_CMD_TIMEDOUT_SOC_WAIT_TIMEOUT] =  (char *)"Command timedout and SOC crash wait timeout";
  map_code_reason_[BT_HOST_REASON_SPURIOUS_WAKEUP_SOC_WAIT_TIMEOUT] =  (char *)"Spurious wake and SOC crash wait timeout";
  map_code_reason_[BT_HOST_REASON_INV_BYTES_SOC_WAIT_TIMEOUT] =  (char *)"Invalid bytes received and SOC crash wait timeout";
  map_code_reason_[BT_HOST_REASON_SOC_WAKEUP_FAILED_SOC_WAIT_TIMEOUT] =  (char *)"SOC Wakeup failed and SOC crash wait timeout";
  map_code_reason_[BT_HOST_REASON_SOC_CRASHED_DIAG_SSR_SOC_WAIT_TIMEOUT] = (char *)"SOC crashed with diag initiated SSR and SOC wait timeout";
  map_code_reason_[BT_HOST_REASON_NONE_SOC_WAIT_TIMEOUT] =  (char *)"Invalid Reason and SOC crash wait timeout";
  map_code_reason_[BT_HOST_REASON_SOC_DEINIT_STUCK] =  (char *)"SOC Deinit Stuck";
  map_code_reason_[BT_HOST_REASON_SSR_INTERNAL_CMD_TIMEDOUT] = (char *)"SSR due to internal Command timeout";
  map_code_reason_[BT_HOST_REASON_FAILED_TO_SEND_INTERNAL_CMD] = (char *)"Failed to send internal command";
  map_code_reason_[BT_HOST_REASON_SSR_SLEEP_IND_NOT_RCVD] = (char *)"Failed to receive SLEEP IND during close";
  map_code_reason_[BT_HOST_REASON_DIAG_LOG_API_STUCK] = (char *)"DIAG log API stuck";
#ifdef IS_PERI_ENABLED
  map_code_reason_[BT_HOST_REASON_PERI_SOC_CRASHED] = (char *)"Peri SOC crashed";
  map_code_reason_[BT_HOST_REASON_PERI_SOC_CRASHED_DIAG_SSR] =  (char *)"Peri SOC crashed with diag initiated SSR";
  map_code_reason_[BT_HOST_REASON_PERI_SOC_CRASHED_ON_OTHER_SUB_SYSTEM] = (char *)"Peripheral core crash deteced in UWB SS";
  map_code_reason_[BT_HOST_REASON_PERI_SUB_SYSTEM_FAILED_UPDATE_SSR_COMPLETE] = (char *)"UWB Sub-System failed to update SSR complete status";
#endif
  map_code_reason_[BT_HOST_REASON_SOC_CALIBRATION_DATA_RECV_TIMEOUT] = (char *)"SOC Calibration data evt receive timed out";
  map_code_reason_[BT_HOST_REASON_STORE_SOC_CALIBRATION_FAILED] = (char *)"Failed to Store Calibration data";
  map_code_reason_[BT_HOST_REASON_THERMAL_THROTTLE_IS_SEVERE] = (char *)"Host thermal sensors crossed severe state";
}
...
void PacketBuff::AddBuffNode(short int dir, HciPacketType type, uint16_t len, uint8_t *pkt)
{
  ...
  switch (type) {
    ...
    case HCI_PACKET_TYPE_ANT_DATA:
      header_length = pkt[0] + ANT_PACKET_HDR;
    ...
    case HCI_PACKET_TYPE_ISO_DATA:
      header_length = ((pkt[3] & 0x3f) << 8) + pkt[2] + ACL_PACKET_HDR;
      if (dir == RingBuffer::HOST_TO_SOC)
        flags = 0;
      else
        flags = 1;
    ...
    case HCI_PACKET_TYPE_PERI_EVT:
      header_length = pkt[2] + 3 + 1;
      flags = 3;
      break;
    case HCI_PACKET_TYPE_PERI_CMD:
    case HCI_PACKET_TYPE_PERI_DATA:
      header_length = pkt[3] + HCI_COMMAND_HDR_SIZE + 2;
      flags = 2;
      break;
#endif
    default:
...
void PacketBuff::DumpData()
{
  ...
  /* Total data must be atleast [size of header + 1 byte of packet data]
   * IF no then no data is actually dumped in ring buffer.
   */
  if ((buffsz1 + buffsz2) < (snoop_hdr_len + 1)) {
