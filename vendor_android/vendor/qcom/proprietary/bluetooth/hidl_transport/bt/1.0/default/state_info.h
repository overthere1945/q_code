...
struct resources_state {
  char com[MAX_PWR_RSRC_INFO_SIZE];
  char bt[MAX_PWR_RSRC_INFO_SIZE];
  char uwb[MAX_PWR_RSRC_INFO_SIZE];
};
...
typedef struct {
  ...
  char fw_su_build_info[BUILD_LABEL_LEN];
  char bt_soc_id[MAX_BUFF_SIZE];
  struct resources_state power_resources_state;
  ...
  char ts_peri_getver_start_send_cmd[MAX_BUFF_SIZE];
  char ts_peri_getver_start_read_rsp[MAX_BUFF_SIZE];
  char ts_peri_getver_rcvd[MAX_BUFF_SIZE];
  char ts_peri_arbit_ntf[MAX_BUFF_SIZE];
  char peri_fw_dwnld_success[MAX_BUFF_SIZE];
  char peri_fw_su_build_info[BUILD_LABEL_LEN];
  char peri_soc_id[MAX_BUFF_SIZE];
  char peri_post_patch_open[MAX_BUFF_SIZE];
  char peri_pre_patch_open[MAX_BUFF_SIZE];
  char ts_peri_arbit_rcvd[MAX_BUFF_SIZE];
  char ts_peri_arbitration_sent[MAX_BUFF_SIZE];
  char peri_pre_nvm_open[MAX_BUFF_SIZE];
  char peri_post_nvm_open[MAX_BUFF_SIZE];
  char peri_pre_chk_patch_path[MAX_BUFF_SIZE];
  char peri_post_chk_patch_path[MAX_BUFF_SIZE];
  char peri_pre_version_info[MAX_BUFF_SIZE];
  char peri_post_version_info[MAX_BUFF_SIZE];
  char peri_pre_su_build_info[MAX_BUFF_SIZE];
  char peri_post_su_build_info[MAX_BUFF_SIZE];
  char ts_peri_reset_start_send_cmd[MAX_BUFF_SIZE];
  char ts_peri_reset_start_read_rsp[MAX_BUFF_SIZE];
  char ts_peri_reset_rcvd[MAX_BUFF_SIZE];
  struct timeval peri_fw_su_build_info_set_time;
#endif
...
class BtState
{
 ...
 public:
  ...
  void SetPreStackIso(char* buf);
  void SetPostStackIso(char* buf);
  ...
  void SetPowerResourcesState(struct power_buff_struct *buf);
...
#ifdef IS_PERI_ENABLED
  void SetPeriArbitEvtTS(char *buf);
  void SetPeriArbitReqTS(char *buf);
  void SetPeriActivateSsResEvtTS(char *buf);
  void SetPeriActivateSsReqTS(char *buf, int type);
  void SetPeriGetVerResEvtTS(char *buf);
  void SetPeriGetVerReqTS(char *buf, int type);
  void SetPeriArbitNtfTS(char *buf);
  void SetPeriFwDwnldSucceed(char *);
  void SetPeriFwSuBuildInfo(char *, struct timeval);
  void SetPeriSocId(char *);
  void SetPrePeriPatchOpenTS(char* buf);
  void SetPostPeriPatchOpenTS(char* buf);
  void SetPrePeriNVMOpenTS(char* buf);
  void SetPostPeriNVMOpenTS(char* buf);
  void SetPrePeriVersioInfoTS(char* buf);
  void SetPostPeriVersioInfoTS(char* buf);
  void SetPrePeriSUBuildInfoTS(char* buf);
  void SetPostPeriSUBuildInfoTS(char* buf);
  void SetPeriResetReqTS(char *buf);
  void SetPeriResetResEvtTS(char *buf);
#endif
};
