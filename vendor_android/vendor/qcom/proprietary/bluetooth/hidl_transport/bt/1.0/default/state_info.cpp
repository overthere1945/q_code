...
BtState::BtState()
{
  ...
  strlcpy(stat_info_obj.fw_dwnld_success, "00:00:00:000:Soc Initialization not completed",
          MAX_BUFF_SIZE);
  strlcpy(stat_info_obj.fw_su_build_info, "BT SoC FW SU Build info: Not Known", BUILD_LABEL_LEN);
#ifdef IS_PERI_ENABLED
  strlcpy(stat_info_obj.peri_fw_dwnld_success, "00:00:00:000:Peri Soc Initialization not completed",
          MAX_BUFF_SIZE);
  strlcpy(stat_info_obj.peri_fw_su_build_info, "Peri SoC FW SU Build info: Not Known", BUILD_LABEL_LEN);
...
void BtState::SetPeriFwSuBuildInfo(char *buf, struct timeval tv) {
  ...
  memset(stat_info_obj.peri_fw_su_build_info, 0, BUILD_LABEL_LEN);
...
void BtState::SetFwSuBuildInfo(char *buf, struct timeval tv) {
  ...
  memset(stat_info_obj.fw_su_build_info, 0, BUILD_LABEL_LEN);
...
void BtState::DumpBtState() {
  ...
  if (soc_need_reload_patch) {
    ...
    write(fd, " \n", 2);
    char tmp_buf1[BUILD_LABEL_LEN] = {0};
    add_time_str(tmp_buf1, &stat_info_obj.fw_su_build_info_set_time);
    snprintf(tmp_buf1 + strlen(tmp_buf1), sizeof(tmp_buf1) - strlen(tmp_buf1), " %s", stat_info_obj.fw_su_build_info);
    ALOGD("%s", tmp_buf1);
    ...
    write(fd, " \n", 2);
    char tmp_buf2[BUILD_LABEL_LEN] = {0};
    add_time_str(tmp_buf2, &stat_info_obj.peri_fw_su_build_info_set_time);
    snprintf(tmp_buf2 + strlen(tmp_buf2), sizeof(tmp_buf2) - strlen(tmp_buf2), " %s", stat_info_obj.peri_fw_su_build_info);
    ALOGD("%s", tmp_buf2);
...

  {
    ...
    for (int i = 0; i < stat_info_obj.last_tx_pkt_len; i++) {
      snprintf(hex_buff, 4, "%.2X ", stat_info_obj.last_tx_pkt_asc[i]);
...

  {
    ...
    for (int i = 0; i < stat_info_obj.ssr_rx_pkt_len; i++) {
      snprintf(hex_buff, 4, "%.2X ", stat_info_obj.ssr_rx_pkt_asc[i]);
...

  {
    ...
    for (int i = 0; i < stat_info_obj.last_rx_pkt_len; i++) {
      snprintf(hex_buff, 4, "%.2X ", stat_info_obj.last_rx_pkt_asc[i]);
  ...
  if (stat_info_obj.inv_bytes_len) {
...

    {
      ...
      for (int i = 0; i < stat_info_obj.inv_bytes_len; i++) {
        snprintf(hex_buff, 4, "%.2X ", stat_info_obj.inv_bytes_asc[i]);
