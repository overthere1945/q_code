...
*/
  ...
  const std::string instance = std::string() + IBluetoothSocket::descriptor + "/default";
  if (bluetoothSocket) {
    auto result = AServiceManager_addService(bluetoothSocket->asBinder().get(), instance.c_str());
    if (result != STATUS_OK) {
  ...
  if (bluetoothGatt) {
    ...
    joinRpcThreadpool();
    if (isTpiSupported) {
      ALOGI("BTTPI: Main, joinThreadpool for AIDL");
