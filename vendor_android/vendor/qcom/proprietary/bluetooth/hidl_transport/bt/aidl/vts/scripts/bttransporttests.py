...
# 1. execute(as admin) "setup.bat" in the
#      C:\ProgramData\Qualcomm\Alpaca\Python\XPlatform folder.
# 2. run "python -m pip install pypiwin32" to install win32 COM extensions
#
import sys
import time
import os
...
def initAlpaca():
    ...
    if (usbDisc):
        ...
        if (deviceCount > 0) :
            ...
            if (tacDevice != None) :
               if (tacDevice.Open()) :
                  print("UUID:" + tacDevice.Get_UUID() + " protName:" + tacDevice.PortName())
                  batteryState = tacDevice.GetBatteryState()
                  print("Battery State: " + str(batteryState))
               else :
...
def setUpEnvironment():
    ...
    subprocess.call("adb shell mkdir /data/nativetest64/VtsHalBluetoothAidlTransportTest")

    # if the VTS test binary doesn't exist on the DUT, push it to the above path"
    subprocess.call("adb push VtsHalBluetoothAidlTransportTest /data/nativetest64/VtsHalBluetoothAidlTransportTest")
    subprocess.call("adb shell chmod 755 /data/nativetest64/VtsHalBluetoothAidlTransportTest/VtsHalBluetoothAidlTransportTest")
    if listTestCases:
        subprocess.call("adb shell /data/nativetest64/VtsHalBluetoothAidlTransportTest/VtsHalBluetoothAidlTransportTest --gtest_list_tests")
        exit(0)
    ...
    subprocess.call("adb push BT-Transport-Tests.sh /data/nativetest64/VtsHalBluetoothAidlTransportTest")
    subprocess.call("adb shell chmod 755 /data/nativetest64/VtsHalBluetoothAidlTransportTest/BT-Transport-Tests.sh")
...
def executeTests(count):
    ...
    subprocess.call(["adb", "-s", str(adbDevice), "shell", "rm", "/data/nativetest64/VtsHalBluetoothAidlTransportTest/tstlog.txt"])
    subprocess.call(["adb", "-s", str(adbDevice), "shell", "rm", "/data/nativetest64/VtsHalBluetoothAidlTransportTest/logcat.txt"])
    subprocess.call(["adb", "-s", str(adbDevice), "shell", "nohup" , "logcat", "-vthreadtime -b kernel -b main -b system",
                         " > /data/nativetest64/VtsHalBluetoothAidlTransportTest/logcat.txt 2>&1 < /dev/null &"])
    if usbDisc:
        subprocess.call(["adb", "-s", str(adbDevice), "shell", "nohup" , "/data/nativetest64/VtsHalBluetoothAidlTransportTest/BT-Transport-Tests.sh",
                         ...
                         " > /data/nativetest64/VtsHalBluetoothAidlTransportTest/tstlog.txt 2>&1 < /dev/null &"])
        usbState = tacDevice.GetUsb1State()
        print("USB State: " + str(usbState))

        if usbState:
          print("Disconnecting USB")
          tacDevice.Usb1(False)
          usbState = tacDevice.GetUsb1State()
          print("USB State: "+ str(usbState))
          time.sleep(testWaitTime)
          print("Reconnecting USB")
          tacDevice.Usb1(True)
          print("USB State: "+ str(tacDevice.GetUsb1State()))
          time.sleep(3)
        ...
        subprocess.call(["adb", "-s", str(adbDevice), "shell", "/data/nativetest64/VtsHalBluetoothAidlTransportTest/BT-Transport-Tests.sh",
                         str(loop), str(testfilter), str(maxappsleeptimer), str(minappsleeptimer), str(maxiterations),
                         str(maxmsgsperiteration), str(logcommands),
                         str(logevents), str(logdata), str(consolelog),
                         " > /data/nativetest64/VtsHalBluetoothAidlTransportTest/tstlog.txt 2>&1"])

def collectLogs(count):
    subprocess.call(["adb", "-s", str(adbDevice), "shell", "pkill", "logcat"])
    subprocess.call(["adb", "-s", str(adbDevice), "pull", "/data/nativetest64/VtsHalBluetoothAidlTransportTest/tstlog.txt", combinedPath])
    ...
    subprocess.call(["adb", "-s", str(adbDevice), "pull", "/data/nativetest64/VtsHalBluetoothAidlTransportTest/logcat.txt", combinedPath])
    ...
    f_count = len(open(filename).readlines())
    with open(filename) as f:
        for x in range (f_count):
            if ('Global test environment tear-down' in f.readline()):
               break
            continue
