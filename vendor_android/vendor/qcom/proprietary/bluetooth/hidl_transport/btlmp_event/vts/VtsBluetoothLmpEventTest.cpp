...
#define LOG_TAG "VtsBluetoothLmpEventTest"
...
namespace {
    static constexpr std::chrono::milliseconds kEventTimeoutMs(100000);
    static constexpr uint64_t kAddressMask = 0xFFFFFFFFFFFF;

    std::array<uint8_t, 6> GetAddress() {
        auto prop = android::base::GetUintProperty<uint64_t>(
                "persist.vendor.service.btlmpevent.address", 0x112233445566);
        prop = prop & kAddressMask;

        auto prop_p = reinterpret_cast<uint8_t*>(&prop);
        std::array<uint8_t, 6> address;
        std::copy(prop_p + 0, prop_p + 6, address.rbegin());

        return address;
    }

    AddressType GetAddressType() {
        auto prop = android::base::GetProperty(
                "persist.vendor.service.btlmpevent.address_type", "p");

        if (prop == "p" || prop == "P") {
            return AddressType::PUBLIC;
        } else if (prop == "r" || prop == "R") {
    ...
    std::string GetSocType() {
        return android::base::GetProperty("persist.vendor.qcom.bluetooth.soc", "");
    }
}
    ...
    class BluetoothLmpEventCallback : public BnBluetoothLmpEventCallback {
        public:
            ...
            ::ndk::ScopedAStatus onEventGenerated(const Timestamp& timestamp, AddressType address_type,
                ...
                std::stringstream address_hex;
                for (auto i: address) {
                    address_hex << std::setw(2) << std::hex << static_cast<int>(i) << " ";
                }
                ALOGD("%s: Address: %s", __func__, address_hex.str().c_str());
                std::cout << "Address received: " << address_hex.str() << "\n";
                if (direction == Direction::TX) {
                    ALOGD("%s: Transmitting", __func__);
                    std::cout << "Direction: Transmitting\n";
                } else if (direction == Direction::RX) {
                    ALOGD("%s: Receiving", __func__);
                    std::cout << "Direction: Receiving\n";
                }
                ...
                if (lmp_event_id == LmpEventId::CONNECT_IND) {
                    ALOGD("%s: Connection indication", __func__);
                    std::cout << "Connection indication\n";
                } else if (lmp_event_id == LmpEventId::LL_PHY_UPDATE_IND) {
                    ALOGD("%s: PHY update indication", __func__);
                    std::cout << "PHY update indication\n";
                }

                ALOGD("%s: time: %ld counter value: %x", __func__, timestamp.bluetoothTimeUs, conn_event_counter);
...
TEST_P(BluetoothLmpEventTest, RegisterAndReceive) {
    if (GetSocType() != "ganges") {
        ALOGD("%s: Unsupported Platform", __func__);
    ...
    std::cout << "\n";
    std::cout << "AddressType: " << static_cast<int>(address_type) << "\n";
