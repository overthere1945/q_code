...
TEST_P(BtTpiAidl, TpiComboTest) {
   ...
   for(int i = 0; i < maxIter; i++) {
...

     {
       ...
       data.push_back(7);
       ALOGI("=====setTpiParamsRsp with HCI_VS_TX_CONTROL_ENABLE_TX ===iter(%d)===", i);

       status = iBtTpi_->setTpiParamsRsp(0x07, data, tpiStatusRsp_cb, &status_value);
       EXPECT_TRUE(status.isOk());
       ALOGI("wait for StatusRsp Callback");
       ASSERT_TRUE(tpiStatusRsp_cb->waitForStatusRspEvent(timeout));
     }
     ...
     {
       std::vector<uint8_t> data;
       ALOGI("=====setTpiParamsRsp HCI_VS_TX_CONTROL_DISABLE_TX ===iter(%d)===", i);

       status = iBtTpi_->setTpiParamsRsp(0x08, data, tpiStatusRsp_cb, &status_value);
       EXPECT_TRUE(status.isOk());
       ALOGI("wait for StatusRsp Callback");
       ASSERT_TRUE(tpiStatusRsp_cb->waitForStatusRspEvent(timeout));
     }
     ...
     ALOGI("=======TRAFFIC PROFILE===iter(%d)===", i);
     std::vector<uint8_t> data2;
     data2.push_back(0x0A); //uint8_t version
     data2.push_back(0x01); //uint8_t enable
     status = iBtTpi_->setTpiParams(0x01, data2, tpistatus_cb, &status_value);
     EXPECT_TRUE(status.isOk());
     ALOGI("Traffic Profile Data Size:%d, wait for Status Callback", data2.size());
     ASSERT_TRUE(tpistatus_cb->waitForStatusEvent(timeout));
     if (status_value == 0)
       ALOGV("Traffic profile Test case succeed");
     else
      ALOGI("Traffic Profile Test case failed");
