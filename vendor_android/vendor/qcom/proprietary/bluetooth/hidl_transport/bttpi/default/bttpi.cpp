...
#define HCI_VS_BT_TRAFFIC_PROFILE_OPCODE (0x00DB | HCI_GRP_VENDOR_SPECIFIC)
...
std::shared_ptr<IBtTpiEventsCb> eventCb;
...
AIBinder_DeathRecipient* deathObj;
...
void BtTpi::sendFragment(int8_t* retValPtr) {
  ...
  uint16_t opcode;
  if (pendingReq == HCI_VS_BT_TRAFFIC_PROFILE_COLLECTION_CFG)
    opcode = HCI_VS_BT_TRAFFIC_PROFILE_OPCODE;
  else
    opcode = (HCI_GRP_VENDOR_SPECIFIC | HCI_VS_LINK_POWER_CTRL_REQ_OPCODE);
...
::ndk::ScopedAStatus BtTpi::setTpiParamsRsp(int32_t in_reqType,
  ...
  *_aidl_return = TPI_CMD_STATUS_SUCCESS;
  ...
  if (in_reqType == 0x07 || in_reqType  == 0x08){
    uint16_t opcode = (HCI_GRP_VENDOR_SPECIFIC | HCI_VS_LINK_POWER_CTRL_REQ_OPCODE);
    std::vector<uint8_t> data;
    data.push_back((uint8_t)opcode);
    data.push_back((uint8_t)(opcode >> 8));
    data.push_back(1);//length of remaning  data
    pendingReq = (uint8_t)in_reqType;
    data.push_back((uint8_t)pendingReq);//sub opcode
    nextSeqNum = 0;
    setCmdPendingStatus(true);
          ...
          case HCI_PACKET_TYPE_EVENT: {
            ...
            if (getCmdPendingStatus()) {
              if (pendingReq == HCI_VS_STX_FEATURE_SUPPORT_REQ && statusRspCb != nullptr) {
                 ...
                 statusRspCb = nullptr;
              } else if ((statusRspCb != nullptr) && (remainingLen == 0)) {
                 rspEvent.clear();
              ...
              if (remainingLen > 0) {
                ...
                if (write_fd_ != -1) {
                  ...
                  if (Notifythread(SEND_BTTPI_DATA) < 0) {
                    ALOGI("%s: Abort sending of remaining fragments due to write failure", __func__);
      ...
      [this](HciPacketType type,
        ...
        switch ((uint8_t)eventType) {
          ...
          case HCI_VS_COEX_STX_BT_MAX_PWR_IND:
            ALOGD("%s: TPI Max Pwr Report, dataLen-%d", __func__, dataLen);
            if (eventCb)
              eventCb->sendTpiEvent(eventType, asyncData);
            break;
          case HCI_VS_COEX_STX_BT_STATE_IND:
            ...
            if (eventCb)
              eventCb->sendTpiEvent(eventType, asyncData);
            break;
          case HCI_VS_COEX_STX_V3_BT_NE_REPORT_IND:
            ALOGD("%s: TPI Async V3 Bt Ne Report, dataLen-%d", __func__, dataLen);
            if (eventCb)
              eventCb->sendTpiEvent(eventType, asyncData);
            break;
          case HCI_VS_BT_TRAFFIC_PROFILE_REPORT:
            ALOGD("%s: TPI Async Traffic Profile Report, dataLen-%d", __func__, dataLen);
            if (eventCb)
              eventCb->sendTpiEvent(eventType, asyncData);
            break;
          default:
...
::ndk::ScopedAStatus BtTpi::UnRegisterTpiEventsCallback(int8_t* _aidl_return) {
  ...
  if (deathObj != nullptr) {
    ...
    AIBinder_DeathRecipient_delete(deathObj);
    deathObj = nullptr;
  }
