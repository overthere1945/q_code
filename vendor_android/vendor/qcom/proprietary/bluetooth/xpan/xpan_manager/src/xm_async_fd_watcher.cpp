...
#define LOG_TAG "vendor.qti.xpan@1.0-xm_async_fd_watcher"
...
int XMAsyncFdWatcher::WatchFdForNonBlockingReads(
  int fd1, int fd2, const ReadCallback& on_read_fd_ready_callback)
{
  // Add file descriptor and callback
  {
    std::unique_lock<std::mutex> guard(internal_mutex_);
    watched_fds_[fd1] = on_read_fd_ready_callback;
    watched_fds_[fd2] = on_read_fd_ready_callback;
  }
...
void XMAsyncFdWatcher::StopWatchingFileDescriptors()
{
  StopThread();
}

XMAsyncFdWatcher::XMAsyncFdWatcher()
{
...
int XMAsyncFdWatcher::TryStartThread()
{
...
int XMAsyncFdWatcher::StopThreadRoutine()
{
...
int XMAsyncFdWatcher::StopThread()
{
  if (!std::atomic_exchange(&running_, false)) return 0;
...
int XMAsyncFdWatcher::NotifyThread()
{
  uint8_t buffer[] = { 0 };
...
void XMAsyncFdWatcher::ThreadRoutine()
{
  ...
  ALOGE("%s: End of XMAsyncFdWatcher::ThreadRoutine", __func__);
}
