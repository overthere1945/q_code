		                              ...
		                              uint32_t len, uint8_t *buf)
{
  ...
  switch (opcode) {
    ...
    delete []cmd;
    break;
    } case BTM_CP_CTRL_TWT_EST_RSP: {
    ALOGI("%s: Received BTM_CP_CTRL_TWT_EST_RSP", __func__);
    CHECK(BTM_TWT_EST_RSP_LEN== (len + BTM_PKT_HEADER_LEN));
    cmd = (uint8_t *)new uint8_t[len + BTM_PKT_HEADER_LEN];
    UINT32_TO_STREAM(cmd, BTM_BTFMCODEC_USECASE_START_RSP);
    UINT32_TO_STREAM(cmd + BTM_OPCODE_LEN, len);
    ...
    XpanManager::Get()->PostMessage(msg);
    break;
  } case BTM_CP_LOG_EVENT_IND:
    case BTM_CP_TIME_SYNC_IND: {
    CpLogEventAndTimestamp(opcode, (xm_ipc_msg_t *)buf);
    break;
  } case BTM_CP_CTRL_REMOTE_PARAMS_RSP: {
    ALOGI("%s: Received BTM_CP_CTRL_REMOTE_PARAMS_RSP", __func__);
    CHECK(BTM_CP_CTRL_REMOTE_PARAMS_RSP_LEN == (len + BTM_PKT_HEADER_LEN));
    uint8_t status;
    STREAM_TO_UINT8(status, buf);
    WARUseCaseRsp((RspStatus)status);
    break;
  } default:
...
void XMPacketizer::OnDataReady(int fd)
{
  ...
  if (bytes_read == -1) {
    ...
    for (i = BTM_PKT_HEADER_LEN; i < BTM_PKT_HEADER_LEN + len ; ++i) {
		                              ...
		                              uint32_t len, uint8_t *buf)
{
  ...
  switch (opcode) {
    ...
    msg->KpAudioBearerRsp.current_transport = buf[0];
    msg->KpAudioBearerRsp.status = buf[1];
    XpanManager::Get()->PostMessage(msg);
    break;
    ...
    break;
  } case BTM_BTFMCODEC_PORT_STATE_IND: {
    uint8_t port_state;
    CHECK(BTM_KP_PORT_STATE_IND_LEN == (len + BTM_PKT_HEADER_LEN));
    STREAM_TO_UINT8(port_state, buf);
    ALOGI("%s: Received port state indications with port state:%u", __func__, port_state);
    if (port_state == 0) {
      xm_ipc_msg_t *msg = (xm_ipc_msg_t *) malloc(XM_IPC_MSG_SIZE);
      if( msg == NULL)
      {
      ...
      msg->PortState.eventId = KP_XM_PORT_STATE_IND;
      msg->PortState.port_state = port_state;
      XpanManager::Get()->PostMessage(msg);
    } else {
      ALOGI("%s unhandled port state %d", __func__, port_state);
    }
    break;
  } case BTM_BTFMCODEC_USECASE_START_REQ: {
    uint8_t transport;
    uint8_t stream_id;
    CHECK(BTM_USECASE_START_LEN == (len + BTM_PKT_HEADER_LEN));
    STREAM_TO_UINT8(transport, buf);
    STREAM_TO_UINT8(stream_id, buf);
    ALOGI("%s: Received usecase start indication message, transport=%d, stream_id=%d",
          __func__, transport, stream_id);
    if (transport == 2) {
      xm_ipc_msg_t *msg = (xm_ipc_msg_t *) malloc(XM_IPC_MSG_SIZE);
      if( msg == NULL)
      {
...
void XMPacketizer::WARUseCaseRsp(RspStatus status)
{
  uint8_t data = (uint8_t)(status & 0x000000FF);
  decode_and_dispatch(BTM_BTFMCODEC_USECASE_START_RSP, 1, &data); 
}


/*  This method will receives the events/erros and timestamp
 *  from the connection proxy and will print them in logcat
 *  log to enable basic debugging from logcat when QXDM logs
 *  are not available.
 */
void XMPacketizer::CpLogEventAndTimestamp(uint32_t opcode, xm_ipc_msg_t *msg)
{
  struct timespec ts;
  bool flag = 1;

  // Implementaiton of the print mechanism depending on msg type
  ALOGI("%s ", __func__);
  if (clock_gettime(CLOCK_MONOTONIC, &ts) == -1)
  {
    ALOGE("%s: failed to get the time clock.", __func__);
    flag = 0;
  }
  ...
  {
    case BTM_CP_LOG_EVENT_IND: {
      /* Print the timestamp here */
      ALOGI("%s: Module Instance ID : %d", __func__, msg->CpLogEventInd.ModuleInstId);
      ALOGI("%s: is_sink : %s", __func__, (msg->CpLogEventInd.is_sink ? "Sink" : "Source"));
      ALOGI("%s: Sub Module Identifier : %d", __func__, msg->CpLogEventInd.SubModuleId);
      ALOGI("%s: Log Event : %d", __func__, msg->CpLogEventInd.LogEventId);
      if (flag)
        ALOGI("%s: TS kernel %lu:%lu", __func__, ts.tv_sec, ts.tv_nsec);
      break;
    } case BTM_CP_TIME_SYNC_IND: {
      /* Print the event/error here */
      ALOGI("%s: Module Instance ID : %d", __func__, msg->CpTimeSyncInd.ModuleInstId);
      ALOGI("%s: is_sink : %s", __func__, (msg->CpTimeSyncInd.is_sink ? "Sink" : "Source"));
      ALOGI("%s: TS Event : %lu:%lu", __func__, PARSE_TO_FIRST32(msg->CpTimeSyncInd.timestamp),
                                                 PARSE_TO_SECOND32(msg->CpTimeSyncInd.timestamp));
      if (flag)
        ALOGI("%s: TS kernel %lu:%lu", __func__, ts.tv_sec, ts.tv_nsec);
      break;
    } default: {
      ALOGE("%s: Wrong event received from CP: %d.", __func__, opcode);
      break;
    }
  }
}
...
void XMPacketizer::ProcessMessage(XmIpcEventId eventId, xm_ipc_msg_t *msg)
{
  switch (eventId) {
    case DH_XM_COP_VER_IND: {
      ...
      free(msg->CoPInd.data);
      msg->CoPInd.data = NULL;
      break;
    }
    case XM_CP_COP_VER_IND: {
      ...
      if (eventId == XP_XM_TWT_SESSION_REQ) {
        // stream_id
        len += 1;
      }
      uint8_t *cmd = (uint8_t*)new uint8_t[len];
      ...
      if (eventId == XP_XM_TWT_SESSION_EST) {
        ...
        cmd[i++] = msg->TwtParams.stream_id;
        decode_and_dispatch(BTM_CP_CTRL_TWT_EST_REQ, len, cmd);
      }
      delete []cmd;
      free(params);
      break;
      ...
      int i = 0, j = 0;
      ALOGI("%s: %s total len : %d", __func__, ConvertMsgtoString(eventId), len);
      ...
      if (eventId == XAC_CP_REMOTE_PARAMS_REQ) {
        cmd[i++] = msg->CpRemoteParamsInd.stream_id;
        decode_and_dispatch(BTM_CP_CTRL_REMOTE_PARAMS_REQ, len , cmd);
      } else {
      ...
      uint8_t cmd[6];
      memcpy(cmd, &msg->RoleSwitchInd.macaddr, sizeof(macaddr_t));
      decode_and_dispatch(BTM_CP_CTRL_ROLE_SWITCH_IND, sizeof(macaddr_t), cmd);
      break;
    } case XAC_CP_BSSID_CHANGE_IND: {
      uint8_t cmd[6];
      memcpy(cmd, &msg->BssidUpdateInd.hs_ap_bssid, sizeof(macaddr_t));
      decode_and_dispatch(BTM_CP_CTRL_BSSID_UPDATE_IND, sizeof(macaddr_t), cmd);
      break;
    } default: {
      ALOGE("%s: this :%04x ipc message is not handled", __func__, eventId);
    }
  }
}
