...
void XMXprofileIf::GetWifiDrvStatus(void)
{
  ALOGD("%s", __func__);
  xm_ipc_msg_t *msg = (xm_ipc_msg_t *) malloc(XM_IPC_MSG_SIZE);
  if( msg == NULL)
  ...
  msg->eventId = XP_XM_GET_WIFI_STATUS;
  XpanManager::Get()->PostMessage(msg);
}
...
void XMXprofileIf::SetRemoteFeatureMask(bdaddr_t addr, uint64_t feature_mask)
{
  ALOGD("%s  %s  %lld", __func__, ConvertRawBdaddress(addr), feature_mask);
  xm_ipc_msg_t *msg = (xm_ipc_msg_t *) malloc(XM_IPC_MSG_SIZE);
  if( msg == NULL)
  {
    ALOGE("%s: nullptr", __func__);
  ...
  msg->RemoteFeatureMask.eventId = XP_REMOTE_FEATURE_MASK;
  msg->RemoteFeatureMask.feature_mask = feature_mask;
  memcpy(&msg->RemoteFeatureMask.bdaddr, &addr, sizeof(bdaddr_t));
  XpanManager::Get()->PostMessage(msg);
}
...
*/
  ...
  ALOGD("%s: bssid %s", __func__, msg->ApParams.bssid.toString().c_str());
...
bool XMXprofileIf::BearerPreferenceReq(Requester requester, uint8_t transport_type, uint8_t reason)
{
  ALOGI("%s: Transport type %s reason %d ", __func__, TransportTypeToString((TransportType)transport_type), reason);
  if(XpanProviderIf::GetIf())
    XpanProviderIf::GetIf()->BearerPreferenceCb((uint8_t)requester, (uint8_t) transport_type, reason);
  else
    ALOGE("Discarding %s AIDL service is not up and running", __func__);
  return true;
}
...
void XMXprofileIf::UpdateDisConnectedClients(macaddr_t left_eb, macaddr_t right_eb)
{
  xm_ipc_msg_t *msg = (xm_ipc_msg_t *) malloc(XM_IPC_MSG_SIZE);
  if( msg == NULL)
  {
  ...
  msg->eventId = XP_WIFI_DISCONNECTED_EB_DETAILS;
  ALOGI("%s: %s , %s", __func__, left_eb.macString().c_str(),right_eb.macString().c_str());

  for (int j =0; j < sizeof(macaddr_t); j++) {
        msg->WiFiDisConnectedEbs.left_eb.b[j] = left_eb.b[j];
        msg->WiFiDisConnectedEbs.right_eb.b[j] = right_eb.b[j];
  }
  XpanManager::Get()->PostMessage(msg);

}

void XMXprofileIf::TeardownTwt(macaddr_t left_eb, macaddr_t right_eb)
{
  xm_ipc_msg_t *msg = (xm_ipc_msg_t *) malloc(XM_IPC_MSG_SIZE);
  if( msg == NULL)
  {
  ...
  msg->eventId = XP_TEARDOWN_TWT;
  ALOGI("%s: %s , %s", __func__, left_eb.macString().c_str(),right_eb.macString().c_str());

  for (int j =0; j < sizeof(macaddr_t); j++) {
        msg->TeardownTwt.left_eb.b[j] = left_eb.b[j];
        msg->TeardownTwt.right_eb.b[j] = right_eb.b[j];
  }
  XpanManager::Get()->PostMessage(msg);
...
bool XMXprofileIf::UseCaseUpdate(bdaddr_t addr, UseCaseType usecase)
{
   ...
   */
  if(XpanProviderIf::GetIf())
    XpanProviderIf::GetIf()->UsecaseUpdateCb(addr, usecase);
  else
    ALOGE("Discarding %s AIDL service is not up and running", __func__);
...
bool XMXprofileIf::RegisterMdnsService(bdaddr_t addr, uint8_t state)
{
  ALOGD("%s: with for %s state: %d", __func__, ConvertRawBdaddress(addr), state);

  if(XpanProviderIf::GetIf())
    XpanProviderIf::GetIf()->MdnsRegisterUnRegister(addr, state);
  else
    ALOGE("Discarding %s AIDL service is not up and running", __func__);
...
bool XMXprofileIf::XpWifiDrvStatus(uint8_t status)
{
  ALOGD("%s: status:%d", __func__, status);
  if(XpanProviderIf::GetIf()) {
    WifiDrvStatus actual_status = (WifiDrvStatus) status;
    uint8_t xp_status = 1;
    if (actual_status == WIFI_DRIVER_SUPPORTING_XPAN)
      xp_status = 0;
    else if (actual_status == WIFI_DRIVER_NOT_SUPPORTING_XPAN)
      xp_status = 2;
    bool status = XpanProviderIf::GetIf()->OnWifiDriverStatus(xp_status);
    XpanManager::Get()->ReScheduleDrvStatus(status);
  } else
    ALOGE("Discarding %s AIDL service is not up and running", __func__);
...
bool XMXprofileIf::CountryCodeUpdate(char *code)
{
  ALOGD("%s: country:%s", __func__, code);
  if(XpanProviderIf::GetIf()) {
    XpanProviderIf::GetIf()->OnCountryCodeUpdate(code);
  } else
    ALOGE("Discarding %s AIDL service is not up and running", __func__);
...
bool XMXprofileIf::XpPossibleToCreateApIface()
{
  ALOGD("%s", __func__);
  if(XpanProviderIf::GetIf()) {
    XpanProviderIf::GetIf()->PossibleToCreateApIface();
  } else
    ALOGE("Discarding %s AIDL service is not up and running", __func__);
...
bool XMXprofileIf::XpXacBearerSwitchFailed(bdaddr_t addr, uint8_t reason, uint8_t role)
{
  ALOGD("%s", __func__);
  if(XpanProviderIf::GetIf()) {
    XpanProviderIf::GetIf()->onBearerSwitchFailed(addr, reason, role);
  } else
    ALOGE("Discarding %s AIDL service is not up and running", __func__);
...
bool XMXprofileIf::XpXacConnect(bdaddr_t addr, uint8_t reason)
{
  ALOGD("%s", __func__);
  if(XpanProviderIf::GetIf()) {
    XpanProviderIf::GetIf()->onConnect(addr, reason);
  } else
    ALOGE("Discarding %s AIDL service is not up and running", __func__);
...
void XMXprofileIf::ReUpdateTwtSessionParams(TwtParameters *TwtParams, uint8_t stream_id)
{
  ...
  ALOGD("%s, stream_id=%d", __func__, stream_id);
  if (num_devices) {
  ...
  UpdateTWTParams(rightOffset, periodicity, num_devices, twtparams, true, stream_id);
}
...
{
  UpdateTWTParams(rightOffset, periodicity, num_devices, twt_params, false, 0);
}

void XMXprofileIf::UpdateTWTParams(uint32_t rightOffset, uint8_t periodicity,
                      uint8_t num_devices, std::vector<tXPAN_Twt_Session_Params> twt_params, bool type,
                      uint8_t stream_id)
{
  ...
  if (twtconfig_prop_read == TWT_CONFIG_ENABLE) {
    ...
    if (type == false)
      ...
      msg->TwtParams.eventId = XP_XM_TWT_SESSION_REQ;
    ...
    msg->TwtParams.stream_id = stream_id;
...
void XMXprofileIf::XpBearerPreferenceReq(bdaddr_t bdaddr, uint8_t transport,
                             uint8_t requestor, uint8_t reason, bool urgency)
{
  xm_ipc_msg_t *msg = (xm_ipc_msg_t *) malloc(XM_IPC_MSG_SIZE);

  ALOGI("%s for bdaddr_t:%s with transport: %d reason :%d ", __func__,
        ConvertRawBdaddress(bdaddr), transport, reason);
  if( msg == NULL)
  {
    ALOGE("%s: nullptr", __func__);
    return;
  ...
  msg->BearerPreference.reason = reason;
  msg->BearerPreference.urgency = urgency;
  XpanManager::Get()->PostMessage(msg);
}
...
void XMXprofileIf::ProcessMessage(XmIpcEventId eventId, xm_ipc_msg_t *msg)
{
  ...
  switch (eventId) {
    case QHCI_XM_REMOTE_SUPPORT_XPAN: {
	  ...
	  BearerPreferenceReq(QHCI, msg->QhciBearerPreference.transport, msg->QhciBearerPreference.reason);
      break;
    } case QHCI_XM_USECASE_UPDATE: {
      QhciXmUseCase UseCase = msg->UseCase;
      bdaddr_t bdaddr = UseCase.bdaddr;
      UseCaseUpdate(bdaddr, UseCase.usecase);
      break;
      ...
      RegisterMdnsService(msg->MdnsReq.bdaddr, msg->MdnsReq.status);
      break;
      ...
      XpWifiDrvStatus(msg->DrvState.status);
      break;
    } case WIFI_XM_COUNTRY_CODE_UPDATE: {
      CountryCodeUpdate(msg->CcUpdate.code);
      delete msg->CcUpdate.code;
      break;
    } case WIFI_XM_POSSIBLE_TO_CREATE_AP_IFACE: {
      XpPossibleToCreateApIface();
      break;
    } case XAC_XP_BEARER_SWITCH_FAILED_RSP: {
      XpXacBearerSwitchFailed(msg->xacBearerFailed.bdaddr, msg->xacBearerFailed.reason,
          msg->xacBearerFailed.role);
      break;
    } case XAC_INITIATE_BLUETOOTH_CONNECTION_IND: {
      XpXacConnect(msg->xacBearerFailed.bdaddr, msg->xacBearerFailed.reason);
      break;
    } default: {
...
void XMXprofileIf::Initialize(void)
{
  XpanProviderCb = {
    ...
    .update_disconnected_clients = UpdateDisConnectedClients,
    .teardown_twt = TeardownTwt,
	...
	.get_wifi_driver_status = GetWifiDrvStatus,
    .set_remote_feature_mask = SetRemoteFeatureMask,
  };
