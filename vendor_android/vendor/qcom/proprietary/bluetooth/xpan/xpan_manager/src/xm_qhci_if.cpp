 ...
 *
 * Description      This API is invoked by QHCI for bearer change.
 *
 * Parameters:      bdaddr_t - Bluetooth address of the remote.

 *                  TransportType   - Bearer type

 *                   reason - reson for bearer preference
 *
 * Return:          bool - return the status wether the message is
 *                  queued to process by XM.
 ******************************************************************/
bool XMQhciIf::BearerPreferenceReq(bdaddr_t addr, TransportType transport,  uint8_t reason)
{
  xm_ipc_msg_t *msg = (xm_ipc_msg_t *) malloc(XM_IPC_MSG_SIZE);

  ALOGI("%s with bd address %s and transport: %d  reason %d ", __func__,
        ConvertRawBdaddress(addr), transport,reason );
  if( msg == NULL)
  {
    ALOGE("%s: nullptr", __func__);
    return false;
  }
  msg->QhciBearerPreference.eventId = QHCI_XP_BEARER_PREFERENCE_REQ;
  memcpy(&msg->QhciBearerPreference.bdaddr, &addr, sizeof(bdaddr_t));
  msg->QhciBearerPreference.transport = transport;
  msg->QhciBearerPreference.reason = 0;
  return XpanManager::Get()->PostMessage(msg);
}
 ...
 *                  queued to process by XM.
 ******************************************************************/
bool XMQhciIf::UseCaseUpdate(bdaddr_t addr, UseCaseType usecase)
{
  xm_ipc_msg_t *msg = (xm_ipc_msg_t *) malloc(XM_IPC_MSG_SIZE);
  ...
  memcpy(&msg->UseCase.bdaddr, &addr, sizeof(bdaddr_t));
...
{
  int i = 0, total_len = num_limit * XM_ENCODER_LIMIT_PARAMS_SIZE;
  ...
  if(!msg->EncoderLimitParams.data) {
    ALOGE("%s: failed allocate the heap to store the data", __func__);
    return false;
  }
...
bool XMQhciIf::isLeLinkconnected(bdaddr_t bdAddr)
{
  return qhci_xm_if.isLeLinkconnected(bdAddr);
}

bool XMQhciIf::isLeTransportReadyForSwitch()
{
  return qhci_xm_if.isLeTransportReadyForSwitch();
}

bool XMQhciIf::isStreamingActive(bdaddr_t bdAddr)
{
  return qhci_xm_if.isStreamingActive(bdAddr);
}

bool XMQhciIf::BearerPreferenceFailed(bdaddr_t bd_addr,
                              TransportType transport_type,
                              uint8_t status)
{
  qhci_xm_if.BearerPreferenceFailed(bd_addr, transport_type, status);
  return true;
}


bool XMQhciIf::UpdateSapInterface(uint8_t state, uint8_t status)
{
  ALOGD("%s  %d  %d ", __func__, state, status);
  qhci_xm_if.SapEnabledStatus(state, status);
  return true;
}
...
void XMQhciIf::ProcessMessage(XmIpcEventId eventId, xm_ipc_msg_t *msg)
{
  switch (eventId) {
    case XP_XM_TRANSPORT_ENABLED: {
      ...
      UpdateSapInterface(msg->CreateSapInfStatusParams.req_state,
                         msg->CreateSapInfStatusParams.status);
      break;
    } default: {
