...
WifiDrvStatus wifi_vndr_status;
macaddr_t ap_mac_addr;
xpan_wifi_status wifi_lib_status;
bool cookie_wait = false;
std::condition_variable cookie_wait_cv;
...
static inline char * WiFiStatus(xpan_wifi_status type)
{
  ...
  else if (type == XPAN_WIFI_STATUS_FAILURE_EEXIST)
    return "XPAN_WIFI_STATUS_FAILURE_EEXIST";
  else if (type == XPAN_WIFI_STATUS_FAILURE_NOT_SUPPORTED)
    return "XPAN_WIFI_STATUS_FAILURE_NOT_SUPPORTED";
  else if (type == XPAN_WIFI_STATUS_FAILURE_DRIVER_NOT_READY)
    return "XPAN_WIFI_STATUS_FAILURE_DRIVER_NOT_READY";
  else if (type == XPAN_WIFI_STATUS_SUCCESS_DRIVER_INITIALIZED)
    return "XPAN_WIFI_STATUS_SUCCESS_DRIVER_INITIALIZED";
  else
...
static inline char *TransportSwitchReason(uint8_t reason)
{
  if (reason == TRANSPORT_SWITCH_REASON_UNSPECIFIED)
    return "TRANSPORT_SWITCH_REASON_UNSPECIFIED";
  else if(reason == TRANSPORT_SWITCH_REASON_TERMINATING)
    return "TRANSPORT_SWITCH_REASON_TERMINATING";
  else
...
void cb_country_code_update(char *);
...
struct wifi_callbacks xm_callback = {cb_wifi_acs_results,
                                     ...
                                     cb_driver_ready_event,
                                     cb_possible_to_create_ap_iface};
...
void cb_wifi_audio_transport_switch(transport_switch_type type,
        transport_switch_status status, transport_switch_reason reason)
{
  XMWifiIf::WifiAudioTransportSwtich((uint8_t)type, (uint8_t)status, (uint8_t)reason);
}
...
void cb_country_code_update(char * code)
{
  XMWifiIf::CbCountryCodeUpdate(code);
}

void cb_possible_to_create_ap_iface()
{
  XMWifiIf::CbPossibleToCreateApIface();
}

void cb_driver_ready_event(xpan_wifi_status status)
{
  XMWifiIf::CbDrvreadyEvent((uint8_t)status);
}
...
static inline uint16_t getcookie(bdaddr_t addr)
{
  ...
  for (auto& it : bdaddr_mapper) {
     if (!memcmp((const char *)&addr, (const char *)&it.second, sizeof(bdaddr_t))) {
       cookie = it.first;
...
void XMWifiIf:: WifiAudioTransportSwtich(uint8_t type, uint8_t status, uint8_t reason)
{
  ...
  ALOGI("%s type %s with status:%s reason:%s", __func__, TransportSwitchType(type),
    TransportSwitchStatus(status),TransportSwitchReason(reason) );
  xm_ipc_msg_t *msg = (xm_ipc_msg_t *) malloc(XM_IPC_MSG_SIZE);
  if( msg == NULL)
  {
    ALOGE("%s: nullptr", __func__);
  ...
  msg->WiFiTransportSwitch.reason = reason;
  XpanManager::Get()->PostMessage(msg);
}
...
void XMWifiIf:: CbSapPowerState(void *data)
{
  ...
  {
    std::unique_lock<std::mutex> lck(cookie_id_mtx);
    if (cookie_wait == false) {
      cookie_wait_cv.wait_for(lck, std::chrono::milliseconds(500), []
                               {return cookie_wait;}); 
    }
  }
  uint8_t dialog_id = getDialogId(cookie);
  if (!dialog_id) {
    ALOGE("%s: dialog_id is missing for cookie %u.discarding here",
	  __func__, cookie);
    return;
  }
...
void XMWifiIf:: CbCountryCodeUpdate(char *code)
{
  int len = strlen(code);
  if (len <= 0) {
    ALOGE("%s not a valid code", __func__);
    return;
  }
  ALOGI("%s country code:%s", __func__, code);
  ...
  msg->eventId = WIFI_XM_COUNTRY_CODE_UPDATE;
  msg->CcUpdate.code = (char *)new char[len+1];
  int i; 
  for (i = 0; i < len; i++) {
    msg->CcUpdate.code[i] = code[i];
  }
  code[i] = '\0';
  ALOGE("%s: len %d i %d code %s", __func__, len, i, msg->CcUpdate.code);
  XpanManager::Get()->PostMessage(msg);
}

void XMWifiIf:: CbDrvreadyEvent(uint8_t wifi_status)
{
  xpan_wifi_status status = (xpan_wifi_status) wifi_status; 
  
  ALOGI("%s: event %s", __func__, WiFiStatus(status));
  if (wifi_vndr_status == WIFI_DRIVER_SUPPORTING_XPAN) {
    ALOGI("%s: skipping this notification as wifi supports xpan", __func__);
    return;
  } else if (status == XPAN_WIFI_STATUS_SUCCESS_DRIVER_INITIALIZED) {
    {
      std::unique_lock<std::mutex> lck(cookie_id_mtx);
      wifi_vndr_status = WIFI_DRIVER_ACTIVE;
    }
    if (callback && callback->get_xpan_supported) {
      uint8_t current_status;
      ALOGI("%s: reading whether wifi support xpan", __func__);
      status = callback->get_xpan_supported(wifi_if_data);
      ALOGI("%s: wifi replied by %s", __func__, WiFiStatus(status));
      {
        std::unique_lock<std::mutex> lck(cookie_id_mtx);
        if (status == XPAN_WIFI_STATUS_FAILURE_NOT_SUPPORTED) {
          wifi_vndr_status = WIFI_DRIVER_NOT_SUPPORTING_XPAN; 
        }
        else if (status == XPAN_WIFI_STATUS_SUCCESS) {
          wifi_vndr_status = WIFI_DRIVER_SUPPORTING_XPAN;
          if (callback->wifi_delete_ap_iface)
            callback->wifi_delete_ap_iface(wifi_if_data, NULL);
        } else
	  wifi_vndr_status = WIFI_DRIVER_NOT_SUPPORTING_XPAN;
      }
      xm_ipc_msg_t *msg = (xm_ipc_msg_t *) malloc(XM_IPC_MSG_SIZE);
      ...
      msg->eventId = WIFI_XM_DRV_STATE;
      msg->DrvState.status = (uint8_t)wifi_vndr_status;
      XpanManager::Get()->PostMessage(msg);
    } 
  } else {
    ALOGE("%s: unhandled status", __func__);
  }
}
...
void XMWifiIf:: CbPossibleToCreateApIface()
{
  ALOGI("%s ", __func__);
  xm_ipc_msg_t *msg = (xm_ipc_msg_t *) malloc(XM_IPC_MSG_SIZE);
  if( msg == NULL)
  ...
  msg->eventId = WIFI_XM_POSSIBLE_TO_CREATE_AP_IFACE;
  XpanManager::Get()->PostMessage(msg);
}
...
void XMWifiIf::UpdateWifiDrvStatus(void)
{
  xm_ipc_msg_t *msg = (xm_ipc_msg_t *) malloc(XM_IPC_MSG_SIZE);
 ...
 ******************************************************************/
WifiDrvStatus XMWifiIf::InitWifiIf()
{
  xpan_wifi_status status;

  ALOGI("%s: initializing wifi vendor lib", __func__);

  wifi_vndr_status = WIFI_DRIVER_INIT_FAILED;
  ...
  if (callback->init_xpan_wifi_lib) {
    status = callback->init_xpan_wifi_lib(&wifi_if_data);
    ALOGI("%s: init wifi vendor lib with status %s", __func__, WiFiStatus(status));
    if (status == XPAN_WIFI_STATUS_SUCCESS)
        wifi_vndr_status = WIFI_DRIVER_ACTIVE;
    else if (status == XPAN_WIFI_STATUS_FAILURE_DRIVER_NOT_READY)
      wifi_vndr_status = WIFI_DRIVER_NOT_ACTIVE;
    else {
  ...
  if (status == XPAN_WIFI_STATUS_SUCCESS) {
    status = callback->get_xpan_supported(wifi_if_data);
    ALOGI("%s: is xpan feature supported %s", __func__, WiFiStatus(status));
    if (status == XPAN_WIFI_STATUS_SUCCESS) {
      wifi_vndr_status = WIFI_DRIVER_SUPPORTING_XPAN;
  ...
  UpdateWifiDrvStatus();
  if (wifi_vndr_status == WIFI_DRIVER_NOT_SUPPORTING_XPAN) {
    ALOGI("%s: as xpan is not supporting stopping unregistering with wifi lib", __func__);
...
bool XMWifiIf::DeInitWifiIf()
{
  ...
  pthread_t wifi_thread_id = xm_wifi_if_thread.native_handle();
  pthread_t this_thread_id = pthread_self();
  ALOGD("%s: wifi_thread_id %u this_thread_id %u", __func__, wifi_thread_id, this_thread_id);

  if (wifi_thread_id != this_thread_id && xm_wifi_if_thread.joinable()) {
   ALOGD("%s: joining XM WiFI If thread", __func__);
   xm_wifi_if_thread.join();
   ALOGI("%s: joined XM WiFI If thread", __func__);
  } else if (wifi_thread_id == this_thread_id) {
    ALOGW("%s: not cleaning up xm main thread as caller and calling TID are same", __func__);
  }

  /* acquire lock here to ensure no other threads are processing
   * data further
   */
  std::unique_lock<std::mutex> lck(xm_wifi_if_mtx);
  while(!xm_wifi_if_workqueue.empty()) {
...
{
  {
    std::unique_lock<std::mutex> lck(cookie_id_mtx);
    if (wifi_vndr_status == WIFI_DRIVER_NOT_SUPPORTING_XPAN) {
      ALOGE("%s: xpan is not supporting by wifi, returning from here", __func__);
      return false;
    }
  }
  ...
  if (wifi_vndr_status == WIFI_DRIVER_INIT_FAILED) {
    ALOGE("%s: retrying to start wifi vendor interface", __func__);
    if (InitWifiIf() == WIFI_DRIVER_INIT_FAILED) {
  ...
  switch (eventId) {
    ...
    case XM_WIFI_USECASE_UPDATE: {
      ...
      if (!memcmp((const char *)&msg->ApParams.ap_mac_address,
          (const char *)&ap_mac_addr, sizeof(macaddr_t))) {
        ALOGD("%s: no change in ap macaddr %s  xm holding mac add %s",
	      __func__, msg->ApParams.ap_mac_address.toString().c_str(),
	      ap_mac_addr.toString().c_str());
        free(wifi_if_msg);
        return true;
      } else if (!memcmp((const char *)&ap_mac_addr,
      ...
      break;
        ...
        wifi_if_msg->WiFiDisConnectedEbs.eventId = msg->WiFiDisConnectedEbs.eventId;
        memcpy(&wifi_if_msg->WiFiDisConnectedEbs.left_eb, &msg->WiFiDisConnectedEbs.left_eb,
            sizeof(macaddr_t));
        memcpy(&wifi_if_msg->WiFiDisConnectedEbs.right_eb, &msg->WiFiDisConnectedEbs.right_eb,
            sizeof(macaddr_t));
        break;
      } case XP_TEARDOWN_TWT: {
          wifi_if_msg->TeardownTwt.eventId = msg->TeardownTwt.eventId;
          memcpy(&wifi_if_msg->TeardownTwt.left_eb, &msg->TeardownTwt.left_eb,
              sizeof(macaddr_t));
          memcpy(&wifi_if_msg->TeardownTwt.right_eb, &msg->TeardownTwt.right_eb,
              sizeof(macaddr_t));
          break;
        } default :{
...
void XMWifiIf::UpdateDisConnectedEbDetails(xm_ipc_msg_t *msg)
{
  xpan_wifi_status ret = XPAN_WIFI_STATUS_FAILURE;
  if (callback) {
    wifi_if_data->cmd_iface = IFACE_TYPE_SAP;
    if (!msg->WiFiDisConnectedEbs.left_eb.isEmpty()) {
      ret = callback->xpan_peer_disconnect(wifi_if_data, msg->WiFiDisConnectedEbs.left_eb.b);
    } if (!msg->WiFiDisConnectedEbs.right_eb.isEmpty()) {
      ret = callback->xpan_peer_disconnect(wifi_if_data, msg->WiFiDisConnectedEbs.right_eb.b);
    }
  }
...
void XMWifiIf::TeardownTwt(xm_ipc_msg_t *msg)
{
  xpan_wifi_status ret = XPAN_WIFI_STATUS_FAILURE;
  if (callback) {
    wifi_if_data->cmd_iface = IFACE_TYPE_SAP;
    if (!msg->TeardownTwt.left_eb.isEmpty()) {
      ret = callback->xpan_twt_teardown(wifi_if_data, msg->TeardownTwt.left_eb.b);
    } if (!msg->TeardownTwt.right_eb.isEmpty()) {
      ret = callback->xpan_twt_teardown(wifi_if_data, msg->TeardownTwt.right_eb.b);
    }
  }
...
void XMWifiIf::CreateSapInterface(xm_ipc_msg_t *msg)
{
  ...
  if(callback && state) {
    ...
    wifi_if_data->cmd_iface = IFACE_TYPE_SAP;
    status = callback->wifi_delete_ap_iface(wifi_if_data, NULL);
    if (status == XPAN_WIFI_STATUS_SUCCESS) {
  ...
  switch (eventId) {
    case QHCI_XM_USECASE_UPDATE: {
      ...
      break;
        ...
        UpdateDisConnectedEbDetails(msg);
        break;
    } case XP_TEARDOWN_TWT : {
        TeardownTwt(msg);
        break;
