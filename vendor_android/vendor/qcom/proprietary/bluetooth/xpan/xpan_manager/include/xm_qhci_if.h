...
class XMQhciIf
{
 ...
 bool UpdateSapInterface(uint8_t state, uint8_t status);
 bool isStreamingActive(bdaddr_t bd_addr);
 bool isLeLinkconnected(bdaddr_t bdAddr);
 bool isLeTransportReadyForSwitch();
 bool BearerPreferenceReq(bdaddr_t, TransportType, uint8_t);
 bool BearerPreferenceFailed(bdaddr_t bd_addr,
                TransportType transport_type, uint8_t status);
 private:
