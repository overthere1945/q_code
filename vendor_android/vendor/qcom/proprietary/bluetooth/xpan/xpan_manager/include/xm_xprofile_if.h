...
class XMXprofileIf
{
  public:
    ...
    void ReUpdateTwtSessionParams(TwtParameters *, uint8_t);
    bool UseCaseUpdate(bdaddr_t, UseCaseType);
    bool BearerPreferenceReq(Requester, uint8_t, uint8_t);
  ...
  private:
    ...
    static void UpdateTWTParams(uint32_t, uint8_t, uint8_t,
                    std::vector<tXPAN_Twt_Session_Params>, bool, uint8_t);
#endif
    static void XpBearerPreferenceReq(bdaddr_t, uint8_t, uint8_t, uint8_t, bool urgency = false);
    ...
    static void UpdateDisConnectedClients(macaddr_t, macaddr_t);
    static void TeardownTwt(macaddr_t, macaddr_t);
    bool ChannelSwitchStarted(xm_ipc_msg_t *);
    static void UpdateWifiScanStarted(bdaddr_t, uint8_t);
    static void GetWifiDrvStatus(void);
    bool XpWifiDrvStatus(uint8_t);
    bool CountryCodeUpdate(char *);
    bool XpPossibleToCreateApIface();
    bool XpXacBearerSwitchFailed(bdaddr_t addr, uint8_t reason, uint8_t role);
    bool XpXacConnect(bdaddr_t addr, uint8_t state);
    static void SetRemoteFeatureMask(bdaddr_t addr, uint64_t feature_mask);
};
