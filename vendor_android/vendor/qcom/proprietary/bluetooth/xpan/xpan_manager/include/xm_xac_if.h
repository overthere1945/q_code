...
class XMXacIf
{
 public:
   ...
   bool PrepareAudioBearerReq(bdaddr_t, TransportType, bool urgency = false);
   bool BearerPreferenceReq(bdaddr_t, TransportType, bool urgency = false);
   bool CheckIpDetailsAvailibility (bdaddr_t bdaddr, TransportType type, bool urgency = false);
   bool InitiateLmpBearerSwitch(bdaddr_t, TransportType);
   bool BearerSwitchInd(bdaddr_t, TransportType, RspStatus, uint8_t reason = 0);
   bool UpdateApDetails(uint8_t);
   bool FetchAndUpdateIpDetailstoCp(bdaddr_t addr, uint32_t usage, void *context);
   bool IpDetailsFetched(uint32_t usage, void *context,
   ...
   bool RegisterMdnsService(bdaddr_t, uint8_t);
   ...
   bool RoleSwitchInd(macaddr_t);
   bool BearerSwitchFailedRsp(bdaddr_t addr, uint8_t reason, uint8_t role);
   bool UnPauseCompleted(TransportType type);
   bool InitiateBluetoothConnection(bdaddr_t addr, uint8_t reason);
   bool isTransportReadyForSwitch(bdaddr_t addr);
 private:
   ...
   bool UpdateRemoteEbDetailstoCp(bool encryption, uint8_t *psk, uint8_t *identity,
                            macaddr_t hs_ap_bssid, macaddr_t hs_mac_addr,
                            ipaddr_t hs_ip_addr,
                            uint32_t center_freq,
                            uint16_t time_sync_tx_port,
   ...
   bool SetRemoteFeatureMask(bdaddr_t addr, uint64_t featre_mask);
