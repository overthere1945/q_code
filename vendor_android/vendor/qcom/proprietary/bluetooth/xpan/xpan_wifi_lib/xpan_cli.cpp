		     ...
		     transport_switch_reason reason_code)
{
	...
	ALOGI("Received switch_reason: %d", reason_code);
}
...
void dummy_cb_cc_update(char* cc)
{
	ALOGI("Received country code update: %s", cc);
}

void dummy_cb_driver_ready(xpan_wifi_status status)
{
	ALOGI("Received driver ready event");
}
...
{
	if (fn.wifi_delete_ap_iface(data, NULL) == XPAN_WIFI_STATUS_SUCCESS)
		return 0;
	else
		return -1;
}
...
static int xpan_cli_get_supported_features(int argc, char *argv[])
{
	xpan_wifi_status status = fn.get_xpan_supported(data);

	switch (status) {
	case XPAN_WIFI_STATUS_SUCCESS:
		printf ("XPAN : Supported");
		return 0;
	case XPAN_WIFI_STATUS_FAILURE_NOT_SUPPORTED:
		printf ("XPAN : Not Supported");
	default:
		printf ("XPAN: Generic Failure");
	}

	return -1;
}

static int xpan_cli_peer_disconnect(int argc, char *argv[])
{
	u8 mac[ETH_ALEN];
	...
	if (fn.xpan_peer_disconnect(data, mac) == XPAN_WIFI_STATUS_SUCCESS) {
		ALOGI("peer disconnection successful");
		return 0;
	}

	ALOGE("Failed to remove peers");
	return -1;
}

static int xpan_cli_twt_teardown(int argc, char *argv[])
{
	u8 mac[ETH_ALEN];
	...
	if (fn.xpan_twt_teardown(data, mac) == XPAN_WIFI_STATUS_SUCCESS) {
		ALOGI("TWT teardown successful");
		return 0;
	}

	ALOGE("Failed to teardown twt sessions");
	return -1;
}
...
static const struct xpan_cli_cmd xpan_cli_commands[] = {
	...
	{ "ENABLE_STA_STATS", xpan_cli_enable_sta_stats,
	  ...
	  "<interval> = Enable xpan stats collection with the given interval" },
	{ "get_supported_features", xpan_cli_get_supported_features,
	  "Get information about xpan feature support" },
	{ "PEER_DISCONNECT", xpan_cli_peer_disconnect,
	  "<mac> = mac address of the peer to be disconnected"
	  "Disconnect a peer" },
	{ "TWT_TEARDOWN", xpan_cli_twt_teardown,
	  "<mac> = mac address of the client with which all the twt sessions need to be teardown"
	  "Teardown all the twt sessions"},
...
int main(int argc, char *argv[])
{
	...
	ret = fn.init_xpan_wifi_lib(&data);
	if(!data) {
	...
	dummy_cb.cb_country_code_update = dummy_cb_cc_update;
	dummy_cb.cb_driver_ready_event = dummy_cb_driver_ready;


	//register for callbacks
	fn.get_xpan_supported(data);
