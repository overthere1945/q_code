...
{
	iface_info *temp = ifinfo;
	char *default_ifname = XPAN_DEFAULT_SAP_IFACE;
	...
	while (temp != NULL) {
		if (temp->type == type &&
		    ((strcmp(temp->ifname, default_ifname) == 0
		    || type == IFACE_TYPE_STA)))
			return temp;
		temp = temp->next;
	}
...
static int get_xsap_iface_id(struct xpan_wifi_data *data)
{
	if (!data)
		return 0;

	iface_info *iface = get_ifaceinfo_from_list(data->ifinfo,
						    IFACE_TYPE_SAP, NULL);
	return (iface) ? iface->if_index : 0;
}

static interface_info *get_interface_by_id(struct xpan_wifi_data *data,
					   int index)
{
	if (!data)
		return NULL;
	...
	dl_list_for_each(iface, &data->interfaces, interface_info, list) {
		if (iface->id == index)
			return iface;
	}
...
static int get_valid_wifi_base_ifindex(xpan_wifi_data *data)
{
	char buf[PROPERTY_VALUE_MAX] = {};
	interface_info *iface = NULL;
	int id = 0;
	...
	property_get("wifi.interface", buf, "wlan0");
	id = if_nametoindex(buf);

	if (!id || dl_list_empty(&data->interfaces)) {
		ALOGE("%s: Error, No interfaces found yet", __func__);
		return -1;
	}

	iface = get_interface_by_id(data, id);
	if (iface)
		return id;

	ALOGE("%s: No valid wifi base interface present", __func__);
	return -1;
}
...
static xpan_wifi_status wifi_error_to_xpan_status(wifi_error code)
{
	switch (code) {
	case WIFI_SUCCESS:
		return XPAN_WIFI_STATUS_SUCCESS;
	case WIFI_ERROR_NOT_SUPPORTED:
		return XPAN_WIFI_STATUS_FAILURE_NOT_SUPPORTED;
	case WIFI_ERROR_UNINITIALIZED:
		return XPAN_WIFI_STATUS_FAILURE_DRIVER_NOT_READY;
	default:
		return XPAN_WIFI_STATUS_FAILURE;
	}
}

static xpan_wifi_status validate_data(xpan_wifi_data *data, bool sap_iface_check)
{
	if (!data) {
		ALOGE("%s: Error xpan_wifi_data NULL", __func__);
		return XPAN_WIFI_STATUS_FAILURE;
	...
	if (data->driver_ready_state != XPAN_WIFI_STATUS_SUCCESS_DRIVER_INITIALIZED) {
		ALOGE("%s: Error, Driver not initialized", __func__);
		return XPAN_WIFI_STATUS_FAILURE_DRIVER_NOT_READY;
	}

	if (!data->xpan_support) {
		ALOGE("%s: Error, Xpan feature not supported", __func__);
		return XPAN_WIFI_STATUS_FAILURE_NOT_SUPPORTED;
	}

	if (sap_iface_check && data->cmd_iface != IFACE_TYPE_SAP) {
...
xpan_wifi_status get_chan_switch_tsf(xpan_wifi_data *data, channel_params *chan_params)
{
	...
	xpan_wifi_status status = validate_data(data, false);
	if (status != XPAN_WIFI_STATUS_SUCCESS)
		return status;
	status = XPAN_WIFI_STATUS_FAILURE;
	...
	iface = get_ifaceinfo_from_list(data->ifinfo, IFACE_TYPE_SAP, NULL);
	if (!iface) {
...
{
	xpan_wifi_data *data = (xpan_wifi_data *)arg;
	...
	if (cmd == NL80211_CMD_VENDOR) {
		...
		if (ifidx || wiphy || wdev) {
			 ...
			 */
			ifsap = get_ifaceinfo_from_list(data->ifinfo, IFACE_TYPE_SAP, NULL);
		...
		if (data->cb && data->cb->cb_channel_switch_started)
		   ...
		   (cmd == NL80211_CMD_WIPHY_REG_CHANGE)) {
		char country_code[3];
		struct nlattr* tb;
		tb = event.get_attribute(NL80211_ATTR_REG_ALPHA2);
		if (tb) {
			strlcpy(country_code,  (const char*) nla_data(tb),
				sizeof(country_code));
			ALOGI("Received country code changed event with"
			      "country: %s, Previous country code: %s",
			      country_code, data->country_code);
			if ((strcmp(data->country_code, country_code) != 0) &&
			    data->cb && data->cb->cb_country_code_update) {
				data->cb->cb_country_code_update(country_code);
				strlcpy(data->country_code, country_code,
					sizeof(data->country_code));
			}
		}
	} else {
...
static void wifi_deinit_interfaces(xpan_wifi_data *data) {
	interface_info *tmp, *prev;
	if (!data)
		return;

	dl_list_for_each_safe(tmp, prev, &data->interfaces, interface_info, list) {
		if (tmp) {
			dl_list_del(&tmp->list);
			free(tmp);
		}
	}
	data->num_interfaces = 0;
}
	...
	while ((de = readdir(d))) {
		...
		if (is_wifi_interface(de->d_name) ) {
			...
			if (ifinfo == NULL) {
				...
				wifi_deinit_interfaces(data);
				return WIFI_ERROR_OUT_OF_MEMORY;
			}

			strlcpy(ifinfo->name, de->d_name, (IFNAMSIZ + 1));
			ifinfo->id = if_nametoindex(de->d_name);
			ifinfo->is_dynamic = false;
			ifinfo->handle = handle;
			// read ifi_flags from /sys/class/net/[iface]flags
			dl_list_add(&data->interfaces, &ifinfo->list);
		}
	}
	closedir(d);
	data->num_interfaces = dl_list_len(&data->interfaces);
	if (data->num_interfaces == 0) {
		ALOGI("wlan base interface not present");
		return WIFI_ERROR_UNINITIALIZED;
	}

	data->driver_ready_state = XPAN_WIFI_STATUS_SUCCESS_DRIVER_INITIALIZED;
	return WIFI_SUCCESS;
}
...
xpan_wifi_status get_xpan_supported(void *ctx)
{
	...
	bool flag = false;
	int ifidx_base_iface;

	xpan_wifi_status status = validate_data(data, false);

	if (status == XPAN_WIFI_STATUS_SUCCESS) {
		ALOGI("wifi enhance_audio_feature: enabled");
		return XPAN_WIFI_STATUS_SUCCESS;
	} else if (status != XPAN_WIFI_STATUS_FAILURE_NOT_SUPPORTED) {
		return status;
	}

	status = XPAN_WIFI_STATUS_FAILURE;

	ifidx_base_iface = get_valid_wifi_base_ifindex(data);
	if (ifidx_base_iface < 0) {
		ALOGE("%s: Failed to get base Wifi interface index. ret=%d",
		      __func__, ifidx_base_iface);
		return status;
	}
	...
	ret = cmd.put_u32(NL80211_ATTR_IFINDEX, ifidx_base_iface);
	if (ret != WIFI_SUCCESS) {
	...
	if (flag) {
		data->xpan_support = true;
		ALOGI("wifi enhance_audio_feature: enabled");
		return XPAN_WIFI_STATUS_SUCCESS;
	} else {
		ALOGI("wifi enhance_audio_feature: not supported");
		return XPAN_WIFI_STATUS_FAILURE_NOT_SUPPORTED;
	}
}

static void print_interface_list(xpan_wifi_data *data)
{
	if (!data)
		return;

	ALOGD("%s: num_interfaces:%d, list_size: %d",
	      __func__, data->num_interfaces, dl_list_len(&data->interfaces));

	interface_info *tmp;
	dl_list_for_each(tmp, &data->interfaces, interface_info, list)
		ALOGD("%s: ifindex:%d ifname:%s flags:0x%x dynamic:%d",
		      __func__, tmp->id, tmp->name, tmp->ifi_flags,
		      tmp->is_dynamic);
}

static void add_new_interface_info(xpan_wifi_data *data, struct ifinfomsg *ifi,
				   const char *ifname)
{
	interface_info *ifinfo, *tmp;
	bool new_iface = true;
	...
	if (new_iface)
		ifinfo = (interface_info *)malloc(sizeof(interface_info));
	else
		ifinfo = tmp;
	...
	strlcpy(ifinfo->name, ifname, (IFNAMSIZ + 1));
	ifinfo->handle = (wifi_handle) data;
	ifinfo->id = ifi->ifi_index;
	ifinfo->ifi_flags = ifi->ifi_flags;

	if (new_iface) {
		ifinfo->is_dynamic = true;
		dl_list_add(&data->interfaces, &ifinfo->list);
		data->num_interfaces++;
	}
}

static int netlink_msg_handler(struct xpan_wifi_data *data, struct sockaddr_nl *nl,
			       struct nlmsghdr *msg)
{
	struct ifinfomsg *ifi;
	struct rtattr *attr;
	interface_info *iface;
	int id = 0, xsap_index, attrlen;
	char buf[PROPERTY_VALUE_MAX] = {};
	char ifname[IFNAMSIZ + 1] = {};

	if (!data) {
		ALOGE("Error, xpan_wifi_data is NULL");
		return 0;
	}
	if (NLMSG_PAYLOAD(msg, 0) < sizeof(struct ifinfomsg)) {
		ALOGE("Ignore unexpected netlink message.");
		return 0;
	}

	ifi = (struct ifinfomsg *) NLMSG_DATA(msg);
	attr = (struct rtattr *) ((u8 *) NLMSG_DATA(msg) + NLMSG_ALIGN(sizeof(struct ifinfomsg)));
	...
	// Ignore non-wlan interfaces.
	if (!is_wifi_interface(ifname))
		return 0;

	property_get("wifi.interface", buf, "wlan0");
	xsap_index = get_xsap_iface_id(data);

	switch (msg->nlmsg_type) {
	case RTM_NEWLINK:
		ALOGD("RTM_NEWLINK Event Received for %d (%s) flags:0x%x",
		      ifi->ifi_index, ifname, ifi->ifi_flags);
		if (xsap_index) {
			iface = get_interface_by_id(data, xsap_index);
			if (iface)
				iface->ifi_flags = ifi->ifi_flags;
			else
				add_new_interface_info(data, ifi, ifname);
			break;
		}
		iface = get_interface_by_id(data, ifi->ifi_index);
		if (iface) {
			// One of existing static interface.
			if (!iface->is_dynamic
			    && !(ifi->ifi_flags & IFF_UP)
			    && (iface->ifi_flags & IFF_UP)) {
				/* Indicate callback when iface move to Down from UP state*/
				ALOGD("RTM_NEWLINK: Trigger cb_possible_to_create_ap_iface");
				if (data->cb && data->cb->cb_possible_to_create_ap_iface)
					data->cb->cb_possible_to_create_ap_iface();
			}
			iface->ifi_flags = ifi->ifi_flags;
		} else {
			// New interface found. Add entry.
			add_new_interface_info(data, ifi, ifname);
		}

		if (data->driver_ready_state != XPAN_WIFI_STATUS_SUCCESS_DRIVER_INITIALIZED
		    && ifi && ifi->ifi_index == if_nametoindex(buf)) {
			/* During driver load, all static interfaces are initialized
			 * in sequence. Wait for 100ms to allow creation of all
			 * interfaces (ex: p2p0, wifi-aware0, etc). Since this happens
			 * only once after bootup, or xpan_wifi_lib init, delay of
			 * 100ms should be fine */
#define STATIC_IFACES_CREATE_WAIT_TIME	100 * 1000
			usleep(STATIC_IFACES_CREATE_WAIT_TIME);

			wifi_init_interfaces((wifi_handle) data);
			if (data->num_interfaces > 0) {
				ALOGI("RTM_NEWLINK: Trigger cb_driver_ready_event");
				/* trigger the callback for driver ready event */
				if (data->cb && data->cb->cb_driver_ready_event)
					data->cb->cb_driver_ready_event(data->driver_ready_state);
			}
		}
		break;
	case RTM_DELLINK:
		ALOGD("RTM_DELLINK Event Received for %d (%s) flags:0x%x",
		      ifi->ifi_index, ifname, ifi->ifi_flags);
		iface = get_interface_by_id(data, ifi->ifi_index);
		if (iface) {
			...
			data->num_interfaces--;
			if (xsap_index) {
				remove_iface_node(data,
					get_ifaceinfo_from_list(data->ifinfo, IFACE_TYPE_SAP, ifname));
			} else {
				/* non-XPAN Interface deleted. Indicate possibility. */
				ALOGD("RTM_DELLINK: Trigger cb_possible_to_create_ap_iface");
				if (data->cb && data->cb->cb_possible_to_create_ap_iface) {
					data->cb->cb_possible_to_create_ap_iface();
				}
			}
...
static void read_netlink_events(xpan_wifi_data *data)
{
	int status, ret;
	char buf[4096];
	...
	status = recvmsg(data->rtnl_sock, &msg, 0);
	if (status < 0) {
		ALOGE("%s: Error in recvmsg: %d", __func__, status);
		return;
	}
	...
	while (NLMSG_OK(h, (unsigned int)status)) {
		...
		if (h->nlmsg_type == NLMSG_ERROR) {
			ALOGE("%s: Error message", __func__);
			return;
		}

		ret = netlink_msg_handler(data, &snl, h);
		if (ret < 0) {
...
static void netlink_deinit(struct xpan_wifi_data *data)
{
	if (!data->rtnl_sock)
		return;

	if (data->rtnl_sock > 0)
		remove_event_handler(data, data->rtnl_sock);
	data->rtnl_sock = 0;
}

static int netlink_init(struct xpan_wifi_data *data)
{
	...
	data->rtnl_sock = socket(PF_NETLINK, SOCK_RAW, NETLINK_ROUTE);
	...
	if (bind(data->rtnl_sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
		ALOGE("Failed to bind netlink socket, errno %d (%s)", errno, strerror(errno));
		goto cleanup;
	}

	if (add_event_handler(data, data->rtnl_sock, read_netlink_events)) {
		ALOGE("Unable to initialize netlink event loop handler");
		goto cleanup;
	}

	ALOGE("Generic netlink initialization successful");
	return 0;

cleanup:
	if (data->rtnl_sock > 0)
		close(data->rtnl_sock);
	return -1;
}

static void nl80211_event_handler(struct xpan_wifi_data *data)
{
	...
	if (data == NULL) {
		ALOGE("nl80211_event_handler: data is null");
		return;
	}

	socket = data->nl_event;
	cb = nl_socket_get_cb(socket);

	res = nl_recvmsgs(socket, cb);
	if (res < 0) {
		ALOGE("Error while reading nl messages %d", res);
	}
}

static int nl80211_init(struct xpan_wifi_data *data)
{
	...
	data->nl_cmd = xpan_wifi_create_nl_socket(XPAN_WIFI_CMD_SOCK_PORT,
					      NETLINK_GENERIC);
	if (data->nl_cmd == NULL) {
		ALOGE("Could not create command sock port");
		goto cleanup;
	}
	data->nl_event = xpan_wifi_create_nl_socket(XPAN_WIFI_EVENT_SOCK_PORT,
						NETLINK_GENERIC);
	if (data->nl_event == NULL) {
		ALOGE("Could not create event sock port");
		goto cleanup;
	}

	/* Set the socket buffer size */
	if (nl_socket_set_buffer_size(data->nl_event, (256*1024), 0) < 0) {
		ALOGE("Could not set nl_socket RX buffer size for event_sock: %s",
				strerror(errno));
		/* continue anyway with the default (smaller) buffer */
	}

	cb = nl_socket_get_cb(data->nl_event);
	...
	data->nl80211_family_id = genl_ctrl_resolve(data->nl_cmd, "nl80211");
	...
	fd = nl_socket_get_fd(data->nl_event);
	if ( fd == -1) {
		ALOGE("File descriptor not available");
		goto cleanup;
	}

	if (add_event_handler(data, fd, (event_callback_t)nl80211_event_handler)) {
		ALOGE("Unable to initialize nl80211 event handler");
		goto cleanup;
	}

	ALOGI("nl80211 events registration successful");
	return 0;
cleanup:
	if (data && data->nl_cmd)
		nl_socket_free(data->nl_cmd);
	if (data && data->nl_event)
		nl_socket_free(data->nl_event);
	return -1;
}

static void nl80211_deinit(struct xpan_wifi_data *data)
{
	if (data->nl_cmd) {
		nl_socket_free(data->nl_cmd);
	}
	...
	if (data->nl_event) {
		int fd = nl_socket_get_fd(data->nl_event);
...
xpan_wifi_status init_xpan_wifi_lib(xpan_wifi_data **ctx)
{
	...
	if (eloop_handler_init(data) < 0) {
		ALOGE("Event loop handler initialization failed");
		goto unload;
	}

	pthread_mutex_init(&data->lock, NULL);

	if (nl80211_init(data) < 0) {
		ALOGE("nl80211 initialization failed");
		goto unload;
	}

	/* Register for monitoring events for network interface changes */
	if (netlink_init(data) < 0) {
		ALOGE("netlink initialization failed.");
		goto unload;
	}

	ret = wifi_init_interfaces((wifi_handle) data);
	if (!(ret == WIFI_ERROR_UNINITIALIZED || ret == WIFI_SUCCESS)) {
		ALOGI("failed to init interfaces");
		goto unload;
	}

	wifi_add_membership((wifi_handle) data, "scan");
	wifi_add_membership((wifi_handle) data, "mlme");
	wifi_add_membership((wifi_handle) data, "regulatory");
	wifi_add_membership((wifi_handle) data, "vendor");

	*ctx = data;

	ALOGI("Initialized Wifi_Xpan_Lib Successfully");
	return wifi_error_to_xpan_status(ret);
unload:
	eloop_handler_deinit(data);
	netlink_deinit(data);
...
static int send_nlmsg(xpan_wifi_data *data, struct nl_msg *nlmsg,
		      nl_recvmsg_msg_cb_t customer_cb, void *arg)
{
	int err = 0;
	struct nl_cb *cb = nl_cb_alloc(NL_CB_DEFAULT);
	struct nl_sock *cmd_sock = data->nl_cmd;
						    ...
						    struct resp_info *info)
{
	...
	status = send_nlmsg(data, nlmsg, NULL, NULL);
					     ...
					     int interval)
{
	...
	xpan_wifi_status ret = validate_data(data, false);
	if (ret!= XPAN_WIFI_STATUS_SUCCESS)
		return ret;
	ret = XPAN_WIFI_STATUS_FAILURE;
	...
	iface = get_ifaceinfo_from_list(data->ifinfo, data->cmd_iface, NULL);
	if (!iface) {
					 ...
					 struct xpan_usecase_params *params)
{
	...
	wifi_error ret;
	struct nlattr *nlData = NULL;
	WiFiConfigCommand *wifiConfigCommand = NULL;
...
xpan_wifi_status wifi_create_ap_iface(void *ctx, char *ifname)
{
	...
	if (if_nametoindex(new_ifname)) {
		ALOGE("%s: Requested SAP iface:%s exists", __func__, new_ifname);
		return XPAN_WIFI_STATUS_FAILURE_EEXIST;
	}
	...
	ret = wifiConfigCommand->put_u32(NL80211_ATTR_IFINDEX, ifidx_base_iface);
	if (ret != WIFI_SUCCESS) {
		ALOGE("%s: Unable to put NL80211_ATTR_IFINDEX :%d", __func__, ret);
		goto done;
	}
...
xpan_wifi_status wifi_delete_ap_iface(void *ctx, char *ifname)
{
	...
	xpan_wifi_data *data = (xpan_wifi_data *)ctx;
	iface_info *ifsap;
	interface_info *ifinfo = NULL;
	char *del_ifname = XPAN_DEFAULT_SAP_IFACE;
	int id = 0;
	...
	if (ifname && strlen(ifname) > 0)
		del_ifname = ifname;
	id = if_nametoindex(del_ifname);
	if (!id) {
		ALOGI("xsap interface is not present");
		return XPAN_WIFI_STATUS_SUCCESS;
	}

	ALOGD("%s: ifname=%s delete", __func__, del_ifname);
	wifiConfigCommand = new WiFiConfigCommand(handle, get_requestid(),
						  0, 0);
	if (wifiConfigCommand == NULL) {
		ALOGE("%s: Error wifiConfigCommand NULL", __func__);
	...
	ret = wifiConfigCommand->put_u32(NL80211_ATTR_IFINDEX, id);
	if (ret != WIFI_SUCCESS) {
		ALOGE("%s: Unable to put NL80211_ATTR_IFINDEX %d", __func__, ret);
		goto done;
	}
...
xpan_wifi_status xpan_get_usable_channels(void *ctx, int *freqs,
					  int max_size, int *size)
{
	...
	if (!data || data->num_interfaces < 1) {
		...
		return status;
	} else if (!freqs || max_size < MAX_WIFI_FREQS) {
		ALOGE("%s: frequency list null/emtpy", __func__);
		return status;
	}
	...
	if (ret != WIFI_SUCCESS) {
		ALOGE("%s: Unable to put BAND_MASK :%d", __func__, ret);
		return status;
	}
	...
	if (ret != WIFI_SUCCESS) {
		ALOGE("%s: Unable to put MODE_MASK :%d", __func__, ret);
		return status;
	}

	cmd.attr_end(nlData);

	cmd.setXpanUsableFreqs(freqs, max_size, size);
...
xpan_wifi_status wifi_do_acs(void *ctx, int *freq_list, int freq_list_size)
{
	...
	struct nlattr *nlData = NULL;
	wifi_handle wifiHandle = NULL;
	int i, j, i_size, drv_freq_size = 0;
	int hw_mode = HOSTAPD_MODE_IEEE80211A;
	u16 ch_width = ACS_CHANNEL_WIDTH_DEFAULT;
	int age_out = SCAN_AGE_OUT_DEFAULT;
	u32 *freqs;
	int drv_usable_freqs[MAX_WIFI_FREQS];
	bool is2Ghz = false, is5Ghz = false;
	wifi_error ret;
	iface_info *iface;

	xpan_wifi_data *data = (xpan_wifi_data*)ctx;
	xpan_wifi_status status = validate_data(data, true);
	if (status != XPAN_WIFI_STATUS_SUCCESS)
		return status;

	status = XPAN_WIFI_STATUS_FAILURE;

	if (!freq_list || freq_list_size <= 0) {
		ALOGE("%s: Error, Invalid Frequency list. size %d",
		      __func__, freq_list_size);
		return status;
	}

	if (xpan_get_usable_channels(ctx, drv_usable_freqs, MAX_WIFI_FREQS,
			&drv_freq_size) != XPAN_WIFI_STATUS_SUCCESS) {
		ALOGE("%s: Failed to get usable channel list.", __func__);
		return status;
	}
	...
	i_size = 0;		// Intersection list size
	for (i = 0; i < freq_list_size; i++) {
		for (j = 0; j < drv_freq_size; j++) {
			if (drv_usable_freqs[j] == freq_list[i])
				break;
		}
		if (j == drv_freq_size)
			continue;

		freqs[i_size] = freq_list[i];
		if (freqs[i_size] > 5000)
			is5Ghz = true;
		else if (freqs[i_size] > 2000)
			is2Ghz = true;
		i_size++;
	}
	if (!i_size) {
		ALOGE("%s: No common frequencies.");
		goto cleanup;
	}
	...
	ALOGI("ACS Params: HW_MODE: %d HT: 1 VHT: 1 BW: %d SCAN_AGE: %d Num_freqs: %d",
	      hw_mode, ch_width, age_out, i_size);
...
xpan_wifi_status register_iface(void *ctx, u8 *mac, iface_type type)
{
	...
	dl_list_for_each(iface, &data->interfaces, interface_info, list) {
		...
		if (rem < PATH_SIZE)
			rem = strlcat(path, iface->name, PATH_SIZE - rem);
		if (rem < PATH_SIZE)
		...
		ifinfo->if_index = iface->id;
		strlcpy(ifinfo->ifname, iface->name, (IFNAMSIZ + 1));
		ifinfo->type = type;
		ALOGI("MAC of interface:%s is " MACSTR,
		      iface->name, MAC2STR(sta_mac));
		add_iface_node(data, ifinfo);
		return XPAN_WIFI_STATUS_SUCCESS;
	}
...
xpan_wifi_status unregister_iface(void *ctx, iface_type type)
{
	...
	ifinfo = get_ifaceinfo_from_list(data->ifinfo, type, NULL);
	if (!ifinfo) {
		ALOGE("%s iface is not present in the list", __func__);
		return status;
	}
...
xpan_wifi_status xpan_peer_disconnect(void *ctx, u8 *mac)
{
	...
	wifiConfigCommand = new WiFiConfigCommand((wifi_handle)data,
						  get_requestid(), 0, 0);
	if (wifiConfigCommand == NULL) {
		ALOGE("%s: Error wifiConfigCommand NULL", __func__);
		return status;
	...
	wifi_error ret = wifiConfigCommand->create_generic(NL80211_CMD_DEL_STATION);
	if (ret != WIFI_SUCCESS) {
		ALOGE("%s: nlcommand_create Error:%d", __func__, ret);
		goto done;
	}

	ret = wifiConfigCommand->put_u32(NL80211_ATTR_IFINDEX, iface->if_index);
	if (ret != WIFI_SUCCESS) {
		ALOGE("%s: Failed to put NL80211_ATTR_IFINDEX %d", __func__, ret);
		goto done;
	}

	ret = wifiConfigCommand->put_addr(NL80211_ATTR_MAC, mac);
	if (ret != WIFI_SUCCESS) {
		ALOGE("%s: Unable to put NL80211_ATTR_MAC %d", __func__, ret);
		goto done;
	}
	...
	ALOGI("Peer disconnection is successful for " MACSTR, MAC2STR(mac));
	status =  XPAN_WIFI_STATUS_SUCCESS;
...
xpan_wifi_status xpan_twt_teardown(void *ctx, u8 *mac)
{
	...
	WifihalGeneric cmd((wifi_handle)data, get_requestid(), OUI_QCA,
	...
	ret = cmd.put_u8(QCA_WLAN_VENDOR_ATTR_CONFIG_TWT_OPERATION,
		       QCA_WLAN_TWT_TERMINATE);
	if (ret != WIFI_SUCCESS) {
		ALOGE("TWT: Failed to put QCA_WLAN_TWT_TERMINATE");
		return status;
	}

	twt_attr = cmd.attr_start(QCA_WLAN_VENDOR_ATTR_CONFIG_TWT_PARAMS);
	if (!twt_attr) {
		ALOGE("%s: failed attr_start for CONFIG_TWT_PARAMS", __func__);
		return status;
	}


	ret = cmd.put_u8(QCA_WLAN_VENDOR_ATTR_TWT_SETUP_FLOW_ID, TWT_ALL_SESSIONS_DIALOG_ID);
	if (ret != WIFI_SUCCESS) {
		ALOGE("%s: Unable to put QCA_WLAN_VENDOR_ATTR_TWT_SETUP_FLOW_ID %d", __func__, ret);
		return status;
	}

	ret = cmd.put_addr(QCA_WLAN_VENDOR_ATTR_TWT_SETUP_MAC_ADDR, mac);
	if (ret != WIFI_SUCCESS) {
		ALOGE("%s: Unable to put QCA_WLAN_VENDOR_ATTR_TWT_SETUP_MAC_ADDR %d", __func__, ret);
		return status;
	}

	cmd.attr_end(twt_attr);
	cmd.attr_end(nlData);
	...
	ALOGI("TWT teardown is successful for " MACSTR, MAC2STR(mac));
	return XPAN_WIFI_STATUS_SUCCESS;
...
static int eloop_handler_init(struct xpan_wifi_data *data)
{
	...
	data->eloop.epoll_fd = epoll_create(EPOLL_SIZE);

	if (data->eloop.epoll_fd == -1) {
		ALOGE("event loop initialization failed.");
		return -1;
	}

	for (i = 0; i < MAX_FD_COUNT; i++)
		data->eloop.handler[i].fd = -1;

	ALOGI("event loop initialization success");
	return 0;
}

static struct event_handler *get_event_handler(struct xpan_wifi_data *data,
					       int fd)
{
	int i;

	for (i = 0; i < MAX_FD_COUNT; i++) {
		if (data->eloop.handler[i].fd == -1 ||
		    data->eloop.handler[i].fd != fd)
			continue;

		return &data->eloop.handler[i];
	}

	return NULL;
}

static int add_event_handler(struct xpan_wifi_data *ctx, int fd, event_callback_t callback)
{
	struct xpan_wifi_data *data = (xpan_wifi_data *)ctx;
	...
	for (i = 0; i < MAX_FD_COUNT; i++) {
		...
		e.data.fd = fd;
		e.events = EPOLLIN;
		if (epoll_ctl(data->eloop.epoll_fd, EPOLL_CTL_ADD, fd, &e) == -1) {
			ALOGE("Not able to add fd entry to epoll file descriptor");
			return -1;
		}

		ALOGD("add fd =%d", fd);
		data->eloop.handler[i].fd = fd;
		data->eloop.handler[i].callback = callback;
		data->eloop.handler[i].data = ctx;
		return 0;
	}

	ALOGE("add fd failed. reached max limit");
	return -1;
}
...
static void remove_event_handler(struct xpan_wifi_data *data, int fd)
{
	...
	for (i = 0; i < MAX_FD_COUNT; i++) {
		if (data->eloop.handler[i].fd != fd)
			continue;

		ALOGD("remove fd =%d", data->eloop.handler[i].fd);
		epoll_ctl(data->eloop.epoll_fd, EPOLL_CTL_DEL, fd, NULL);
		data->eloop.handler[i].fd = -1;
		data->eloop.handler[i].callback = NULL;
		data->eloop.handler[i].data = NULL;
		close(fd);
		break;
	}
}

static void process_event_loop(xpan_wifi_data *data)
{
	int wait_time = -1;
	int num_events = 0;
	...
	num_events = epoll_wait(data->eloop.epoll_fd, events, EPOLL_SIZE,
				    wait_time);
	if (num_events == -1) {
		ALOGE("event loop wait error");
		return;
	}

	while (curr_event < num_events) {
		fd = events[curr_event].data.fd;
		flags = events[curr_event].events;

		if (flags & EPOLLHUP) {
			ALOGE("file descriptor '%d' sent HUP.", fd);
			remove_event_handler(data, fd);
		} else if (flags & EPOLLIN) {
			handler = get_event_handler(data, fd);
			if (handler != NULL && handler->callback != NULL) {
				handler->callback(handler->data);
			}
		} else {
			ALOGE("Unknown event - %x", flags);
		}
		curr_event++;
	}
}
...
void xpan_wifi_event_loop(void *ctx)
{
	...
	ALOGI("xpan_wifi_event_loop() exiting");
}
...
xpan_wifi_status init_xpan_wifi_lib_function_table(struct wifi_drv_ops *fn)
{
	...
	fn->get_xpan_supported = get_xpan_supported;
	fn->xpan_peer_disconnect = xpan_peer_disconnect;
	fn->xpan_twt_teardown = xpan_twt_teardown;
