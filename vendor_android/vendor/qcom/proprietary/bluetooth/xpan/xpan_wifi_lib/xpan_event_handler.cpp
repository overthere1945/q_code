...
wifi_error EventHandler::switchEventHandler(WifiEvent &event)
{
	...
	transport_switch_reason switch_reason = TRANSPORT_SWITCH_REASON_UNSPECIFIED;
	...
	if(tb[QCA_WLAN_VENDOR_ATTR_AUDIO_TRANSPORT_SWITCH_REASON])
	{
		switch (nla_get_u8(tb[QCA_WLAN_VENDOR_ATTR_AUDIO_TRANSPORT_SWITCH_REASON])) {
			case QCA_WLAN_AUDIO_TRANSPORT_SWITCH_REASON_TERMINATING:
				ALOGI("Reason for switch request is terminating");
				switch_reason = TRANSPORT_SWITCH_REASON_TERMINATING;
				break;
			default:
				ALOGE("Invalid switch reason received");
				return WIFI_ERROR_INVALID_ARGS;
		}
	}
