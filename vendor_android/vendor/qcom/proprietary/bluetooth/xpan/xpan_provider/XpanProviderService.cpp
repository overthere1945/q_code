...
std::shared_ptr<IXpanProviderCallback> xmXpanProviderCb = NULL;
...
::ndk::ScopedAStatus XpanProviderService::registerXpanCallbacks(
  ...
  if (in_cb != NULL) {
    std::string propXpan = GetProperty("persist.vendor.service.bt.adv_transport", "false");
    if (propXpan.compare("false")==0) {
        ALOGI("%s ", propXpan.c_str());
  ...
  deathRecipient = AIBinder_DeathRecipient_new(
...
::ndk::ScopedAStatus XpanProviderService::sendXpanCommand(
  ...
  switch(opcode) {
    ...
    case SET_AP_AVB_REQ:
     ...
     case UPDATE_WIFI_CHANNEL_SWITCH_REQ:
         updateWifiChannelSwitchReq(cmd_data);
         break;

     case GET_WIFI_DRIVER_STATUS:
         getWifiDriverStatus(cmd_data);
         break;

     case UPDATE_DISCONNECTED_SAP_CLIENTS:
         updateDisConnectedClients(cmd_data);
         break;

     case TEARDOWN_TWT:
         teardownTwt(cmd_data);
         break;

    case UNREGISTER:
        unRegisterXpanCallbacks();
        break;

    case REMOTE_FEATURE_MASK:
    	setRemoteFeatureMask(cmd_data);
        break;
...
void XpanProviderService::unRegisterXpanCallbacks() {
  ALOGE("%s:", __func__);
  if (deathRecipient != NULL && xpanProviderCb != NULL) {
    ALOGE("%s: AIBinder_unlinkToDeath", __func__);
    AIBinder_unlinkToDeath(xpanProviderCb->asBinder().get(),
                           deathRecipient,
                           reinterpret_cast<void*>(this));
    ...
    ALOGE("%s: xpanProviderCb or deathRecipient is null, Failed to do AIBinder_unlinkToDeath", __func__);
  }
}
...
void XpanProviderService::updateBearerPreferenceReq(const std::vector<uint8_t>& data) {
  /* expectedLength = address(6) + requestor (1) + bearer(1) + reason(1) + urgency (1) */
  uint8_t bytesParsed = 0, expectedLength = 10;
  bdaddr_t addr = {0};
  uint8_t requestor, bearer, reason;
  ...
  bearer = data[bytesParsed++];
  reason = data[bytesParsed++];
  bool urgency = (bool) data[bytesParsed];
  ALOGD("%s: address(%02X:%02X:%02X:%02X:%02X:%02X), bearer = %d  requestor = %d reason = %d urgency %d ",
        __func__, addr.b[5], addr.b[4], addr.b[3], addr.b[2],
        addr.b[1], addr.b[0], bearer, requestor, reason, urgency);

  if (xm_intf && xm_intf->update_bearer_preference_req) {
    xm_intf->update_bearer_preference_req(addr, bearer, requestor, reason, urgency);
  }
}
...
void XpanProviderService::updateApDetailsLocal(const std::vector<uint8_t>& data){
  ...
  uint8_t bytesParsed = 0, expectedLength = 37;
  ...
  ALOGD("%s: len = (%d) ", __func__, data.size());
  ...
  ALOGD("%s: bssid %s", __func__,
      ConvertRawAddress(localParams.bssid));
  ALOGD("%s: mac %s", __func__,
      ConvertRawAddress(localParams.randomMac));
  ALOGD("%s: ipaddr %s mdns %s freq %d", __func__,
...
/* Indication from Xpan Profile that Received Wifi channel switch request from Peer device */
void XpanProviderService::updateWifiChannelSwitchReq(const std::vector<uint8_t>& data) {
	  /* expectedLength = BluetoothDevice(6) + channel (2) */
  ...
  bdaddr_t addr = {0};
  std::copy(data.begin(), data.begin() + BD_ADDR_LENGTH, addr.b);
  uint16_t channel =  ((data[bytesParsed]) | (data[bytesParsed + 1] << 8));
  ALOGD("%s:  %s  %d ",   __func__, addr.toString().c_str(), channel);

  if (xm_intf && xm_intf->update_wifi_channel_switch_req) {
    xm_intf->update_wifi_channel_switch_req(addr, channel);
  }
}

/* Check Wi-Fi driver  status*/
void XpanProviderService::getWifiDriverStatus(const std::vector<uint8_t>& data) {
  /* expectedLength = 0 */

  uint8_t bytesParsed = 0, expectedLength = 0;
  if (data.size() != expectedLength) {
    ALOGE("%s: Incorrect parameters length (%d)", __func__, data.size());
    return;
  }
  ...
  if (xm_intf && xm_intf->get_wifi_driver_status) {
    xm_intf->get_wifi_driver_status();
  }
}

/* Update Disconnected clients*/
void XpanProviderService::updateDisConnectedClients(const std::vector<uint8_t>& data) {
  /* expectedLength = 0 */

 /* expectedLength =  mac_left (6) + mac_right (6) */
  uint8_t bytesParsed = 0, expectedLength = 12;
  if (data.size() < expectedLength) {
    ALOGE("%s: Incorrect parameters length (%d)", __func__, data.size());
    return;
  }
  ...
  std::copy((data.begin()+ bytesParsed), (data.begin() + bytesParsed + mac_size), right.b);
  bytesParsed += mac_size;
  ALOGD("%s: %s , %s ", __func__, left.macString().c_str(), right.macString().c_str());

  if (xm_intf && xm_intf->update_disconnected_clients) {
    xm_intf->update_disconnected_clients(left, right);
  }
}

/* Terminate TWT session*/
void XpanProviderService::teardownTwt(const std::vector<uint8_t>& data) {
 ...
 /* expectedLength =  mac_left (6) + mac_right (6) */
  ...
  if (xm_intf && xm_intf->teardown_twt) {
    xm_intf->teardown_twt(left, right);
  }
}

void XpanProviderService:: setRemoteFeatureMask(const std::vector<uint8_t>& data) {

    /* expectedLength =  bdaddr_t (6) + featureMask (8) */
    uint8_t bytesParsed = 0, expectedLength = 6 + 4;
    if (data.size() < expectedLength) {
      ALOGE("%s: Incorrect parameters length (%d)", __func__, data.size());
	  return;
	  }
	...
	bdaddr_t addr;
	copy(data, bytesParsed, BD_ADDR_LENGTH, addr.b);
    featureMask = data[bytesParsed++];
    featureMask |= (data[bytesParsed++] << 8);
    featureMask |= (data[bytesParsed++] << 16);
    featureMask |= (data[bytesParsed++] << 24);
	ALOGD("%s: %s int feature mask %d ", __func__, addr.toString().c_str(), featureMask);
	 if (xm_intf && xm_intf->set_remote_feature_mask) {
	   xm_intf->set_remote_feature_mask(addr, featureMask);
	 }
}
...
void XpanProviderService::sendXpanEvent(const std::vector<uint8_t>& data) {
  ALOGD("%s", __func__);
  /* XM main thread and XpanProviderService are accessing xpanProviderCb without
     any synchronization.
     This is leading to a race condition, when BinderDied or
     unRegisterXpanCallbacks is called in XpanProviderService thread and
     parallely SendXpanEvent is triggered in XM thread.
     Cannot use a mutex to synchronize, because signal maybe delivered to
     XpanProviderService thread and there is a chance of deadlock with XM main
     thread, while handling the signal.
     To solve this, Creating xmXpanProviderCb, which is only used by XM (in
     SendXpanEvent). The value of xmXpanProviderCb is updated only in XM thread
     context, when xpanProviderCb changes */
  if (xpanProviderCb != xmXpanProviderCb) {
    ALOGD("%s xpanProviderCb=%p xmXpanProviderCb=%p", __func__,
          xpanProviderCb.get(), xmXpanProviderCb.get());
    xmXpanProviderCb = xpanProviderCb;
  }

  if (xmXpanProviderCb == NULL) {
    ALOGE("%s: XpanProvider Client not bound to the service", __func__);
    return;
  }

  xmXpanProviderCb->xpanEventReceivedCb(data);
}
...
/* Callback from XpanManager when xpan supporting device is connected */
  ...
  void BearerPreferenceCb (uint8_t requester, uint8_t bearer, uint8_t reason) {
    ALOGD("%s requester %d bearer %d reason %d ", __func__, requester, bearer, reason);
    std::vector<uint8_t> data;
    data.push_back(XPAN_BEARER_PREFERENCE_REQ);
    data.push_back(requester);
    data.push_back(bearer);
...
void MdnsRegisterUnRegister(bdaddr_t bdAddr, uint8_t state) {
    ALOGD("%s device = %s action = %d", __func__, bdAddr.toString().c_str(), state);
    std::vector<uint8_t> data;
...
void WifiAPAvbRsp (bdaddr_t bdAddr, uint8_t state) {
  ...
  bool OnWifiDriverStatus(uint8_t status) {
     ALOGD("%s", __func__);
     std::vector<uint8_t> data;
     data.push_back(WIFI_DRIVER_STATUS);
     data.push_back(status);
     if (service_) {
       service_->sendXpanEvent(data);
       return true;
  ...
  void OnCountryCodeUpdate(char * country) {
     ALOGD("%s %s ", __func__, country);
     std::vector<uint8_t> data;
     data.push_back(COUNTRY_CODE_UPDATE);
     int len = strlen(country);
     if (len > 0) {
       for (int i = 0; i < len ; i++) {
         uint8_t byte = (uint8_t)country[i];
         data.push_back(byte);
       }
     }
     if (service_) {
  ...
  void PossibleToCreateApIface() {
     ALOGD("%s ", __func__);
     std::vector<uint8_t> data;
     data.push_back(POSSIBLE_TO_CREATE_AP_IFACE);
     if (service_) {
       service_->sendXpanEvent(data);
     }
  }

  void onBearerSwitchFailed(bdaddr_t bdAddr, uint8_t reason, uint8_t role) {
     ALOGD("%s ", __func__);
     std::vector<uint8_t> data;
     data.push_back(BEARER_SWITCH_FAILED);
     addBdAddrToEventData(data, bdAddr);
     data.push_back(reason);
     data.push_back(role);
     if (service_) {
  ...
  void onConnect(bdaddr_t bdAddr, uint8_t reason) {
    ALOGD("%s ", __func__);
    std::vector<uint8_t> data;
    data.push_back(ON_CONNECT);
    addBdAddrToEventData(data, bdAddr);
    data.push_back(reason);
    if (service_) {
      service_->sendXpanEvent(data);
