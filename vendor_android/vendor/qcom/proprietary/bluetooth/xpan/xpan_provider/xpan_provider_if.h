...
enum {
  ...
  UPDATE_DISCONNECTED_SAP_CLIENTS,
  TEARDOWN_TWT,
  UNREGISTER,
  REMOTE_FEATURE_MASK,
};
...
enum {
  ...
  WIFI_DRIVER_STATUS,
  COUNTRY_CODE_UPDATE,
  POSSIBLE_TO_CREATE_AP_IFACE,
  BEARER_SWITCH_FAILED,
  ON_CONNECT,
...
/* Update eb SAP Disconnection details */
typedef void (XM_UpdateDisConnectedClients) (macaddr_t left, macaddr_t right);

/* Terminate twt session */
typedef void (XM_TeardownTwt) (macaddr_t left, macaddr_t right);

/* Received Wifi Channel switch request from PEER */
typedef void (XM_UpdateWifiChannelSwitchReq) (bdaddr_t addr, uint16_t channel);

/* Get wifi driver status  */
typedef void (XM_GetWifiDriverStatus) ();

/* Received sub set feature mask*/
typedef void (XM_SetRemoteFeatureMask) (bdaddr_t addr, uint64_t feature_mask);
...
typedef struct {
  ...
  XM_UpdateDisConnectedClients* update_disconnected_clients = NULL;
  XM_TeardownTwt* teardown_twt = NULL;
  XM_UpdateWifiScanStarted* update_wifi_scan_started = NULL;
  XM_UpdateWifiChannelSwitchReq* update_wifi_channel_switch_req = NULL;
  XM_GetWifiDriverStatus* get_wifi_driver_status = NULL;
  XM_SetRemoteFeatureMask* set_remote_feature_mask = NULL;
} tXPAN_MANAGER_API;
...
class XpanProviderIf {
 public:
  ...
  virtual void BearerPreferenceCb (uint8_t requester, uint8_t state, uint8_t reason) = 0;
  ...
  virtual bool OnWifiDriverStatus(uint8_t status) {return false;};
  virtual void OnCountryCodeUpdate(char *country) = 0;
  virtual void PossibleToCreateApIface() = 0;
  virtual void onBearerSwitchFailed(bdaddr_t bdAddr, uint8_t reason, uint8_t role) = 0;
  virtual void onConnect(bdaddr_t bdAddr, uint8_t reason) = 0;
};
