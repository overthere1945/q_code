...
class XpanProviderService : public BnXpanProvider {
  public:
    ...
    void updateWifiChannelSwitchReq(const std::vector<uint8_t>& data);
    void getWifiDriverStatus(const std::vector<uint8_t>& data);
    void updateDisConnectedClients(const std::vector<uint8_t>& data);
    void teardownTwt(const std::vector<uint8_t>& data);
    void setRemoteFeatureMask(const std::vector<uint8_t>& data);
    void unRegisterXpanCallbacks();
