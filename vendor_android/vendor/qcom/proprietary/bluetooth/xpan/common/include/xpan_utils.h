...
#define WLAN_WAKE_INTERVAL_MUSIC 70
#define WLAN_WAKE_INTERVAL_VOICE 30

/* XPAN Feature bit Mask */
#define XPAN_FEATURE_2_4_GHZ_SAP_SUPPORT                0x1
#define XPAN_FEATURE_5_GHZ_SAP_SUPPORT                  0x2
#define XPAN_FEATURE_6_GHZ_SAP_SUPPORT                  0x4
#define XPAN_FEATURE_P2P_TOPOLOGY_SUPPORT               0x8
#define XPAN_FEATURE_AP_TOPOLOGY_SUPPORT                0x10
#define XPAN_FEATURE_P2P_AUDIO_SUPPORT                  0x20
#define XPAN_FEATURE_AP_AUDIO_SUPPORT                   0x40
#define XPAN_FEATURE_BT_TO_AP_OVER_2_4_GHZ_SUPPORT      0x80
#define XPAN_FEATURE_BT_TO_AP_OVER_5_GHZ_SUPPORT        0x100
#define XPAN_FEATURE_BT_TO_P2P_SWITCH_SUPPORT           0x200
#define XPAN_FEATURE_AP_AP_2_4_TO_5_GHZ_SUPPORT         0x400
#define XPAN_FEATURE_BEARER_SWITCH_DURING_LEA_STREAM    0x800
#define XPAN_FEATURE_SWITCH_PRIMARY_SUPPORT             0x1000
#define XPAN_FEATURE_AP_TLS_1_2_SUPPORT                 0x2000
#define XPAN_FEATURE_AP_DTLS_SUPPORT                    0x4000
#define XPAN_FEATURE_VOICE_OVER_P2P_SUPPORT             0x8000
#define XPAN_FEATURE_AP_MDNS_SUPPORT                    0x10000
#define XPAN_FEATURE_AP_GAMING_SUPPORT                  0x20000
#define XPAN_FEATURE_AP_TLS_1_3_SUPPORT                 0x40000
#define XPAN_FEATURE_ENTERPRISE_AP_SUPPORT              0x80000
#define XPAN_FEATURE_ENTERPRISE_AP_CERTIFICATE_SUPPORT  0x100000
...
typedef struct macaddr_t {
  ...
  std::string macString() const {
    char buf[18];
    snprintf(buf, sizeof(buf), "%02X:%02X:%02X:%02X:%02X:%02X", b[0],
             b[1], b[2], b[3], b[4], b[5]);
    buf[17] = '\0';
    std::string str(buf);
    return str;
...
typedef struct ipaddr_t{
  ...
  std::string toString() const {
    ...
    if (type == IPv4) {
      snprintf(buf, sizeof(buf), "%d.%d.%d.%d", ipv4[0], \
...
/* IDLE transition status (LE->AP or AP->LE)*/
typedef enum {
  IDLE_TRANSITION_COMPLETED,
  IDLE_TRANSITION_STARTED,
  IDLE_TRANSITION_FAILED,
} IdleTransitionStatus;
...
static inline char * StatusToString(RspStatus status)
{
  ...
  else if (status == XM_AC_AUDIO_SERVER_EXCEPTION)
    return "XM_AC_AUDIO_SERVER_EXCEPTION";
  else
...
#define ConvertRawBdaddress(bdaddr) \
 ({\
    android::base::StringPrintf ("%02X:%02X:%02X:%02X:%02X:%02X", \
    (bdaddr).b[5], (bdaddr).b[4], (bdaddr).b[3], (bdaddr).b[2], (bdaddr).b[1], \
    (bdaddr).b[0]).c_str(); \
 })

#define ConvertRawMacaddress(addr) \
 ({\
    android::base::StringPrintf ("%02X:%02X:%02X:%02X:%02X:%02X", \
    (addr).b[0], (addr).b[1], (addr).b[2], (addr).b[3], (addr).b[4], \
    (addr).b[5]).c_str(); \
 })
...
static inline char* UseCaseToString(UseCaseType usecase)
{
  ...
  else if(usecase == USECASE_LE_HQ)
    return "USECASE_LE_HQ";
  else
