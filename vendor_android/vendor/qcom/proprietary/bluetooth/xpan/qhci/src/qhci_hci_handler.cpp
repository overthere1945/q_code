...
void QHci::QHciSendCisEstablishedEvt(qhci_dev_cb_t *rem_info,
  ...
  if (rem_info) {
    ...
    uint8_t data[QHCI_CIS_ESTABLISHED_EVT_SIZE]
      = {0x3E, 0x1D, 0x19, 0x00, 0x00, 0x00, 0x26, 0x1C, 0x00, 0x26,
        0x1C, 0x00, 0x26, 0x1C, 0x00, 0x26, 0x1C, 0x00, 0x10, 0x10,
        0x03, 0x01, 0x00, 0x01, 0x01, 0xBC, 0x00, 0x00, 0x00, 0x08,
        0x00};
    //CIG Sync Delay - 0x001c26 -- 7206usec
    //Transport Latency - 0x001c26 -- 7206usec
    //ISO interval - 0x0008
    //num of subevents - 3
...
void QHci::QHciSendUsecaseUpdateCfm(uint8_t usecase) {
  ...
  if ((usecase == (uint8_t) XPAN_WIFI_HQ_PROFILE)
    ...
    || (usecase == (uint8_t) XPAN_WIFI_AP_HQ_PROFILE)
    || (usecase == (uint8_t) XPAN_AP_VOICE_CALL_PROFILE)) {
...
void QHci::QHciVsCodecEvtForUsecase(uint8_t usecase) {
  uint8_t vs_qhci_evt_pkt[QHCI_CODEC_MODE_UPDATE_EVT_LEN]
    = {0xFF, 0x08, 0x51, 0x18, 0x00, 0x00, 0x01, 0x01, 0x00, 0x00};
  ...
  vs_qhci_evt_pkt[9] = usecase;

  DataHandler *data_handler = DataHandler::Get();
  if (data_handler) {
#ifdef XPAN_SUPPORTED
    ALOGD("%s: ++++ Sending Usecase mode to stack %d", __func__,
      vs_qhci_evt_pkt[9]);

    hidl_vec<uint8_t> *hidl_data = new hidl_vec<uint8_t>;
    hidl_data->resize(QHCI_CODEC_MODE_UPDATE_EVT_LEN);
    memcpy(hidl_data->data(), vs_qhci_evt_pkt, QHCI_CODEC_MODE_UPDATE_EVT_LEN);
    data_handler->OnPacketReadyFromQHci(HCI_PACKET_TYPE_EVENT,
                                        hidl_data, false);
#endif
  }
...
void QHci::QHciVsCodecEvtForDelay(qhci_msg_t *msg) {
  uint8_t vs_qhci_evt_pkt[QHCI_CODEC_DELAY_REPORT_EVT_LEN]
    = {0xFF, 0x0A, 0x51, 0x18, 0x00, 0x00, 0x01, 0x04, 0x01, 0xFF, 0xFF, 0x0};

  vs_qhci_evt_pkt[9] = msg->DelayReport.delay_report & 0x00FF;
  vs_qhci_evt_pkt[10] = (msg->DelayReport.delay_report & 0xFF00) >> 8;

  DataHandler *data_handler = DataHandler::Get();
  if (data_handler) {
#ifdef XPAN_SUPPORTED
    ALOGE("%s: ++++ Sending Delay reporting value %d", __func__,
          ((vs_qhci_evt_pkt[10] << 8) | vs_qhci_evt_pkt[9]));
    hidl_vec<uint8_t> *hidl_data = new hidl_vec<uint8_t>;
    hidl_data->resize(QHCI_CODEC_DELAY_REPORT_EVT_LEN);
    memcpy(hidl_data->data(), vs_qhci_evt_pkt, QHCI_CODEC_DELAY_REPORT_EVT_LEN);
    data_handler->OnPacketReadyFromQHci(HCI_PACKET_TYPE_EVENT,
                                        hidl_data, false);
#endif
  }
...
void QHci::QHciLeCreateConnectToSoc(bdaddr_t bd_addr) {
  ...
  qhci_ap_soc_cmd_pending = (BIT_WAIT_FOR_LE_CONN_CMD_STATUS |
                              BIT_WAIT_FOR_LE_CONN_CMPL_EVENT);
...
void QHci::SendCreateCisToSoc(qhci_dev_cb_t *rem_info) {
  ...
  uint16_t handle;
  uint16_t cis_handle_1_collision = 0;
  uint16_t cis_handle_2_collision = 0;
  ...
  if (verifyCisHandleCollision(cig_params.cis_handles[0])) {
    auto it = cis_collison_handle_map.find(cig_params.cis_handles[0]);
    if (it != cis_collison_handle_map.end()) {
      cis_handle_1_collision = it->second;
    }
  } else if (verifyCisHandleCollision(cig_params.cis_handles[1])) {
    auto it = cis_collison_handle_map.find(cig_params.cis_handles[1]);
    if (it != cis_collison_handle_map.end()) {
      cis_handle_2_collision = it->second;
    }
  }
  ALOGD("%s Cis collion handles %d %d", __func__, cis_handle_1_collision,
          cis_handle_2_collision);
  //Move this logic to other thread _if there any crashes
  uint16_t length = 12;
  uint8_t *data = (uint8_t *)malloc(length);
  ...
  if (cis_handle_1_collision != 0) {
    //Currently create cis going each handle separately
    data[4] = cis_handle_1_collision & 0x00FF;
    data[5] = cis_handle_1_collision & 0xFF00;
  } else {
    //Currently create cis going each handle separately
  ...
  if (cis_handle_2_collision != 0) {
    data[8] = cis_handle_2_collision & 0x00FF;
    data[9] = cis_handle_2_collision & 0xFF00;
  } else {
  ...
  DataHandler *data_handler = DataHandler::Get();
  if (data_handler) {
    SET_BIT(qhci_soc_cis_evt_status, BIT_FIRST_CIS_ESTABLISHING_TO_SOC);
    SET_BIT(qhci_soc_cis_evt_status, BIT_SECOND_CIS_ESTABLISHING_TO_SOC);
    if (data_handler->GetControllerRef() != nullptr)
      data_handler->UpdateRingBufferFromQHci(HCI_PACKET_TYPE_COMMAND,
                                             data, length);
      ALOGD("%s Sending data to the soc", __func__);
...
void QHci::QHciPrepareAndSendHciDisconnect(uint16_t handle) {
  ...
  if (handle > 0) {
    ...
    if (verifyCisHandleCollision(handle)) {
      ...
      ALOGD("%s:  SOC Mapping Handle %d", __func__, handle);
    }
...
void QHci::QHciSendLeEnableEncryption(qhci_dev_cb_t *rem_info) {

  if (rem_info) {
    uint16_t handle = rem_info->handle;
    ...
    memcpy(&data[5], &rem_info->le_enc_cmd_info.rand_num[0], 8);
    data[13] = rem_info->le_enc_cmd_info.encrypted_div[0];
    data[14] = rem_info->le_enc_cmd_info.encrypted_div[1];
    memcpy(&data[15], &rem_info->le_enc_cmd_info.ltk[0], 16);

    SET_BIT(qhci_hci_cmd_wait, BIT_LE_ENABLE_ENC_QHCI_TO_SOC);
...
void QHci::QHciSendAclToSoc(qhci_dev_cb_t *rem_info, uint8_t *acl_data,
                                  uint8_t acl_length) {
  ...
  if (rem_info) {
    ...
    ALOGD("%s handle %d aclLength %d", __func__, handle, acl_length);
    uint16_t length = acl_length + 4;
    uint8_t *data = (uint8_t *)malloc(length);
    ...
    //Opcode
    data[0] = (handle & 0x00FF);
    data[1] = (handle & 0xFF00) >> 8;;

    //length
    data[2] = (acl_length & 0x00FF);
    data[3] = (acl_length & 0xFF00) >> 8;;

    memcpy(&data[4], acl_data, acl_length);

#ifdef XPAN_SUPPORTED
    DataHandler *data_handler = DataHandler::Get();
    if (data_handler) {
      if (data_handler->GetControllerRef() != nullptr)
        data_handler->UpdateRingBufferFromQHci(HCI_PACKET_TYPE_ACL_DATA,
                                               data, length);
        ALOGD("%s Sending ACL data to the soc", __func__);
        data_handler->GetControllerRef()->SendPacket(HCI_PACKET_TYPE_ACL_DATA,
                                                     data, length);
    }
#endif
    free((uint8_t *)data);
...
void QHci::QHciSendTargetedAdvEvent(bdaddr_t bd_addr) {
  ALOGD("%s ", __func__);

  uint8_t qhci_adv_event[QHCI_TARGETTED_ADV_EVT_LEN]
      = {0x3E, 39, 0x0D, 0x01, 0x01, 0x0, 0x0, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
         0xFF, 0x01, 0x01, 0x0E, 0x7F, 0xD0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
         0x0, 0x0, 0x0, 0x0D, 0x02, 0x01, 0x1A, 0x04, 0x16, 0x4E, 0x18, 0x01,
         0x04, 0x16, 0x53, 0x18, 0x01};

  DataHandler *data_handler = DataHandler::Get();
  qhci_adv_event[7] = bd_addr.b[0];
  qhci_adv_event[8] = bd_addr.b[1];
  qhci_adv_event[9] = bd_addr.b[2];
  qhci_adv_event[10] = bd_addr.b[3];
  qhci_adv_event[11] = bd_addr.b[4];
  qhci_adv_event[12] = bd_addr.b[5];

  if (data_handler) {
#ifdef XPAN_SUPPORTED
    ALOGD("%s Send targetted advertising pkt event to stack ", __func__);
    hidl_vec<uint8_t> *hidl_data = new hidl_vec<uint8_t>;
    hidl_data->resize(QHCI_TARGETTED_ADV_EVT_LEN);
    memcpy(hidl_data->data(), qhci_adv_event,
      QHCI_TARGETTED_ADV_EVT_LEN);
    data_handler->OnPacketReadyFromQHci(HCI_PACKET_TYPE_EVENT,
                                        hidl_data, false);
#endif
  } else {
...
void QHci::QHciCodecCmdCmplEvt(uint8_t status) {
  ALOGD("%s ", __func__);

  uint8_t qhci_cmd_cmplt[QHCI_CMD_CMPLT_LEN]
      = {0x0e, 0x05, 0x01, 0x51, 0xFC, 0x00, 0x3C};
  ...
  if (data_handler) {
    ...
    ALOGD("%s event to stack ", __func__);
    hidl_vec<uint8_t> *hidl_data = new hidl_vec<uint8_t>;
    hidl_data->resize(QHCI_CMD_CMPLT_LEN);
    memcpy(hidl_data->data(), qhci_cmd_cmplt,
      QHCI_CMD_CMPLT_LEN);
    data_handler->OnPacketReadyFromQHci(HCI_PACKET_TYPE_EVENT,
                                        hidl_data, false);
...
void QHci::QHciVsCodecEvtForTransportType(bdaddr_t bd_addr,
                                          uint8_t transport_type,
                                          uint8_t conn_status) {
  //Type - 2, conn handle - 2B, transport type - 1B, conn_status - 1B
  uint8_t vs_qhci_evt_pkt[QHCI_CODEC_MODE_TRANSPORT_TYPE_EVT_LEN]
    = {0xFF, 0x0F, 0x51, 0x18, 0x00, 0x00, 0x01, 0x08, 0x02,
       0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

  vs_qhci_evt_pkt[9] = bd_addr.b[0];
  vs_qhci_evt_pkt[10] = bd_addr.b[1];
  vs_qhci_evt_pkt[11] = bd_addr.b[2];
  vs_qhci_evt_pkt[12] = bd_addr.b[3];
  vs_qhci_evt_pkt[13] = bd_addr.b[4];
  vs_qhci_evt_pkt[14] = bd_addr.b[5];


  vs_qhci_evt_pkt[15] = transport_type;
  vs_qhci_evt_pkt[16] = conn_status;

  DataHandler *data_handler = DataHandler::Get();
  if (data_handler) {
#ifdef XPAN_SUPPORTED
    ALOGD("%s: Sending transport_type to stack %d for addr %s", __func__,
            vs_qhci_evt_pkt[15], ConvertRawBdaddress(bd_addr));

    hidl_vec<uint8_t> *hidl_data = new hidl_vec<uint8_t>;
    hidl_data->resize(QHCI_CODEC_MODE_TRANSPORT_TYPE_EVT_LEN);
    memcpy(hidl_data->data(), vs_qhci_evt_pkt, QHCI_CODEC_MODE_TRANSPORT_TYPE_EVT_LEN);
    data_handler->OnPacketReadyFromQHci(HCI_PACKET_TYPE_EVENT,
                                        hidl_data, false);
#endif
  }
...
void QHci::QHciPrepareAndSendLeCancelConnect() {
  ALOGD("%s: ", __func__);
...
void QHci::QHciVsMetaDataCmdCmplEvt(uint8_t status) {
  ALOGD("%s ", __func__);

  uint8_t qhci_cmd_cmplt[QHCI_CMD_CMPLT_LEN]
      = {0x0e, 0x05, 0x01, 0x51, 0xFC, 0x00, 0x3D};
