...
void QHciXmIntf::PrepareAudioBearerRspToXm(bdaddr_t bd_addr,
  ...
  xm_qhci_if.PrepareAudioBearerRsp(bd_addr, status, QHCI_To_XM);
 ...
 * return         true/false
 ******************************************************************/
bool QHciXmIntf::UseCaseUpdateToXm(bdaddr_t bd_addr, UseCaseType usecase) {
     ALOGE("%s usecase %d ", __func__, usecase);
     //call XM APi from here
     xm_qhci_if.UseCaseUpdate(bd_addr, usecase);
     return true;
}
...
void QHciXmIntf::BearerSwitchInd(bdaddr_t addr, TransportType transport,
                                       uint8_t status) {
  ALOGD("%s  status %d Transport %s", __func__, status,
 ...
 *
 * Description    Is LE ACL link present for given Device
 *
 * Arguments      bdaddr_t - Remote bluetooth address
 *
 * return         bool
 ******************************************************************/
bool QHciXmIntf::isLeLinkconnected(bdaddr_t bdAddr) {
...
/******************************************************************
 *
 * Function       SapEnabledStatus
 *
 * Description    This API is informs QHci about SAP enabled status
 *
 * Arguments      uint8_t - Bluetooth address of the remote.
 *                uint8_t - status
 *
 * return         none
 ******************************************************************/
void QHciXmIntf::SapEnabledStatus(uint8_t state, uint8_t status) {
  ALOGD("%s  state %d status %d ", __func__,
            state, status);
  ...
  msg->eventId = QHCI_SAP_ENABLE_STATUS;
  msg->SapEnableStatus.eventId = QHCI_SAP_ENABLE_STATUS;
  msg->SapEnableStatus.status = status;
  msg->SapEnableStatus.state = state;
...
/******************************************************************
 *
 * Function       isLeTransportReadyForSwitch
 *
 * Description    This API is infoto check If LE Transport is ready for switch
 *
 * Arguments      uint8_t - Bluetooth address of the remote.
 *                uint8_t - status
 *
...
bool QHciXmIntf::isLeTransportReadyForSwitch() {
  ALOGD("%s ", __func__);

  return QHci::Get()->isLeTransportReadyForSwitch();
}

/******************************************************************
 *
 * Function       BearerPreferenceReq
 *
 * Description    Send the bearer preference request to Profile
 *
 * Arguments      remote bluetooth addres - bd_addr_t
 *                       transport type : 2 - LE
 *                                              3 - P2P
 ...
 * return         none
 ******************************************************************/
void QHciXmIntf::BearerPreferenceReq(bdaddr_t bd_addr,
                                    TransportType transport_type) {
  ALOGD("%s BD Address %s transport_type %s ", __func__,
  ConvertRawBdaddress(bd_addr), TransportTypeToString(transport_type));
  //call XM APi from here
  xm_qhci_if.BearerPreferenceReq(bd_addr, transport_type, QHCI_To_XM);
...
/******************************************************************
 *
 * Function       BearerPreferenceFailed
 *
 * Description    Update to QHCI that brearer Preference has failed
 *
 * Arguments      remote bluetooth addres - bd_addr_t
 *                transport type : 2 - LE
 *                                 3 - P2P
 ...
 * return         none
 ******************************************************************/
void QHciXmIntf::BearerPreferenceFailed(bdaddr_t addr,
                                        TransportType transport_type,
                                        uint8_t status) {
  ...
  msg->eventId = QHCI_INITIATED_SEAMLESS_FAILED;
  msg->InitiatedSeamlessFailed.eventId = QHCI_INITIATED_SEAMLESS_FAILED;
  msg->InitiatedSeamlessFailed.transport_type = transport_type;
  msg->InitiatedSeamlessFailed.status = status;

  for (int i = 0; i < 6; i++)
    msg->InitiatedSeamlessFailed.bd_addr.b[i] = addr.b[i];
