...
#define HCI_LE_VENDOR_QBCE_CMD 0xFC51
#define HCI_SUB_OPCODE_ENCODER_LIMIT_CMD 0x24
#define HCI_LE_VENDOR_CIG_CONTEXT_TYPE_OPCODE 0x3C
#define HCI_VS_SET_CIG_METADATA_OPCODE 0x3D

#define QHCI_TLV_TYPE_STREAM_INDICATION 0x1
#define QHCI_TLV_TYPE_BAP_TIMEOUT_INDICATION 0x2

#define QHCI_AUDIO_SERVER_CRASHED 0x1
#define QHCI_AUDIO_SERVER_RECOVERED 0x2
#define QHCI_AUDIO_STREAM_STOP 0x4
...
#define HCI_LE_APCF_VENDOR_CMD 0xFD57
#define HCI_LE_APCF_SUBOPCODE_SERVICE_DATA 0x07

#define HCI_ADV_TARGET_ANNOCEMENTS 0x01
#define HCI_LE_ASCS_UUID 0x184E
...
#define QHCI_TARGETTED_ADV_EVT_LEN 41
#define QHCI_CODEC_DELAY_REPORT_EVT_LEN 12
#define QHCI_CMD_CMPLT_LEN 7
#define QHCI_CODEC_MODE_UPDATE_EVT_LEN 10
#define QHCI_CODEC_MODE_TRANSPORT_TYPE_EVT_LEN 17
...
#define QHCI_QLL_CONN_CMPL_EVT 0x09
...
#define QHCI_CONN_FAILED_ESTABLISH 0x3E

//Reason codes for Transport Enable
#define TRANSPORT_DISABLE_DUE_TO_WIFI_SSR 0x08
...
#define CIS_ACL_COLLISION_MAP_HANDLE 128
...
typedef enum {
  ...
  STREAM_BAP_TIMEOUT,
} QHCI_AC_STREAM_STATUS;

typedef enum {
  QHCI_IDLE_TRANSTITION_COMPLETED,
  QHCI_IDLE_TRANSTITION_STARTED,
  QHCI_IDLE_TRANSTITION_FAILED,
} QHCI_IDLE_TRANSTITION_STATUS;
...
#define BIT_BREDR_DISCONNECT_CMD_TO_SOC 0x00
...
#define BIT_LE_ENABLE_ENC_QHCI_TO_SOC 0x06
#define BIT_LE_QLL_CONN_CMPL_EVENT 0x07

#define BIT_FIRST_CIS_ESTABLISH_TO_SOC 0x01
#define BIT_SECOND_CIS_ESTABLISH_TO_SOC 0x02
#define BIT_FIRST_CIS_ESTABLISHING_TO_SOC 0x03
#define BIT_SECOND_CIS_ESTABLISHING_TO_SOC 0x04

#define BIT_WAIT_FOR_LE_CONN_CMD_STATUS 0x01
#define BIT_WAIT_FOR_LE_CONN_CMPL_EVENT 0x02

#define QHCI_MSG_QUEUE                   0
#define QHCI_STOP_THREAD                 1
...
typedef struct
{
  ...
  le_enc_cmd_info_t le_enc_cmd_info;
  uint8_t idle_transition_status;
  bool qhci_initiated_seamless;
} qhci_dev_cb_t;
...
class QHci {
  public:
    ...
    uint8_t IsQhciTxPkt(uint8_t *data, size_t length);
    ...
    bool isLeLinkconnected(bdaddr_t bd_addr);
    bool isLeTransportReadyForSwitch();
    ...
    bool isL2capPauseActive(uint16_t handle);
    void GetLocalLeVersion(uint8_t *version, uint16_t *companyId,
                             uint16_t *subversion);
    uint64_t GetLocalLeFeatures();
    bool isIdleTransitionPending(bdaddr_t bd_addr);
    bool isQhciInitiatedSeamlessPending(bdaddr_t bd_addr);
  ...
  private:
    ...
    void PostQHciStateChange(uint16_t cis_handle,
                             QHCI_CSM_STATE state,
    ...
    void QHciSendCreateConnectionToAc(bdaddr_t bd_addr,
                                            uint16_t supervision_tout, bool is_bg = false);
    void QHciLeConnCmplEvent(uint16_t handle, bdaddr_t bd_addr,
                                       uint8_t status);
    void QHciCodecCmdCmplEvt(uint8_t status);
    void QHciVsMetaDataCmdCmplEvt(uint8_t status);
    void QHciVsCodecEvtForDelay(qhci_msg_t *msg);
    void QHciVsCodecEvtForUsecase(uint8_t usecase);
    void QHciVsCodecEvtForTransportType(bdaddr_t bd_addr, uint8_t transport_type,
                                        uint8_t conn_status);
    void QHciFetchUsecaseUpdateCmd(uint16_t context_type,
                                               uint16_t acl_handle);
    uint8_t QHciFetchUsecaseforStackFromXmUsecase(UseCaseType usecase);
    UseCaseType GetCurrentUsecase(TransportType trans);
    ...
    void PostQHciTransportStateChange(uint16_t handle,
    ...
    void QHciSendLeEnableEncryption(qhci_dev_cb_t *rem_info);
    void QHciLeEncCmdInfo(const uint8_t *data);
    void QHciSendAclToSoc(qhci_dev_cb_t *rem_info, uint8_t *acl_data,
                                  uint8_t acl_length);
    void QHciSendTargetedAdvEvent(bdaddr_t bd_addr);
    void updateCisCollisionHandler(uint8_t *data);
    bool verifyCisHandleCollision(uint16_t cis_handle);
    void QHciPrepareAndSendLeCancelConnect();
    ...
    std::vector <bdaddr_t> qhci_xpan_support_devices;
    qhci_cig_params_t cig_params;
    ...
    std::map<uint16_t, uint16_t> cis_collison_handle_map;
    ...
    bdaddr_t apcf_whitelist_address;
    ...
    uint64_t qhci_ap_soc_cmd_pending;
    bool dbg_direct_ap_prop;
    uint8_t qhci_ap_connect;
    uint64_t qhci_hci_cmd_wait;
    uint64_t qhci_soc_cis_evt_status;
    QHciCigContextType qhci_context_type;
    bool usecase_update_cmd_rcv;
    std::vector<std::vector<uint8_t>> l2c_data_q;
    int read_fd;
    int write_fd;
    bool qhci_le_soc_cancel_conn;
    bool host_le_cancel_conn_cmd;
};
