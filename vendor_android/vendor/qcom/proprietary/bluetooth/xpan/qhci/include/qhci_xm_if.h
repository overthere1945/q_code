...
class QHciXmIntf
{
  public:
    ...
    bool UseCaseUpdateToXm(bdaddr_t bd_addr, UseCaseType);
    ...
    void SapEnabledStatus(uint8_t state, uint8_t status);
    bool isLeTransportReadyForSwitch();
    void BearerPreferenceReq(bdaddr_t bd_addr,
                            TransportType transport_type);
    void BearerPreferenceFailed(bdaddr_t bd_addr,
                              TransportType transport_type,
                              uint8_t status);
};
