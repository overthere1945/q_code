...
XpanLmpManager::XpanLmpManager() {
  ALOGD("%s", __func__);
  mSupervisionTimeout = XPAN_TCP_SUPERVISION_TIMEOUT;
  mPingTimeout = mSupervisionTimeout/3;
  mPingInterval = 0;
}
...
void XpanLmpManager::CleanLmpCommandQueue() {
  mLmpCmdQue.clear();
}
...
ipaddr_t XpanLmpManager::GetIpAddress() {
  return mIpAddr;
}
...
uint8_t XpanLmpManager::parseLmpVersionReq(uint8_t *p, uint16_t len, ipaddr_t ip) {
  ...
  mdns_uuid_t mdnsUuid = XpanApplicationController::GetLocalMdnsUuid();
  ...
  addUintArrToData(cmd_data, mdnsUuid.b, 16);
  cmd_data.push_back(info.version);
  addUint16ToData(cmd_data, info.companyId);
  addUint16ToData(cmd_data, info.subversion);

  void *buf = (void *)cmd_data.data();
  size_t len1 = (size_t)cmd_data.size();
  ...
  if (XpanApplicationController::IsTLSEnabled()) {
    ...
    if(!shandler) {
      ALOGE("%s: Couldn't find associated socketHandler. Ignore this response.", __func__);
      return XPAN_CONN_DOESNT_EXIST;
    }

    ret = shandler->SendData(XPAN_LMP_MSG, cmd_data);
  } else {
    ret = send(XpanSocketHandler::acceptTempFd, buf, len1, 0);
  }
  ALOGD("%s: %d bytes written", __func__, ret);
  if (ret <= 0) {
...
XpanSocketHandler* XpanLmpManager::GetSocketHandler() {
  return mSocketHdlr;
}

void XpanLmpManager::ConfigureSupervisionTimers() {
  char xpan_prop_1[PROPERTY_VALUE_MAX];
  int supervision_to = 0, ping_to = 0;
  property_get("persist.vendor.qcom.btadvaudio.target.support.xpan_supervision_to", xpan_prop_1, "");
  if (!strcmp(xpan_prop_1, "")) {
    ALOGI("%s: Using default supervision & ping interval", __func__);
    return;
  } else {
    supervision_to = atoi(xpan_prop_1);
  }

  char xpan_prop_2[PROPERTY_VALUE_MAX];
  property_get("persist.vendor.qcom.btadvaudio.target.support.xpan_ping_interval", xpan_prop_2, "");
  if (!strcmp(xpan_prop_2, "")) {
    ALOGI("%s: Using default supervision & ping interval", __func__);
    return;
  } else {
    ping_to = atoi(xpan_prop_2);
  }

  if (supervision_to <= ping_to) {
    ALOGI("%s: unacceptable values, supervision_to: %d, ping_interval: %d",
          __func__, supervision_to, ping_to);
    return;
  }

  mSupervisionTimeout = supervision_to;
  mPingTimeout = ping_to + 100;
  mPingInterval = ping_to;
}


void XpanLmpManager::setSupervisionTimeout(uint16_t supervision_timeout, uint16_t interval) {
  ...
  ConfigureSupervisionTimers();
  ALOGD("%s: Supervision timeout updated -> %d, Ping Timeout = %d interval = %d",
        __func__, mSupervisionTimeout, mPingTimeout, mPingInterval);
}
...
void XpanLmpManager::StartSupervisionTimer(XpanLmpManager *lmp) {
  ...
  if (lmp->mPingInterval > 0) {
    lmp->mPingTimer->SetTimerInterval(lmp->mPingInterval);
  }
  lmp->mPingTimer->StartTimer(lmp->mPingTimeout);

  ALOGD("%s: Timer started for Ping (%d ms) interval(%d)", __func__,
        lmp->mPingTimeout, lmp->mPingInterval);
...
void XpanLmpManager::ResetPingTimer(ipaddr_t ip) {
  ...
  // Start the timers again in main thread
  XpanResetSupervisionEvt *msg = (XpanResetSupervisionEvt *)malloc(sizeof(XpanResetSupervisionEvt));
  if (msg == NULL) {
    ALOGE("%s: failed to allocate memory", __func__);
    return;
  }

  msg->event = XPAN_RESET_SUPERVISION_EVT;
  msg->ip = ip;
  xac->PostMessage((xac_handler_msg_t *)msg, MSG_PRIORITY_DEFAULT);
}
...
void XpanLmpManager::HandleLmpPingTimeout(void *data) {
  ...
  lmp->SendLmpPingReq();
  ALOGD("LMP PING Request sent to %s", lmp->GetIpAddress().toString().c_str());
}
...
void XpanLmpManager::SendLmpCancelBearerSwitchInd(uint8_t bearer_from, uint8_t bearer_to,
                                                  uint16_t err_code) {
  ALOGD("%s: bearer_from:%d, bearer_to:%d, err_code:%d", \
        __func__, bearer_from, bearer_to, err_code);
  std::vector<uint8_t> cmd_data;
...
void XpanLmpManager::SendLmpLstoInd() {
  std::vector<uint8_t> cmd_data;
  const uint16_t length = 3;
  uint16_t lsto = mSupervisionTimeout * 1.6; // ms to slots conversion
  cmd_data.push_back((uint8_t)(0xFF & XPAN_LMP_MSG));
  addUint16ToData(cmd_data, length);
  ...
  mSocketHdlr->SendData(XPAN_LMP_MSG, cmd_data);
  ALOGD("%s: Link Supervision timer sent to remote", __func__);
}
