...
void XpanAcTimer::SetTimerInterval(uint32_t timer_ms) {
  mPeriodicTimerMs = timer_ms;
}
...
bool XpanAcTimer::StartTimer(uint32_t timeout_ms) {
  ...
  timer_var.it_interval.tv_sec = mPeriodicTimerMs/1000;
  timer_var.it_interval.tv_nsec = 1000000 * (mPeriodicTimerMs % 1000);;
