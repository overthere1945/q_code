...
class XpanAcQhciIf {
 ...
 public:
  ...
  bool CreateConnection (bdaddr_t addr, uint16_t supervision_timeout, bool is_bg);
  ...
  bool IsXpanWhcDeviceReady(bdaddr_t addr);
  bool LeLinkConnectionStatusCb(bdaddr_t addr, uint8_t state, TransportType);
  bool StreamStatusUpdate(bdaddr_t addr, uint8_t status);
};
