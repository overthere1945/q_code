...
class XpanAcXmIf {
 ...
 public:
  ...
  bool Initialize();
  bool Deinitialize();
  bool PrepareBearer (bdaddr_t addr, TransportType bearer, bool urgency = false);
  bool BearerPreference (bdaddr_t addr, TransportType bearer, bool is_urgent = false);
  bool CheckIpDetailsAvailibility (bdaddr_t addr, TransportType bearer, bool urgency = false);
  bool UpdateRemoteApParams (tXPAN_Remote_Params params);
  bool RequestRemoteApParams (bdaddr_t addr, uint32_t usage, void *context);
  ...
  bool BearerSwitchInd (bdaddr_t addr, TransportType bearer, uint8_t status, uint8_t reason = 0);
  ...
  bool SetRemoteFeatureMask(bdaddr_t addr, long feature_mask);
};
