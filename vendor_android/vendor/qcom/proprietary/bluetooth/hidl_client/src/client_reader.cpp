...
extern std::shared_ptr<IBluetoothHci> btHci;
#else
using android::hardware::bluetooth::V1_0::IBluetoothHci;
...
void *process_tool_data(void *arg) {
    ALOGE("Start of process_tool_data()");
    int retval;
...
static int get_payload_len(char packet_type, unsigned char *preamble) {
    ALOGE("Start of get_payload_len()");
    int len_offset;
...
bool command_is_get_rts_status(const unsigned char* buf, int len) {
    ALOGE("Start of command_is_get_rts_status()");
    bool result = false;
...
bool command_is_get_cts_status(const unsigned char* buf, int len) {
    ALOGE("Start of command_is_get_cts_status()");
    bool result = false;
