 ...
 *
 ******************************************************************************/
#ifdef CONFIG_BT_AIDL_SUPPORTED
...
#define BT_ISO_PACKET_TYPE 0x05
...
#define COMMAND_SET_FTM_MODE  \
  { 0x00, 0x00, 0x05 }
static sem_t s_cond;
static int mode_type;
#ifdef CONFIG_BT_AIDL_SUPPORTED
static volatile bool aidl_init = false;
#endif
...
#ifdef CONFIG_BT_AIDL_SUPPORTED
class AidlHciCallbacks;
std::shared_ptr<IBluetoothHci> btHci;
...
class AidlHciCallbacks : public ::aidl::android::hardware::bluetooth::BnBluetoothHciCallbacks {
  ...
  ::ndk::ScopedAStatus initializationComplete(AidlStatus status) {
    ...
    if (status == AidlStatus::SUCCESS) {
        aidl_init = true;
        ALOGE("%s (status = %d)", __func__, static_cast<int>(status));
    } else {
        ALOGE("Error in AIDL initialization");
    }
    sem_post(&s_cond);
  ...
  ::ndk::ScopedAStatus isoDataReceived(const std::vector<uint8_t>& data) {
    ...
    int len = static_cast<int>(data.size());
    unsigned char val = BT_ISO_PACKET_TYPE;
    if (safe_write(server_fd, &val, 1) == -1) {
        ALOGE("%s: failed to write event to tool socket", __func__);
        return ndk::ScopedAStatus::fromServiceSpecificError(
        STATUS_FAILED_TRANSACTION);
...
void start_aidl()
{
    ...
    ALOGI("%s: Initialize AIDL with Mode BT", __func__);
    btHci = IBluetoothHci::fromBinder(binder);
    if (btHci != nullptr)
    {
        ...
        ALOGE("Value of AIDL callback is: %p", aidl_callbacks_.get());
        //Sending SET_FTM_MODE
        hidl_vec<uint8_t> cmd = COMMAND_SET_FTM_MODE;
        btHci->sendHciCommand(cmd);
        btHci->initialize(aidl_callbacks_);
    ...
    switch (mode) {
        case MODE_BT:
            ...
            if (aidl_init == true) {
                ...
                ALOGE("%s: AIDL failed to initialize, sending invalid FD to tool", __func__);
                return false;
            }
#else
...
void hidl_client_close() {
#ifdef CONFIG_BT_AIDL_SUPPORTED
    if ((aidl_init == false) && (ant_init_status == 1)) {
        return;
    }
#else
    ...
    switch (mode_type) {
        case MODE_BT:
#ifdef CONFIG_BT_AIDL_SUPPORTED
            ALOGI("%s: Close AIDL with Mode BT", __func__);
            btHci->close();
            btHci = nullptr;
#else
