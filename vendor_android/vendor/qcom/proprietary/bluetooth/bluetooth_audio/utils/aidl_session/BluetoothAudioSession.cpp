...
void BluetoothAudioSession::OnSessionStarted(
  ...
  const AptxAdaptiveConfiguration *aptxAdaptiveConfig = nullptr;
  const CodecConfiguration *codecConfig = nullptr;

  if(audio_config.getTag() == AudioConfiguration::a2dpConfig) {
    codecConfig = &audio_config.get<AudioConfiguration::a2dpConfig>();
  }
  if (codecConfig && codecConfig->codecType == CodecType::APTX_ADAPTIVE) {
    aptxAdaptiveConfig =
        &codecConfig->config.get<CodecConfiguration::CodecSpecific::aptxAdaptiveConfig>();
  }
  is_local_remote_delay_valid = false;
  if (stack_iface == nullptr) {
    LOG(ERROR) << __func__ << " - SessionType=" << toString(session_type_)
               << ", IBluetoothAudioPort Invalid";
  } else if (!UpdateAudioConfig(audio_config)) {
    if(!(codecConfig && codecConfig->codecType == CodecType::APTX_ADAPTIVE) ||
         (aptxAdaptiveConfig != nullptr && !aptxAdaptiveConfig->aptxAdaptiveConfigStream.empty())) {
      LOG(ERROR) << __func__ << " - SessionType=" << toString(session_type_)
...
std::vector<uint16_t> BluetoothAudioSession::getCisHandles() {
  std::lock_guard<std::recursive_mutex> guard(stack_ops_mutex_);
  if (IsSessionReady()) {
    ...
    switch (session_type_) {
      ...
      case SessionType::LE_AUDIO_HARDWARE_OFFLOAD_DECODING_DATAPATH:
       ALOGW("%s: cis_handles_ size: %d", __func__, cis_handles_.size());
       for (int i = 0; i < cis_handles_.size(); i++) {
         ALOGW("%s: cis cis_handles_: [ %d ] = %d", __func__, i, cis_handles_[i]);
       }
       return cis_handles_;
    }
  }
...
void BluetoothAudioSession::ReportA2dpExtensiblityEnabled(bool is_a2dp_extensiblity_enabled) {
  LOG(INFO) << __func__<< " is a2dp extensiblity enabled: "<< is_a2dp_extensiblity_enabled;
  is_a2dp_extensiblity_enabled_ = is_a2dp_extensiblity_enabled;
}
  ...
  if (!IsSessionReady()) {
    switch (session_type_) {
      ...
      case SessionType::A2DP_HARDWARE_OFFLOAD_DECODING_DATAPATH:
        return is_a2dp_extensiblity_enabled_ ? AudioConfiguration(A2dpStreamConfiguration{})
                                             : AudioConfiguration(CodecConfiguration{});
...
void BluetoothAudioSession::ReportMetadataChanged(uint32_t cig_id,
     uint32_t cis_id, const std::vector<uint8_t>& data) {
  ...
  for (auto& observer : observers_) {
    ...
    if (callback->metadata_changed_cb_ != nullptr) {
      callback->metadata_changed_cb_(cookie, cig_id, cis_id, data);
    }
  }
}
...
void BluetoothAudioSession::ReportAudioConfigChanged(
  ...
  if (session_type_ !=
      ...
      LOG(ERROR) << __func__ << " for SessionType=" << toString(session_type_)
                 << " Updated Audio Config Received";

    if (audio_config.getTag() != AudioConfiguration::a2dpConfig) {
      LOG(ERROR) << __func__ << " invalid non-a2dpConfig type for SessionType ="
                 << toString(session_type_);
      return;
    }

    auto new_a2dp_config =
        audio_config.get<AudioConfiguration::a2dpConfig>();

    if(new_a2dp_config.codecType == CodecType::APTX_ADAPTIVE) {
        AptxAdaptiveConfiguration aptxAdConfig =
            new_a2dp_config.config.get<CodecConfiguration::CodecSpecific::aptxAdaptiveConfig>();

        if(aptxAdConfig.aptxMode == AptxMode::HIGH_QUALITY) {
          local_remote_delay = 320;
          is_local_remote_delay_valid = true;
          LOG(INFO) << __func__ << " for SessionType=" << toString(session_type_)
                   << " Aptx Adaptive updated local latency: "
                   << local_remote_delay;
        } else if(aptxAdConfig.aptxMode == AptxMode::LOW_LATENCY ||
                  aptxAdConfig.aptxMode == AptxMode::ULTRA_LOW_LATENCY) {
          local_remote_delay = 70;
          is_local_remote_delay_valid = true;
          LOG(INFO) << __func__ << " for SessionType=" << toString(session_type_)
                   << " Aptx Adaptive updated local latency: "
                   << local_remote_delay;
    ...
    LOG(INFO) << __func__ << "begin to update codec for SessionType=" << toString(session_type_);
    if (!UpdateAudioConfig(audio_config)){
        LOG(INFO) << __func__ << "fail to update codec for SessionType=" << toString(session_type_);
    } else {
      for (auto& observer : observers_) {
        uint16_t cookie = observer.first;
  ...
  if (is_streaming_) {
    ...
    if (audio_config.getTag() == AudioConfiguration::leAudioBroadcastConfig) {
      LOG(ERROR) << __func__ << " for SessionType=" << toString(session_type_)
                 << " Broadcast audio_config_ reported during streaming. It "
...
void BluetoothAudioSession::ReportUniqueCishandles(std::vector<uint16_t> cis_hndles) {
  std::lock_guard<std::recursive_mutex> guard(stack_ops_mutex_);
  LOG(INFO) << __func__ << " for SessionType=" << toString(session_type_)
            << ", cis_hndles size = " << cis_hndles.size();
  cis_handles_ = cis_hndles;

  for (int i = 0; i < cis_hndles.size(); i++) {
    ALOGW("%s: After assigning cis_handles_: [ %d ] = %d", __func__, i, cis_handles_[i]);
  }
}
...
bool BluetoothAudioSession::UpdateAudioConfig(
  ...
  LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_);
  LOG(INFO) << __func__ << " - audio_config_tag=" << toString(audio_config_tag);
  ...
  LOG(INFO) << __func__ << ": is_software_audio_config: " << is_software_audio_config
                        << ", is_a2dp_offload_audio_config: " << is_a2dp_offload_audio_config
                        << ", is_hfp_offload_audio_config: " << is_hfp_offload_audio_config
                        << ", is_le_audio_offload_unicast_audio_config: "
                        << is_le_audio_offload_unicast_audio_config
                        << "is_le_audio_broadcast_offload_audio_config: "
                        << is_le_audio_broadcast_offload_audio_config;

  if (!is_software_audio_config && !is_a2dp_offload_audio_config &&
      !is_hfp_offload_audio_config &&
      !is_le_audio_offload_unicast_audio_config && !is_le_audio_broadcast_offload_audio_config) {
    return false;
  }
  audio_config_ = std::make_unique<AudioConfiguration>(audio_config);
  return true;
...
void BluetoothAudioSession::ReportSessionStatus() {
  ...
  for (auto& observer : observers_) {
    ...
    LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_)
              ...
              << ::android::base::StringPrintf("%04x", cookie);
    if (callback->session_changed_cb_ != nullptr) {
...
void BluetoothAudioSession::ReportControlStatus(bool start_resp,
  ...
  for (auto& observer : observers_) {
    ...
    if (callback->control_result_cb_ != nullptr) {
...
bool BluetoothAudioSession::GetPresentationPosition(
  ...
  if(is_local_remote_delay_valid) {
    presentation_position.remoteDeviceAudioDelayNanos = (uint64_t)local_remote_delay*1000000;
    LOG(INFO) << __func__ << " for SessionType=" << toString(session_type_)
                   << " Return local latency value: "
                   << local_remote_delay;
  } else if (stack_iface_->getPresentationPosition(&presentation_position).isOk()) {
    LOG(WARNING) << __func__ << " - IBluetoothAudioPort SessionType="
                 << toString(session_type_) << " failed";
    local_remote_delay = (uint16_t)(presentation_position.remoteDeviceAudioDelayNanos/1000000);
    is_local_remote_delay_valid = true;
  } else {
    return false;
  }
...
void BluetoothAudioSession::UpdateSinkMetadata(
  ...
  if(session_type_ == SessionType::A2DP_HARDWARE_OFFLOAD_DECODING_DATAPATH ) {
     float gain = sink_metadata.tracks[0].gain;
     LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_)
               << " gain : " << gain;
  }
