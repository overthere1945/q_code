...
struct PortStatusCallbacks {
  ...
  std::function<void(uint16_t cookie, uint32_t cig_id, uint32_t cis_id,
                     const std::vector<uint8_t>& data)> metadata_changed_cb_;
};
...
class BluetoothAudioSession {
 public:
  ...
  void ReportMetadataChanged(uint32_t cig_id, uint32_t cis_id,
                             const std::vector<uint8_t>& data);
  void ReportUniqueCishandles(std::vector<uint16_t> cis_hndles);
  std::vector<LatencyMode> GetSupportedLatencyModes();
  void SetLatencyMode(const LatencyMode& latency_mode);
  void ReportA2dpExtensiblityEnabled(bool is_a2dpextensiblity_enabled);
  ...
  std::vector<uint16_t> getCisHandles();
 ...
 private:
  ...
  bool is_local_remote_delay_valid = false;
  bool is_a2dp_extensiblity_enabled_ = false;
  uint16_t local_remote_delay;
  std::vector<uint16_t> cis_handles_ = {};
