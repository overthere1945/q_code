...
static const SbcCapabilities kDefaultOffloadSbcCapability = {
    ...
    .channelMode = {SbcChannelMode::MONO, SbcChannelMode::DUAL,
                    SbcChannelMode::STEREO, SbcChannelMode::JOINT_STEREO},
    .blockLength = {4, 8, 12, 16},
    .numSubbands = {4, 8},
    .allocMethod = {SbcAllocMethod::ALLOC_MD_S, SbcAllocMethod::ALLOC_MD_L},
    .bitsPerSample = {16},
    .minBitpool = 2,
    .maxBitpool = 250};
    ...
    switch (codec_capability.codecType) {
      case CodecType::SBC: {
        codec_capability.capabilities
            .set<CodecCapabilities::Capabilities::sbcCapabilities>(kDefaultOffloadSbcCapability);
      } break;
      case CodecType::AAC: {
        codec_capability.capabilities
            .set<CodecCapabilities::Capabilities::aacCapabilities>(kDefaultOffloadAacCapability);
      } break;
      case CodecType::LDAC:
        codec_capability.capabilities
...
CodecInfo composeBroadcastCodecInfo(ChannelMode channelMode,
    const Lc3Capabilities& capability) {
  CodecInfo::Transport codec_info_transport =
      CodecInfo::Transport::make<CodecInfo::Transport::Tag::leAudio>();
  auto& transport =
      codec_info_transport.get<CodecInfo::Transport::Tag::leAudio>();
  transport.samplingFrequencyHz.push_back(capability.samplingFrequencyHz[0]);
  transport.bitdepth.push_back(capability.octetsPerFrame[0]);
  transport.frameDurationUs.push_back(capability.frameDurationUs[0]);
  transport.channelMode.push_back(channelMode);
...
std::vector<CodecInfo> BluetoothAudioCodecs::GetLeAudioOffloadCodecInfo(
  ...
  if (session_type ==
      ...
      kDefaultOffloadLeAudioCodecInfoMap.end()) {
    LOG(INFO) << __func__
                 << ": GetLeAudioOffloadCodecInfo for broadcast session";
    kDefaultOffloadLeAudioCodecInfoMap[session_type] = std::vector<CodecInfo>();
    for (auto capability : supportedBcastLc3CapabilityList) {
      CodecInfo bcast_stereo_codec_info = composeBroadcastCodecInfo(ChannelMode::STEREO, capability);
      CodecInfo bcast_mono_codec_info = composeBroadcastCodecInfo(ChannelMode::MONO, capability);
      LOG(INFO) << __func__
                   << ": bcast_stereo_codec_info " << bcast_stereo_codec_info.toString();
      LOG(INFO) << __func__
                   << ": bcast_mono_codec_info " << bcast_mono_codec_info.toString();
      kDefaultOffloadLeAudioCodecInfoMap[session_type].push_back(bcast_stereo_codec_info);
      kDefaultOffloadLeAudioCodecInfoMap[session_type].push_back(bcast_mono_codec_info);
    }
  }
