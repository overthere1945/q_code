...
#define LOG_TAG "A2dpOffloadCodecAptxHd"
...
/**
 * Aptx HD Signaling format Vendor Specific Value of [A2DP - 4.7 Vendor Specific A2DP Codec]
 */
...
constexpr size_t kCapabilitiesSize = 40/8;
...
/**
 * APTX-HD Class implementation
 */

A2dpOffloadCodecAptxHd::A2dpOffloadCodecAptxHd()
    : A2dpOffloadCodec(info_),
      info_({.name = "APTX-HD"}) {
  auto Vendor_AptxHd = CodecId::Vendor();
  Vendor_AptxHd.codecId = A2DP_APTX_HD_CODEC_ID_BLUETOOTH;
  Vendor_AptxHd.id = A2DP_APTX_HD_VENDOR_ID;
  info_.id = Vendor_AptxHd;
...
A2dpStatus A2dpOffloadCodecAptxHd::ParseConfiguration(
    const std::vector<uint8_t>& configuration,
    CodecParameters* codec_parameters, AptxHdParameters* aptxhd_parameters) const {
  ...
  if (aptxhd_parameters) {
  }

  return A2dpStatus::OK;
}

bool A2dpOffloadCodecAptxHd::BuildConfiguration(
  ...
  if (sf_hint >= 0 && lcaps.get(sf_hint) && rcaps.get(sf_hint))
    ...
    LOG(INFO) << ": Unable to Set APTX-HD frequency";
    return false;
  }
  ...
  if (cm_hint >= 0 && lcaps.get(cm_hint) && rcaps.get(cm_hint))
    ...
    LOG(INFO) << ": Unable to Set APTX-HD Channel Mode";
    return false;
  }
