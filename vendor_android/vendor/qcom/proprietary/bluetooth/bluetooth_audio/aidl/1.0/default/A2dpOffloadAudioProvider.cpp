...
ndk::ScopedAStatus A2dpOffloadAudioProvider::startSession(
  ...
  if (audio_config.getTag() == AudioConfiguration::Tag::a2dp) {
    ...
    if (codec->info.id == CodecId(CodecId::A2dp::SBC)) {
      ...
      LOG(INFO) << __func__ << " Parse SBC - SessionType=" << toString(session_type_)
                << " - CodecId=" << a2dp_config.codecId.toString();
      ...
      LOG(INFO) << __func__ << " Parse AAC - SessionType=" << toString(session_type_)
                << " - CodecId=" << a2dp_config.codecId.toString();
    } else if (codec->info.id.getTag() == CodecId::Tag::vendor) {
      auto a2dp_vendor_codec = codec->info.id.get<CodecId::vendor>();
      if (a2dp_vendor_codec.codecId == A2DP_APTX_CODEC_ID_BLUETOOTH &&
          a2dp_vendor_codec.id == A2DP_APTX_VENDOR_ID) {
        AptxParameters aptx_parameters;

        auto codec_aptx =
            std::static_pointer_cast<const A2dpOffloadCodecAptx>(codec);
        a2dp_status = codec_aptx->ParseConfiguration(a2dp_config.configuration,
                                                     &aptx_parameters);

        LOG(INFO) << __func__ << " Parse APTX - SessionType=" << toString(session_type_)
                  << " - CodecId=" << a2dp_config.codecId.toString();
      } else if (a2dp_vendor_codec.codecId == A2DP_APTX_HD_CODEC_ID_BLUETOOTH &&
          a2dp_vendor_codec.id == A2DP_APTX_HD_VENDOR_ID) {
        AptxHdParameters aptxhd_parameters;

        auto codec_aptxhd =
            std::static_pointer_cast<const A2dpOffloadCodecAptxHd>(codec);
        a2dp_status = codec_aptxhd->ParseConfiguration(a2dp_config.configuration,
                                                     &aptxhd_parameters);

        LOG(INFO) << __func__ << " Parse APTXHD - SessionType=" << toString(session_type_)
                  << " - CodecId=" << a2dp_config.codecId.toString();
      } else if (a2dp_vendor_codec.codecId == A2DP_LDAC_CODEC_ID_BLUETOOTH &&
          a2dp_vendor_codec.id == A2DP_LDAC_VENDOR_ID) {
        LdacParameters ldac_parameters;

        auto codec_ldac =
            std::static_pointer_cast<const A2dpOffloadCodecLdac>(codec);
        a2dp_status = codec_ldac->ParseConfiguration(a2dp_config.configuration,
                                                     &ldac_parameters);

        LOG(INFO) << __func__ << " Parse LDAC - SessionType=" << toString(session_type_)
                  << " - CodecId=" << a2dp_config.codecId.toString();
      } else if (a2dp_vendor_codec.codecId == A2DP_APTX_ADAPTIVE_CODEC_ID_BLUETOOTH &&
          a2dp_vendor_codec.id == A2DP_APTX_ADAPTIVE_VENDOR_ID) {
        AptxAdaptiveParameters aptxadaptive_parameters;

        auto codec_aptxadaptive =
            std::static_pointer_cast<const A2dpOffloadCodecAptxAdaptive>(codec);
        a2dp_status = codec_aptxadaptive->ParseConfiguration(a2dp_config.configuration,
                                                     &aptxadaptive_parameters);

        LOG(INFO) << __func__ << " Parse APTXADAPTIVE - SessionType=" << toString(session_type_)
                  << " - CodecId=" << a2dp_config.codecId.toString();
      } else {
        LOG(WARNING) << __func__ << " - Unknown A2DP Vendor Codec";
      }
    }
...
ndk::ScopedAStatus A2dpOffloadAudioProvider::parseA2dpConfiguration(
  ...
  if (!BluetoothCodecExtensiblityFlags::kEnableA2dpCodecExtensibility) {
