...
BluetoothAudioProvider::BluetoothAudioProvider() {
  ...
  char vsc_enable_val[PROPERTY_VALUE_MAX];
  property_get("persist.vendor.qcom.bluetooth.vsc_enabled", vsc_enable_val, "false");
  if (!strcmp(vsc_enable_val, "true")) {
    LOG(INFO) << __func__ << " VSC Extension is enabled";
...
ndk::ScopedAStatus BluetoothAudioProvider::startSession(
  ...
  if (session_type_ == SessionType::A2DP_HARDWARE_OFFLOAD_ENCODING_DATAPATH)
    BluetoothAudioSessionReport::ReportA2dpExtensiblityEnabled(session_type_,
        BluetoothCodecExtensiblityFlags::kEnableA2dpCodecExtensibility);
...
ndk::ScopedAStatus BluetoothAudioProvider::updateAudioConfiguration(
  ...
  audio_config_ = std::make_unique<AudioConfiguration>(audio_config);
  LOG(INFO) << __func__ << " Before filling vsm SessionType="
            << toString(session_type_)
            << " | audio_config_=" << audio_config_->toString();

  if (BluetoothCodecExtensiblityFlags::kEnableLeAudioCodecExtensibility) {
    MetadataLtv::VendorSpecific cached_metadata = MetadataLtv::VendorSpecific();
    cached_metadata = getVendorSpecificMetadataBySessionType(session_type_, *audio_config_);

    LOG(INFO) << __func__ << ": audio_config_ tag: " << toString(audio_config_->getTag());
    if (audio_config_->getTag() == AudioConfiguration::leAudioConfig) {
      LeAudioConfiguration &audioconfig =
      ...
      for (int i = 0; i < audioconfig.streamMap.size(); ++i) {
        LOG(INFO) << __func__ << ": cached_metadata.companyId: "
                              << cached_metadata.companyId;
        if (cached_metadata.companyId != 0) {
          //Setting vsm in session as part of audio_config_
          auto &le_audio_direction_ase_config = audioconfig.streamMap[i].aseConfiguration;

          auto direction_cfg_vendor_metadata = MetadataLtv::VendorSpecific();
          direction_cfg_vendor_metadata.companyId = cached_metadata.companyId;
          direction_cfg_vendor_metadata.opaqueValue =
                  std::vector<uint8_t>(cached_metadata.opaqueValue.begin() + 2,
                                           cached_metadata.opaqueValue.end());
          if (le_audio_direction_ase_config.has_value()) {
            le_audio_direction_ase_config.value().metadata = {direction_cfg_vendor_metadata};
          }

          for (int i = 0; i < direction_cfg_vendor_metadata.opaqueValue.size(); i++) {
            LOG(DEBUG) << ": direction_cfg_vendor_metadata.opaqueValue[" << i << "]: "
                       << (unsigned)direction_cfg_vendor_metadata.opaqueValue[i];
          }
        }
      }
    }
  ...
  LOG(INFO) << __func__ << " After filling vsm SessionType="
            << toString(session_type_)
            << " | audio_config_=" << audio_config_->toString();
...
/** Remove above 2 lines and uncomment this for spatial audio in future!!
...
BluetoothAudioProvider::getVendorSpecificMetadataBySessionType(
     SessionType session_type_, const AudioConfiguration& audio_config) {
  MetadataLtv::VendorSpecific temp;
  (void)session_type_;
  (void)audio_config;
  temp.companyId = 0;
  return temp;
...
#ifdef ANDROID_PLATFORM_VERSION_17
ndk::ScopedAStatus
