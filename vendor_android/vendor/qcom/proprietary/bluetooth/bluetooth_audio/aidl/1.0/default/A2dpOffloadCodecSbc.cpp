...
#define LOG_TAG "A2dpOffloadCodecSbc"
...
static int GetBitrate(const A2dpBits& configuration, int bitpool) {
  ...
  int sbc_bitrate = (bits * sampling_frequency) / (blocks * subbands);
  LOG(INFO) << ": For Bitpool " << bitpool << " and framesize " << bits/8;
  LOG(INFO) << ": SBC BitRate is " << sbc_bitrate;
  return sbc_bitrate;
}
...
bool A2dpOffloadCodecSbc::BuildConfiguration(
  ...
  if (sf_hint >= 0 && lcaps.get(sf_hint) && rcaps.get(sf_hint))
    ...
    LOG(INFO) << ": Unable to Set SBC frequency";
    return false;
  }
  ...
  if (cm_hint >= 0 && lcaps.get(cm_hint) && rcaps.get(cm_hint))
    ...
    LOG(INFO) << ": Unable to Set SBC channel mode";
    return false;
  }
  ...
  if (lcaps.get(kBlockLength16) && rcaps.get(kBlockLength16))
    ...
    LOG(INFO) << ": Unable to Set SBC Block length";
    return false;
  }
  ...
  if (lcaps.get(kSubbands8) && rcaps.get(kSubbands8))
    ...
    LOG(INFO) << ": Unable to Set SBC Subbands";
    return false;
  }
  ...
  if (lcaps.get(kAllocationMethodLoudness) &&
    ...
    LOG(INFO) << ": Unable to Set SBC Alloc";
    return false;
  }
  ...
  LOG(INFO) << ": Negotiated min bitpool " << static_cast<int>(min_bitpool);
  LOG(INFO) << ": Negotiated max bitpool " << static_cast<int>(max_bitpool);
