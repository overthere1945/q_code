...
class BluetoothAudioSessionControl {
 public:
  ...
  static bool isClient_2_0(const SessionType& session_type) {
     std::shared_ptr<BluetoothAudioSession> session_ptr =
        BluetoothAudioSessionInstance::GetSessionInstance(session_type);
     if (session_ptr != nullptr) {
       return session_ptr->isClient_2_0();
     }
     return false;
  }

  static const AudioConfiguration_2_0& GetAudioConfig_2_0(
      const SessionType& session_type) {
    std::shared_ptr<BluetoothAudioSession> session_ptr =
        BluetoothAudioSessionInstance::GetSessionInstance(session_type);

    if (session_ptr != nullptr) {
      return session_ptr->GetAudioConfig_2_0();
    } else if (session_type == SessionType::A2DP_HARDWARE_OFFLOAD_DATAPATH) {
      return BluetoothAudioSession::kInvalidOffloadAudioConfiguration_2_0;
    } else {
      return BluetoothAudioSession::kInvalidSoftwareAudioConfiguration_2_0;
    }
  }
   ...
   */
  static bool a2dpGetSinkLatency(const SessionType& session_type,
                                        uint64_t* remote_delay_report_ns,
                                        uint64_t* total_bytes_readed,
                                        timespec* data_position) {
    bool ret = false;
    std::shared_ptr<BluetoothAudioSession> session_ptr =
        BluetoothAudioSessionInstance::GetSessionInstance(session_type);
    if (session_ptr != nullptr) {
      ret = session_ptr->a2dpGetSinkLatency(session_type, remote_delay_report_ns,
