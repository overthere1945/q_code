...
int audio_stream_start_qti()
{
    ...
    tA2DP_CTRL_ACK orig_status = A2DP_CTRL_ACK_UNKNOWN;
    ALOGD("%s: state = %s", __func__, dump_a2dp_hal_state(audio_stream.state));
    ...
    if (status != A2DP_CTRL_ACK_SW_ERROR) {
      audio_stream.state = AUDIO_A2DP_STATE_STARTING;
    }
    orig_status = status;
    ...
    if (status == A2DP_CTRL_ACK_SUCCESS) {
         ...
         goto end;
    } else if (status == A2DP_CTRL_ACK_SW_ERROR) {
         ALOGD("%s: status = %s", __func__, dump_a2dp_ctrl_ack(status));
         retry_count = SW_WAIT_TRIAL_COUNT;
         wait_time = 100;
    }
    ...
    for (int i = 0; i < retry_count; i++) {
        ...
        if (orig_status == A2DP_CTRL_ACK_SW_ERROR) {
          ALOGI("%s: orig_status = %s, wait for %dmsec", __func__,
              dump_a2dp_ctrl_ack(orig_status), wait_time);
        ...
        if (orig_status == A2DP_CTRL_ACK_SW_ERROR) {
          if (status == A2DP_CTRL_ACK_SUCCESS) {
            ...
            ALOGI("%s: a2dp stream start failed, ret=%d, status=%d, retry", __func__, ret, status);
            continue;
          }
        }
...
uint16_t audio_get_a2dp_sink_latency_qti()
{
    ...
    if (audio_stream.session_ready) {
        ...
        audio_stream.sink_latency = (remoteDelay/1000000);
